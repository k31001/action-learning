...
## 10. 동급 비교·KV 캐시 실측 공백·보증 관행 (2026-09-19 추가, 덱 v5.0 근거)

사용자 지적(2026-09-19): "호스트 공동 설계가 유일한 솔루션이라는 표현은 수정. OP·보증연수·다이 세대 등 다른 시도도 모두 중요한 축. 동급 비교로 과장 없이. CacheLib 결과는 더 정합성 높은 실험으로 대체하고 없으면 가설로 명시. 수명 보증·SLA 리스크는 별도 슬라이드."

| ID | 사실 | 출처 | 등급 |
|---|---|---|---|
| F48 | **동급 용량 정격 DWPD**: Solidigm D5-P5336 61.44TB(QLC) **0.58 DWPD**, 5년 보증 · Micron 6550 ION 61.44TB(QLC, G8/G9) **1.0 RDWPD**(16KB 랜덤) · Kioxia CM9(TLC, KV 캐시 계층 제품군) 읽기 집중 **1 DWPD** / 혼합용 **3 DWPD** · Kioxia LC9 245.76TB(QLC) **0.3 DWPD**. 따라서 동급 61TB 비교는 QLC 0.58~1.0 vs TLC 1~3(약 1~5배), 최대 용량 245TB QLC 0.3 vs 혼합용 TLC 3은 10배, 극단 조합(0.075 vs 3)만 40배 | Solidigm P5336 제품 사양, Micron 6550 ION 발표(ServeTheHome·StorageNewsletter), Kioxia CM9 제품 브리프, Kioxia LC9 보도자료 | ✅ |
| F49 | **KV 캐시 오프로드 I/O 특성**(CHEOPS 2025, VU Amsterdam·IBM): DeepSpeed·FlexGen의 KV 캐시 오프로드 블록 계층 트레이스는 128KiB 요청이 지배적이며 **읽기 평균 2.0GiB/s vs 쓰기 11MiB/s**로 읽기 편중. 즉 쓰기 강도는 프레임워크·공유 캐시 구조(LMCache·Mooncake의 다중 세션 프리픽스 캐시)에 따라 크게 달라지며, FDP 적용 WAF는 미보고. SNIA 2026 세션은 LMCache+vLLM 블록 계층 특성화(예측성·처리량 트레이드오프)를 다루나 WAF 수치는 공개 요약에 없음 | ACM CHEOPS'25 논문(atlarge-research.com PDF), SNIA Developer 세션 19768 | ✅(논문)·🟡(SNIA 요약) |
| F50 | **호스트 쓰기 패턴으로 SSD WAF = 1 보장**(VLDB 2026 "How to Write to SSDs", TUM): 범용 SSD에서 GC 전에 완전 무효화된 슈퍼블록을 항상 확보하는 NoWA 패턴으로 SSD WAF = 1을 보장, YCSB-A 플래시 쓰기 6.2~9.8배 감소·처리량 1.65~2.24배, TPC-C 플래시 쓰기 7.2배 감소. ZNS(zone-append)와 FDP(배치 힌트로 NoWA 대체) 모두 지원 — 데이터베이스 영역에서 호스트가 WAF를 결정한다는 실증 | PVLDB 19(7) 1469~1483, arXiv 2603.09927, GitHub ZLeanStore | ✅(학술) |
| F51 | **FDP 범용 실측**(삼성·NVM Express): 랜덤 워크로드 50% 사용률에서 WAF 약 3 → 약 1, 전력 43.6% 감소(전형 응용). RocksDB에서는 ZNS가 FDP보다 약간 더 큰 WAF 감소 | NVM Express FDP 블로그, 삼성 기술 블로그(하이퍼스케일러 FDP 채택·RocksDB), Blocks & Files 2023-08 | 🟡 |
| F52 | **KV 캐시 워크로드의 FDP WAF 실측은 공개 문헌에 없음**(2026-09-19 검색 기준): CacheLib(F15)·범용 랜덤(F51)·DB(F50)·WARP 조건부(fdp-technical-limits §3)만 존재. 따라서 "QLC로 KV 캐시 요구 DWPD 충족"은 **검증할 가설**이며 Phase 2 실측(고객 트레이스 기반 RUH 정책·WAF)이 그 검증이다 | 본 노트(검색 결과 종합) | ⚠️ |
| F53 | **보증 관행**: 엔터프라이즈 SSD 보증은 "기간(통상 5년) 또는 TBW·DWPD 선도달" — 삼성 PM9A3 "5년 또는 DWPD 선도달", Micron 엔터프라이즈 SSD 보증, Biwin 등 다수 | 삼성 PM9A3 제품 페이지, Micron Enterprise SSD Warranty, Biwin 보증 정책 | ✅ |
| F54 | **호스트 측 WAF 관측**: OCP Cloud/Datacenter NVMe SSD 사양의 SMART Cloud Health 로그(C0)에 "Physical Media Units Written"(NAND 기록량)을 두어 표준 SMART "Data Units Written"(호스트 기록량)과의 비로 **호스트가 WAF를 계산**할 수 있게 함(사양 문구가 WAF 산출 목적을 명시) | OCP NVMe Cloud SSD Spec v1.0(2020)·Datacenter NVMe SSD Spec, Qiita·9to5Tutorial 해설, Micron WAF 기술 노트 | ✅ |

**독해**: 격차는 동급 비교로 2~10배가 정직한 범위다(F48). 호스트 배치는 WAF ≈3 → ≈1(×2.9, F15·F51)로 격차의 일부를 닫고, 나머지는 다이 세대(P/E)·OP·보증연수·SLC 캐시·워크로드 재정의 등 병행 중인 축과 결합해야 한다 — 그래서 "유일한 경로"가 아니라 "호스트가 쥔 남은 지렛대"다. KV 캐시 워크로드의 FDP WAF는 미실측(F52)이고 쓰기 강도 자체가 프레임워크에 따라 크게 다르므로(F49), 유효 DWPD 보증은 WAF 밴드 조건부여야 하며(F53 관행 + F54 관측), 이것이 보증·SLA 슬라이드의 근거다.

## 5. 검색 원문 목록

- §10 동급 비교·KV 실측 공백·보증: P5336 https://www.solidigm.com/content/dam/solidigm/en/site/products/technology/p5336-product-brief/documents/solidigm-d5-p5336-product-brief.pdf · 6550 ION https://www.servethehome.com/micron-6550-ion-61-44tb-pcie-gen5-nvme-ssd-launched/ · CM9 https://www.kioxia.com/content/dam/kioxia/shared/business/ssd/enterprise-ssd/asset/productbrief/eSSD-CM9-R-Product-brief.pdf · LC9 https://www.storagereview.com/news/245tb-kioxia-lc9-ssd-sets-new-ssd-density-record · CHEOPS'25 https://atlarge-research.com/pdfs/2025-cheops-llm.pdf · SNIA https://www.snia.org/sniadeveloper/session/19768 · VLDB'26 https://www.vldb.org/pvldb/vol19/p1469-lee.pdf · NVMe FDP 블로그 https://nvmexpress.org/nvmeflexible-data-placement-fdp-blog/ · 삼성 PM9A3 보증 https://www.samsung.com/us/business/computing/memory-storage/enterprise-solid-state-drives/pm9a3-nvme-u-2-ssd-1-9tb-mz-ql21t900/ · Micron 보증 https://www.micron.com/sales-support/sales/returns-and-warranties/enterprise-ssd-warranty · OCP Cloud SSD https://www.opencompute.org/documents/nvme-cloud-ssd-specification-v1-0-3-pdf · WAF 산출 https://qiita.com/ken-yossy/items/5bae983e0918daf2dcf8
