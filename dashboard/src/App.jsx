  ...
  return (
    <div className="min-h-screen">
      <main className="max-w-screen-2xl mx-auto px-5 py-6">
        ...
        <nav
          aria-label="페이지 탭"
          className="flex items-center gap-0.5 mb-6 px-1 py-1 rounded-xl bg-white/70 ring-1 ring-slate-200 shadow-soft backdrop-blur-sm"
        >
          {TOP_TABS.map(t => {
            const Icon = t.icon
            ...
            return (
              <button
                ...
                className={`relative flex items-center gap-2 px-4 py-2 text-[13px] font-medium rounded-lg transition-all ${
                  active
                    ? 'text-slate-900 bg-white ring-1 ring-slate-200 shadow-soft'
          ...
          {/* 버전 배지 — 탭 우측에 정렬, 미세하게 */}
          <span className="ml-auto mr-2 px-2 py-0.5 text-[10px] font-medium font-mono rounded text-slate-500 ring-1 ring-slate-200 bg-white">
            {VERSION}
          </span>
        </nav>
              ...
              <div>
                  ...
                  {MAIN_TABS.map(t => (
                    ...
                    >
                      ...
                      {t.id === 'triggers' && activeTriggers.length > 0 && (
                        <span className="ml-1.5 px-1.5 py-0.5 rounded-full bg-red-100 text-red-700 text-xs">
                          {activeTriggers.length}
                        </span>
                      )}
                      {t.id === 'ewi' && (criticalCount + warningCount) > 0 && (
                        <span className="ml-1.5 px-1.5 py-0.5 rounded-full bg-yellow-100 text-yellow-700 text-xs">
                ...
                {mainTab === 'ewi' && (
                  <div className="bg-white border border-slate-200 rounded-xl p-4">
                    <h2 className="text-sm font-semibold text-slate-800 mb-4">EWI 수동 지표</h2>
