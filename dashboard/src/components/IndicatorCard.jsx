  ...
  return (
    <div className="flex items-center gap-1.5 mt-1">
      <svg width={w} height={h} className="overflow-visible">
        <path d={path} fill="none" stroke="currentColor" strokeWidth="1.5" className="text-zinc-500" />
...
export default function IndicatorCard({ indicator, onEdit, onAutoUpdate }) {
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-1 shrink-0">
          {indicator.autoUpdateId && (
            <button
              ...
              disabled={updating}
              className="p-1.5 rounded-lg hover:bg-zinc-200/60 text-zinc-500 hover:text-sky-600 transition-colors disabled:opacity-40"
              title="자동 업데이트"
            >
      ...
      <p className="text-xs text-zinc-500 mb-3">{indicator.source}</p>
      ...
      {indicator.scenarioSignals?.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mb-3">
          ...
          <span className="text-xs text-zinc-500 self-center">{indicator.scenarioText}</span>
        </div>
      )}
      ...
      {indicator.autoUpdateSource && (
        <div className="text-xs text-zinc-400 mb-1">
          <span className="text-zinc-300">자동출처:</span> {indicator.autoUpdateSource}
        </div>
      )}
      ...
      {indicator.lastUpdated && (
        <div className="text-xs text-zinc-400 border-t border-zinc-200/60 pt-2 mt-2">
          업데이트: {indicator.lastUpdated}
          {indicator.note && <span className="ml-2 text-zinc-400 italic truncate">— {indicator.note}</span>}
        </div>
      )}
    </div>
  )
