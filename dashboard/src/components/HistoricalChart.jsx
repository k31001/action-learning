...
const CustomTooltip = ({ active, payload, label, unit, formatValue }) => {
  ...
  return (
    <div className="bg-stone-100 border border-stone-300 rounded-lg p-2.5 text-xs shadow-xl">
      <div className="text-stone-500 mb-1.5">{label}</div>
      {payload.map((p, i) => (
    ...
    <div>
      ...
      <ResponsiveContainer width="100%" height={160}>
        <AreaChart data={filtered} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
          ...
          <XAxis dataKey="date" tickFormatter={tickDate} tick={{ fill: '#57534e', fontSize: 10 }} interval="preserveStartEnd" />
          <YAxis tick={{ fill: '#57534e', fontSize: 10 }} width={50}
...
export function CapExChart({ data }) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-2">
        <span className="text-xs text-stone-500">분기별 합산 AI CapEx</span>
        <div className="flex gap-1">
          {RANGE_OPTIONS.slice(0, 4).map(o => (
            <button key={o.months} onClick={() => setRange(o.months)}
              className={`px-2 py-0.5 rounded text-xs transition-colors ${
                range === o.months ? 'bg-stone-200 text-stone-900' : 'text-stone-500 hover:text-stone-700'
              }`}>
              {o.label}
            </button>
          ))}
      ...
      <ResponsiveContainer width="100%" height={160}>
        <BarChart data={filtered} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="#1f2937" />
          <XAxis dataKey="period" tick={{ fill: '#57534e', fontSize: 10 }} />
          <YAxis tick={{ fill: '#57534e', fontSize: 10 }} width={40}
            tickFormatter={v => `$${v}B`} />
    ...
    <div>
      <div className="flex items-center justify-between mb-2">
        <div className="flex gap-1">
          <button onClick={() => setView('hbm')}
            className={`px-2 py-0.5 rounded text-xs ${view === 'hbm' ? 'bg-stone-200 text-stone-900' : 'text-stone-500 hover:text-stone-700'}`}>
            HBM
          </button>
          <button onClick={() => setView('dram')}
            className={`px-2 py-0.5 rounded text-xs ${view === 'dram' ? 'bg-stone-200 text-stone-900' : 'text-stone-500 hover:text-stone-700'}`}>
            DRAM 전체
          </button>
        </div>
      </div>
      <ResponsiveContainer width="100%" height={180}>
        <LineChart data={source} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="#1f2937" />
          <XAxis dataKey="period" tick={{ fill: '#57534e', fontSize: 9 }} />
          <YAxis domain={[0, 80]} tick={{ fill: '#57534e', fontSize: 10 }} tickFormatter={v => `${v}%`} width={36} />
...
export function StockCompareChart({ stocksData }) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-2">
        <span className="text-xs text-stone-500">기준: 선택 기간 시작 = 100</span>
        <div className="flex gap-1">
          {RANGE_OPTIONS.slice(0, 4).map(o => (
            <button key={o.months} onClick={() => setRange(o.months)}
              className={`px-2 py-0.5 rounded text-xs transition-colors ${
