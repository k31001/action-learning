        ...
        {STEEP_DATA.categories.map(cat => {
          ...
          return (
            <div key={cat.id} className="bg-white border border-zinc-200 rounded-hig-lg shadow-hig-1 overflow-hidden">
              ...
              {isOpen && (
                <div className="border-t border-zinc-200 px-4 py-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {cat.factors.map(f => {
                    ...
                    return (
                      <div key={f.id} className={`flex items-center gap-2 py-1.5 px-2.5 rounded-lg border ${tierCls}`}>
                        <span className="text-[10px] font-mono text-zinc-500 shrink-0 w-7">{f.id}</span>
                        <span className="text-xs text-zinc-700 flex-1 truncate">{f.name}</span>
                        <span className="text-[10px] font-mono text-zinc-500 shrink-0">I {f.impact}/U {f.uncertainty}</span>
        ...
        <ResponsiveContainer width="100%" height={400}>
          <ScatterChart margin={{ left: 8, right: 8, top: 8, bottom: 8 }}>
            ...
            <XAxis
              ...
              {...AXIS}
              label={{ value: 'Impact (1~5)', position: 'insideBottom', offset: -5, fill: '#71717a', fontSize: 11 }}
            />
            <YAxis
            ...
            <Tooltip
              ...
              content={({ active, payload }) => {
                ...
                return (
                  <div className="bg-zinc-100 border border-zinc-300 rounded-lg p-2.5 text-xs shadow-xl max-w-xs">
      ...
      <Card title="Top 10 핵심 불확실성 (I×U 기준)" source="wiki/driving-forces/impact-uncertainty-matrix.md">
        <div className="space-y-1.5">
          {DRIVING_FORCES_DATA.top10.map(item => {
            ...
            return (
              <div key={item.id} className="flex items-center gap-3 py-1.5 px-2.5 rounded bg-zinc-50 border border-zinc-200">
                ...
                <span className="text-xs text-zinc-800 flex-1">{item.name}</span>
        ...
        {DRIVING_FORCES_DATA.drivers.map((d, i) => {
          ...
          return (
            ...
            >
              ...
              <h4 className="text-sm font-semibold text-zinc-900 mb-3">{d.title}</h4>
              ...
              <div className="space-y-2 mb-3">
                <div className="border-l-2 border-emerald-500 pl-2.5">
                  ...
                  <div className="text-xs text-zinc-700 mt-0.5">{d.poleA.narrative}</div>
                </div>
                <div className="border-l-2 border-red-500 pl-2.5">
                  ...
                  <div className="text-xs text-zinc-700 mt-0.5">{d.poleB.narrative}</div>
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1.4fr_1fr] gap-4">
        <Card title="시나리오 매트릭스 (DF1 × DF2)" source="wiki/scenarios/scenario-matrix.md">
          <div className="relative">
            ...
            {/* X-axis label */}
            <div className="text-center text-[10px] text-zinc-500 mb-1">
              DF1: AI 거품 ← / AI 지속 →
            </div>
              ...
              {quadrants.map(({ scenarioId }) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p className="text-[11px] text-zinc-700 leading-snug mt-1">{s.summary}</p>
                  </div>
                )
              })}
            </div>
            {wildcard && (
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[11px] text-zinc-500 mt-0.5">{wildcard.summary}</p>
                </div>
            ...
            <tbody>
              {SCENARIOS_DATA.branchingPoints.map((bp, i) => (
                <tr key={i} className="border-b border-zinc-200/50 last:border-0">
                  <td className="py-2 pr-3 text-zinc-800">{bp.factor}</td>
                  <td className="py-2 pr-3 font-mono text-amber-700">{bp.decisionDate}</td>
                  <td className="py-2 text-zinc-500">{bp.monitor}</td>
                </tr>
              ))}
            </tbody>
          </table>
  ...
  return (
    <div className="space-y-4">
          ...
          {BENCHMARK_DATA.cyclicalPatterns.map(p => (
            ...
            >
              <div className="flex items-center gap-2 mb-1.5">
                <span
                  className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold text-zinc-900"
              ...
              <p className="text-[11px] text-zinc-700 leading-snug mb-2">{p.mechanism}</p>
              <div className="flex flex-wrap gap-1">
                {p.cases.map((c, i) => (
                  <span key={i} className="text-[10px] px-1.5 py-0.5 rounded bg-zinc-100 text-zinc-700 border border-zinc-300">{c}</span>
                ))}
              </div>
            </div>
          ))}
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {BENCHMARK_DATA.industries.map((c, i) => (
                <tr key={i} className="border-b border-zinc-200/50 last:border-0">
                  <td className="py-2 pr-3 text-zinc-900 font-semibold">{c.name}</td>
                  <td className="py-2 pr-3 text-zinc-500">{c.industry}</td>
                  <td className="py-2 pr-3 text-zinc-700">{c.pattern}</td>
                  <td className="py-2 text-zinc-500 text-[11px]">{c.keyMetric}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          {BENCHMARK_DATA.agriHedging.map(h => (
            <div key={h.id} className="border border-zinc-200 rounded-hig-md shadow-hig-1 p-3 bg-zinc-50">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[11px] ml-7">
                <div>
                  <span className="text-zinc-500">원리: </span>
                  <span className="text-zinc-700">{h.원리}</span>
                </div>
                <div>
      ...
      <Card title="호황 참여 헤지 구조 5종" source="wiki/benchmark/upside-participation-hedging.md">
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {BENCHMARK_DATA.upsideStructures.map(s => {
                ...
                return (
                  <tr key={s.id} className={`border-b border-zinc-200/50 last:border-0 ${rowCls}`}>
                    <td className="py-2 pr-2 text-zinc-900 font-semibold">{s.name}</td>
                    <td className="py-2 pr-2 text-center text-zinc-500">{s.complexity}</td>
                    <td className="py-2 pr-2 text-center text-zinc-700 font-mono">{s.floor}</td>
                    <td className="py-2 pr-2 text-center text-zinc-700 font-mono">{s.upside}</td>
                    <td className="py-2 pr-2 text-center text-zinc-500 font-mono">{s.premium}</td>
                    <td className="py-2 text-zinc-700">{s.note}</td>
                  </tr>
                )
              })}
            </tbody>
        ...
        <p className="text-[10px] text-zinc-400 mt-2 flex items-start gap-1.5">
