...
const STATUS_LABELS = {
  ...
  unknown: { label: '미입력', color: 'text-slate-500' },
}
    ...
    >
      <div className="bg-white border border-slate-300 rounded-2xl w-full max-w-lg shadow-2xl">
        ...
        <div className="flex items-start justify-between p-5 border-b border-slate-200">
          <div>
            <h3 className="font-semibold text-slate-900">{indicator.name}</h3>
            <p className="text-xs text-slate-500 mt-0.5">{indicator.source}</p>
          </div>
          <button onClick={onClose} className="text-slate-500 hover:text-slate-900 transition-colors ml-4">
            <X size={18} />
          </button>
        </div>
        ...
        <div className="p-5 space-y-4">
          {/* Alert info */}
          <div className="bg-slate-100 rounded-lg p-3 text-xs space-y-1">
            <div className="text-slate-500">
              <span className="text-slate-700 font-medium">경보 기준: </span>
              {indicator.alertDescription}
            </div>
            <div className="text-slate-500">
              <span className="text-slate-700 font-medium">시나리오 신호: </span>
              {indicator.scenarioText}
            </div>
            {indicator.hint && (
              <div className="text-slate-500 italic">{indicator.hint}</div>
            )}
          </div>
          ...
          <div>
            ...
            {indicator.inputType === 'select' ? (
              <select
                ...
                className="w-full bg-slate-100 border border-slate-300 rounded-lg px-3 py-2 text-sm text-slate-900 focus:outline-none focus:border-slate-400"
              >
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-slate-100 border border-slate-300 rounded-lg px-3 py-2 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:border-slate-400"
          ...
          {indicator.alertCondition === 'manual' && !indicator.inputType === 'select' && (
            <div>
              <label className="block text-xs text-slate-500 mb-1.5">상태 수동 지정</label>
              <div className="flex gap-2">
                {Object.entries(STATUS_LABELS).filter(([k]) => k !== 'unknown').map(([k, v]) => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded text-xs font-medium border transition-colors ${
                      manualStatus === k
                        ? `border-current ${v.color} bg-slate-100`
          ...
          {/* Note */}
          <div>
            <label className="block text-xs text-slate-500 mb-1.5">메모 (선택)</label>
            <input
              type="text"
              value={note}
              onChange={e => setNote(e.target.value)}
              placeholder="출처, 맥락 등을 간단히 기록..."
              className="w-full bg-slate-100 border border-slate-300 rounded-lg px-3 py-2 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:border-slate-400"
            />
          </div>
              ...
              <div className="space-y-1 max-h-32 overflow-y-auto">
                {[...indicator.history].reverse().slice(0, 8).map((h, i) => (
                  <div key={i} className="flex items-start gap-2 text-xs">
                    <span className="text-slate-400 shrink-0 font-mono">{h.date}</span>
        ...
        <div className="flex gap-3 p-5 border-t border-slate-200">
          <button
            onClick={onClose}
            className="flex-1 py-2 text-sm rounded-lg border border-slate-300 text-slate-500 hover:text-slate-900 hover:border-slate-400 transition-colors"
          >
            취소
          </button>
          <button
