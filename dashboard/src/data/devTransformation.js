...
export const DT_AXES = [
  ...
  },
  {
    axis: '조직',
    color: '#059669',
    items: ['고객별 Co-Design Pod (=메모리판 FDE) — 개발 엔지니어를 고객 아키텍트 옆에 상주 (Palantir Forward Deployed Engineer 모델)', '시스템 아키텍트·모델링 전문 조직 신설 — 외부 채용+내부 육성, 시스템 모델 소유, Pod에 아키텍트 공급', 'Distinguished Engineer(스타 아키텍트) 트랙 — 관리자 승진과 분리된 임원급 기술직, 고객 기술 리더십의 얼굴', '워크로드 인텔리전스 기능 — RS-9의 개발실 버전', '기술 마케팅 승격 — 로드맵 제안서·백서를 1급 기능으로'],
  },
  {
    axis: '일하는 방식',
    color: '#0a1b5c',
...
export const DT_STAR_ENGINEER = {
  headline:
    'FDE·Co-Design Pod·시스템 모델링의 중심에는 고객사와 대등하게 기술을 토론·주도할 개인이 있어야 한다. 익명의 팀이 아니라 이름으로 불리는 스타 엔지니어(Distinguished Engineer)가 그 자리다.',
  quote: {
  ...
  hiringEdge: {
    intro: '2026 반도체 특수로 메모리 임직원 처우가 급등(삼성 DS 특별 성과급으로 ~6억원 성과급 시대, SK하이닉스 영업이익 10% 성과급 명문화·연봉 상한 철폐). 이 국면이 스타 영입 협상에 유리한 위치를 만든다.',
    points: [
      { k: '재원', v: '메모리 사상 최대 이익 → 스타급 처우를 감당할 여력 확보' },
      { k: '정상화', v: '업계 보상 상향 리셋 → 일반 직원도 이미 6억대이므로 스타 최상위 밴드 제시의 사내 형평성 마찰이 작다' },
      { k: '명분', v: '"반도체 특수"의 핵심 인재 확보·유지는 이사회·주주가 납득하는 투자' },
      { k: '이탈 방어', v: '같은 국면이 SK하이닉스 poaching 압력도 키움 → DE 트랙은 영입뿐 아니라 스타 retention 수단' },
    ],
  },
  form: '관리자 승진과 분리된 이원 경력 경로(dual-ladder) — DE/Technical Fellow를 임원급으로 대우(관리 보직 없이 최고 처우). 컨퍼런스 발표·기술 백서 저자·특허 대표발명자·고객 공동설계 리드로 이름을 남기게 한다.',
...
export const DT_PHASES = [
  {
    ...
    theme: '증명',
    color: '#1428a0',
    actions: ['파일럿 고객 1사 Co-Design Pod 1호 발족', '워크로드 랩 최소 구성 + 시스템 모델 v0.1 (디바이스→서버 레벨, 실측 오차 검증)', '선제 제안 1호 — 파일럿 고객 차세대 로드맵 대상', 'KPI 개정안 설계 (제안·공동설계 지표 추가)', 'Distinguished Engineer 트랙 설계 — 처우 급등 국면 활용한 스타 영입/유지 프레임'],
  },
  {
    phase: 'Phase 2 · 1년',
    theme: '제도화',
    color: '#d97706',
    actions: ['Co-Design Pod 3~5개 확대 (모델사+하이퍼스케일러+ASIC)', '시스템 아키텍트·모델링 전문 조직 신설 — 모델 v1.0 (랙 레벨)', '★ DE 1호 임명 + 스타 아키텍트 영입 1명 (호명 문화 가동)', '로드맵 교차 리뷰 정례화 (분기)', 'PoA 사이클 제도화 — 연 4회', '개정 KPI 전면 적용', '★ SCA형 계약 1건 이상 수주 (공동설계 조항 포함)'],
  },
  {
    phase: 'Phase 3 · 3년',
    theme: '표준화',
...
export const DT_KPIS = [
  ...
  { label: 'Distinguished Engineer(스타) 수', now: '0 (트랙 없음)', y1: '1명+ 임명·영입', y3: '5~10명·대외 호명 정착' },
]
