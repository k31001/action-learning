    ...
    sections: [
        ...
        blocks: [
          ...
          },
          {
            ...
            rows: [
              ...
              ['CMO-3 · 다운사이클 (2022~23)', '3강 과점 · 코로나 수요 절벽 · 삼성 현금 ~$63B', '1차 승리 공식 재복제 — "인위적 감산 없다" · CapEx 47.7조 · Taylor 착공', '결과 이질성 — 범용 게임 승리(사상 최대 실적) / HBM 인증 게임 상실(40%→17% · 33년 만의 역전)', '메커니즘은 발화해도, 맥락이 이동한 곳에서는 결과를 재생하지 못한다'],
              ['CMO-4 (설계) · 다음 다운턴 (2028~29 창)', '§3의 6대 변화 — 절제 균형+CXMT · 계약 바닥 · 인증 게임 · 비동기 사이클 · CAPEX/ROI 경로 · 추격자 위치', '§5 액션 판정·§6의 감사 통과 메커니즘만 재조립 — 계약 바닥 선점 · 역사이클 대상 교정 · 치킨게임 봉인 · 맥락 EWI', '(목표) 매출 바닥이 계약으로 보장된 다운턴 · 다음 세대 인증 선점 · 절제 균형 유지', '과거 공식의 복제가 아니라, 맥락 감사를 통과한 재조립'],
            ],
          },
        ],
      },
      ...
      },
      {
        ...
        blocks: [
          {
            type: 'table',
            headers: ['메커니즘', '과거 발화 사례', '2026~28 판정', '판정 근거 (§3 맥락 변수)', '전략 배선'],
            rows: [
      ...
      },
      {
        ...
        blocks: [
          {
            type: 'p',
            text: '§2가 다운턴을 "사례" 단위로 분해했다면, 이 절은 같은 역사를 삼성이 실제로 한 액션 단위로 다시 분해한다. 사고의 과정은 네 단계다: ① §2 사례에서 삼성의 실제 액션을 추출하고, ② 결과 증거로 효과를 판정하고(◎ 분명 / △ 조건부 / ✕ 역효과), ③ §3 맥락 감사·§4 메커니즘 감사로 "그 액션이 통한 조건이 지금도 성립하는가"를 심사한 뒤, ④ 판정 ◎이면서 감사를 통과한 액션만 §6의 전략으로 번역한다. 조건이 소멸한 액션(△)은 폐기·대체하고, 역효과 액션(✕)은 반면교사로 역이용한다.',
          },
          {
            type: 'trace',
            items: [
              ...
              {
                action: '"인위적 감산 없다" + CapEx 47.7조 유지',
                period: '2022~23',
                outcome: '범용 게임 승리 — 2025~26 사상 최대 실적. 그러나 이는 다수의 이윤 극대화 경기자를 상대로 한 6강형 퇴출 유도 공식의 마지막 발화.',
                verdict: 'partial',
                reason: '불성립 — 3강 절제 균형에선 자해(§3 #1), CXMT는 가격으로 퇴출 불가(§4)',
                strategy: '폐기·대체 — 절제 공개 신호(D16) + 게임 분리(RS-6·RS-2·MB-4)',
                refs: 'wiki/benchmark/cyclical-strategy-benchmark.md · wiki/entities/cxmt.md',
              },
              {
          ...
          {
            type: 'quote',
            text: '도출 원리 — 판정 ◎ 액션(구조 대응·역사이클 매수·재무 요새·다운턴 착공)의 공통 분모는 "다운턴을 버티는 시간이 아니라 사용하는 시간으로 썼다"는 것이다. §6 전략 2순위(역사이클 대상 교정)는 이 액션들의 직계 후손이고, △는 폐기·대체(3순위), ✕는 반면교사(R&D 하한·별동대)로 역이용된다. 1순위(계약 바닥)만이 과거 액션 목록에 없는 신규다 — 효과가 분명했던 과거 액션들조차 전부 "다운턴 도착 후"의 대응이었지만, 계약 바닥은 도착 전에만 만들 수 있고 그 창이 지금 열려 있다.',
      ...
      },
      {
        ...
        blocks: [
          {
            type: 'p',
            text: 'CMO가 묻는 질문은 하나다: "다음 다운턴에서 원하는 결과(O)에서 출발해, 2026~28의 맥락(C)에서 실제로 발화하는 메커니즘(M)만으로 대응을 재조립하면 무엇부터 해야 하는가?" 발화의 창이 닫히는 순서 — 지금 하지 않으면 영영 못 하는 것부터 — 로 배열한다. 각 순위 옆 괄호는 §5 액션 판정과의 연결이다.',
          },
          {
            type: 'strategy',
            items: [
              {
                ...
                why: '§5 액션 목록의 유일한 공백 — 과거 세 다운턴에 존재하지 않았던 새 메커니즘이며, 발화 조건(공급 부족 호황 + 고객의 물량 불안)이 지금만 성립하기 때문이다 — 고객이 스스로 선수금을 예치하고 다년 계약에 서명하는 국면은 다운턴이 시작되는 순간 끝난다. 과거 공식이 "폭락을 체력으로 버틴다"였다면, 새 공식은 "폭락이 매출에 도달하지 못하게 한다"다.',
                links: [
              ...
              },
              {
                ...
                why: '§5 판정 ◎ 액션 A2(역사이클 매수)·A3(재무 요새)·A4(다운턴 착공)의 직계이자 A6(✕ HBM 실기)의 반면교사다. 여전히 발화하는 메커니즘 중 배당이 가장 크지만 발화 대상을 교정해야 한다. 인증이 배분을 결정하는 맥락에서 인증 없는 캐파는 점유율로 전환되지 않는다. 엘피다는 다운턴 중 기술 전환 실패로 죽었고, 2022~23의 삼성은 범용 캐파를 지키느라 HBM 전환을 놓쳤다 — 두 사례가 같은 경고를 두 번 반복한다.',
                links: [
              ...
              },
              {
                ...
                why: '§5에서 △ 조건부 판정을 받은 A5("인위적 감산 없다")의 폐기·대체다. 소모전 메커니즘의 발화 조건은 "상대가 이윤 극대화 경기자"인데 CXMT는 이 조건을 충족하지 않는다 — 손실을 국가가 메우는 상대는 가격으로 퇴출되지 않는다. 부러진 메커니즘의 재발화 시도는 결과를 만들지 못할 뿐 아니라 3강 절제 균형까지 파괴하는 이중 비용을 치른다.',
