...
export const STORYLINE_LENSES = [
  ...
  // ── CMO (Context-Mechanism-Outcome) ───────────────────────────────────────
  {
    ...
    sections: [
      ...
      {
        num: 2,
        title: '사례 분해 — 네 번의 다운턴, 네 개의 CMO 구성',
        blocks: [
          {
            type: 'p',
          ...
          {
            type: 'p',
            text: 'CMO-2019 (2019 다운사이클 2018 Q4~2019): 3강 과점 성립 후 첫 다운턴. 서버·모바일 재고 조정발 절벽(DRAM -37~38%)에서 후발 2사가 먼저 감산(Micron 2019-03 최초 공식화 → SK 2019-07)하고 요카이치 정전이 공급을 조이는 동안, 삼성은 무임승차형 버티기 — 무감산("인위적 웨이퍼 투입 감소 검토 안 함")·CapEx 유지(22.6조) — 로 4Q19 출하 증가·1Q20 DRAM 점유 44.1%를 회수했고 기술 4축(1z·136단 V낸드·Flashbolt·EUV) 세대 전환을 완주했다. 그러나 같은 해의 HBM팀 축소는 이 다운턴의 성적표에 안 보이던 비용을 3차 다운턴의 출발 위치(C)로 청구했다 — 다운턴 대응의 판정은 그 다운턴 안에서 완결되지 않는다.',
          ...
          },
          {
            ...
            rows: [
              ...
              ['CMO-2019 · 2019 다운사이클 (2018 Q4~2019)', '3강 과점 첫 다운턴 · 재고 조정발 절벽(DRAM -37~38%) · 후발 2사가 먼저 감산(Micron 2019-03·SK 2019-07)+요카이치 정전 · 삼성은 정점 확장 직후(2017 CapEx 최대·재고 고점·배당 3년 고정)', '무임승차형 버티기 — 무감산·CapEx 유지로 후발의 감산 위에서 버팀 + 기술 4축 세대 완주, 동시에 [배분] HBM팀 축소', '1Q20 DRAM 44.1%·2020 반등 흡수로 버티기 회수 / HBM 축소가 3차 다운턴의 C를 생성', '버티기의 ◎는 경쟁사가 대신 감산해줄 때만 — 같은 M이 3차에서 ✕. 다운턴 대응의 성적표는 그 다운턴 안에서 끝나지 않는다'],
      ...
      },
      {
        ...
        blocks: [
          ...
          { type: 'h', text: 'CMO-2019 · 2019 다운사이클 (2018 Q4~2019)' },
          {
            type: 'p',
            text: 'M/C 판별: 2018 Q4 진입 시점의 소여(3강 과점, 재고 조정발 절벽, 후발의 감산 개시·요카이치 정전, 정점 확장·재고 고점·배당 고정 출발 위치) = C, 기간 중 선택(무감산 유지·CapEx 유지·기술 세대 완주·HBM팀 축소·비전 2030·수출규제 대응) = M. 증거 등급 주의: 근거 소스가 원문 프록시 차단으로 전 항목 검색 요약 경유 — 첨자는 전부 ²(◎¹ 없음).',
            refs: 'sources/articles/samsung-2019-downturn-2017-2019-actions-2026-08-16.md',
          },
          {
            type: 'matrix',
            headers: [
              'M (액션 · 메커니즘)',
              'C19-a 경기자 구조 = 후발이 먼저 감산(Micron 2019-03·SK 2019-07)+요카이치 정전',
              'C19-b 배분 규칙 = 캐파·원가 게임 유지 (HBM은 아직 니치)',
              'C19-c 수요·거래 = 재고 조정발 절벽(DRAM -37~38%)·수출규제 외생 충격',
              'C19-d 출발 위치 = 정점 확장 직후·반도체 OP -69%',
            ],
            rows: [
              {
                label: '무감산 공식 기조 유지 ("인위적 웨이퍼 투입 감소 검토 안 함" 2Q19 컨콜 + 12라인 R&D 용도 전환) — 메커니즘: 무임승차형 버티기',
                cells: [
                  { text: '◎² C가 "후발이 먼저 감산+정전"이었으므로 버티기는 무임승차로 발화 → 삼성만 캐파 유지, 4Q19 출하 증가로 매출 보합·1Q20 DRAM 점유 44.1% (무감산 유지는 삼성 단독 — 1위의 버티기가 후발의 감산 위에서 작동)', tone: 'clear' },
                  { text: '—', tone: 'none' },
                  { text: '◎² ↖(무감산×C19-a 참조) — 절벽 국면에서 출하 확대가 가격 하락을 상쇄한 경로', tone: 'clearSoft' },
                  { text: '△² C가 이익 -69%였으므로 버티기 비용도 노출 — 부분 발화. 단 재무 요새와 배당 고정 이행이 동시에 감당됨', tone: 'partial' },
                ],
              },
              {
                label: 'CapEx 삭감 없이 유지 (2019 반도체 22.6조) — 메커니즘: 재무 요새 기반 역사이클 유지',
                cells: [
                  { text: '◎² ↖(CapEx×C19-d 참조) — 경쟁사 삭감 국면에서의 유지 자체가 점유 압박 병행', tone: 'clearSoft' },
                  { text: '—', tone: 'none' },
                  { text: '—', tone: 'none' },
                  { text: '◎² C가 정점 확장 직후·이익 급감이었음에도 유지 → Counterpoint "지속 투자가 2020 반등기 선도 지위 강화" — 2020 COVID 반등을 온전한 캐파로 흡수 (Micron $10.5B→$9B 삭감·SK 축소 예고와 대조)', tone: 'clear' },
                ],
              },
              {
                label: '기술 세대 전환 완주 (1z DRAM·136단 V낸드·HBM2E Flashbolt·EUV D1x 준비) — 메커니즘: 다운턴 중 기술 전환',
                cells: [
                  { text: '—', tone: 'none' },
                  { text: '◎² C가 원가=점유율 게임이었으므로 세대 선행 발화 → 2020~21 상승기 원가·세대 우위로 회수 (1z 세계 최초·9x단+ V낸드 유일 — 단 같은 해 HBM 조직 축소와 병존)', tone: 'clear' },
                  { text: '◎² ↖(기술 완주×C19-b 참조) — 회복기(2020 서버 수요) 방향과 정렬', tone: 'clearSoft' },
                  { text: '—', tone: 'none' },
                ],
              },
              {
                label: '[배분] HBM 전담팀 축소 (2019 — "니치" 판단·자원 범용 재배치) — 메커니즘: 주력 논리에 의한 니치 배제',
                cells: [
                  { text: '—', tone: 'none' },
                  { text: '✕² C가 아직 캐파 게임이었으므로 축소가 당시엔 합리적으로 보였다 — C의 미래 이동(인증 게임화)을 읽지 못한 배분 → 이 다운턴 성적표엔 비용이 안 보였고, 3차의 C(HBM 후순위 출발)로 청구됨: 40%→17%·33년 만의 역전. 같은 해 SK는 반대 방향', tone: 'adverse' },
                  { text: '—', tone: 'none' },
                  { text: '—', tone: 'none' },
                ],
              },
              {
                label: '[배분] 반도체 비전 2030 선언 (2019-04-24 — 시스템반도체 133조·15,000명) — 메커니즘: 다운턴 중 포트폴리오 다각화',
                cells: [
                  { text: '—', tone: 'none' },
                  { text: '—', tone: 'none' },
                  { text: '—', tone: 'none' },
                  { text: '△² C가 요새 체력이었으므로 절벽 한복판의 초장기 공표가 가능 — 파운드리 확장으로 계승(2021 171조 증액). 단 메모리 사이클 대응 효과는 간접(2차 5대 신수종 △과 같은 계열)', tone: 'partial' },
                ],
              },
              {
                label: '수출규제 대응 (공급처 다변화·JSR-imec 벨기에 레지스트 우회·국산화 착수) — 메커니즘: 공급망 이원화',
                cells: [
                  { text: '—', tone: 'none' },
                  { text: '—', tone: 'none' },
                  { text: '◎² C에 지정학 외생 충격(2019-07)이 겹쳤으므로 이원화 발화 → 생산 차질 없이 통과, 소재 국산화(솔브레인 2020-01 등)로 구조화 — 공급망 리스크 관리 제도화의 계기', tone: 'clear' },
                  { text: '—', tone: 'none' },
                ],
              },
            ],
            footnotes: [
              '반사실 한계+증거 등급: "삼성도 감산했다면"의 대조군 부재 — 경쟁사 대조로 보완하되, 근거 소스 전부 검색 요약 경유라 모든 첨자가 ²(미확인 15건은 소스 말미 참조).',
              'C 교호작용: C19-a(후발 감산)×C19-d(1위 체력)가 결합해야 무임승차 발화 — 무감산 ◎는 경쟁사 행동에 의존한 조건부 성공이며, 3차에서 같은 M이 ✕가 된 것은 이 결합의 부재 때문. C19-b가 아직 캐파 게임이었다는 사실이 HBM 축소를 당시 합리적으로 보이게 했다 — 한 다운턴의 M이 다음 다운턴의 C가 되는 순환이 이 표에서 가장 선명하다.',
              '채움률: 10/24 = 42%.',
            ],
          },
