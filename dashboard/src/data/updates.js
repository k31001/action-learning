...
export const UPDATES = [
  // ── 2026-08-05 ───────────────────────────────────────────────────────────────
  {
    ...
    summary:
      '사용자 제공 Apple Podcasts 링크의 MAD Podcast(Matt Turck) × Dylan Patel 2번째 출연(2026-07 초)을 2차 보도 3건(Podcast Alpha·BigGo·KuCoin) 교차로 수집(원문 접근 차단 — 한계 소스 헤더 명시). 핵심: ① 메모리 다년 구조적 부족 — 가격 상방 2~3배·진짜 증분 공급 2028(팹 연 +20~30% 한계)·컨슈머 전치가 랠리 재원, ② KV 캐시 = 추론·에이전트 수요의 미시 메커니즘(권석준 "추론 100배" 구체화), ③ Anthropic FCF 전환 주장(연환산 $50B+ — 3자 마스터 변수 "AI 수익화"의 첫 긍정 사례, 주장 단계), ④ CPO 양산 2029 지연(Street 2027)·구리 붐 연장·NVIDIA 800V. 위키 기존 신호(DRAM Q3 감속 조짐)와의 해석 대립(정점 vs 중간 조정)을 명시 기록 — 다음 정기 재평가 참고 입력. DF1·DF2 변경 없음. 인터뷰 메뉴 5번째 항목. 패치 v2.32.12.',
