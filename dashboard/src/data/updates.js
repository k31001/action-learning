...
export const UPDATES = [
  // ── 2026-09-23 (QLC 덱 1장 교훈 시점 오류 정정) ──────────────────────────────
  {
    ...
    type: 'lint',
    version: 'v2.46.47',
    title: 'QLC 덱 v7.5 — 1장 교훈의 시점 오류 정정: "갈린 것은 규격이 아니라 양산 시점"',
    summary:
      '①구획이 JEDEC HBM4 8 Gbps 대 NVIDIA 10~13 Gbps로 2022년 design-in을 설명하고 있었다. HBM4 표준은 2025-04 제정이라 3년 뒤 근거로 과거를 해석한 셈이다. 2019~2023 창 안으로 재조사하니 2022년에는 규격 격차 자체가 없었고(양사 6.4 Gbps·819 GB/s 동일), 갈린 것은 양산 시점이었다.',
    tags: ['QLC', 'eSSD', 'HBM3', '팩트체크', '발표자료'],
    items: [
      { label: '반직관 — 규격 격차가 없었다', detail: 'SK하이닉스와 삼성의 HBM3 공표 스펙은 모두 6.4 Gbps/pin · 819 GB/s. "삼성이 NVIDIA가 필요로 한 규격에 미달했다"는 2021~2022년 공개 근거가 존재하지 않는다' },
      { label: '인과는 일자로만 선다', detail: 'SK하이닉스 개발완료 2021-10(JEDEC 표준보다 3개월 앞섬) → 양산·NVIDIA 공급 2022-06-09 → 2022 Q3 H100 8단 16GB 단독 공급. 삼성은 로드맵만 있고 2022년 중 양산 공표를 확보하지 못했으며 2023 Q4 진입 — 약 5개 분기 공백' },
      { label: '슬라이드 교체', detail: '스펙 막대 도식 → 두 레인 타임라인. 세로 기준선 두 개(JEDEC 표준 회색 / 고객 일정 H100 2022 Q3 파랑 강조)와 삼성 레인의 「약 5개 분기」 공백 바' },
      { label: 'MR-MUF는 확정되지 않는다', detail: '업계가 원인으로 지목하나 인과 서술이 전부 2023년 이후 사후 해석이고, 수율 수치가 출처 간 정면 충돌한다(TrendForce 삼성 10~20% 대 SK 60~70% ↔ EE Times SK 75~80% 대 삼성 60~65%, 벤더 확인값 없음) → 수율 숫자 사용 금지' },
      { label: '비지지 8종 고정', detail: '공동 설계 · 표준 참여 · "스펙 미달"(2024년 HBM3E, 2년 시차) · "요구 속도 미달"(HBM4 시대, 창 밖) · "12단 수율"(H100은 8단) · CapEx(SK는 2년 연속 감소) · 가격 · "밀려났다"(2022년 점유율 40%). 마이크론이 HBM3를 건너뛴 사실도 병기해 단일 원인 서사 차단' },
      { label: '위키 개명', detail: 'codesign-demand-lesson.md → hbm3-designin-lesson.md. 리서치가 공동 설계를 비지지로 판정했으므로 파일명이 그 주장을 담으면 안 된다. 참조 6곳 링크 갱신' },
    ],
    links: [
      { label: '위키 — HBM3 소켓을 가른 것', href: 'wiki/concepts/hbm3-designin-lesson.md' },
      { label: '원장 — 2022 design-in 원인', href: 'sources/articles/qlc-v9-hbm3-designin-causes-2026-09.md' },
    ],
  },
  // ── 2026-09-23 (QLC 덱 수치 검증) ────────────────────────────────────────────
  {
    date: '2026-09-23',
    type: 'lint',
    version: 'v2.46.46',
    title: 'QLC 덱 v7.4 — 수치 검증: DWPD 범위 확정, DC NAND EB 오류 정정, Gartner 추론 역전',
    summary:
      '웹 리서치 전수 조사로 1장의 숫자 셋을 고쳤다. QLC 정격 상한 1.0은 128K 순차 기준이라 헤드라인이 아니었고, "DC NAND 2028년 1,807EB"는 전체 flash 수치를 잘못 가져온 오류였으며(DC는 909EB), 추론 근거는 규모의 역전을 직접 말하는 Gartner로 격상했다.',
    tags: ['QLC', 'eSSD', 'DWPD', '팩트체크', '발표자료'],
    items: [
      { label: 'QLC 정격 범위 확정', detail: '헤드라인 0.26~0.6 · 245TB급 0.3~0.6 · 워크로드 기준 포함 0.075~1.0(13.3배). 삼성 BM1773(245.76TB, 0.6 DWPD, >260PBW, 2026-09-18 출시)을 신규 확인' },
      { label: '비교 불가가 더 중요한 사실', detail: '245TB급 세 제품 중 둘(Kioxia LC9 0.3, 삼성 BM1773 0.6)이 측정 워크로드를 공개하지 않는다. 2배 차이가 셀·컨트롤러 차이인지 측정 기준 차이인지 분리 불가 → 슬라이드 주석으로 고정' },
      { label: 'EB 오류 정정', detail: '"DC NAND 295EB → 1,807EB"는 오류. 1,807EB는 전체 flash CY28이고 데이터센터는 909EB(CAGR 46%). v7 원장 V-21에서 유입돼 위키·덱으로 전파됐다. sources/는 불변 계층이라 v8 원장이 정정을 보유' },
      { label: 'Gartner 추론 역전', detail: '2026년 AI 최적화 IaaS 지출의 55%가 추론, 추론 $23.3B가 학습 $19.0B를 사상 최초로 추월, 2027년 59%(Gartner 2026-08-10). Kioxia의 CAGR 86%는 성장률이지 점유율이 아니라 규모의 역전을 말하지 못한다' },
      { label: '가격 %는 산술적으로 불가능', detail: '"20~30% 저렴"은 출처도 없고 물리 상한도 넘는다. 비트당 원가 절감 천장은 −25%(셀당 3→4비트)이고 컨트롤러·DRAM·PCB가 공통 원가라 드라이브 $/TB 절감은 그보다 작다. VDURA 지수도 QLC/TLC 비율이 3개 시점 중 2개가 정확히 0.8000이라 모델 가정 의심' },
      { label: '반대 신호·부정 확인', detail: '전력 용량 기준으로는 학습 5GW 대 추론 2GW라는 집계(원 출처 미확인) · TrendForce는 2029년을 전환점으로 봄(분모 다름) · "D5-P5810"은 존재하지 않음 · 국내 매체가 Gartner $42B를 "59조 달러"로 오역' },
    ],
    links: [
      { label: '원장 — DWPD·가격·추론 검증', href: 'sources/articles/qlc-v8-dwpd-price-inference-2026-09.md' },
      { label: '보고서 v4.4', href: 'outputs/report/qlc-ssd-strategy-report.md' },
    ],
  },
  // ── 2026-09-23 (QLC 덱 1장 논리 전환) ────────────────────────────────────────
  {
    date: '2026-09-23',
    type: 'build',
    version: 'v2.46.45',
    title: 'QLC 덱 v7.3 — 1장 핵심 논리를 design-in 판별로 전환',
    summary:
      '1장이 "삼성이 HBM에서 시장을 작게 봐 졌다"로 읽히면 과거 의사결정 논쟁이 슬라이드의 중심이 된다. 명제를 "표준 충족만으로는 부족했고 고객 사양을 먼저 맞춘 쪽이 design-in을 가져갔다"로 바꾸고, 팩트체크되지 않는 서술 4종을 전량 삭제했다.',
    tags: ['QLC', 'eSSD', 'HBM', 'design-in', '발표자료'],
    items: [
      { label: '새 명제', detail: 'JEDEC HBM4 표준 최대 8 Gbps 대 NVIDIA 요구 10~13 Gbps를 막대 두 개로 그리고 그 사이를 "표준을 만족해도 닿지 않는 구간"으로 표시. design-in은 표준선이 아니라 고객 요구선에서 갈렸다' },
      { label: '삭제한 서술 4종', detail: '"시장을 작게 봤다"(해석이지 사실 아님) · "조직 축소"(팩트체크 불가, 단일 보도 계열) · "5년 공백"(사실 아님 — 삼성 사업보고서 기준 제품 개발 지속) · 내부 판단 서술. 지연 구간은 "전략적 우선순위가 약화된 구간"으로만 표기' },
      { label: '요구는 누적된다', detail: '구매 기준 3국면 시대 구분은 프레임워크이지 객관적 사실이 아니어서 폐기. 「기존(용량당 비용·밀도·전력) → + 내구성」 누적 도식으로 교체. 증거는 OCP 사양 v2.5~2.7이 QoS를 빼지 않고 추가했다는 개정 이력' },
      { label: 'QLC 정격 범위화', detail: '"0.6 단일 값"은 Solidigm P5336 값이며 Kioxia LC9(245TB)은 0.3이다. 같은 Micron 6600 ION도 기준 블록에 따라 4K 0.075 / 16K 0.3 / 128K 순차 1.0으로 13배 차이 → 0.3~1.0 범위와 "245TB급은 0.3"만 표기' },
      { label: '출처 없는 수치 제거', detail: '"QLC가 TB당 20~30% 저렴" 삭제(출처 없음, 인용 가능한 것은 VDURA 현물 지수 −13~20%뿐) · 추론 CAGR 86%/16% 삭제(원장 간 CAGR과 "증분 수요의 86%"로 정의 엇갈림) → Kioxia EB 전망으로 대체' },
      { label: '질문으로 마무리', detail: 'TLC(3 DWPD 급·비용 높다) 대 QLC(0.3~1.0·245TB급·비용 낮다) 대비 후 "고용량 QLC의 밀도·비용 이점을 유지하면서 AI 추론이 요구하는 내구성을 확보할 수 있는가?"' },
    ],
    links: [
      { label: '위키 — design-in을 가른 것', href: 'wiki/concepts/codesign-demand-lesson.md' },
      { label: '보고서 v4.3', href: 'outputs/report/qlc-ssd-strategy-report.md' },
    ],
  },
  ...
  },
  {
    ...
    summary:
      '사용자가 v3.x 4장 덱의 실행 슬라이드를 제시하며 그 형식을 선호해, 표 중심 레이아웃을 그림 구도로 되돌렸다. ' +
      '좌 삼성 개발실, 중앙 ① FDE 상주 화살표와 선례 카드, 우 고객 시스템 계층이 주고받는 관계를 한 장으로 보인다.',
    tags: ['QLC', 'eSSD', 'FDE', '발표자료'],
    items: [
      { label: '구도', detail: '좌 삼성 개발실(제공하는 것 4 + 송용호 인용) / 중앙 ① FDE 상주 → · 고객 선별·채널 카드 · 선례 카드(Palantir FDE 도해, Micron↔Anthropic SCA 4요소) · ← ② 접근권 확보(시한 2027H1) / 우 고객 시스템 4계층 + FDE 아이콘 + 고객 로고' },
      { label: '유지', detail: '하단 조직·인사·문화 타일 3, 단계별 통과 조건 3, 판돈·비용. v5.0에서 제외한 자회사·별도 보상·지분 참여는 복원하지 않았고 개발실 제공 항목 네 번째는 WAF 급등 감지·런타임 대응으로 채움' },
      { label: '도형 수정', detail: 'RIGHT/LEFT_ARROW 몸통 두께를 기본 0.5에서 0.86으로 조정 — 기본값에서는 두 줄 중 아래 줄이 몸통 밖으로 나가 잘렸다' },
    ],
    links: [
      { label: '기획서 v6.1', href: 'outputs/presentation/qlc-ssd-strategy-outline.md' },
    ],
  },
  // ── 2026-09-22 (v6.0 원본 덱 개정) ──────────────────────────
  {
    date: '2026-09-22',
    type: 'build',
    version: 'v2.46.37',
    title: 'QLC 원본 덱 v6.0 — 교훈 팩트체크 정정, 신뢰성 ppm 모델, 차트 확대, 보증에서 런타임 대응 기술로',
    summary:
      '사용자 피드백 6건을 받아 원본 6장 덱을 개정했다(시각화판은 동결). 리서치 6건으로 다운턴 교훈 3건을 모두 뒤집었고 — 표준 비준이 아니라 구매자의 프로덕션 코드가 신호이며(ZNS 반례), OCP 조달 사양 기고자는 구매자이고, 차기 전환점은 2028이다 — ' +
      '신뢰성 축을 ppm 단위로 정량화해 512TB급에서 이중 패리티가 필요하다는 판정을 세웠으며, 6장을 조건부 보증에서 WAF 급등 런타임 감지·대응 기술 확보로 옮겼다.',
    tags: ['QLC', 'eSSD', 'FDP', '신뢰성', 'WAF', '발표자료', '팩트체크'],
    items: [
      { label: '1장 문제', detail: '교훈 ① 표준 비준 → 구매자 프로덕션 코드(FDP 2022-11 비준 → CacheLib 2024-01 머지 → 커널 2025-05 vs ZNS 2020-06 비준 후 물량 0) · ② 규격 기고 → 구매자 사양 적중(OCP 기고자는 Meta·MS·Google·Dell·HPE) · ③ 2027H2 → 2028(2027은 수급률 전환, 신증설 비트 도달은 2028). HBM 서술은 "조직 축소"에서 "시장 가능성 과소평가와 투자 후순위"로. 구매 기준 3국면(성능 → QoS → 밀도·전력·공급)과 SSD 지표 목록. 격차 2~5배(현 QLC 0.6 기준). 수요는 연도별 경로 차트' },
      { label: '2장 신뢰성', detail: '요구 OCP MTBF 2,000,000시간 = 5년 누적 21,662 ppm 고정, 다이 수 512(61TB) → 1,024(245TB) → 2,133(512TB), 보호 없음 43 → 10 ppm. 단일 패리티 79배(1,679 ppm)·이중 패리티 395배(8,455)·여분 다이 1,000배 이상 완화, 용량 비용 6.25%/12.5%. 현 수준 역산 200~2,700 ppm과 겹쳐 512TB급은 이중 패리티 또는 여분 다이 필요. 내부 해법은 도해로 교체' },
      { label: '3장 해법 사다리', detail: 'solution_ladder_slide v3.0 — 셀 차트 3.95×2.75in(면적 약 3배). 신설: 정격 DWPD 57배↓ 대 드라이브당 허용 기입량 67배↑, WAF before→after 쌍(CacheLib 3.22→1.03 사용률 100% / 1.22→1.03 사용률 50% / XD8 2.8→1.0 / LLM KV 미실측). 연도축 WAF 추이선은 사용률 교란 때문에 배제' },
      { label: '6장 대응 기술', detail: '조건부 보증은 리스크를 고객에게 되돌려 수용성이 낮다는 판단에 따라 런타임 감지·대응 기술 확보로 축 이동. 목표 감지 1시간·회복 4시간, 루프 4단계(관측·감지·대응·검증), 기술 요소 T1~T6(핸들별 WAF 텔레메트리·온디바이스 감지·무중단 FDP 재구성·경보 규격·정책 엔진·에뮬레이터). 보증은 TBW 기준 유지 + 대응 이력 부속으로 축소' },
      { label: 'RUH 200+ 제거', detail: 'ScaleFlux 원문은 "RUH"가 아니라 "FDP write streams"였고 제3자 검증이 없다. 덱·보고서·위키 5종에서 전량 제거하고 요구 수준을 16개 안팎(XFS 사용자 스트림 상한 16·f2fs 3·xNVMe 8, 스펙 상한은 네임스페이스당 128, 출하 제품 지원 수는 업계 미공개)으로 정정' },
      { label: '신설 자산', detail: '소스 6종(qlc-v6-*), 위키 3종(ssd-die-reliability-ppm·essd-purchase-criteria-shift·waf-runtime-response), 차트 스크립트 generate_qlc_v6_charts.py(7종), 보고서 v3.0, 기획서 v6.0 절. 지식 그래프 재생성(노드 111·엣지 602)' },
      { label: '범위 제외', detail: '시각화 강화판(qlc-ssd-strategy-visual.pptx v2.0)은 사용자 지시로 개정하지 않았다. 시각화판은 v5.2 내용 기준이며 원본과 수치가 다르다(의도된 차이)' },
    ],
    links: [
      { label: '보고서 v3.0', href: 'outputs/report/qlc-ssd-strategy-report.md' },
      { label: '기획서 v6.0', href: 'outputs/presentation/qlc-ssd-strategy-outline.md' },
    ],
  },
  // ── 2026-09-22 ───────────────────────────────────────────────────────────────
  {
  ...
  },
  {
    ...
    summary:
      ...
      '"지렛대" → "잔여 변수", "닫힌다" → "충족·해소" 등으로 덱·공용 빌더 v2.3·위키 §2.5·사다리 보고서 용어 통일. 수치·근거 변경 없음. 패치 v2.46.33.',
    tags: ['QLC', '덱', '액션 타이틀', '해법 사다리'],
    items: [
      { label: '제목 규율', detail: '30pt 한 줄 폭 실측(≤ 2,100px @110dpi)으로 다듬고, 제목이 놓친 논거는 2·3장 리드가 받음' },
      { label: '용어', detail: '지렛대 → 잔여 변수·격차 축소 변수 / 닫힌다 → 충족·해소 / 워크로드를 여는 → 개방하는 / 리스크를 안으므로 → 수반하므로' },
    ],
    links: [
      { label: '기획서 v5.2', href: 'outputs/presentation/qlc-ssd-strategy-outline.md' },
      { label: '위키 §3.4 덱 표기 원칙', href: 'wiki/concepts/qlc-ssd-market.md' },
    ],
  },
  {
    date: '2026-09-19',
  ...
  },
  {
    ...
    summary:
      '1장을 문제 제기로: 다운턴 교훈(타임라인 + 교훈 카드 3) + 요구의 이동 패널(AI 수요가 구매 기준을 추론 캐시 계층의 대용량·1~3 DWPD로 옮김, 10배·350EB·10~40배, 해법 필요). 2장 해법 사다리(당위성), 3장 역량(솔루션), 4장 실행. 구 3장(고객 채택 동인)은 1장 패널로 흡수. ' +
      '위키 §3.4 덱 표기 원칙 4장 스파인으로 갱신, 기획서 v4.0·보고서 PPT 맵 동기화. 패치 v2.46.30.',
    tags: ['QLC', '덱', '스토리라인', '다운턴 교훈'],
    items: [
      { label: '덱 v4.0(4장)', detail: '① 교훈·문제(왜 지금, 무엇이 필요한가) → ② 해법 사다리(왜 호스트 협력인가) → ③ 역량(어떻게 해소하는가) → ④ 실행(누가 어떻게)' },
    ],
    links: [
      { label: '기획서 v4.0', href: 'outputs/presentation/qlc-ssd-strategy-outline.md' },
  ...
  // ── 2026-09-03 (ii) ───────────────────────────────────────────────────────────
  {
    ...
    type: 'ingest',
    version: 'v2.46.15',
    title: '용어 정정 — 리얼라인 SSD → 니어라인(Nearline) SSD (송용호 인터뷰)',
    summary:
      '송용호 인터뷰의 SSD 제품 컨셉 용어를 자동 전사 오인식("리얼라이·리얼라인")에서 니어라인(Nearline) SSD로 정정하고 맥락을 보강. 니어라인은 데이터센터 스토리지 계층에서 핫 SSD와 콜드 아카이브 사이의 대용량 티어(현재 니어라인 HDD 영역)로, 이를 SSD로 대체하려면 성능이 아니라 용량당 원가·전력으로 승부해야 한다 — 인터뷰의 "실체는 원가 절감, 프로모션은 파워로, PBSSD는 박스 단위 파워 최적화" 논지가 이 티어 정의와 맞물린다. 패치 v2.46.15.',
    tags: ['인터뷰', '송용호', '니어라인 SSD', 'PBSSD', '용어 정정', 'dashboard'],
    items: [
      { label: '정정 범위', detail: 'interviews.js 섹션 14 제목·본문·태그·시사점(id nearline-ssd-pbssd), 소스 헤더 정정표(사용자 확인 2026-09-03 명시), index.md·updates.js·log.md 표기. 녹취 전문은 원본 불변 유지' },
      { label: '맥락 보강', detail: '니어라인 티어 정의(HDD 대체·용량당 원가·전력), 다운턴 판매 제품(대용량·저원가) vs 업턴 판매 제품(고성능 엔터프라이즈) 양쪽 포트폴리오 논의와 연결, 과제팀의 "경쟁사 니어라인 SSD 발표 대응 보고서" 회고 반영' },
    ],
    links: [
      { label: 'song-yongho-ax-pi-interview-2026-09-03.md', href: 'https://github.com/k31001/action-learning/blob/main/sources/raw-notes/song-yongho-ax-pi-interview-2026-09-03.md' },
    ],
  ...
  // ── 2026-09-03 ────────────────────────────────────────────────────────────────
  {
    ...
    items: [
      { label: '녹취록 원본 보존', detail: 'sources/raw-notes/song-yongho-ax-pi-interview-2026-09-03.md 신설 — 참석자 구성·과제 맥락 헤더 + 자동 전사 녹취록 전문(원본·불변) + ASR 정정 대조(RNA→RMA, HDM 3f→HBM3E, 리얼라이→니어라인 SSD 등)' },
      { label: '인터뷰 메뉴 구조화', detail: 'data/interviews.js INTERVIEWS 최상단에 추가(최신순, 내부 4번째·전체 6번째). 16개 섹션, keyQuotes 4개(대규모 RMA·중국 가격 통제·실행력의 역설·고객 지향), 다운턴 센싱 3대 관전 포인트 표' },
      { label: '핵심 논지', detail: '네 내부 인터뷰의 층위 완성 — 매크로·수익화(신문섭) → 제품·캐파(최장석) → 계약·오퍼레이션(이창수) → 다운턴 시뮬레이션·기본기·조직 체질(송용호). 공통 마스터 변수(AI 수익화·현금흐름) 위에 "기본(품질·원가)"과 "중국의 가격 결정권" 두 바닥 변수 추가' },
      { label: '과제 시사점 10건', detail: '2차 저지선에 품질·원가 축 추가 / RMA 시나리오 매뉴얼화 / 멀티이어 한계 명시 / EWI 전기 축 / 저전력 요구 전환 예고 / 중국 "가격 결정권 이전" 프레임 / 플랫폼+최저 원가 커스텀 / 고객 지향 체계 / AX 전제 조건 / 파일럿·시드 유지 원칙' },
      { label: '인덱스·로그', detail: 'index.md raw-notes 섹션·log.md 갱신. 위키·outputs 미변경(인터뷰 시리즈 동일 스코프)' },
    ],
    links: [
      { label: 'song-yongho-ax-pi-interview-2026-09-03.md', href: 'https://github.com/k31001/action-learning/blob/main/sources/raw-notes/song-yongho-ax-pi-interview-2026-09-03.md' },
    ],
  ...
  // ── 2026-08-08 (ii) ──────────────────────────────────────────────────────────
  {
    ...
    type: 'build',
    version: 'v2.38.1',
    title: 'CMO-4 예측 매트릭스 — 다음 다운사이클(2028~29 창)의 M×C, O는 원인 3종 조건부',
    summary:
      '요청("동일 형태의 표로 다음 다운사이클 예측 — C는 예측 가능, M은 제안, O는 다운사이클 원인에 따라 갈리므로 원인별로 셀 안에서 구분")에 따라 CMO 렌즈 §5 매트릭스에 4번째 표(예측) 추가. 원인 3종 정의 — ①수요발(AI CAPEX/ROI 재평가, C·D ~29%) ②공급발(2028~29 신규 캐파 동시 도래+CXMT, A·B 내 조정) ③전환발(3D DRAM·CXL·zHBM, E 5~10%) — 각각 M6 EWI 확인 트리거와 배선. M 6행 = §7 전략+§6 벤치마킹 제안(계약 바닥·M&A+PMI·별동대·옵션 캐파·게임 분리·맥락 EWI). O는 가설이므로 증거 첨자 대신 ᶠ 표기, 색 규칙 변경: 세 원인 모두 ◎ᶠ인 cause-robust 셀만 딥 그린(M3 별동대·M4 옵션 캐파·M6 EWI — 프레임워크 불변 전략과 일치). 원인 의존 셀(M1·M2·M5)은 셀 내부를 원인별 변이 칩으로 분해(matrix 렌더러 parts 확장). 채움률 13/24=54%. 패치 v2.38.1.',
    tags: ['스토리라인', 'CMO', '예측 매트릭스', '다운사이클 원인', 'cause-robust', 'dashboard'],
    items: [
      { label: '원인 3종 표', detail: '경로·조기 확인 트리거(FCF 반전/CapEx -25%/GPU 임대가 -35% · 증설 공시/재고/CXMT 캐파 · 전력 50%/CXL 채택)·시나리오 연결(C·D / A·B 조정 / E)' },
      { label: 'cause-robust 판정', detail: 'M3(별동대)·M4(옵션 캐파+재무 요새)·M6(EWI)만 세 원인 모두 ◎ᶠ — Robust 개념의 매트릭스 재확인(통찰 ⑤). M1 계약 바닥은 ①②◎ᶠ이나 ③전환발엔 갱신 시점 커버리지 공동화 △ᶠ' },
      { label: '원인 의존 셀 분해', detail: 'M1×C4-c·M2×C4-a·M5×C4-a — 셀 내부를 ①②③ 변이 칩(개별 톤)으로 렌더링, 실행 순서 각주("M6 상시 → M1 즉시 → 나머지 트리거 발동 시")' },
      { label: '렌더러 확장', detail: 'Storyline.jsx matrix 블록에 cell.parts 지원 — 원인별 미니 행을 톤별 배경으로 스택' },
    ],
    links: [
  ...
  // ── 2026-08-08 ───────────────────────────────────────────────────────────────
  {
    ...
    type: 'build',
    version: 'v2.38.0',
    title: 'CMO 렌즈 M×C→O 매트릭스 3종 — 3에이전트 협업(조사·방법론·정리) + A5 판정 강등',
    summary:
      '요청("다운턴별로 행=M·열=C·셀=O 매트릭스, 효과 있었던 것만 컬러로 눈에 띄게, 방법론 전문가·자료 조사·정리 3에이전트 체제, 삼성의 실제 M 면밀 조사")에 따라 제작. ① 조사 에이전트: 삼성 다운턴 실제 액션 웹 리서치 → 신규 소스(2023-04-07 감산 공식화 선회·2019 HBM팀 축소·2008 SanDisk 인수 시도·철회·Line-16 역사이클 12조·HDD 매각·엘피다 입찰 불참·2023 CapEx 53.1조/R&D 28.34조 사상 최대). ② 방법론 에이전트: 리얼리스트 평가 규격(C열 4축·M/C 판별·O셀 템플릿·◎△✕— 4등급+증거 첨자·채움률 40~60%·생존자 편향 대조 각주). ③ 정리 에이전트: 매트릭스 3종 작성(각 5행×4열·채움률 40%). 검증 후 통합 — 핵심 정합: 매트릭스가 실증한 소모전 불발(무감산→2023-04 자진 철회)에 따라 §5 A5 판정 △→✕ 강등. 대시보드 matrix 렌더러 신설(◎ primary만 딥 그린 강조·5톤·범례·각주). 마이너 v2.38.0(새 시각화·데이터 카테고리).',
    tags: ['스토리라인', 'CMO', 'M×C 매트릭스', '멀티에이전트', '감산 선회', 'A5 강등', 'dashboard'],
    items: [
      { label: 'CMO-1 (2007~09)', detail: '5행(무감산 소모전·40nm 선행·조직 통합·SanDisk 시도→철회·역사이클 증설)×4열 — ◎ 6(소모전 완전 발화·40nm·조직 통합·2010 증설)·△ 2·✕ 0' },
      { label: 'CMO-2 (2010~13)', detail: '5행(Line-16·세대 선행·포트폴리오 재배분·HDD 매각·[배분] 엘피다 불참)×4열 — ◎ 4·△ 4·✕ 0. 엘피다 불참=다운턴 저가 매수 불행사(마이크론 인수와 대조)' },
      { label: 'CMO-3 (2022~23)', detail: '5행(A5 무감산→감산 선회 전 궤적·감산 분리 CapEx/R&D 역사이클·Taylor·A6 HBM 후순위·조직 무대응)×4열 — ◎ 1(투자·R&D 분리 집행)·△ 4·✕ 3. 소모전 불발·자진 철회가 ✕¹ 1차 증거' },
      { label: 'A5 판정 강등', detail: '§5 액션 판정 표 A5: △ 조건부 → ✕ 불발·자진 철회 (2023-04-07 감산 공식화 — Q1 영업이익 -96%·DS -4.58조). §7 3순위·§2 요약 표·도출 원리 동반 정합' },
      { label: '신규 소스', detail: 'sources/articles/samsung-downturn-actions-2007-2023-2026-08-07.md — Research Agent 수집(해석 없음), index.md 등록' },
      { label: 'matrix 렌더러', detail: 'Storyline.jsx 신규 블록 — clear(딥 그린+흰 글자, 유일하게 튀는 층)·clearSoft(참조 셀)·partial·adverse(좌측 레드 보더)·none 5톤 + 범례 + 고정 각주(반사실 한계·C 교호작용·채움률)' },
    ],
    links: [
      { label: 'storyline-cmo.md', href: 'https://github.com/k31001/action-learning/blob/main/wiki/storyline/storyline-cmo.md' },
      { label: 'samsung-downturn-actions-2007-2023-2026-08-07.md', href: 'https://github.com/k31001/action-learning/blob/main/sources/articles/samsung-downturn-actions-2007-2023-2026-08-07.md' },
    ],
  },
  // ── 2026-08-07 (iv) ──────────────────────────────────────────────────────────
  {
