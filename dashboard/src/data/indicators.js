...
export const INITIAL_INDICATORS = [
  ...
  // ────────────────────────────────────────────
  {
    ...
    history: [
      ...
      { date: '2026-09-08', value: -20, note: '[정기 점검] 09-08 리서치 결과 상충 — 한쪽은 장기 하락(Vast.ai 스팟 저점 $1.49) 보고, 다른 쪽은 "Blackwell 가용 확대에도 2026년 대부분 기간 임대가 상승세"로 보고(관측 기간·바스켓 차이 추정). -35% 급락 임계는 어느 해석으로도 미도달, 조기경보 미발동. Vast.ai 자동 갱신 값으로 재확인 필요' },
    ],
  ...
  },
  {
    ...
    history: [
      ...
      { date: '2026-09-08', value: 40, note: '[정기 점검] HYG 프록시 자체는 소폭 타이트닝(HY OAS 285→266~275bp)이나, 프록시가 못 잡는 개별 신용 이벤트 급증 — CoreWeave 5년물 CDS 디폴트확률 ~50%·Oracle CDS 7년 최고(75bp)+S&P BBB- 강등(정크 한 단계 위)·$16.3B 부채딜에 은행 이탈→PIMCO 앵커·하이퍼스케일러 IG채권 발행 YoY+1,300%. +150bps 트리거는 미도달이나 프록시 한계가 처음으로 뚜렷해짐 — 개별 CDS를 보조 지표로 병행 추적 권고' },
    ],
    note: 'capex 상당분이 부채·SPV·ABS(Meta-Blue Owl, Oracle, CoreWeave). 스프레드 확대 = 신규 빌드 조달 위축. [2026-09-08] 다운턴 센싱 관전 포인트 ②(하이퍼스케일러 자금 구조) 축으로 승격(송용호 인터뷰 09-03) — 그동안은 자체 캐시플로우 투자였으나 FCF가 네거티브로 가면 순서대로 대출→지분 매각→NVIDIA가 금융업체처럼 자금을 대는 악성 체인 가능성. 이창수 "꼭짓점은 CapEx가 아니라 FCF" 프레임과 정합. 스프레드 확대는 이 체인의 실측 선행 신호 → demand_inflection_divergence와 함께 감시. 같은 날 5개 병렬 리서치로 CoreWeave CDS 50%·Oracle 강등 등 개별 이벤트 확인(HYG 프록시가 못 담는 리스크) — [september-2026-market-update-2026-09-08.md] 참조',
    inputType: 'number',
