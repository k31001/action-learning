// EWI 초기 데이터 — 시나리오 플래닝 보고서 기준 (2026-05-31)

// ── 4사분면 포지션 히스토리 ────────────────────────────────────────────────────
// DF1 (df1): AI 수요 강도  -10(버블붕괴) ~ +10(슈퍼사이클)
// DF2 (df2): 디커플링 강도 -10(완전공존) ~ +10(전면디커플링)
export const INITIAL_QUADRANT_POSITIONS = [
  { key: 'twoYear',    date: '2024-05', df1: 3.5,  df2: 4.5,  note: 'H100 CapEx 급증 시작, VEU 완화' },
  { key: 'oneYear',    date: '2025-05', df1: 5.0,  df2: 6.0,  note: 'HBM3E 품질 이슈, 삼성 점유율 17%' },
  { key: 'sixMonth',   date: '2025-11', df1: 5.5,  df2: 2.5,  note: 'H20 부분 재허용, 삼성 35% 반등, 디커플링 완화' },
  { key: 'threeMonth', date: '2026-02', df1: 6.5,  df2: 1.5,  note: 'MATCH법안 위원회 미통과, 공존 신호 강화' },
  { key: 'oneMonth',   date: '2026-04', df1: 7.5,  df2: 0.5,  note: '빅테크 Q1 실적 호조, 관리된 공존 신호' },
  // 최신 기준 (2026-07-14 재평가) — DF1 8.5 유지·DF2 1.0→0.5 (관리된 공존 쪽으로 소폭 이동). 직전 07-07 재평가(DF1 8.5·DF2 1.0) 이후 수집분 반영.
  //   유일한 물질적 신규 소스: 애플–CXMT 건 (FT 2026-07-08, 07-11 ingest). DF1(수요 강도)에는 구조 변화 없음(범용 DRAM +55~60% 급등은 기존 슈퍼사이클과 정합) → DF1 8.5 정점 유지.
  //   DF2 하향(관리된 공존 쪽)의 근거: (a) 애플이 중국 내수용 기기에 中 CXMT DRAM 기술 검증 착수 + 미 행정부에 사용 승인 로비 — 프리미엄 고객이 중국 공급사를 조달 후보로 세우는 것 자체가 공존 정합 발전.
  //     (b) 로이터(2026-06): 트럼프 행정부가 대베이징 긴장 회피 차원에서 CXMT 추가 블랙리스트 등재 보류 = 디에스컬레이션. → 실현된 사실 2건 모두 공존 쪽 약한 신호.
  //   절제: 아직 '테스트' 단계(양산 채택·승인 미확정)·1260H 리스트 등재·의회 반대·YMTC 2022 무산 전철 리스크 상존 → 리트머스(승인/차단) 미해결이므로 이동폭 소폭(1.0→0.5)에 그침.
  //   하방 병기(공급 씨앗): CXMT 글로벌 DRAM 캐파 점유 11%(2025)→15%(2028E)·애플 price leverage로 삼성·SK 과점 '균열 신호' — 중기 마진 리스크로 EWI 감시(cxmt_apple_qualification·cxmt_dram_share).
  // [2026-07-28 정기 재평가] DF1 8.5·DF2 0.5 유지 — 07-21 이후 git log 변경분은 전부 실행/조직 계층(개발실 전환 발표·수주산업화 8개 전략 옵션 질의)으로 거시 축(DF1·DF2) 무영향. 웹 리서치로 확인한 in-window 신호는 전부 정점 재확인 방향:
  //   (상방 확인) Alphabet Q2(2026-07-22) 2026 CapEx 가이던스 $180~190B→$195~205B 상향·Cloud 백로그 QoQ +$50B→$514B(CNBC·Seeking Alpha). (정점 확인) SK하이닉스 Q2(~07-28) 영업이익 ~KRW 64T(~$43.7B)·OPM ~76% 사상 최대(Korea Times·TrendForce). GPU 임대가 firming/flat(H100 ~$3.46·H200 ~$4.11)·DRAM Q3 +13~18% 유지.
  //   이미 정점(8.5)이므로 Alphabet 상향은 신규 상방 레그가 아닌 정점 재확인 → DF1 8.5 유지. 절제 이유: 가장 큰 3개 tell(MSFT·Meta 07-29·Amazon 07-30)이 창 직후 발표 예정 — DF1 핵심 확인 이벤트가 다음 주 대기.
  //   DF2: 창 내 미중 실현 사실 부재(MATCH 위원회 단계 유지·CXMT HBM 미확정·애플 CXMT 테스트 단계·신규 수출통제 없음) → 방향·위치 무변화. 삼성 HBM4 볼륨 발주 미전환도 무변화(EWI 추적).
  // [2026-07-21 정기 재평가] DF1 8.5·DF2 0.5 유지 — 직전 07-14 재평가 이후 신규 거시 축 신호 부재. 이번 주 물질적 발전은 삼성 개별 실행 신호(HBM4 인증 통과에도 볼륨 발주 미전환, Winbuzzer 07-17)로 거시 축이 아닌 EWI·트리거·시나리오 B 실행 리스크로 반영.
  //   확증(무변화) 신호: 빅테크 2026 CapEx ~$725B(+77% YoY)·GPU 임대가 firming(H200 +8% YoY)·DRAM Q3 +13~18% 감속(정제)·MATCH 위원회 단계 유지 → DF1 정점·DF2 방향 모두 유지.
  // [2026-08-04 정기 재평가] DF1 8.5·DF2 0.5 유지 — 07-28 이후 git log 변경분은 내부 인터뷰 2건(최장석 상무 07-29·이창수 부사장 08-03)뿐. 거시 축은 "실현된 외부 사실"이 움직인다는 일관 방법론에 따라 내부 전문가 프레이밍으로 분류.
  //   내용도 양면적: (상방·바닥 경직화) 이창수 take-or-pay 멀티이어(선수금 수백억 달러 예치)·NTB(Not-To-Below) 가격 하한·NTE 상한으로 컬랩스 와도 상당 이익률 바닥 계약 고정 = 기존 LTA→SCA 락인의 삼성 1차 자료 재확인, 핸드투마우스(재고 없음)로 근단기 수급 견조.
  //   (하방·유보) 최장석 HBM 편중 다운사이드(HBM 꺼지면 캐파 stranded·HBM↔DDR 상쇄로 shortage→oversupply 반전, D/S 60~70%)·이창수 중복수요·NAND 조정 우려(YMTC 에코 확대, 내년 어느 시점)·원가 비교열위. 영업 수장 본인 "충분히 올랐다·파티할 때 아니다·2차 방어선 필요"로 정점+경계 톤.
  //   순효과: 바닥 경직화(상방)와 HBM편중·중복수요·NAND(하방)가 상쇄 → 정점 재확인, 거시 축 무이동. 커스텀 HBM 퇴조(이창수) vs zHBM 커스텀 시대(최장석) 논쟁은 DF3/제품믹스 축(EWI custom_hbm_revenue_share 병기)이지 DF1·DF2 아님.
  //   DF2: 신규 미중 실현 사실 부재 — 이창수 "중국 비동조화(미주 꺼져도 자동 연동 아님)"·미주 집중+중국 페이즈2 멀티이어는 관리된 공존(0.5) 정합 전망 → 위치·방향 유지.
  // [2026-08-11 정기 재평가] DF1 8.5·DF2 0.5 유지 — 08-04 이후 git log 변경분은 전부 제품믹스(SSD)·역사 리서치·외부 전문가 프레이밍 계층:
  //   enterprise SSD 1Q26 실측(삼성 1위 38.2%·Top5 $18.46B QoQ +86.1% 사상 최대)·CMO 다운턴 역사 렌즈·메모리 3사 CAPEX 역사·크리스 밀러(Chip War)·Sachin Katti(OpenAI) 인터뷰.
  //   거시 축은 "실현된 외부 사실"이 움직인다는 일관 방법론상 이들은 제품믹스(DF3/NAND)·역사·전문가 해석으로 분류 → DF1 무이동. 삼성 SSD 1위는 AI-스토리지 NAND 강세 확인이나 삼성 개별 실행 신호(EWI enterprise_ssd_tam).
  //   [핵심 신규 실현 사실] 07-28 재평가가 "다음 주 DF1 핵심 확인 이벤트"로 지목한 MS·Meta·Amazon Q2(07-29·07-30) 실적 실현 — 4사 CapEx 전원 상향(Amazon 2026 $200B→~$220B·MS FY27 $255~260B·Meta 하한 $130~145B·Google $195~205B, 합산 ~$745~750B ≈ +82% YoY)·삭감 0건.
  //     이미 정점(8.5)이므로 전원 상향은 신규 상방 레그가 아닌 재확인 → DF1 8.5 유지.
  //   [후기순환 tell 실측 등장] 08-04가 이창수 프레임("진짜 꼭짓점은 CapEx가 아니라 FCF")으로 선행 관전 대상 지목한 CapEx-vs-FCF 다이버전스가 Q2 실적에서 가시화 — Meta FCF -91%→$784M·Amazon TTM FCF 마이너스 전환(~-$7.6B).
  //     단 수요 붕괴가 아닌 조달 구조 경보(CapEx 자체 상승·핸드투마우스 재고 없음·Google Cloud 백로그 $514B 견조) → 축 무이동, EWI(demand_inflection_divergence·bigtech_capex_growth FCF 렌즈) 감시 강화로 반영.
  //   GPU 임대가 firming/flat(H100 ~$2.69·H200 ~$4.38, 붕괴 없음)로 수요 변곡 조기경보 미발동. DF2: 신규 미중 실현 사실 부재(크리스 밀러 "초크포인트 GPU→HBM 이동"은 외부 해석·MATCH 본회의 movement 없음·CXMT HBM 미확정) → 방향·위치 유지.
  // [2026-08-25 정기 재평가] DF1 8.5·DF2 0.5 유지 — 08-18 이후 git log 변경분(v2.46.2~v2.46.9·v2.42.1)은 전부 ② 역사 리서치(지난 20년 다운턴 복기 전체 메모리 D+N 통합 산점도 전환·NAND 분기 낙폭 4Q08 -19.3%·4Q22 -25% 검증) ③ SP-2 다운턴 트랙 심화(시나리오별 삼성 영향 진단 — S/W 코드 고정 좌표계, 별도 축 DF-D1×DF-D2라 SP-1 무이동) ④ 전략·outputs(FDP 생태계 확산·락인 실행전략 3종) ⑤ 발표 층(PPTX 확률 표기 제거 — 발표/분석 층 분리·영향 진단 슬라이드 리디자인)으로 신규 실현된 외부 거시 수요 사실 0건 → 축 무이동.
  //   08-25 웹 리서치 in-window 신호도 전부 정점 재확인·정제: 빅테크 4사 2026 CapEx ~$725B(+77% YoY)·삭감 0건(08-11 스냅샷 무변화)·DRAM Q3 계약가 +13~18%·NAND +10~15% QoQ(Q2 +58~63%/+70~75% 대비 감속=소비자 지불한계 정제, 서버·AI 견조)·HBM4 가격 전세대 +30%. 직전 실현 외부 데이터는 여전히 Q2 하이퍼스케일러 실적(08-11) → DF1 8.5 정점 유지.
  //   DF2: MATCH(H.R.8170) 04-22 위원회 보고 후 본회의 표결 movement 없음·애플–CXMT '테스트'→'인증(qualifying)' 소폭 진전했으나 백악관 사인오프 대기·Schumer 반대 서한(07-29) 역풍으로 리트머스 미해결 → DF2 0.5 방향·위치 유지(EWI cxmt_apple_qualification 갱신). CXMT BIS 엔티티 리스트 미등재. 발동 트리거 0건.
  // [2026-08-18 정기 재평가] DF1 8.5·DF2 0.5 유지 — 08-11 이후 git log 변경분(v2.40.0~v2.46.0)은 전부 다섯 계층: ① 제품믹스·삼성 실행(SSD·UFS 축 보강 v2.42.4·삼성 SSD design win v2.42.5·CAPEX 탭 솔루션 오버레이 v2.46.0) ② 역사 리서치(CMO 통합 매트릭스 v2.41.0·2019 다운사이클 편입 v2.44.0·CAPEX 히스토리 2006 소급 v2.42.2·시장 전환기 전략 v2.42.6) ③ 신규 분석 트랙(SP-2 다운턴 시나리오 플래닝 v2.42.0 — 별도 축 DF-D1×DF-D2, SP-1 거시 축 무영향) ④ outputs/보고서(2차 저지선 전략·키엔스 벤치마크·SSD 전략적 방향성 보고서 v1.1) ⑤ 내부 문서(회의록 메뉴 v2.40.0).
  //   거시 축은 "실현된 외부 사실"이 움직인다는 일관 방법론상 위 전부 제품믹스(DF3/NAND)·역사·내부 분석·outputs로 분류 → 신규 실현된 외부 거시 수요 사실 0건. 직전 실현 외부 데이터는 여전히 Q2 하이퍼스케일러 실적(08-11) → DF1 8.5 정점 유지, 축 무이동.
  //   [이번 사이클 최대 산출물] SP-2 다운턴 트랙 신설 + CMO 다운턴 역사 매트릭스(관측 3→4개)는 다운턴 대비·감별 차원을 심화하나 설계상 별도 축(DF-D1 발원지×DF-D2 속도)을 써 SP-1 DF1 무이동 — 공급발 다운턴은 A·B 사분면 안에서도 발생하므로 두 트랙 병존이 방법론적으로 정합.
  //   [삼성 실행 진전] SSD design win(PM1763 양산 개시 2026-07-08 공식·CMX V-NAND 캐파 60%+ NVIDIA 배정·DGX Spark PM9E1 실탑재)은 AI-스토리지 NAND 수요·삼성 실행 진전 확인이나 삼성 개별 실행 신호(DF3/NAND)라 거시 축 아님 → EWI(samsung_ai_ssd_position·nvidia_storage_next_partner·enterprise_ssd_tam) 갱신으로 반영.
  //   DF2: 08-11 이후 신규 미중 실현 정책 사실 0건(MATCH 본회의 movement 없음·CXMT HBM 미확정·애플–CXMT 테스트 단계·신규 수출통제 없음). SP-2에서 DF2는 배경 변수로 강등되어 SP-1 위치 무영향 → DF2 0.5 방향·위치 유지. 발동 트리거 0건.
  { key: 'current',    date: '2026-09', df1: 8.5,  df2: 0.5,  note: '[09-22 재평가] DF1 8.5·DF2 0.5 유지 — 09-15 이후 git log 커밋은 전부 QLC eSSD 덱/보고서/위키(v2.46.18~v2.46.40) 내부 제품믹스(DF3/NAND)·발표 층으로 신규 실현 외부 거시 사실 0건 → 포지션 맵 무이동, 이번 사이클 갱신은 EWI 웹 리서치 in-window 신호 기반. DF1(수요) 정점 재확인: 빅테크 4사 2026 CapEx ~$725B(+77% YoY·삭감 0건)·2027 전망 상향 유지(JPM·Goldman·Evercore·BofA >$1T — 36.5% 성장만으로 $1조 도달)·DRAM Q3 계약가 +13~18%·NAND +10~15%·소비자 DRAM 부족 DDR2까지 확대·구조적 쇼티지 2027 지속·HBM 계약가 2027 multiples 상향·GPU 임대가 firming(H100 9월 온디맨드 중앙값 ~$3.25/GPU-h·H200 ~$4.40/GPU-h — 붕괴 없음)·Micron Q4 FY26 실적 09-30이 다음 확인점. 후기순환 조달/파이낸싱 tell 심화(수요 붕괴 아님): NVIDIA-OpenAI-Oracle $745B 순환 파이낸싱 루프가 첫 스트레스 테스트(금리) 직면·Oracle $43B 차입으로 마이너스 FCF 커버·OpenAI 2026 매출 ~$20B·손실 ~$14B·AI 순환경제 버블 인덱스 73/100·NVIDIA $105B OpenAI 베팅(CFO Kress "once-in-a-generation 플랫폼 전환"으로 방어). CapEx 자체 상향·재고 없음이라 조달 구조 경보로 EWI 갱신(ai_dc_credit_spread·enterprise_ai_roi_proven·demand_inflection_divergence), 축 무이동. DF2(디커플링): Trump-Xi 정상회담 09-24 워싱턴 개최 확정(2일 후) — 3T(무역·기술·타이완) 의제, 기술이 AI 급진전으로 긴급성 부각·시장은 건설적 결과 선반영(반도체·아시아 기술주·위안·홍콩 강세)·Busan 휴전 연장이 최선 기대·베이징은 미 규칙 집행 연기 압박·워싱턴은 희토류 정상화 요구·MATCH 법안 의회 진전(150일 동맹 정렬 조항)·베이징 비판 격화. 09-15가 지목한 "9월 정상회담=다음 분기점"의 날짜가 09-24로 확정·임박했으나 타결·라이선스 언어 변경·MATCH 통과·신규 통제 등 실현된 정책 사실은 09-24 이후 확정 → 리트머스 미해결로 DF2 0.5 유지, 양방향 트리거 대기(완화=us_china_semiconductor_deal / MATCH 진전·결렬=match_act_passed). 삼성 HBM4 볼륨 발주 여전히 미확보(유상 평가 단계)·16-Hi HBM4 삼성 skip(SK 볼륨·Micron 샘플)·Micron HBM 캐파 2배 증설 = B 시간 리스크 지속. 발동 트리거 0건. [09-15 재평가] DF1 8.5·DF2 0.5 유지 — 09-08 이후 git log 커밋 0건(신규 ingest·외부 소스 0건)이라 이번 사이클 갱신은 EWI 웹 리서치 in-window 신호 기반(포지션 맵·확률은 "실현된 외부 거시 사실"이 움직인다는 일관 방법론상 무이동). DF1(수요) 정점 재확인: 빅테크 4사 2026 CapEx ~$725B(+77% YoY·삭감 0건)·JPM 2027 ~$1T·Goldman 2027 $1.01T 상향·DRAM Q3 계약가 +13~18%·NAND +10~15%·구조적 쇼티지 2027까지 지속(TrendForce/SK하이닉스)·GPU 임대가 firming(H100 온디맨드 ~$2.65~2.89/h·H200 +20~40%·"메모리 병목이지 실리콘 병목 아님")·Micron Q4 FY26 가이던스 $50B·HBM 10만 wpm 램프(09-30 실적이 다음 DF1 확인점). 후기순환 조달·파이낸싱 tell 심화(수요 붕괴 아님): NVIDIA의 OpenAI 오하이오 DC 파이낸싱 개런티 $250B→~$105B(<$120B) 축소(Fortune "보도 대비 $145B↓·인공 수요 우려 신호")·Oracle 주가 YTD -24%·HSBC "OpenAI 2030년 $200B 매출에도 +$207B 추가 조달 필요" → 송용호 관전 ②(FCF→대출→지분 매각→NVIDIA 금융업체화)·①(AI ROI/수익화 한계)의 실측 보강. CapEx 자체는 상향·핸드투마우스 재고 없음이라 조달 구조 경보로 EWI 갱신(ai_dc_credit_spread·enterprise_ai_roi_proven·demand_inflection_divergence), 축 무이동. DF2(디커플링): 9월 미중 정상회담 개최 국면 진입 — MATCH 법안이 가장 절차적으로 진전된 기술 파일로 의제 상정·베이징 비판 격화. 09-08이 지목한 "9월 정상회담=다음 분기점"이 실제 열렸으나 타결·MATCH 본회의 통과·신규 수출통제 등 실현된 정책 사실 미확정 → 리트머스 미해결로 DF2 0.5 방향·위치 유지, 결과에 따라 양방향 트리거 대기(완화=us_china_semiconductor_deal / MATCH 진전=match_act_passed). 애플–CXMT 9월 승인/차단 미확정(Schumer 08-21 시한 경과)·CXMT $42억 상하이 IPO(DRAM 증설 자금 조달)는 공급 씨앗(시나리오 D 배경 재확인, 기 반영). 발동 트리거 0건. [09-08 재평가] DF1 8.5·DF2 0.5 유지 — 09-01 이후 git log 변경분은 전부 내부 인터뷰(송용호 부사장 AX/PI센터장 09-03)·제품믹스 리서치(대용량 QLC eSSD 벤더별 타임라인·FDP RUH 갭 09-02)·발표 층(향후 계획 덱·내부 인터뷰 인용문 덱·FDP 컨설팅 덱)으로 신규 실현 외부 거시 사실 0건 → 축 무이동. 송용호 인터뷰는 다운턴 시뮬레이션·기본기(품질·원가)·조직 체질 층위의 내부 전문가 프레이밍으로, 그의 자체 관측(가격 피크 26년 가능성·28~30 슬로우 가능·단 AI 수요는 아직 갈 길 멀어 존재)은 이미 DF1 8.5 후기순환 정점·D 시나리오 2027~2028 조정 타이밍과 정합 → DF1 8.5 정점 재확인. 다운턴 센싱 3대 관전 포인트(① AI 기술 ROI 진보 지속성 — OpenAI 광고 진입·IPO는 한계 인식 신호 ② 하이퍼스케일러 자금 구조 — FCF 마이너스→대출→지분 매각→NVIDIA 금융업체화 ③ 전기 — 美 27~28 쇼티지가 DC 건설 속도 지연 가능)와 중국 만성적 공급과잉·가격 통제 프레임(5% 오버서플라이면 가격 하락·중국 점유 10%·소비 50%·정부가 뒷배)은 시나리오 상대 확률이 아니라 하락 변곡 선행 EWI·트리거로 반영(enterprise_ai_roi_proven·ai_dc_credit_spread·demand_inflection_divergence·dc_cancellation_count·cxmt_asp_gap 노트 갱신). QLC eSSD 타임라인(245TB 첫 상용 출하 Micron 2026-05·삼성 용량 리더십 후발·CMX 타깃 200+ FDP 스트림·AI 서버 시장 $187B·서버가치 65%)은 DF3/NAND 제품믹스 신호로 EWI(enterprise_ssd_tam·nvidia_storage_next_partner) 갱신. DF2: MATCH 본회의 movement 없음·애플–CXMT 리트머스 미해결·신규 수출통제 없음(09-01 무변화, 9월 시진핑 방미 정상회담이 다음 분기점). 발동 트리거 0건. [09-01 재평가] DF1 8.5·DF2 0.5 유지 — 08-25 이후 git log 변경분은 전부 SP-2 다운턴 트랙(다섯 시나리오 발현 근접도 dt-b/c/d/e-signals 08-28·별도 축 DF-D1×DF-D2)·전략/FDP(FDP 협업 3유형·스토리지 벤더 딜 구조·KV Cache SSD 수요)·역사 리서치(2023 NAND 다운턴 벤더 데이터)·발표 층(영향 덱 6p)으로 신규 실현 외부 거시 사실 0건 → 축 무이동. 08-28 수집 in-window 신호도 정점 재확인·유보: HBM 비트수요 성장 감속(2025 +130%→2026 +70%→2027 +50~60%, 단 여전히 공급부족)·TrendForce 메모리 시장 2027 $1.28T 상향(Agentic AI 구조적 확장)·재고 역대최저(DRAM 2~3주·NAND 3~4주)·SEMI 장비지출 사상최대(2026 $133B·2027 $151B)·NVIDIA Rubin Ultra HBM 구성 축소 검토(공급 타이트발, 수요 붕괴 아님)·토큰 소비 폭증(구글 3.2 quadrillion/월 +7배, Jevons 상쇄). DF2: MATCH 본회의 movement 없음·애플–CXMT 인증(qualifying) 진전(CXMT DDR5 수율 >90%·HP/Asus/Acer 인증 완료)했으나 Schumer 반대 서한 답변시한(08-21) 경과·백악관 사인오프 대기·9월 시진핑 방미 정상회담 관측(저신뢰)으로 리트머스 미해결 → DF2 이동 근거 미충족(EWI 갱신). 발동 트리거 0건. [08-25 재평가] DF1 8.5·DF2 0.5 유지 — 08-18 이후 git log 변경분(v2.46.2~v2.46.9·v2.42.1)은 전부 역사 리서치(지난 20년 다운턴 복기 D+N 통합 산점도·NAND 분기 낙폭 검증)·SP-2 삼성 영향 진단(S/W 코드 좌표계, 별도 축)·FDP 생태계 실행전략·발표 층 분리(PPTX 확률 표기 제거)로 신규 실현 외부 거시 사실 0건 → 축 무이동. 08-25 웹 리서치 in-window 신호도 정점 재확인·정제(CapEx ~$725B·삭감 0건·DRAM Q3 +13~18%·NAND +10~15% QoQ 감속·HBM4 +30%). DF2: MATCH 본회의 movement 없음·애플–CXMT 인증(qualifying) 진전했으나 백악관 사인오프 대기·Schumer 반대 서한으로 리트머스 미해결(EWI 갱신). 발동 트리거 0건. [08-18 재평가] DF1 8.5·DF2 0.5 유지 — 08-11 이후 git log 변경분(v2.40.0~v2.46.0)은 전부 제품믹스·삼성 실행(SSD·UFS·CAPEX 솔루션)·역사 리서치(CMO 매트릭스·2019 편입·CAPEX 히스토리)·신규 분석 트랙(SP-2 다운턴 — 별도 축 DF-D1×DF-D2)·outputs/보고서(2차 저지선·SSD 전략)·회의록으로 신규 실현된 외부 거시 사실 0건 → 축 무이동. 이번 사이클 최대 산출물 SP-2 다운턴 트랙은 다운턴 대비 심화이나 별도 축이라 SP-1 무이동. 삼성 SSD design win(PM1763 양산·CMX·DGX Spark)은 삼성 개별 실행 신호(DF3/NAND)라 EWI 갱신으로 반영. DF2 신규 미중 실현 사실 부재. 발동 트리거 0건. [08-11 재평가] DF1 8.5·DF2 0.5 유지 — git log 변경분은 제품믹스(SSD 1Q26 삼성 1위 38.2%)·역사 리서치·외부 전문가 프레이밍(크리스 밀러·Sachin Katti)으로 거시 축 무이동. 07-28이 지목한 확인 이벤트(MS·Meta·Amazon Q2 07-29·07-30) 실현 — 4사 CapEx 전원 상향(합산 ~$745~750B·+82% YoY)·삭감 0건 = 정점 재확인. Meta FCF -91%·Amazon TTM FCF 마이너스 전환의 CapEx-vs-FCF 다이버전스 실측 등장은 후기순환 tell이나 조달 경보(수요 붕괴 아님)라 축 무이동·EWI 감시 강화. GPU 임대가 firming/flat. [08-04 재평가] DF1 8.5·DF2 0.5 유지 — git log 변경분은 내부 인터뷰 2건(최장석 07-29·이창수 08-03)뿐, 내부 전문가 프레이밍으로 거시 축 무이동. 양면적: take-or-pay/NTB 바닥 경직화(상방 재확인) vs HBM 편중·중복수요·NAND 조정(하방 유보) 상쇄 → 정점 재확인. DF2: 이창수 중국 비동조화·미주 집중은 관리된 공존 정합. [07-28 재평가] in-window 신호 정점 재확인: Alphabet CapEx 상향·SK하이닉스 사상 최대. [07-14 재평가] DF2 1.0→0.5 — 애플–CXMT 건이 관리된 공존 쪽 약한 신호' },
]

// 시나리오 확률 — wiki/scenarios/scenario-matrix.md 기반 (2026-09-15 정기 재평가 — 유지)
//   A 25~29 / B 35~39 / C 6~10 / D 19~23 / E 5~8  → 합 100
//   [2026-09-22 정기 재평가 — 유지] 09-15 이후 git log 커밋은 전부 QLC eSSD 덱/보고서/위키(v2.46.18~v2.46.40) 내부 제품믹스(DF3/NAND)·발표 층으로 신규 실현 외부 거시 사실 0건 → A26·B39·C8·D21·E6 전부 유지, 발동 트리거 0건. 이번 사이클 갱신은 EWI 웹 리서치 in-window 신호 기반이며 방법론상 상대 확률을 움직이지 않음.
//     DF1 신호는 전부 정점 재확인·후기순환 조달 경보: 빅테크 CapEx ~$725B(+77%)·2027 >$1T 상향(JPM·Goldman·Evercore·BofA)·삭감 0건·DRAM Q3 +13~18%·NAND +10~15%·GPU 임대가 firming(H100 ~$3.25/h·H200 ~$4.40/h)·구조적 쇼티지 2027 지속은 이미 정점(DF1 8.5)인 축의 재확인. NVIDIA-OpenAI-Oracle $745B 순환 파이낸싱 첫 스트레스 테스트·Oracle $43B 차입·OpenAI 손실 ~$14B·버블 인덱스 73/100은 후기순환 조달/ROI 경보이나 수요 붕괴(D 재배분)가 아닌 조달 구조 경보(CapEx 상향·재고 없음) → EWI 감시 강화, 상대 확률 무영향.
//     DF2: Trump-Xi 정상회담 09-24 워싱턴 개최 확정(2일 후) — 09-15가 지목한 분기점의 날짜 확정·임박이나 실현된 정책 사실(타결·통과·통제) 미확정 → A↔B 재배분 미실행, 결과 대기. 삼성 HBM4 볼륨 발주 미전환(16-Hi 삼성 skip·SK 락인 심화) = B 시간 리스크 지속. 발동 트리거 0건.
//   [2026-09-15 정기 재평가 — 유지] 09-08 이후 git log 커밋 0건(신규 ingest·외부 소스 0건) → A26·B39·C8·D21·E6 전부 유지. 이번 사이클 갱신은 EWI 웹 리서치 in-window 신호 기반이며 방법론상 시나리오 간 상대 확률을 움직이지 않음.
//     DF1 in-window 신호는 전부 정점 재확인·후기순환 조달 경보: 빅테크 4사 CapEx ~$725B(+77%)·JPM/Goldman 2027 ~$1T 상향·삭감 0건·DRAM Q3 +13~18%·NAND +10~15%·GPU 임대가 firming·Micron Q4 $50B 가이던스는 이미 정점(DF1 8.5)인 축의 재확인. NVIDIA-OpenAI 오하이오 파이낸싱 개런티 $250B→~$105B 축소·Oracle YTD -24%·HSBC OpenAI +$207B 조달 갭은 후기순환 조달/ROI 경보이나 수요 붕괴(D로의 재배분) 아님(CapEx 상향·재고 없음) → EWI 감시 강화로 반영, 상대 확률 무영향.
//     DF2: 9월 미중 정상회담이 실제 개최 국면(MATCH이 가장 절차 진전된 기술 파일로 의제) — 09-08이 지목한 분기점이 열렸으나 실현된 정책 사실(타결·통과·통제) 미확정 → 재배분 미실행, 결과 대기. 애플–CXMT 리트머스 미해결·CXMT $42억 상하이 IPO는 시나리오 D 배경 강화이나 이미 D 21에 반영된 추세 재확인 — 상대 확률 무영향. 발동 트리거 0건.
//   [2026-09-08 정기 재평가 — 유지] 09-01 이후 git log 변경분은 전부 내부 인터뷰(송용호 부사장 AX/PI센터장 09-03)·제품믹스 리서치(대용량 QLC eSSD 타임라인·FDP RUH 갭 09-02)·발표 층(향후 계획 덱·인용문 덱·FDP 컨설팅 덱)으로 신규 실현된 외부 거시 사실 0건 → A26·B39·C8·D21·E6 전부 유지.
//     송용호 인터뷰는 다운턴 시뮬레이션·기본기(품질·원가)·조직 체질 층위의 내부 전문가 프레이밍으로 방법론상 상대 확률을 움직이지 않음. 그의 자체 관측(가격 피크 26년 가능성·28~30 슬로우 가능·단 AI 수요 아직 존재)은 이미 D 21에 반영된 2027~2028 조정 타이밍·DF1 8.5 후기순환 정점 판정과 정합. 다운턴 센싱 3대 관전 포인트(AI ROI·하이퍼스케일러 자금구조·전기)와 중국 만성적 공급과잉·가격 통제 프레임은 하락 변곡 선행 EWI·트리거로 반영(enterprise_ai_roi_proven·ai_dc_credit_spread·demand_inflection_divergence·dc_cancellation_count·cxmt_asp_gap). QLC eSSD 타임라인(245TB 첫 출하 Micron·삼성 후발·CMX 200+ FDP 스트림·AI 서버 $187B/65%)은 DF3/NAND 제품믹스 신호로 EWI(enterprise_ssd_tam·nvidia_storage_next_partner) 갱신, 상대 확률 무영향. 발동 트리거 0건.
//   [2026-09-01 정기 재평가 — 유지] 08-25 이후 git log 변경분은 전부 SP-2 다운턴 트랙(dt-b/c/d/e-signals 08-28·별도 축)·전략/FDP(FDP 협업 3유형·스토리지 벤더 딜 구조·KV Cache)·역사 리서치(2023 NAND 다운턴 벤더 데이터)·발표 층(영향 덱 6p)으로 신규 실현된 외부 거시 사실 0건 → A26·B39·C8·D21·E6 전부 유지.
//     08-28 수집 in-window 신호도 정점 재확인·유보뿐: HBM 비트수요 성장 감속(2025 +130%→2026 +70%→2027 +50~60%, 여전히 공급부족)·TrendForce 2027 메모리 $1.28T 상향·재고 역대최저·SEMI 장비지출 사상최대·NVIDIA Rubin Ultra HBM 구성 축소 검토(공급 타이트발)·토큰 소비 폭증(Jevons)은 이미 정점(DF1 8.5)인 축의 재확인이지 시나리오 간 상대 확률 변경 요인 아님. DF2: MATCH 본회의 movement 없음·애플–CXMT 인증 진전(CXMT DDR5 수율 >90%·HP/Asus/Acer 인증)했으나 Schumer 서한 답변시한(08-21) 경과·백악관 사인오프 대기로 리트머스 미해결 → 재배분 미실행(EWI cxmt_apple_qualification 갱신). 중국 추격 실측(CXMT DRAM 7.67% 세계4위·YMTC NAND 14% 세계3위)은 시나리오 D 배경 강화이나 이미 D 21에 반영된 추세의 재확인 — 상대 확률 무영향. 발동 트리거 0건.
//   [2026-08-25 정기 재평가 — 유지] 08-18 이후 git log 변경분(v2.46.2~v2.46.9·v2.42.1)은 전부 역사 리서치(지난 20년 다운턴 복기 D+N 통합 산점도·NAND 분기 낙폭 검증)·SP-2 삼성 영향 진단(S/W 코드 좌표계 — 별도 축 DF-D1×DF-D2)·FDP 생태계 실행전략·발표 층 분리(PPTX 확률 표기 제거)로 신규 실현된 외부 거시 사실 0건 → A26·B39·C8·D21·E6 전부 유지.
//     08-25 웹 리서치 in-window 신호도 정점 재확인·유보뿐: 빅테크 4사 CapEx ~$725B(+77% YoY)·삭감 0건·DRAM Q3 +13~18%·NAND +10~15% QoQ(Q2 대비 감속=정제)·HBM4 +30%는 이미 정점(DF1 8.5)인 축의 재확인이지 시나리오 간 상대 확률 변경 요인 아님. DF2: MATCH 본회의 movement 없음·애플–CXMT 인증(qualifying) 진전했으나 백악관 사인오프 대기·Schumer 반대 서한(07-29)으로 리트머스 미해결 → 재배분 미실행(EWI cxmt_apple_qualification 갱신).
//     발표 층 확률 미표기 원칙(사용자 지시 2026-08-25 — 추정 정밀도가 낮아 진단 신뢰를 깎음)은 분석 층 확률 추적과 별개 — SCENARIOS는 분석 층 단일 소스로 유지. 발동 트리거 0건.
//   [2026-08-18 정기 재평가 — 유지] 08-11 이후 git log 변경분(v2.40.0~v2.46.0)은 전부 제품믹스·삼성 실행(SSD·UFS·CAPEX 솔루션 오버레이)·역사 리서치(CMO 통합 매트릭스·2019 다운사이클 편입·CAPEX 히스토리)·신규 분석 트랙(SP-2 다운턴 — 별도 축 DF-D1×DF-D2)·outputs/보고서(2차 저지선·SSD 전략)·회의록으로 신규 실현된 외부 거시 사실 0건 → A26·B39·C8·D21·E6 전부 유지.
//     이번 사이클 최대 산출물인 SP-2 다운턴 트랙은 다운턴 대비 차원을 심화하나 설계상 별도 축을 써 이 매트릭스(SP-1 DF1×DF2) 상대 확률을 바꾸지 않음(공급발 다운턴은 A·B 안에서도 발생). 삼성 SSD design win(PM1763 양산·CMX·DGX Spark)은 삼성 개별 실행 신호(DF3/NAND)라 EWI 갱신으로 반영, 상대 확률 무영향. DF2 무변화(신규 미중 실현 사실 부재). 발동 트리거 0건.
//   [2026-08-11 정기 재평가 — 유지] 08-04 이후 git log 변경분은 제품믹스(SSD 1Q26 삼성 1위 38.2%)·역사 리서치(CMO·CAPEX)·외부 전문가 프레이밍(크리스 밀러·Sachin Katti)으로 거시 축 무이동 → A26·B39·C8·D21·E6 전부 유지.
//     07-28이 지목한 DF1 확인 이벤트(MS·Meta·Amazon Q2 07-29·07-30) 실현 — 4사 CapEx 전원 상향(합산 ~$745~750B·+82% YoY)·삭감 0건은 이미 정점(DF1 8.5)인 축의 재확인이지 시나리오 간 상대 확률 변경 요인 아님(이미 A+B 행 반영).
//     Meta FCF -91%·Amazon TTM FCF 마이너스 전환의 CapEx-vs-FCF 다이버전스 실측 등장은 후기순환 tell이나 수요 붕괴가 아닌 조달 경보 → D로의 재배분 미실행, EWI(demand_inflection_divergence·bigtech_capex_growth FCF 렌즈) 감시 강화로 반영. DF2 무변화.
//   [2026-08-04 정기 재평가 — 유지] 07-28 이후 git log 변경분은 내부 인터뷰 2건(최장석 07-29·이창수 08-03)뿐 — 내부 전문가 프레이밍으로 거시 축 무이동.
//     양면적: 이창수 take-or-pay/NTB 1차 확인은 이미 C 8·D 21에 반영된 LTA→SCA 락인의 재확인(신규 증분 아님)·최장석 HBM 편중 다운사이드는 "AI가 꺼질 확률"이 아니라 "꺼졌을 때 삼성 캐파 리스크"(조건부 심도)라 상대 확률 무변화 → A26·B39·C8·D21·E6 전부 유지.
//     커스텀 HBM 퇴조(이창수) vs zHBM 부상(최장석)은 DF3/제품믹스 신호로 EWI custom_hbm_revenue_share 병기, 시나리오 확률 무영향. DF2 무변화(중국 비동조화·미주 집중은 관리된 공존 정합).
//   [2026-07-28 정기 재평가 — 유지] 07-21 이후 git log 변경분은 전부 실행/조직 계층(개발실 전환·수주산업화 질의)으로 거시 축 무영향.
//     in-window 신호(Alphabet CapEx 상향·SK하이닉스 사상 최대)는 DF1 정점 재확인이지 시나리오 간 상대 확률을 바꾸는 요인이 아님(이미 A+B 행 DF1 8.5 반영). DF2 무변화 → 확률 A26·B39·C8·D21·E6 전부 유지.
//     MSFT·Meta(07-29)·Amazon(07-30) Q2 CapEx 가이던스가 다음 주 확인 이벤트 — 상향 시 B 강화·삭감 시 D 경보로 EWI bigtech_capex_growth 추적.
//   [2026-07-14] 애플–CXMT 건(FT 07-08) 반영 — DF2가 관리된 공존 쪽으로 소폭 이동(1.0→0.5)한 데 따른 순수 DF2 재배분.
//     공존 신호는 AI-지속 행(A+B, DF1 8.5 불변) 내부에서 디커플링(A)→공존(B)으로 확률을 이동시킴 → A 27→26·B 38→39.
//     C·D·E 불변(DF1 무변화·C·D는 이미 낮음·E는 DF3 근거 아님). AI-지속 행 합계 65 유지, C+D+E 합계 35 유지 → 합 100.
//     이동폭은 소폭 — 애플–CXMT는 아직 '테스트' 단계이고 승인/차단 리트머스가 미해결이기 때문(양방향 EWI cxmt_apple_qualification로 추적).
//   직전 [2026-07-07]: A27·B38·C8·D21·E6.
export const SCENARIOS = [
  { id: 'A', name: '황금 요새', probability: 26, color: 'blue', description: 'AI 지속 + 디커플링' },
  { id: 'B', name: 'AI 르네상스', probability: 39, color: 'emerald', description: 'AI 지속 + 공존', mainBet: true },
  { id: 'C', name: '기술 냉전', probability: 8, color: 'red', description: 'AI 붕괴 + 디커플링' },
  { id: 'D', name: '조용한 재편', probability: 21, color: 'orange', description: 'AI 붕괴 + 공존' },
  { id: 'E', name: '패러다임 전환', probability: 6, color: 'purple', description: 'HBM 패러다임 붕괴 (와일드카드)' },
]

// alertCondition:
//   'lte'  — currentValue <= alertThreshold → alert
//   'gte'  — currentValue >= alertThreshold → alert
//   'pct_drop_6m' — 6개월 평균 대비 X% 하락
//   'manual' — 수동 상태 입력

export const INITIAL_INDICATORS = [
  // ────────────────────────────────────────────
  // 월간 모니터링
  // ────────────────────────────────────────────
  {
    id: 'hbm_spot_price_change',
    name: 'HBM 현물 가격 (6개월 변동률)',
    category: 'monthly',
    source: 'DRAM Exchange·TrendForce',
    unit: '%',
    unitLabel: '% (6개월 변동)',
    currentValue: null,
    alertThreshold: -30,
    alertCondition: 'lte',
    alertDescription: '6개월 내 -30% 이하 → 경보',
    scenarioSignals: ['C', 'D'],
    scenarioText: 'C 또는 D 방향 경보',
    warningThreshold: -15,
    status: 'unknown',
    lastUpdated: null,
    history: [],
    note: '',
    inputType: 'number',
    hint: '현재 HBM 현물 가격의 6개월 전 대비 변동률 입력 (예: -12.5)',
    autoUpdateId: 'hbm_spot_price_change',
    autoUpdateSource: 'NVDA 주가 6개월 변동률 프록시 (Yahoo Finance)',
    autoUpdateIsProxy: true,
  },
  {
    id: 'nvidia_hbm_order_change',
    name: 'NVIDIA HBM 수주 계획 변동',
    category: 'monthly',
    source: 'NVIDIA 공급망 채널',
    unit: '%',
    unitLabel: '% (직전 계획 대비)',
    currentValue: null,
    alertThreshold: -20,
    alertCondition: 'lte',
    alertDescription: '-20% 이상 하향 조정 → 긴급 경보',
    scenarioSignals: ['C', 'D'],
    scenarioText: '긴급 경보 — 전략 위기',
    warningThreshold: -10,
    status: 'unknown',
    lastUpdated: null,
    history: [],
    note: '',
    inputType: 'number',
    hint: 'NVIDIA의 HBM 수주 계획 변동률 입력 (예: -5). 하향이면 음수.',
    isCritical: true,
    autoUpdateId: 'nvidia_hbm_order_change',
    autoUpdateSource: 'NVDA 주가 3개월 모멘텀 프록시 (Yahoo Finance)',
    autoUpdateIsProxy: true,
  },
  {
    id: 'samsung_hbm4_yield',
    name: '삼성전자 HBM4 수율 (로직 다이)',
    category: 'monthly',
    source: '삼성전자 내부 KPI',
    unit: '%',
    unitLabel: '%',
    currentValue: 70,
    alertThreshold: 85,
    alertCondition: 'lte',
    alertDescription: '목표 90% 대비 5%p+ 미달 → 즉시 개선팀',
    scenarioSignals: [],
    scenarioText: '즉시 개선팀 소집',
    warningThreshold: 87,
    status: 'warning',
    lastUpdated: '2026-05-05',
    history: [{ date: '2026-05-05', value: 70, note: '보고서 기준 초기값' }],
    note: '2026년 9월 30일까지 90%+ 목표',
    inputType: 'number',
    hint: '현재 HBM4 로직 다이 수율 % 입력 (예: 72.5). 목표: 90%+',
    isCritical: true,
  },
  {
    id: 'bigtech_ai_churn',
    name: '빅테크 AI 구독 해지율',
    category: 'monthly',
    source: '빅테크 각사 공시/추정치',
    unit: '%',
    unitLabel: '% (분기 해지율)',
    currentValue: null,
    alertThreshold: 10,
    alertCondition: 'gte',
    alertDescription: '분기 해지율 10% 초과 → C·D 경보',
    scenarioSignals: ['C', 'D'],
    scenarioText: 'C·D 방향 경보',
    warningThreshold: 7,
    status: 'unknown',
    lastUpdated: null,
    history: [],
    note: '',
    inputType: 'number',
    hint: '빅테크 AI 서비스 분기 해지율 % 입력',
  },

  // ────────────────────────────────────────────
  // 분기별 모니터링
  // ────────────────────────────────────────────
  {
    id: 'bigtech_capex_growth',
    name: '빅테크 4사 AI CapEx 성장률',
    category: 'quarterly',
    source: '빅테크 분기 실적 발표',
    unit: '%',
    unitLabel: '% YoY',
    currentValue: 82,
    alertThreshold: -5,
    alertCondition: 'lte',
    alertDescription: '5~10% 둔화→D경보 / 절대 삭감→C긴급경보',
    scenarioSignals: ['B', 'C', 'D'],
    scenarioText: '20%+→A·B확인 / 5~10% 둔화→D / 절대삭감→C긴급',
    warningThreshold: 10,
    status: 'normal',
    lastUpdated: '2026-09-22',
    history: [
      { date: '2026-05-05', value: 40, note: '2026년 초기값 (빅테크 4사 합산 +40%, 보수적 하한)' },
      { date: '2026-07-21', value: 77, note: '빅테크 4사 2026 CapEx ~$725B (Amazon $200B·MS $190B·Alphabet $175~185B·Meta $115~135B) = 2025 $410B 대비 +77% (Tom\'s Hardware·Statista). "AI 경제 건강·매출 성장이 자본지출 정당화" — DF1 정점 재확인' },
      { date: '2026-07-28', value: 77, note: 'Q2 실적 시즌 시작 — Alphabet(07-22) 2026 CapEx 가이던스 $180~190B→$195~205B 상향(Cloud 백로그 QoQ +$50B→$514B, CNBC·Seeking Alpha) = 상방 확인. MSFT·Meta(07-29)·Amazon(07-30)은 창 직후 발표 예정으로 4사 합산 재계산은 다음 주 — Alphabet 상향으로 +77% 기준선은 상방 쪽. DF1 정점 재확인' },
      { date: '2026-08-04', value: 77, note: '[해석 렌즈 보강 — 이창수 부사장 인터뷰 08-03] 값 무변화. 이창수 프레임: "꼭짓점은 CapEx가 아니라 FCF" — 과거엔 CapEx↑=메모리 매수라 좋았으나, 지금은 CapEx↑인데 FCF가 흑자→마이너스로 꺾이면 빅테크 주가가 흔들리고(메타 사례) 미래 리스크를 매수하는 것. 3축(CSP FCF·AI 최종수요 진정성·GPU 생태계) 중 하나라도 안 꺼지면 수요 양호. → 향후 이 지표는 절대 CapEx 성장률과 함께 CapEx-vs-FCF 다이버전스를 병행 관전(demand_inflection_divergence 연동)' },
      { date: '2026-08-11', value: 82, note: 'Q2 실적 실현(MS·Meta 07-29·Amazon 07-30) — 4사 전원 상향: Amazon 2026 $200B→~$220B·MS FY27 $255~260B·Meta 하한 $130~145B·Google $195~205B. 합산 ~$745~750B → +77%에서 ~+82% YoY로 상향, 삭감 0건 = DF1 정점 재확인. 단 이창수 FCF tell 실측 등장 — Meta FCF -91%→$784M·Amazon TTM FCF 마이너스 전환(~-$7.6B). CapEx↑는 유지되나 FCF 다이버전스가 후기순환 경보로 가시화(hyperscaler-q2-2026-actuals-gpu-rental-2026-08-11.md)' },
      { date: '2026-09-01', value: 82, note: '[08-28 수집, 값 무변화·간접 확증] Q3 4사 CapEx 실적은 아직(다음 확인점). 08-28 in-window 방증: 하이퍼스케일러 CapEx 중 메모리 비중이 2023~24 ~8%→2026 ~30%로 급등(Marvell 인용)·SEMI 300mm 팹 장비지출 2026 $133B(+18%)→2027 $151B(+14%) 사상 최대·CXL 재활용 DDR4 대규모 배치 등 = CapEx 삭감/변곡 신호 0건, 정점 재확인 방향. 다만 DT-B(수요 효율화) 관전 강화: 토큰당 추론비용 280배 하락·KV캐시 90% 절감·Meta CXL 서버 -25% 등 원단위 절감은 Jevons(구글 3.2 quadrillion 토큰/월 +7배·기업 AI 지출 2배)로 상쇄 유지' },
      { date: '2026-09-15', value: 82, note: '[정기 점검, 값 무변화] Q3 4사 실적은 아직(다음 확인점). in-window 확증: 2026 4사 CapEx ~$725B(+77% YoY)·삭감 0건 유지, 2027 전망 상향 — JP모건 ~$1T·Goldman $1.01T(2026 $765B 대비 +32%). CapEx 삭감/변곡 신호 0건 = 정점 재확인. 단 조달 구조 tell 심화(demand_inflection_divergence 연동): NVIDIA의 OpenAI 오하이오 DC 파이낸싱 개런티 $250B→~$105B(<$120B) 축소(Fortune "보도 대비 $145B↓·인공 수요 우려")·Oracle 주가 YTD -24%·HSBC "OpenAI 2030 $200B 매출에도 +$207B 조달 필요" = CapEx-vs-FCF/조달 다이버전스 후기순환 경보(수요 붕괴 아님)' },
      { date: '2026-09-22', value: 82, note: '[정기 점검, 값 무변화] Q3 4사 실적은 아직. 09-22 in-window 확증: 2026 4사 CapEx ~$725B(+77% YoY)·삭감 0건 유지, 2027 전망 상향 재확인 — Evercore·BofA도 2027 합산 >$1T 모델, 일부 2026 추정 $800~900B 상향(가이던스 계속 드리프트). "capex는 36.5% 성장만으로 2027 $1조 도달 = 성장률을 절반으로 깎아도 달성" 논지. CapEx 삭감/변곡 신호 0건 = 정점 재확인. 조달 구조 tell 심화(demand_inflection_divergence 연동): NVIDIA-OpenAI-Oracle $745B 순환 파이낸싱 루프가 첫 스트레스 테스트(연준 금리) 직면·Oracle $43B 차입으로 마이너스 FCF 커버·OpenAI 2026 매출 ~$20B 대비 손실 ~$14B·AI 순환경제 버블 인덱스 73/100 = CapEx-vs-FCF/조달 다이버전스 후기순환 경보(수요 붕괴 아님·NVIDIA CFO Kress "once-in-a-generation 플랫폼 전환" 방어)' },
    ],
    note: 'Google·MS·Amazon·Meta 합산 CapEx YoY 성장률. 2026 ~+82%(20%+ 임계 대폭 초과) → A·B 확인 신호 강. [08-11] Q2 실적 후 4사 전원 상향(합산 ~$745~750B). [08-04→08-11] 이창수 프레임 실측 검증 — "진짜 꼭짓점은 FCF": CapEx-vs-FCF 다이버전스(Meta FCF -91%·Amazon TTM FCF 마이너스)가 하락 변곡 선행 tell로 등장, 절대 CapEx 성장(상방)과 병행 관전',
    inputType: 'number',
    hint: '빅테크 4사 합산 AI CapEx YoY 성장률 % 입력',
    autoUpdateId: 'bigtech_capex_growth',
    autoUpdateSource: '분기 실적 시드 데이터 YoY 자동 계산',
  },
  {
    id: 'samsung_hbm_share',
    name: '삼성전자 HBM 시장 점유율',
    category: 'quarterly',
    source: 'TrendForce·Counterpoint Research',
    unit: '%',
    unitLabel: '%',
    currentValue: 37,
    alertThreshold: 30,
    alertCondition: 'lte',
    alertDescription: '30% 이하 하락 → 전략 위기 경보',
    scenarioSignals: [],
    scenarioText: '전략 위기 경보',
    warningThreshold: 33,
    status: 'warning',
    lastUpdated: '2026-06-14',
    history: [
      { date: '2025-01-01', value: 17, note: 'Q2 2025 (HBM3E 품질 이슈)' },
      { date: '2025-10-01', value: 35, note: 'Q3 2025 반등' },
      { date: '2026-05-05', value: 35, note: '2026Q1 추정' },
      { date: '2026-06-14', value: 37, note: '2026Q1 확정 37%(hbm-share.json). 병목 점검(06-14): 삼성 HBM 4월 35~40%로 급회복 — 이전 25~30% 추정 초과' },
    ],
    note: '2026Q1 37%(SK 51%·Micron 12%, TrendForce/hbm-share.json 기준). 4월 35~40% 급회복. 교차: Counterpoint Q1 확정치는 삼성 ~32%·SK ~58%로 더 낮음 — 시점·집계 기준 차이(시계열은 hbm-share.json 기준 통일). Counterpoint도 삼성 2026 연간 30%+ 전망 (july-2026-market-update)',
    inputType: 'number',
    hint: '삼성전자 HBM 시장 점유율 % 입력',
    isCritical: true,
    autoUpdateId: 'samsung_hbm_share',
    autoUpdateSource: 'HBM 점유율 시드 데이터 최신값',
  },
  {
    id: 'xian_fab_license',
    name: '시안 팹 라이선스 갱신 상태',
    category: 'quarterly',
    source: 'BIS 결정 후 즉시 확인',
    unit: 'status',
    unitLabel: '상태',
    currentValue: null,
    alertThreshold: null,
    alertCondition: 'manual',
    alertDescription: '6개월 이하·거절 → A·C 긴급 경보',
    scenarioSignals: ['A', 'C'],
    scenarioText: '1년 갱신→B·D / 조건부→D / 6개월이하·거절→A·C긴급',
    warningThreshold: null,
    status: 'unknown',
    lastUpdated: null,
    history: [],
    note: 'VEU 폐지 후 연간 라이선스 갱신 체제',
    inputType: 'select',
    selectOptions: [
      { value: 'annual', label: '1년 갱신 (정상)', status: 'normal', signal: 'B·D 방향' },
      { value: 'conditional', label: '조건부 갱신', status: 'warning', signal: 'D 경보' },
      { value: 'short', label: '6개월 이하 단기 갱신', status: 'critical', signal: 'A·C 긴급 경보' },
      { value: 'denied', label: '거절', status: 'critical', signal: 'A·C 긴급 경보' },
    ],
    hint: '시안 팹 라이선스 갱신 결과 선택',
    isCritical: true,
  },
  {
    id: 'cxmt_ddr5_shipment',
    name: 'CXMT DDR5 분기 출하량',
    category: 'quarterly',
    source: '업계 복수 추정치',
    unit: '만 개',
    unitLabel: '만 개/분기',
    currentValue: null,
    alertThreshold: 1000,
    alertCondition: 'gte',
    alertDescription: 'DDR5 분기 1,000만 개+ 출하 → D 방향 신호',
    scenarioSignals: ['D'],
    scenarioText: 'D 방향 신호 — 범용 DRAM 시장 잠식 경고',
    warningThreshold: 600,
    status: 'unknown',
    lastUpdated: null,
    history: [],
    note: '2025년 Q3 기준 CXMT·YMTC 합산 8~10% 시장 점유',
    inputType: 'number',
    hint: 'CXMT DDR5 분기 출하량 (만 개) 입력',
  },
  // ── 신규 (2026-05-19 SemiAnalysis ingest, CXMT 위협 가속 query 결과) ──
  {
    id: 'cxmt_asp_gap',
    name: 'CXMT 평균 ASP vs 글로벌 DRAM ASP 격차',
    category: 'quarterly',
    source: 'TrendForce / DRAMeXchange',
    unit: '%',
    unitLabel: '% 격차 (CXMT가 글로벌 평균 대비 낮을수록 음수)',
    currentValue: null,
    alertThreshold: -10,
    alertCondition: 'gte',
    alertDescription: '격차 −10%p 이내로 좁혀짐 → CXMT 단가 우위 약화, 정상 마진 경쟁자 부상 신호',
    scenarioSignals: ['C', 'D'],
    scenarioText: 'C·D 위협 가속 — CXMT가 단가 외 물량 점유로 진입장벽 형성',
    warningThreshold: -20,
    status: 'unknown',
    lastUpdated: null,
    history: [],
    note: 'DDR 마진 ~ HBM 계약 수준 근접(SemiAnalysis 2026-03)으로 CXMT 단가 우위 약화 추적. [2026-09-08] 중국 가격 통제 프레임 병기(송용호 인터뷰 09-03) — 5% 오버서플라이면 가격이 떨어지는 시장에서 중국 점유 10%+중국 소비 비중 50%가 만나면 마켓셰어 1등이 아니어도 중국이 시장가를 통제할 수 있고 그 뒤에 중국 정부가 있다. AI가 수급 불균형을 극단 증폭시킨 지금 중국 업체는 흑자로 실력을 키우는 정의 차세를 얻음 → "2035 만성적 공급과잉·중국 가격 결정권" 시대 리스크. 관리된 공존(DF2 0.5) 하 중기 마진 리스크(내부 전망이라 거시 축 무이동)',
    inputType: 'number',
    hint: 'CXMT 평균 ASP 대비 글로벌 평균 ASP 격차 (% 단위, CXMT가 낮으면 음수)',
  },
  {
    id: 'cxmt_fab3_status',
    name: '허페이 Fab 3 가동 상태 (1x nm 도달 시점)',
    category: 'quarterly',
    source: '복수 보도 (Tom\'s Hardware, Digitimes, Morgan Stanley)',
    unit: '단계',
    unitLabel: '단계 (계획 / 건설 / 시험양산 / 양산)',
    currentValue: 'planned',
    alertThreshold: null,
    alertCondition: 'manual',
    alertDescription: '시험양산 또는 양산 단계 도달 시 CXMT 1x nm 진입 — 시나리오 C·D 가속 신호',
    scenarioSignals: ['C', 'D'],
    scenarioText: 'C·D 가속 — CXMT 차세대 노드 진입으로 점유율 추가 상향',
    warningThreshold: null,
    status: 'normal',
    lastUpdated: '2026-05-19',
    history: [
      { date: '2026-05-19', value: 'planned', note: '계획 단계, 2027~2030 1x nm 목표 (CXMT 기술 자립 로드맵 3단계)' },
    ],
    options: [
      { value: 'planned', label: '계획 단계', status: 'normal', signal: '기존 가정 유지' },
      { value: 'construction', label: '건설 단계', status: 'warning', signal: 'C·D 조기 가속 경보' },
      { value: 'pilot', label: '시험양산', status: 'critical', signal: 'C·D 가속 확정' },
      { value: 'mass_production', label: '양산', status: 'critical', signal: '시나리오 매트릭스 재평가 필요' },
    ],
    inputType: 'select',
    hint: 'CXMT 허페이 Fab 3 진척 단계 선택',
  },
  // ── 신규 (2026-07-11 애플–CXMT ingest, FT 2026-07-08 보도) ──
  {
    id: 'cxmt_apple_qualification',
    name: '애플 CXMT DRAM 인증·채택 단계 (중국 내수용)',
    category: 'quarterly',
    source: 'Financial Times / Reuters / 복수 보도',
    unit: '단계',
    unitLabel: '단계 (테스트 / 미승인대기 / 승인·채택 / 차단·무산)',
    currentValue: 'testing',
    alertThreshold: null,
    alertCondition: 'manual',
    alertDescription: '미 행정부 승인·양산 채택 도달 시 CXMT 프리미엄 고객 인증 이정표 + DF2 관리된 공존(Pole B) 신호 — 삼성·SK 과점 균열·애플 price leverage 확대',
    scenarioSignals: ['B', 'D'],
    scenarioText: 'B·D 신호 — 관리된 공존 + 중국 범용 CXMT 잠식. 차단(무산) 시 A·C 디커플링 반전 신호',
    warningThreshold: null,
    status: 'warning',
    lastUpdated: '2026-09-22',
    history: [
      { date: '2026-07-11', value: 'testing', note: 'FT 2026-07-08: 애플이 중국 내수용 기기에 CXMT DRAM 기술 검증 착수 + 미 행정부에 사용 승인 로비. CXMT는 국방부 1260H 리스트 등재.' },
      { date: '2026-08-25', value: 'testing', note: '[08-25 재평가] WSJ 08-09~11: 애플이 iPhone·MacBook 라인 전반에서 CXMT DRAM을 \'테스트\'에서 \'인증(qualifying)\' 단계로 진전 — 양산 승인 직전의 기술 검증 통과 국면. 단 여전히 백악관 사인오프 대기이고, Schumer 상원 원내대표의 對팀쿡 반대 서한(07-29)이 정치적 역풍으로 추가됨. CXMT는 BIS 엔티티 리스트 미등재. 승인/차단 리트머스 미해결 → currentValue \'testing\' 유지(pending_approval 미도달), DF2 이동 근거 미충족.' },
      { date: '2026-09-01', value: 'testing', note: '[09-01 재평가] 08-28 수집(dt-d-china-entrant-signals): 정치전 격화·미결 지속. Schumer 주도 초당파 상원 서한(07-29~30)이 팀 쿡에게 "전 세계 어느 제품에도 CXMT·YMTC 미사용 약속" 요구, 답변시한 08-21 경과 — 애플 공개 약속 보도 없음. 상무부는 06월 CXMT Entity List 추가 권고를 무역협상 맥락에서 보류(보류≠승인). [저신뢰] wccftech(08월): 트럼프 행정부가 9월 시진핑 방미 정상회담 후 애플의 CXMT(메모리)·YMTC(스토리지) 조달 허용 공식 통지 관측. 한편 CXMT DDR5 수율 >90% 돌파(삼성 92~93%와 2~3%p差)·HP/Asus/Acer 인증 완료 후 노트북 제한 채택 개시 = 기술·상업 인증 파이프라인은 실질 진전. 종합: 승인/차단 리트머스 여전히 미해결 → currentValue \'testing\' 유지, DF2 0.5 이동 근거 미충족. 9월 정상회담 결과가 다음 핵심 분기점.' },
      { date: '2026-09-22', value: 'testing', note: '[09-22 재평가] Trump-Xi 정상회담 09-24 워싱턴 개최 확정(2일 후) — 09-01이 지목한 "9월 정상회담 후 조달 허용 통지 관측"의 이벤트가 임박했으나, 애플 CXMT 조달에 대한 미 행정부 공식 승인·양산 채택 확정은 09-22 현재 미보도. 정상회담이 반도체 라이선스 언어를 완화하면 이 리트머스가 승인 쪽으로, 결렬·MATCH 진전이면 차단 쪽으로 갈 수 있는 상위 분기점. currentValue \'testing\' 유지, DF2 0.5 이동 근거 미충족 — 09-24 결과가 pending_approval/approved_adopted/blocked 방향 결정.' },
    ],
    options: [
      { value: 'blocked', label: '차단·무산', status: 'normal', signal: 'YMTC 2022 전철 — DF2 디커플링(A·C) 신호' },
      { value: 'testing', label: '기술 검증 중', status: 'warning', signal: '인증 파이프라인 진입 — CXMT 위상 변화 조짐' },
      { value: 'pending_approval', label: '미 행정부 승인 대기', status: 'warning', signal: '정책 분기점 임박 — 승인 여부가 DF2 리트머스' },
      { value: 'approved_adopted', label: '승인·양산 채택', status: 'critical', signal: 'DF2 관리된 공존(B·D) 확정 + 과점 균열·애플 price leverage' },
    ],
    inputType: 'select',
    hint: '애플의 CXMT DRAM 인증·채택 진척 단계 선택 (중국 내수용 한정)',
  },
  // ── 신규 (2026-05-19 권석준 추가 영상 3건 ingest, 애플 온디바이스 AI 진입 추적) ──
  {
    id: 'apple_ondevice_ai_status',
    name: '애플 온디바이스 AI 메모리 채택 단계',
    category: 'quarterly',
    source: '복수 보도 (Apple Newsroom, Bloomberg, Reuters)',
    unit: '단계',
    unitLabel: '단계 (미발표 / 발표 / 시제품 / 양산 채택)',
    currentValue: 'unannounced',
    alertThreshold: null,
    alertCondition: 'manual',
    alertDescription: '시제품 또는 양산 채택 단계 도달 시 스마트폰 폼팩터 최적화 메모리 시장 폭풍 신호 — Samsung LPDDR·UFS 폼팩터 선점 압력',
    scenarioSignals: ['B', 'E'],
    scenarioText: 'B·E 신호 — 온디바이스 AI 시대 본격 진입, MB-4 5종 메모리 통합 솔루션·SE-3 신흥 시장 가속 명분',
    warningThreshold: null,
    status: 'normal',
    lastUpdated: '2026-05-19',
    history: [
      { date: '2026-05-19', value: 'unannounced', note: '권석준 교수(SBS 2026-05): 애플 진입 시 거대 폭풍. 일본 폼팩터 실패 교훈으로 한국이 놓치면 동일 위험.' },
      { date: '2026-05-19', value: 'unannounced', note: '의미 명확화: Apple Intelligence(2024 WWDC)는 발표됐으나 권석준이 우려한 신규 메모리 폼팩터·LPDDR6/특수 규격 공식 채택은 미발표 상태. EWI는 신규 규격 진척만 추적.' },
    ],
    options: [
      { value: 'unannounced', label: '미발표', status: 'normal', signal: '기존 가정 유지 (Apple Intelligence 자체 발표는 별개)' },
      { value: 'announced', label: '신규 규격 공식 발표', status: 'warning', signal: '폼팩터 전환 신호 — 1~2년 내 표준 결정' },
      { value: 'prototype', label: '시제품 공개', status: 'critical', signal: 'Samsung LPDDR·UFS 폼팩터 선점 필요' },
      { value: 'mass_adoption', label: '양산 채택', status: 'critical', signal: '스마트폰 메모리 시장 재편 — 시나리오 매트릭스 재평가' },
    ],
    inputType: 'select',
    hint: '애플 온디바이스 AI용 신규 메모리 규격·폼팩터 채택 단계 선택 (Apple Intelligence 발표 자체는 별개)',
  },
  // ── 신규 (2026-05-25 Counterpoint ingest, 사이클 정점 신호 + 메모리 수요 전이) ──
  {
    id: 'dram_opm_vs_hbm_opm',
    name: '일반 DRAM 영업이익률 vs HBM 영업이익률 (역전 여부)',
    category: 'quarterly',
    source: 'Counterpoint Research / TrendForce / 3사 IR',
    unit: '단계',
    unitLabel: '단계 (HBM > DRAM 정상 / 근접 / DRAM > HBM 역전 / DRAM 사상최고 초과)',
    currentValue: 'inverted',
    alertThreshold: null,
    alertCondition: 'manual',
    alertDescription: 'DRAM OPM이 HBM OPM을 초과(역전)하는 시점은 통상 사이클 정점 12~18개월 전 — RS-1·5 절제 강도 정점 신호로 격상',
    scenarioSignals: ['C', 'D'],
    scenarioText: '정점 신호 — 다운턴 진입 예고. 호황기 절제 격상 + cushion 확보 가속',
    warningThreshold: null,
    status: 'critical',
    lastUpdated: '2026-06-24',
    history: [
      { date: '2026-02-05', value: 'inverted', note: 'Counterpoint: Q4 2025 DRAM OPM 60%대 진입 — 사상 첫 HBM 계약 마진 초과' },
      { date: '2026-05-25', value: 'inverted', note: 'Counterpoint·Bloomberg(Mehrotra "with discipline") + SemiAnalysis "DDR 마진 HBM 근접/초과" 정합' },
      { date: '2026-06-24', value: 'inverted', note: 'Micron FY26 Q3: GAAP 매출총이익률 84.9% 사상 최고·DRAM ASP +low-60s% QoQ(성장 압도 동인)·DRAM 76% 비중 — 역전·정점 신호 재확인 (micron-q3-fy26.md)' },
    ],
    options: [
      { value: 'normal', label: 'HBM > DRAM (정상)', status: 'normal', signal: '호황 정점 멀음' },
      { value: 'near', label: '근접 (격차 5%p 이내)', status: 'warning', signal: '정점 접근 신호' },
      { value: 'inverted', label: 'DRAM > HBM (역전)', status: 'critical', signal: '정점 임박 — RS-1·5 격상' },
      { value: 'extreme', label: 'DRAM 사상 최고치 초과', status: 'critical', signal: '정점 12~18개월 전 신호 — cushion 확보 가속' },
    ],
    inputType: 'select',
    hint: '일반 DRAM과 HBM의 영업이익률 관계 선택 (Counterpoint·TrendForce 분기 데이터 기준)',
    isCritical: true,
  },
  {
    id: 'smartphone_shipment_yoy',
    name: '글로벌 스마트폰 출하 YoY (메모리 부족 전이 신호)',
    category: 'quarterly',
    source: 'Counterpoint Research / IDC / Canalys',
    unit: '%',
    unitLabel: '% YoY',
    currentValue: -2.1,
    alertThreshold: -1,
    alertCondition: 'lte',
    alertDescription: '스마트폰 출하 YoY < 0% — 메모리 부족이 소비자 시장으로 전이된 신호 (Counterpoint 2025-12: 2026 -2.1% 전망, 메모리 부족 직접 원인)',
    scenarioSignals: ['D'],
    scenarioText: 'D 부분 가속 — 소비자 수요 수축 시 메모리 다운턴 전이 위험. Samsung 메모리(공급자) ↔ 스마트폰(수요자) 양면 노출은 전사 자기상쇄',
    warningThreshold: 0,
    status: 'warning',
    lastUpdated: '2026-05-25',
    history: [
      { date: '2025-12-18', value: -2.1, note: 'Counterpoint 2026 전망: 메모리 부족발 -2.1% (역대 최대 하향), BoM +25/15/10% (저/중/고가)' },
    ],
    note: '단순 거시 수요 둔화 ≠ 본 EWI. 메모리 BoM 압박이 명시적 원인일 때만 작동. ASP 인상(2026 +6.9% YoY)으로 매출 영향은 부분 상쇄.',
    inputType: 'number',
    hint: '글로벌 스마트폰 출하 YoY % (메모리 부족이 명시적 원인일 때)',
  },
  // ── 신규 (2026-05-19 권석준 인터뷰 동아일보, HBM 6세대 양산 추적) ──
  {
    id: 'hbm6_mass_production_status',
    name: 'HBM 6세대 양산 진척 (HBM4E 이후)',
    category: 'quarterly',
    source: 'TrendForce / SK Hynix·Samsung IR',
    unit: '단계',
    unitLabel: '단계 (계획 / 시제품 / 리스크 양산 / 대량 양산)',
    currentValue: 'planned',
    alertThreshold: null,
    alertCondition: 'manual',
    alertDescription: '리스크 양산 또는 대량 양산 진입 시 Main Bet(시나리오 B) 기술 토대 확정 — HBM4E 이후 권 교수가 본 "4~5년 로드맵 안정" 검증',
    scenarioSignals: ['A', 'B'],
    scenarioText: 'A·B 신호 — HBM 로드맵 안정 = Main Bet 가정 검증, MB-2 동서 균형 공급자 가속 명분',
    warningThreshold: null,
    status: 'normal',
    lastUpdated: '2026-05-19',
    history: [
      { date: '2026-05-19', value: 'planned', note: '권석준(동아 2026-05): "빠르면 2026 하반기 HBM 6세대 양산 시작. HBM 로드맵 4~5년 안정." 현 시점은 계획·R&D 단계.' },
    ],
    options: [
      { value: 'planned', label: '계획 단계', status: 'normal', signal: '기존 로드맵 진행 중' },
      { value: 'sample', label: '시제품 공급', status: 'normal', signal: 'NVIDIA·하이퍼스케일러 인증 시작' },
      { value: 'risk_production', label: '리스크 양산', status: 'warning', signal: '2026 H2 권석준 전망 검증 단계 — Main Bet 토대 안정' },
      { value: 'mass_production', label: '대량 양산', status: 'normal', signal: 'HBM 슈퍼사이클 1단계 확정 — HBM5/7 차세대 로드맵 점검 시점' },
    ],
    inputType: 'select',
    hint: 'HBM 6세대 양산 진척 단계 선택 (HBM4E 이후 세대)',
  },
  // ── 신규 (2026-05-19 권석준 인터뷰 SBS, HBM:DRAM 가격 균형선 6:1 진단 기반) ──
  {
    id: 'hbm_dram_price_ratio',
    name: 'HBM:DRAM 단위 GB 가격 비율',
    category: 'quarterly',
    source: 'TrendForce / DRAMeXchange',
    unit: '배',
    unitLabel: '배 (HBM 단가 / DRAM 단가, 단위 GB 기준)',
    currentValue: null,
    alertThreshold: 6,
    alertCondition: 'lte',
    alertDescription: '비율 6배 이하로 떨어지면 HBM이 DRAM 대비 수익기적 메리트 상실 — 캐파를 범용 DRAM 쪽으로 재분배 신호 (권석준 균형선)',
    scenarioSignals: ['B', 'D'],
    scenarioText: 'B·D 신호 — HBM 우위 약화 시 RS2 바벨 "저원가 범용" 축으로 무게 이동',
    warningThreshold: 7,
    status: 'unknown',
    lastUpdated: null,
    history: [],
    note: '권석준 교수(SBS 2026-04): HBM:DRAM 균형선 6:1. 그 미만이면 DRAM이 이익률에서 HBM 추월 가능. SemiAnalysis "DDR 마진 ~ HBM 계약 수준" 신호의 정량 보조 지표.',
    inputType: 'number',
    hint: 'HBM 평균 단가 / DRAM 평균 단가 (단위 GB 기준 배수)',
  },

  // ────────────────────────────────────────────
  // 연간 검토
  // ────────────────────────────────────────────
  {
    id: 'global_hbm_tam',
    name: '글로벌 HBM 시장 TAM',
    category: 'annual',
    source: 'TrendForce·Bank of America',
    unit: '$B',
    unitLabel: '십억 달러',
    currentValue: 54.6,
    alertThreshold: null,
    alertCondition: 'manual',
    alertDescription: '2028년 $1,000억 목표 달성 경로 점검',
    scenarioSignals: ['A', 'B'],
    scenarioText: '2028년 $1,000억 목표 경로 점검',
    warningThreshold: null,
    status: 'normal',
    lastUpdated: '2026-05-05',
    history: [
      { date: '2026-05-05', value: 54.6, note: '2026E 추정 (Bank of America)' },
    ],
    note: '2026E $546억, 2028E $1,000억 목표',
    inputType: 'number',
    hint: '글로벌 HBM 시장 TAM (십억 달러, $B) 입력',
  },
  {
    id: '3d_dram_progress',
    name: '3D DRAM 상용화 진척도',
    category: 'annual',
    source: 'IMEC·마이크론·SK하이닉스 발표',
    unit: 'status',
    unitLabel: '단계',
    currentValue: null,
    alertThreshold: null,
    alertCondition: 'manual',
    alertDescription: 'E 시나리오 조기 전환 판단 지표',
    scenarioSignals: ['E'],
    scenarioText: 'E 시나리오 조기 경보 — 3D DRAM 상용화 가속 여부',
    warningThreshold: null,
    status: 'unknown',
    lastUpdated: null,
    history: [],
    note: 'IMEC/마이크론 PoC에서 HBM3E 대비 전력 50% 개선 시 E 시나리오 트리거',
    inputType: 'select',
    selectOptions: [
      { value: 'research', label: 'R&D 단계 (정상)', status: 'normal', signal: 'E 시나리오 원거리' },
      { value: 'poc', label: 'PoC 성공 (주의)', status: 'warning', signal: 'E 시나리오 모니터링' },
      { value: 'poc_50pct', label: 'PoC 전력 50%+ 개선 (경보)', status: 'critical', signal: 'E 시나리오 트리거 발동' },
      { value: 'pilot', label: '파일럿 양산 (긴급)', status: 'critical', signal: 'E 시나리오 긴급 대응' },
    ],
    hint: '3D DRAM 기술 성숙도 단계 선택',
  },
  // ────────────────────────────────────────────
  // 신규 지표 (2026-05-06 추가) — Q1 2026 데이터 기반
  // ────────────────────────────────────────────
  {
    id: 'samsung_hbm4_rubin_share',
    name: '삼성 HBM4 NVIDIA Rubin 점유율',
    category: 'quarterly',
    source: 'UBS·Counterpoint Research',
    unit: '%',
    unitLabel: '%',
    currentValue: 28,
    alertThreshold: 25,
    alertCondition: 'lte',
    alertDescription: '25% 이하 → HBM4E 1번 자리 탈환 위기, 35% 이하 → 추격 정체',
    scenarioSignals: ['B'],
    scenarioText: '40%+ → B 가속 / 25%↓ → 전략 위기',
    warningThreshold: 35,
    status: 'warning',
    lastUpdated: '2026-09-22',
    history: [
      { date: '2025-04', value: 17, note: 'HBM3E 품질 이슈로 추락' },
      { date: '2025-09', value: 35, note: 'Q3 2025 반등 시작' },
      { date: '2026-03', value: 28, note: 'NVIDIA Rubin HBM4 공급 점유 (UBS)' },
      { date: '2026-07-21', value: 28, note: '인증은 통과(06-05 Vera Rubin 3사 인증)했으나 볼륨 발주 미전환 — 삼성 NVIDIA향 HBM4 매출 여전히 유상 평가용 샘플 수준(Winbuzzer 07-17). SK하이닉스 Rubin 2/3+ 락인 유지. 인증→볼륨 발주 전환 지연 = 시나리오 B 실행 리스크' },
      { date: '2026-07-28', value: 28, note: '무변화 — 07-21~28 창 내 볼륨 발주 전환 보도 없음(07-15 Digitimes·07-17 Winbuzzer 이후 신규 반전 부재). SK하이닉스 Rubin/HBM4 선두 60~70% 유지. 인증→볼륨 발주 전환 지연 지속 = 시나리오 B 시간 리스크' },
      { date: '2026-08-25', value: 28, note: '[08-25 재평가] 볼륨 발주 전환 여전히 미확인(직전 실측 mid-July). 신규 삼성 실행 신호: 레거시 메모리 백엔드의 베트남 이전을 검토해 국내 HBM 캐파를 확보하려는 움직임(TechTimes 08-15) — 볼륨 발주 확정이 아니라 캐파 언락 준비 신호. NVIDIA는 Rubin HBM4에 삼성·SK·Micron 3사 인증 유지(추정 배분 SK 60~70%·삼성 25~30%·Micron 잔여). 인증→볼륨 발주 전환 지연 지속 = 시나리오 B 시간 리스크' },
      { date: '2026-09-01', value: 28, note: '[09-01 재평가] 볼륨 발주 전환 확인서 여전히 미확보. 08-28 수집 신규 구조 신호 2건: ① NVIDIA가 커스텀 HBM \'NVHBM\'을 08-26 공식 발표 — 메모리 컨트롤러를 base die로 이동, NVLink Fusion 파트너가 자기 설계에 통합. 제조는 삼성·SK가 표준 구현으로 담당(첫 파트너 Amazon Annapurna Trainium4, Feynman 세대 2028 첫 채택). 삼성엔 차세대 제조 참여 기회이나 현 Rubin 배분과는 별개. ② NVIDIA가 DRAM 타이트로 Rubin Ultra의 HBM 구성(스택당 용량)을 축소 검토(TrendForce 08-04·최대 -81% 보도) — 공급 타이트발 최적화이지 삼성 배분 확대와 무관. 삼성 zHBM(Hot Chips 2026 스펙 공개, 상용시점 미공개)은 3D 수직적층 차별화 카드. 종합: Rubin HBM4 점유 28% 무변화, 인증→볼륨 발주 전환 지연 지속 = 시나리오 B 시간 리스크' },
      { date: '2026-09-22', value: 28, note: '[09-22 재평가] 볼륨 발주 전환 여전히 미확보 — 삼성 NVIDIA향 HBM4는 유상 평가 단계 유지(2월 양산 시작·NVIDIA/AMD 최종 인증 통과했으나 volume-production order 미확정, tweaktown·digitimes). 신규 실측: NVIDIA가 신규 16-Hi HBM4를 Q4 2026 납품 요청 — SK하이닉스 16층 볼륨·Micron 샘플·**삼성은 16-Hi를 skip**(Benzinga·tweaktown 09월). 삼성 12-hi HBM4는 최종 인증 단계. Micron은 HBM 캐파 2배 증설(Sammy Fans 09-12). 3사 합쳐도 NVIDIA에겐 듀오폴리 구조(SK 락인 심화). 인증→볼륨 발주 전환 지연 지속 = 시나리오 B 시간 리스크, Micron 09-30 실적이 HBM 가격·볼륨 다음 확인점' },
    ],
    note: '목표: 2027년 40%+ (NVIDIA Feynman 1번 공급사 지위). 06-05 인증 통과·07-17 볼륨 발주 대기 — "인증 ≠ 공급 계약" 구간',
    inputType: 'number',
    hint: '삼성 HBM4 NVIDIA Rubin 점유율 % 입력',
    isCritical: true,
  },
  {
    id: 'samsung_q1_memory_revenue',
    name: '삼성 분기 메모리 매출 (조원)',
    category: 'quarterly',
    source: 'Samsung Quarterly IR',
    unit: '조원',
    unitLabel: '조원',
    currentValue: 75,
    alertThreshold: 35,
    alertCondition: 'lte',
    alertDescription: '35조 원 이하 → 다운사이클 진입 신호',
    scenarioSignals: ['C', 'D'],
    scenarioText: '50조 원+ → 호황 / 35조 원↓ → 다운턴 경보',
    warningThreshold: 50,
    status: 'normal',
    lastUpdated: '2026-04-30',
    history: [
      { date: '2025-Q1', value: 19, note: '저점' },
      { date: '2025-Q4', value: 37, note: '회복' },
      { date: '2026-Q1', value: 75, note: '사상 최대 (+292% YoY)' },
    ],
    note: 'Q1 2026 사상 최대 분기. HBM4 Sold Out + 일반 DRAM +55~60% QoQ',
    inputType: 'number',
    hint: '삼성 분기 메모리 매출 (조원) 입력',
  },
  {
    id: 'cxmt_hbm3_production',
    name: 'CXMT HBM3 양산 진척도',
    category: 'quarterly',
    source: 'TrendForce·Tom\'s Hardware',
    unit: 'status',
    unitLabel: '단계',
    currentValue: null,
    alertCondition: 'manual',
    alertDescription: 'CXMT HBM3 → HBM3E → HBM4 진척 단계',
    scenarioSignals: ['D'],
    scenarioText: 'HBM4 양산 도달 시 → 시나리오 D·C 가속',
    warningThreshold: null,
    status: 'warning',
    lastUpdated: '2026-07-28',
    history: [
      { date: '2026-04', value: 'HBM3 양산 개시', note: '월 60K 웨이퍼, 화웨이 우선 공급' },
      { date: '2026-07-21', value: 'HBM3 양산 (일정 지연)', note: '[정기 점검] CXMT는 DDR5/LPDDR5 양산 지속하나, 2026년 내 HBM3 양산 도달은 이제 어려워 보임(일정 슬립, techwireasia·semiconductorsinsight 2026-07). D·C 가속 시점 후퇴 요인 — DF2 무변화 정합' },
      { date: '2026-07-28', value: 'HBM3 양산 (일정 지연)', note: '[정기 점검] 무변화 — 07-21~28 창 내 HBM 대규모 양산 확정 신규 보도 없음. 최근 구체 건은 6월 CXMT–Tencent ~$3B 서버 DRAM 계약(창 이전)으로 DDR5 축. HBM at scale 여전히 불투명 — DF2 무변화 정합' },
    ],
    note: '2026년 양산 개시 목표였으나 HBM 일정 지연(2026 도달 불투명) → 2027 HBM3E → 2028 HBM4 위협 가능성. DDR5/LPDDR5는 양산 중',
    inputType: 'select',
    selectOptions: [
      { value: 'rd', label: 'R&D 단계', status: 'normal', signal: '정상' },
      { value: 'sample', label: '샘플 출하', status: 'warning', signal: '주의' },
      { value: 'hbm3_mp', label: 'HBM3 양산 (현재)', status: 'warning', signal: 'D 모니터링' },
      { value: 'hbm3e_mp', label: 'HBM3E 양산', status: 'critical', signal: 'D 경보' },
      { value: 'hbm4_mp', label: 'HBM4 양산', status: 'critical', signal: 'C·D 긴급' },
    ],
    hint: 'CXMT HBM 양산 단계 선택',
    isCritical: true,
  },
  {
    id: 'taylor_phase2_announced',
    name: '텍사스 테일러 2단계(HBM 전용) 발표 여부',
    category: 'quarterly',
    source: 'Samsung 공식 발표',
    unit: 'status',
    unitLabel: '상태',
    currentValue: null,
    alertCondition: 'manual',
    alertDescription: '2단계 발표 지연 → 마이크론·인텔에 미국 HBM 표준 거점 빼앗김',
    scenarioSignals: ['A'],
    scenarioText: '미발표 → A·C 시나리오에서 미국 거점 부족 위기',
    warningThreshold: null,
    status: 'warning',
    lastUpdated: '2026-05-06',
    history: [],
    note: 'Decision 8 — 2026 Q4 마감. CHIPS 추가 보조금 협상 + 미국계 빅테크 LTA 사전 체결',
    inputType: 'select',
    selectOptions: [
      { value: 'pending', label: '미발표 (현재)', status: 'warning', signal: '결정 임박' },
      { value: 'announced', label: '공식 발표', status: 'normal', signal: 'B·A 가속' },
      { value: 'subsidy', label: '추가 보조금 확정', status: 'normal', signal: 'B 가속' },
      { value: 'delayed', label: '2027년 이후 연기', status: 'critical', signal: 'A·C 위기' },
    ],
    hint: '텍사스 2단계 결정 상태',
    isCritical: true,
  },
  {
    id: 'downcycle_capex_floor',
    name: '다운사이클 Capex 하한선 준수 (RS-5 Nucor 모델)',
    category: 'quarterly',
    source: 'Samsung 분기 Capex 실적',
    unit: '조원',
    unitLabel: '조원/년',
    currentValue: null,
    alertThreshold: 4,
    alertCondition: 'lte',
    alertDescription: '연 4조원 하한 — HBM 차세대 R&D + 패키징 + 3D DRAM 합산',
    scenarioSignals: ['C', 'D'],
    scenarioText: '하한 미달 시 → 회복기 경쟁력 붕괴 (Nucor 사례 교훈)',
    warningThreshold: 5,
    status: 'unknown',
    lastUpdated: null,
    history: [],
    note: 'RS-5 이사회 정책. 호황기에 절제, 다운사이클에 사수 동일 거버넌스',
    inputType: 'number',
    hint: 'HBM R&D + 어드밴스드 패키징 + 3D DRAM 합산 capex (조원/년)',
  },
  {
    id: 'ai_dev_efficiency_adoption',
    name: 'AI 개발 효율화 도구 도입률 (RS-2·RS-3 선행 조건)',
    category: 'quarterly',
    source: '메모리사업부 인사 KPI',
    unit: '%',
    unitLabel: '%',
    currentValue: null,
    alertThreshold: 50,
    alertCondition: 'gte',
    alertDescription: '50%+ 도달 → 잉여 인력 RS-2·RS-3 전환 배치 가능',
    scenarioSignals: [],
    scenarioText: '도입률 진척에 따라 잉여 역량 배분 — Decision 5·7 KPI',
    warningThreshold: 30,
    status: 'unknown',
    lastUpdated: null,
    history: [],
    note: 'AI 코딩·EDA·공정 시뮬레이션·수율 예측 통합 도입률',
    inputType: 'number',
    hint: '메모리사업부 엔지니어 중 AI 효율화 도구 도입 비율 %',
  },

  {
    id: 'emerging_market_orders',
    name: '신흥 AI 시장 수주 (사우디·UAE·인도)',
    category: 'annual',
    source: '삼성전자 영업 실적',
    unit: '$B',
    unitLabel: '십억 달러/년',
    currentValue: null,
    alertThreshold: null,
    alertCondition: 'manual',
    alertDescription: '연간 50억 달러+ 달성 여부 점검',
    scenarioSignals: ['B'],
    scenarioText: 'B 시나리오 가속 — 동서 균형 공급망 지표',
    warningThreshold: null,
    status: 'unknown',
    lastUpdated: null,
    history: [],
    note: 'Humain·G42·Jio 등 신흥 AI 인프라 수주 합산',
    inputType: 'number',
    hint: '사우디·UAE·인도 합산 연간 메모리 수주액 ($B) 입력',
  },

  // ────────────────────────────────────────────
  // RS-6 — NAND 공정 전환 주기 연장 R&D (신규, 2026-05-06)
  // ────────────────────────────────────────────
  {
    id: 'nand_layer_cycle_months',
    name: 'NAND 공정 전환 주기 (개월) — RS-6 KPI',
    category: 'annual',
    source: 'Samsung Memory IR + TrendForce',
    unit: 'months',
    unitLabel: '개월',
    currentValue: 18,
    alertThreshold: 18,
    alertCondition: 'lte',
    alertDescription: '주기 18개월 이하 = capex/수율 부담 가중. 24개월+ 연장이 RS-6 목표',
    scenarioSignals: [],
    scenarioText: 'V8(2023) → V9(2024 TLC, 2026 QLC) → V10(2026 H2) 평균 18개월. RS-6 목표 24개월+',
    warningThreshold: 20,
    status: 'warning',
    lastUpdated: '2026-05-06',
    history: [
      { date: '2023', value: 18, note: 'V7→V8 (236L)' },
      { date: '2024', value: 18, note: 'V8→V9 TLC (286L)' },
      { date: '2026', value: 18, note: 'V9 QLC→V10 (BV NAND 430L) 예정' },
    ],
    note: '주기 연장 = 공정 전환 capex 회피. 양산 ramp 6개월 지연 = 누적 이익 2/3 소실 (Weber/PSU)',
    inputType: 'number',
    hint: 'V_n→V_{n+1} 양산 시점 간격 (개월)',
    isCritical: true,
  },
  {
    id: 'hybrid_bonding_own_ip_share',
    name: 'Hybrid Bonding 자체 IP 비율 (V10 → V11) — RS-6 KPI',
    category: 'quarterly',
    source: 'Samsung 특허청 출원·등록',
    unit: '%',
    unitLabel: '% (자체 IP / 총 사용 IP)',
    currentValue: null,
    alertThreshold: 50,
    alertCondition: 'gte',
    alertDescription: '50%+ 도달 → YMTC 라이선스 의존도 안전권. 70%+ = V11 양산 시 자립',
    scenarioSignals: ['C'],
    scenarioText: '시나리오 C(전면 디커플링) 발생 시 라이선스 차단 리스크. 자체 IP 70%+ 가 생존선',
    warningThreshold: 30,
    status: 'unknown',
    lastUpdated: null,
    history: [],
    note: 'YMTC가 hybrid bonding 핵심 IP·특허 다수 보유 (TrendForce 2025-05). 한국 IP 컨소시엄 검토',
    inputType: 'number',
    hint: 'V10/V11에서 자체 hybrid bonding IP 비율 % 입력',
    isCritical: true,
  },
  {
    id: 'nand_yield_ramp_time_months',
    name: 'NAND 신노드 양산 ramp 시간 (yield 90% 도달 개월) — RS-6 KPI',
    category: 'annual',
    source: 'Samsung 메모리연구소 + 공정 데이터',
    unit: 'months',
    unitLabel: '개월',
    currentValue: null,
    alertThreshold: 12,
    alertCondition: 'lte',
    alertDescription: '12개월 이내 yield 90% 도달 = 학습곡선 우위. 18개월 초과 = 누적 이익 2/3 손실 (Weber)',
    scenarioSignals: [],
    scenarioText: '6개월 단축 효과 = 누적 이익 2배. 6개월 지연 = 이익 2/3 소실 (PSU Weber)',
    warningThreshold: 15,
    status: 'unknown',
    lastUpdated: null,
    history: [],
    note: 'RS-6 트랙 1·2 효과 측정 핵심 KPI. 공정 개발 1분당 약 $5,000 손실로 환산 가능',
    inputType: 'number',
    hint: '신 NAND 노드 양산 개시→yield 90% 도달까지 개월 수',
  },
  {
    id: 'nand_capex_per_bit_growth',
    name: 'NAND Capex/bit 연간 변화율 — RS-6 KPI',
    category: 'annual',
    source: 'TrendForce + Samsung Memory IR',
    unit: '%',
    unitLabel: '% (YoY)',
    currentValue: null,
    alertThreshold: 0,
    alertCondition: 'gte',
    alertDescription: '플러스 = bit 당 capex 증가 = 비용 곡선 우상향. 0 또는 마이너스 = 효율 개선',
    scenarioSignals: [],
    scenarioText: 'RS-6 트랙 1~4 효과로 bit 당 capex 감소 추세 유지가 목표 (마이너스 값)',
    warningThreshold: 5,
    status: 'unknown',
    lastUpdated: null,
    history: [],
    note: '2026년 NAND capex $22.2B (+5%만) — 업계가 capa 확장보다 process upgrade에 집중. bit/capex 효율이 핵심',
    inputType: 'number',
    hint: '연간 NAND capex 증가율 - bit shipment 증가율 (%) 입력',
  },

  // ────────────────────────────────────────────
  // 경쟁사 (Competitors) — SK하이닉스 / Micron / 중국 (2026-05-07 신규)
  // 출처: wiki/entities/sk-hynix.md, wiki/entities/micron.md, wiki/entities/cxmt.md, wiki/entities/ymtc.md, wiki/concepts/dram-market-share.md
  // ────────────────────────────────────────────
  {
    id: 'sk_hynix_hbm_share',
    name: 'SK하이닉스 HBM 시장 점유율 (비교축)',
    category: 'quarterly',
    source: 'TrendForce·Counterpoint Research·Astute Group',
    unit: '%',
    unitLabel: '%',
    currentValue: 53,
    alertThreshold: 60,
    alertCondition: 'gte',
    alertDescription: '60%+ 유지 시 NVIDIA 락인 심화 — 삼성 추격 어려움',
    scenarioSignals: ['B'],
    scenarioText: 'SK 60%+ 고착 시 삼성 HBM4E 1번 자리 회복 경로 봉쇄',
    warningThreshold: 55,
    status: 'warning',
    lastUpdated: '2026-05-06',
    history: [
      { date: '2024', value: 54, note: '연간 평균' },
      { date: '2025-Q2', value: 62, note: '삼성 HBM3E 품질 이슈로 SK 점유 정점' },
      { date: '2025-Q3', value: 53, note: '삼성 35% 회복으로 SK 일부 환원' },
    ],
    note: '비교축 — 삼성 HBM 점유율(samsung_hbm_share) + Micron(micron_hbm_share)와 함께 보면 3강 구도 시각화. NVIDIA Rubin HBM4 70% 점유 전망 (UBS)',
    inputType: 'number',
    hint: 'SK하이닉스 HBM 분기 점유율 % 입력',
  },
  {
    id: 'micron_hbm_share',
    name: 'Micron HBM 시장 점유율 (위협 신호)',
    category: 'quarterly',
    source: 'Astute Group·Counterpoint Research',
    unit: '%',
    unitLabel: '%',
    currentValue: 11,
    alertThreshold: 20,
    alertCondition: 'gte',
    alertDescription: '20%+ 도달 시 삼성 추월 재현 우려 (Q2 2025 21% 사례). 22~23% 목표 (Micron CEO)',
    scenarioSignals: ['B'],
    scenarioText: 'Micron 20%+ → 삼성 HBM 시장 3위 추락 위험. 미국 CHIPS Act + Idaho ID2 가속이 동력',
    warningThreshold: 15,
    status: 'warning',
    lastUpdated: '2026-05-06',
    history: [
      { date: '2024', value: 7, note: '연간 평균 — 후발 진입' },
      { date: '2025-Q2', value: 21, note: '삼성(17%) 추월 — HBM 점유 3배 성장' },
      { date: '2025-Q3', value: 11, note: '삼성 35% 회복으로 환원' },
    ],
    note: 'Idaho ID2 팹 (NY보다 우선 완공) + CHIPS Act $61.65억 보조금 + HBM3E 12Hi NVIDIA 공급으로 추격 가속',
    inputType: 'number',
    hint: 'Micron HBM 분기 점유율 % 입력',
    isCritical: true,
  },
  {
    id: 'sk_hynix_op_margin',
    name: 'SK하이닉스 영업이익률 (삼성 비교 벤치마크)',
    category: 'quarterly',
    source: 'SK하이닉스 IR·KED Global',
    unit: '%',
    unitLabel: '%',
    currentValue: 72,
    alertThreshold: null,
    alertCondition: 'manual',
    alertDescription: '72% (Q1 2026) — HBM 집중 투자 효과. 삼성은 분산 포트폴리오로 후행',
    scenarioSignals: [],
    scenarioText: '삼성 영업이익률과 격차가 5%p+ 지속 시 RS-5 자본 효율 이슈 — HBM CapEx 비중 재배분 필요',
    warningThreshold: null,
    status: 'warning',
    lastUpdated: '2026-04-23',
    history: [
      { date: '2023', value: -23, note: '적자 (-7.7조 원)' },
      { date: '2024', value: 35, note: '회복 (+23.5조 원)' },
      { date: '2025', value: 49, note: '연간 (97.1조 원 매출, 47.2조 원 영업이익)' },
      { date: '2026-Q1', value: 72, note: '기록적 영업이익률 (HBM4 수요 3년치 공급 초과)' },
    ],
    note: 'CapEx 30조원+ 집행하면서도 72% 마진 — 고수익 제품(HBM) 집중이 CapEx 효율성을 극대화. 삼성 메모리 P&L 분리(SD-1) 시 비교 가능',
    inputType: 'number',
    hint: 'SK하이닉스 분기 영업이익률 % 입력',
  },
  {
    id: 'dram_market_leader',
    name: 'DRAM 시장 1위 지위 (33년 만의 교체)',
    category: 'quarterly',
    source: 'TrendForce·Korea Herald·S&P Global',
    unit: 'status',
    unitLabel: '1위',
    currentValue: null,
    alertThreshold: null,
    alertCondition: 'manual',
    alertDescription: '2025 Q1 SK하이닉스가 33년 만에 삼성 추월. 회복 여부가 메모리 사업 정체성 지표',
    scenarioSignals: ['B'],
    scenarioText: '삼성 1위 회복 → B 가속 / SK 1위 고착 → SD-1·MB-4 시급성 강화',
    warningThreshold: null,
    status: 'warning',
    lastUpdated: '2026-05-06',
    history: [
      { date: '2025-Q1', value: 'SK hynix', note: 'SK 36.3% > Samsung 32.7% (33년 만의 교체)' },
      { date: '2025-Q3', value: 'SK hynix', note: 'SK 33.2% > Samsung 32.6% (격차 축소)' },
    ],
    note: '2025년 SK 매출 $496억 vs 삼성 $464억 — 연간 기준도 SK 우위. 회복은 HBM 점유율 회복이 선행 조건',
    inputType: 'select',
    selectOptions: [
      { value: 'Samsung', label: '삼성 1위 (2자리수 격차)', status: 'normal', signal: 'B 가속' },
      { value: 'Samsung_narrow', label: '삼성 1위 (1%p 이내)', status: 'warning', signal: '경합' },
      { value: 'SK hynix', label: 'SK하이닉스 1위 (현재)', status: 'critical', signal: 'SD-1·MB-4 시급' },
    ],
    hint: 'DRAM 시장 1위 업체 선택',
    isCritical: true,
  },
  {
    id: 'cxmt_dram_share',
    name: 'CXMT DRAM 시장 점유율 (중국 추격)',
    category: 'quarterly',
    source: 'TrendForce·Morgan Stanley·Digitimes',
    unit: '%',
    unitLabel: '%',
    currentValue: 8,
    alertThreshold: 12,
    alertCondition: 'gte',
    alertDescription: '12%+ 도달 시 4강 진입 — 범용 DRAM 가격 압박 본격화 (2027E 13.9%)',
    scenarioSignals: ['D'],
    scenarioText: '15%+ → 시나리오 D 가속 (CXMT가 한국 메이커 범용 DRAM 가격 결정권 잠식)',
    warningThreshold: 10,
    status: 'warning',
    lastUpdated: '2026-09-01',
    history: [
      { date: '2023', value: 3, note: '빠른 성장 시작' },
      { date: '2024', value: 5, note: 'Morgan Stanley 추정 (4.9%)' },
      { date: '2025-Q3', value: 8, note: 'TrendForce 추정 — DRAM 4강 진입' },
      { date: '2025-Q4', value: 7.67, note: 'Omdia 매출 기준 글로벌 DRAM 점유율 7.67%, 세계 4위(중국 1위) — 삼성 ~36%·SK ~29%·마이크론 ~24% (dt-d-china-entrant-signals 08-28). Nomura는 현재 ~10%·2028년 18% 전망' },
      { date: '2027E', value: 13.9, note: '업계 예측치' },
      { date: '2028E', value: 15, note: 'FT(2026-07-08): 글로벌 DRAM 웨이퍼 캐파 점유율 11%(2025)→15%(2028) 전망 — 애플–CXMT 보도 기준 (캐파 계열, 매출 점유와 구분). Nomura 매출 점유 18%(2028) 병기' },
    ],
    note: '캐파: 2024년 17만 WSPM → 2026E 30만 WSPM(2026말 35만·2027 목표 42만). FT(2026-07-08) 캐파 점유 11%(2025)→15%(2028E) — 매출 점유(본 EWI)와 별개 계열. 빅펀드 III $470억 + 안후이성 정부 지원. [08-28 수집] 17nm DDR5 수율 >90% 돌파(삼성 92~93%와 2~3%p差·업계 소식통, CXMT 공식 아님)·HP/Asus/Acer DDR5 인증 완료 후 노트북 제한 채택 개시(3사 공급 고갈로 물량 확보 경쟁)·Tencent 다년 200억 위안+ 계약·LPDDR6 샤오미 Xring O3에 SK와 공급 경쟁 진입. 애플이 中 내수용 기기에 CXMT DRAM 테스트→인증 진전(FT 2026-07-08) — cxmt_apple_qualification EWI 연동. [2026-09-15] CXMT $42억(약 300억 위안) 상하이 STAR 마켓 IPO 추진 보도(DRAM 증설 자금 조달) — 캐파 확장 가속 자금줄로 시나리오 D(중국 추격·가격 결정권) 배경 강화. 단 매출 점유(본 EWI 8%)는 무변화, 이미 D 21에 반영된 추세의 재확인. 9월 미중 정상회담 결과가 CXMT 규제 방향(Entity List·MATCH)의 상위 분기점',
    inputType: 'number',
    hint: 'CXMT DRAM 분기 점유율 % 입력',
    isCritical: true,
  },
  {
    id: 'ymtc_nand_share',
    name: 'YMTC NAND 출하 점유율 (중국 추격)',
    category: 'quarterly',
    source: 'Digitimes·Tom\'s Hardware',
    unit: '%',
    unitLabel: '%',
    currentValue: 14,
    alertThreshold: 15,
    alertCondition: 'gte',
    alertDescription: '15%+ 도달 시 NAND 글로벌 3위 고착 — 삼성 NAND 마진 압박, 2027말 세계 1위 목표',
    scenarioSignals: ['D'],
    scenarioText: 'YMTC 15%+ 시 NAND 가격 압박 본격화. 3공장 가동(2027) + DRAM 50% 전환 시 양면 위협',
    warningThreshold: 14,
    status: 'warning',
    lastUpdated: '2026-09-01',
    history: [
      { date: '2023', value: 5, note: '제재 후 회복 단계' },
      { date: '2024', value: 9, note: '연간 평균 8~10%' },
      { date: '2025-Q1', value: 10, note: 'Digitimes — 10% 초과' },
      { date: '2025-Q3', value: 13, note: 'Digitimes — 전분기 대비 상승' },
      { date: '2026-Q2', value: 14, note: '[08-28 수집] YMTC 2026 Q2 NAND 출하 점유율 14%로 세계 3위 등극 — 마이크론·키옥시아 추월(삼성 25%·SK 22%, Digital Citizen·TradingKey 2차 인용). 2027말 세계 1위 목표·월 40만 wpm 캐파 목표 보도(Tom\'s Hardware·Seoul Economic Daily 08-26)' },
      { date: '2025E', value: 15, note: 'YMTC 자체 목표' },
    ],
    note: '294L NAND 양산 (2025) + 우한 3공장 2027 가동 (NAND 50% + DRAM 50%) — 삼성 V-NAND·DRAM 양면 위협. 국산 장비 라인 구축 진행. [08-28] Q2 2026 14% 세계 3위(Micron·Kioxia 추월)·2027말 세계 1위 목표',
    inputType: 'number',
    hint: 'YMTC NAND 분기 출하 점유율 % 입력',
    isCritical: true,
  },

  // ────────────────────────────────────────────
  // SSD · UFS · AI Storage (2026-05-07 신규)
  // 출처: wiki/concepts/ssd-ufs-market.md, wiki/entities/nvidia-cmx-scada.md
  // ────────────────────────────────────────────
  {
    id: 'ai_ssd_iops_max',
    name: 'AI SSD 단일 IOPS 최고치 (NVIDIA Storage-Next)',
    category: 'quarterly',
    source: 'NVIDIA·Micron·SK hynix·Kioxia 발표',
    unit: 'M IOPS',
    unitLabel: '백만 IOPS',
    currentValue: 5.4,
    alertThreshold: 25,
    alertCondition: 'gte',
    alertDescription: '경쟁사 25M IOPS 도달 시 삼성 AI SSD 격차 결정적. 1억 IOPS는 차세대 PCIe 7.0 단계',
    scenarioSignals: [],
    scenarioText: 'SK·Kioxia 1억 IOPS 도달 (2027) 전 삼성 SLC AI SSD 로드맵 공개 시급',
    warningThreshold: 10,
    status: 'warning',
    lastUpdated: '2026-08-18',
    history: [
      { date: '2025-11', value: 5.4, note: 'Micron 9650 PCIe Gen6 — 단일 SSD 최고치' },
      { date: '2026-Q1', value: 5.4, note: 'Samsung PM1763 시연 (수치 미공개)' },
      { date: '2026-07', value: 5.4, note: 'Samsung PM1763 양산 개시(07-08, 16TB 28.4/21.9GB/s) — 대역폭 스펙 공개, 단일 IOPS 최고치는 여전히 미공개(SLC 초고 IOPS 트랙 로드맵 공백 유지)' },
      { date: '2026-2027E', value: 25, note: 'SK하이닉스 AI-N P 목표 (NVIDIA 공동 개발)' },
      { date: '2027E', value: 100, note: 'Kioxia·SK 1억 IOPS 단일 SSD 목표 (PCIe Gen7)' },
    ],
    note: 'SC\'25에서 Micron 9650 × 44개 + SCADA로 2.3억 IOPS 달성 (서버 합산). 삼성 단일 SSD 목표 미공개 — RS-3 정보 공백',
    inputType: 'number',
    hint: 'AI SSD 단일 IOPS 최고치 (백만 IOPS, 예: 5.4)',
    isCritical: true,
  },
  {
    id: 'samsung_ai_ssd_position',
    name: '삼성 AI SSD 포지션 (NVIDIA CMX/SCADA)',
    category: 'quarterly',
    source: 'Samsung GTC 2026 + NVIDIA 발표',
    unit: 'status',
    unitLabel: '단계',
    currentValue: null,
    alertCondition: 'manual',
    alertDescription: 'NVIDIA Storage-Next 핵심 파트너 자리를 SK·Kioxia가 선점. 삼성 SLC AI SSD 로드맵 부재',
    scenarioSignals: [],
    scenarioText: 'PM1753 CMX 공급은 즉각 매출 기회. SLC AI SSD 로드맵 공개가 다음 마일스톤',
    warningThreshold: null,
    status: 'warning',
    lastUpdated: '2026-08-18',
    history: [
      { date: '2026-03', value: 'PM1763 시연', note: 'GTC 2026 NVIDIA SCADA 시연' },
      { date: '2026-Q2', value: 'PM1753 CMX 공급', note: 'NVIDIA Vera Rubin CMX 공식 공급 (TLC V8 PCIe Gen5)' },
      { date: '2026-07', value: 'PM1763 양산 개시', note: '[실측] 2026-07-08 공식 양산 개시 — "차세대 AI 플랫폼 검증 완료"(9세대 V-NAND·4nm 컨트롤러·16TB 28.4/21.9GB/s). Vera Rubin 명시는 매체 해석으로 유보. CMX 물량 구조: 1유닛 576 SSD·9,600TB, 삼성 V-NAND 캐파 60%+ NVIDIA 배정 보도 (samsung-ssd-design-wins-nvidia-aipc-2026-08-16.md)' },
      { date: '2026-08', value: 'DGX Spark PM9E1 실탑재', note: '[실측] NVIDIA DGX Spark 4TB SSD가 삼성 PM9E1(M.2 2242·자체 5nm Presto 컨트롤러·14.5GB/s)임을 분해로 확인(MZALC4T0HBL1) — 개인용 AI 기기 design win. 단 SLC 초고 IOPS(SCADA) 트랙 로드맵은 여전히 공백' },
    ],
    note: 'CMX(즉각 매출 ✅) vs SCADA(구조적 위협 ⚠️) — 삼성 RS-3 다음 결정점은 SLC NAND 기반 초고 IOPS AI SSD 로드맵 공개 (2026 Tech Day 목표). [08-18] PM1763 양산 개시(07-08)·PM9E1 DGX Spark 실탑재로 CMX·클라이언트 축 실행 진전 확인 — "AI SSD 후행"은 SLC 초고 IOPS 트랙 한정으로 재해석. 캐파 60% NVIDIA 배정은 매출 락인이자 단일 고객 집중 리스크(RS-4 긴장) 병기',
    inputType: 'select',
    selectOptions: [
      { value: 'pm1763_demo', label: 'PM1763 GTC 시연', status: 'warning', signal: '생태계 진입' },
      { value: 'pm1753_supply', label: 'PM1753 CMX 공식 공급', status: 'warning', signal: 'CMX 매출 확보' },
      { value: 'pm1763_mass_prod', label: 'PM1763 양산 개시 + DGX Spark 실탑재 (현재)', status: 'warning', signal: 'CMX·클라이언트 축 실행 진전 (SLC 트랙은 공백)' },
      { value: 'slc_roadmap', label: 'SLC AI SSD 로드맵 공개', status: 'normal', signal: 'SCADA 추격 시작' },
      { value: 'nvidia_codev', label: 'NVIDIA 공동 개발 발표', status: 'normal', signal: 'B 가속' },
    ],
    hint: '삼성 AI SSD NVIDIA 파트너십 단계 선택',
    isCritical: true,
  },
  {
    id: 'enterprise_ssd_tam',
    name: '글로벌 Enterprise SSD 시장 TAM',
    category: 'annual',
    source: 'Mordor Intelligence·Intel Market Research',
    unit: '$B',
    unitLabel: '십억 달러',
    currentValue: 32,
    alertThreshold: null,
    alertCondition: 'manual',
    alertDescription: 'AI 워크로드가 SSD 수요 성장의 40% 기여. 북미 DC SSD CAGR 27.6% (2025-2031)',
    scenarioSignals: ['A', 'B'],
    scenarioText: 'TAM $69B 도달 (2031) 시 삼성 AI SSD 점유율 회복 여부가 RS-3 KPI. [08-11] 1Q26 실측 삼성 1위 38.2%',
    warningThreshold: null,
    status: 'normal',
    lastUpdated: '2026-08-11',
    history: [
      { date: '2025', value: 32, note: '글로벌 Enterprise SSD 매출 ($32B, 265 EB)' },
      { date: '2026-1Q', value: 74, note: '[실측] Top5 벤더 매출 $18.46B/분기(QoQ +86.1% 사상 최대) → 연환산 ~$74B. 삼성 1위 38.2%($7.05B·QoQ +92.8%)·SK그룹 25.1%·Micron 16.7%·Kioxia 12.0%·SanDisk 8.0%. 계약가 분기 +80%(공급 제약)·재고 사상 최저. 단 쇼티지 가격 급등이 부풀린 수치(TrendForce, enterprise-ssd-market-1q26-2026-08.md)' },
      { date: '2031E', value: 69, note: '북미 Data Center SSD만으로 $69.08B (CAGR 27.6%)' },
    ],
    note: '하이퍼스케일러가 60% 매출 기여 (2024) → 65% (2025). PCIe Gen5 양산, Gen6 2026 진입, Gen7 2027~2028. [08-11] 1Q26 삼성 enterprise SSD 1위(38.2%) 실측 — AI-스토리지 NAND 수요 강세 확인(삼성 개별 실행 신호, 거시 축 무관). Micron 컨슈머 Crucial 철수로 전 산업 enterprise 집중. [2026-09-08] 대용량 QLC eSSD 용량 리더십 타임라인 병기(qlc-essd-timeline-fdp-ruh-2026-09.md) — Solidigm이 61TB급 12개월 선행(2023-07 vs 삼성 2024-07)·245TB 첫 상용 출하는 Micron(6600 ION, 2026-05)·삼성 122TB급은 아직 announcement/전시 단계. AI 서버 시장 가치 2024 $187B(+69%)·서버 시장 가치의 65%(2023 23%→65%)로 "총량 붕괴가 아니라 재배분" 명제 정량 근거. 삼성 용량 리더십 후발은 다운턴 극복의 "니즈 적중" 경쟁에서 리스크(제품믹스 신호, 거시 축 무관)',
    inputType: 'number',
    hint: '글로벌 Enterprise SSD 시장 TAM ($B) 입력',
  },
  {
    id: 'ufs_lpddr5x_price_change',
    name: 'UFS 4.1 + LPDDR5X 모바일 메모리 가격 변동률',
    category: 'quarterly',
    source: 'abit.ee·Global Semi Research',
    unit: '%',
    unitLabel: '% YoY',
    currentValue: 85,
    alertThreshold: 100,
    alertCondition: 'gte',
    alertDescription: '+100%+ 시 스마트폰 BOM 부담 한계 도달 → 2026 Q4 capex 가동으로 가격 조정 예상',
    scenarioSignals: ['B'],
    scenarioText: 'UFS 4.1 / LPDDR5X 가격 +80~90% (2026 H1) — AI 부족이 모바일로 전이. 2026 Q4 부족 완화 시점',
    warningThreshold: 70,
    status: 'warning',
    lastUpdated: '2026-05-07',
    history: [
      { date: '2026-H1', value: 85, note: 'UFS 4.1 1TB + LPDDR5X 16GB 평균 +80~90% YoY' },
      { date: '2026-Q4E', value: 30, note: 'Samsung·Micron 신규 capex 가동으로 부족 완화 예상' },
    ],
    note: '플래그십 표준이 12GB+512GB → 16GB+1TB UFS 4.1로 이행. 모바일 메모리 시장은 Samsung·SK·Micron 3강 + CXMT LPDDR5X-10667 시연으로 가시화',
    inputType: 'number',
    hint: 'UFS 4.1 + LPDDR5X 평균 가격 변동률 % YoY 입력',
  },

  // ────────────────────────────────────────────
  // CHIPS Act & 보조금 (2026-05-07 신규/보강)
  // 출처: wiki/concepts/chips-act.md
  // ────────────────────────────────────────────
  {
    id: 'taylor_2nm_volume_production',
    name: '텍사스 테일러 2nm 양산(Volume) 시점',
    category: 'annual',
    source: 'Samsung 공식 발표·Tom\'s Hardware·NIST',
    unit: 'status',
    unitLabel: '단계',
    currentValue: null,
    alertCondition: 'manual',
    alertDescription: '2027 H2 양산 목표 — 슬립 시 Tesla AI5/AI6 공급 차질 + Micron Idaho ID2(2027) 추월 위험',
    scenarioSignals: ['A', 'B'],
    scenarioText: '2027 H2 양산 → B 가속 / 2028 슬립 → A 위기 (마이크론 Idaho 추월)',
    warningThreshold: null,
    status: 'warning',
    lastUpdated: '2026-05-06',
    history: [
      { date: '2026-Q1', value: '장비 반입 시작', note: '2nm 공정용 첫 장비 (3월), Tesla 칩 장비 이전 (4월)' },
      { date: '2026-Q2', value: '위험생산 (Risk Production) 시작', note: '목표' },
      { date: '2027-H2', value: '양산 (Volume Production)', note: '목표 — 4nm→2nm 업그레이드로 1~2년 후퇴' },
    ],
    note: 'CHIPS Act $4.745B 연방 + Texas $250M 주정부. Tesla AI5/AI6 $16.5B 계약이 첫 주요 고객. 1단계 가동 슬립 시 D8(2단계 발표·추가 보조금) 협상력 약화',
    inputType: 'select',
    selectOptions: [
      { value: 'on_track', label: '2026 Q2 위험생산 시작 (정상)', status: 'normal', signal: 'B 진행' },
      { value: 'risk_prod', label: '위험생산 진입', status: 'normal', signal: 'B 가속' },
      { value: '2027h2_volume', label: '2027 H2 양산 도달', status: 'normal', signal: '목표 달성' },
      { value: 'delayed_2028', label: '2028 이후 슬립', status: 'critical', signal: 'A·C 위기' },
    ],
    hint: 'Taylor 2nm 양산 단계 선택',
    isCritical: true,
  },
  {
    id: 'us_subsidy_competitor_gap',
    name: '미국 CHIPS Act 보조금 경쟁사 격차 (Samsung vs TSMC·Micron·Intel)',
    category: 'annual',
    source: 'NIST CHIPS Program·AIP.ORG',
    unit: '$B',
    unitLabel: '$B (직접 보조금)',
    currentValue: 4.745,
    alertThreshold: null,
    alertCondition: 'manual',
    alertDescription: 'Samsung $4.745B < Micron $61.6억 < TSMC $66억 < Intel $85억. 2단계 추가 협상 필수',
    scenarioSignals: ['A'],
    scenarioText: 'D8(2단계 추가 보조금) 협상 진척 — 격차 축소 vs Idaho ID2 + Ohio 고착화 위험',
    warningThreshold: null,
    status: 'warning',
    lastUpdated: '2024-12-10',
    history: [
      { date: '2024-04', value: 6.4, note: '예비 발표 ($6.4B)' },
      { date: '2024-12', value: 4.745, note: '최종 확정 ($4.745B) — 협상 과정에서 $1.655B 감소' },
      { date: '2026-Q4-목표', value: 7.5, note: 'D8 — HBM 전용 2단계 추가 보조금 협상 ($2.75B+ 추가 목표)' },
    ],
    note: '비교: TSMC $66억(애리조나) / Micron $61.65억(아이다호+NY) / Intel $85억. Samsung은 3위 규모 + Texas 주정부 $250M 별도. K-칩스법 세액공제 20%(대기업) 병행',
    inputType: 'number',
    hint: 'Samsung 미국 CHIPS Act 보조금 누적액 ($B) 입력',
  },

  // ────────────────────────────────────────────
  // NVIDIA Storage-Next 파트너십 + 중국 빅펀드 III (2026-05-07 신규)
  // ────────────────────────────────────────────
  {
    id: 'nvidia_storage_next_partner',
    name: 'NVIDIA Storage-Next 파트너십 지위',
    category: 'quarterly',
    source: 'NVIDIA 발표·TrendForce·Blocks & Files',
    unit: 'status',
    unitLabel: '지위',
    currentValue: null,
    alertCondition: 'manual',
    alertDescription: 'SK하이닉스 + Kioxia가 Storage-Next 핵심 파트너 선점. Phison 컨트롤러 + SK SLC NAND 레퍼런스 플랫폼화 가능',
    scenarioSignals: ['B'],
    scenarioText: '핵심 파트너 진입 → B 가속 / 후행 고착 → RS-3 SCADA 트랙 위기',
    warningThreshold: null,
    status: 'warning',
    lastUpdated: '2026-08-18',
    history: [
      { date: '2025-09', value: 'Kioxia 1억 IOPS 발표', note: 'Kioxia + NVIDIA 공동 개발' },
      { date: '2025-11', value: 'Micron 9650 SCADA 레퍼런스', note: 'SC\'25 2.3억 IOPS 시연' },
      { date: '2025-12', value: 'SK AI-N P 발표', note: 'NVIDIA 공동 개발 — 1억 IOPS 목표 (SLC NAND)' },
      { date: '2026-03', value: 'Samsung PM1763 시연', note: 'GTC 2026 — 생태계 참여 단계' },
      { date: '2026-08', value: 'CMX 공급자 지위 심화 (PM1763 양산)', note: '[실측] PM1763 양산 개시(07-08)·CMX V-NAND 캐파 60%+ NVIDIA 배정으로 CMX(TLC) 공급자 지위는 심화 — 그러나 핵심 파트너(SLC NAND 초고 IOPS SCADA 레퍼런스)는 여전히 SK·Kioxia 선점. 삼성 SLC AI SSD 로드맵 미공개 = 최대 정보 공백 유지 (samsung-ssd-design-wins-nvidia-aipc-2026-08-16.md)' },
    ],
    note: 'Storage-Next는 NVIDIA AI SSD 생태계의 미래 표준. SLC NAND 기반 초고 IOPS가 핵심 — 삼성 SLC AI SSD 로드맵 미공개가 가장 큰 정보 공백. [08-18] CMX(TLC) 축은 양산·캐파 배정으로 공급자 지위 심화, SLC(SCADA) 축 로드맵은 미도달. [2026-09-08] CMX 생태계 FDP 요구 수준 병기(qlc-essd-timeline-fdp-ruh-2026-09.md) — 현행 표준 enterprise FDP SSD의 RUH(Reclaim Unit Handle)는 통상 2~8개인데, KV cache 오프로드(CMX, BlueField 기반)는 세션·테넌트·공유 프리픽스·수명 등급별 스트림 분리를 위해 훨씬 많은 스트림이 필요 — ScaleFlux는 CMX 타깃 플랫폼에서 200+ FDP 쓰기 스트림·유효 7~10+ DWPD(5년)를 발표(2026-07-30, "200개"는 NVIDIA 공식 스펙이 아니라 ScaleFlux 플랫폼 기준). 삼성 FDP·니어라인 SSD·PBSSD 컨셉(송용호 09-03)이 이 KV cache 락인 축의 대응 제품군',
    inputType: 'select',
    selectOptions: [
      { value: 'observer', label: '관찰자 (생태계 참여)', status: 'warning', signal: 'PM1763 시연 단계' },
      { value: 'supplier', label: 'CMX 공급자 (PM1753)', status: 'warning', signal: '즉각 매출 확보' },
      { value: 'co_dev_announced', label: 'NVIDIA 공동 개발 발표', status: 'normal', signal: 'B 가속' },
      { value: 'core_partner', label: '핵심 파트너 (Reference)', status: 'normal', signal: '시장 주도' },
    ],
    hint: '삼성 NVIDIA Storage-Next 파트너십 지위 선택',
    isCritical: true,
  },
  {
    id: 'china_bigfund_iii_deployment',
    name: '중국 빅펀드 III 집행 진척 (CXMT·YMTC 위협)',
    category: 'annual',
    source: 'Bloomberg·Reuters·Yole Group',
    unit: '$B',
    unitLabel: '십억 달러 (집행 누적)',
    currentValue: null,
    alertThreshold: 30,
    alertCondition: 'gte',
    alertDescription: '$30B+ 집행 시 CXMT·YMTC 캐파 확장 가속. 빅펀드 III 총 규모 ~$47B (3,440억 위안)',
    scenarioSignals: ['D', 'C'],
    scenarioText: '빅펀드 III 본격 집행 → CXMT·YMTC 범용 메모리 점유율 가속 → D 시나리오 강화',
    warningThreshold: 15,
    status: 'unknown',
    lastUpdated: '2024-03',
    history: [
      { date: '2024-03', value: 0, note: '빅펀드 III 설립 (3,440억 위안 / ~$470억)' },
      { date: '빅펀드 I (2014)', value: 19, note: '약 1,387억 위안 (~$190억) — CXMT 전신·YMTC·SMIC' },
      { date: '빅펀드 II (2019)', value: 28, note: '약 2,041억 위안 (~$280억) — 제조·장비·소재' },
    ],
    note: '국가집성전로산업투자기금 III. 첨단 공정·장비·메모리 자립에 집중 투자. 미국 수출통제 강화 대응. 지방정부 직접 지원 (안후이성·후베이성) 별도. 14~15차 5개년 계획 메모리 자급률 40% 목표',
    inputType: 'number',
    hint: '빅펀드 III 집행 누적액 ($B) 입력',
  },

  // ────────────────────────────────────────────
  // 수요 변곡 조기경보 (Demand-Inflection EWI)
  // 인과 사슬: 수요청산가(GPU임대가) → 돈(파이낸싱) → 발주미시(스팟) → 착공 → 메모리(재고)
  // 착공보다 "왼쪽" 신호가 먼저 꺾이면 = 하락 변곡 조기경보. wiki/concepts/demand-inflection-ewi.md
  // ────────────────────────────────────────────
  {
    id: 'gpu_rental_price_trend',
    name: 'GPU 임대가 추세 (H100/B200, 6개월)',
    category: 'monthly',
    source: 'Vast.ai·RunPod·Lambda·SemiAnalysis ClusterMAX',
    unit: '%',
    unitLabel: '% (6개월 변동)',
    currentValue: -20,
    alertThreshold: -35,
    alertCondition: 'lte',
    alertDescription: '6개월 -35% 이하 급락 → AI 컴퓨트 ROI 악화·발주 둔화 선행 (Tier0 최선행)',
    scenarioSignals: ['D', 'C'],
    scenarioText: '컴퓨트 공급과잉 신호 → 메모리 수요 둔화 선행',
    warningThreshold: -20,
    status: 'warning',
    lastUpdated: '2026-07-28',
    history: [
      { date: '2026-06-02', value: -20, note: '2024~25 H100 임대가 급락 후 둔화세 (예시 판단값)' },
      { date: '2026-07-21', value: -20, note: '[정기 점검] 최신 실측은 firming — H200 온디맨드 +8% YoY(2025-07 $3.54→2026-07 $3.82). 6개월 추세 급락 신호 없음, -35% 경보 임계와 여유. 실측 바스켓(gpu_rental_h100_usd) 우선 참조' },
      { date: '2026-07-28', value: -20, note: '[정기 점검] 07-28 스냅샷 firming/flat — H100 온디맨드 중앙값 ~$3.46/h·H200 코호트 중앙값 ~$4.11/h(getdeploying). 주간 급락 신호 없음, Tier0 최선행 조기경보 미발동' },
      { date: '2026-08-11', value: -20, note: '[정기 점검] 08-11 스냅샷 firming/flat — H200 코호트 중앙값 ~$4.38/h(소폭 상승)·H100 온디맨드 ~$2.69/h(Jarvislabs). 6개월 -35% 급락 신호 없음, gpu_rental_collapse 트리거·수요 변곡 조기경보 미발동' },
      { date: '2026-09-15', value: -20, note: '[정기 점검] 09-12 스냅샷 firming/flat — H100 온디맨드 중앙 ~$2.65~2.89/h·H200 +20~40% 프리미엄. "메모리 병목" 진단 유지, 6개월 -35% 급락 신호 없음(-35% 경보 임계와 여유). Tier0 최선행 조기경보 미발동. 실측 바스켓(gpu_rental_h100_usd) 우선 참조' },
    ],
    note: '메모리 수요의 최선행(Tier0) — 컴퓨트 수요 청산가격. 급락 = 과잉 빌드 신호. 2026-09 firming/flat(메모리 병목). demand-inflection-ewi.md',
    inputType: 'number',
    hint: 'GPU 시간당 임대가 6개월 변동률 % (하락이면 음수)',
  },
  {
    id: 'gpu_rental_h100_usd',
    name: 'GPU 현물 임대가 (H100·H200 바스켓, Vast.ai)',
    category: 'monthly',
    source: 'Vast.ai 공개 오퍼 API',
    unit: '$/h',
    unitLabel: '$/GPU·h (바스켓 중앙값)',
    currentValue: 3.21,
    alertThreshold: 2.0,
    alertCondition: 'lte',
    alertDescription: 'AI GPU 바스켓 < $2.0/h 지속 → 컴퓨트 공급과잉·AI 수요 둔화 선행 (Tier0 실측)',
    scenarioSignals: ['D', 'C'],
    scenarioText: 'GPU 현물 급락 = 메모리 수요 둔화 선행',
    warningThreshold: 2.6,
    status: 'normal',
    lastUpdated: '2026-09-22',
    history: [
      { date: '2026-06-02', value: 3.21, note: 'Vast.ai 바스켓 (H100 SXM $2.52 / H200 $3.91, 0.5:0.5)' },
      { date: '2026-07-21', value: 3.21, note: '[정기 점검] H200 온디맨드 중앙값 $3.82 (전년 $3.54 대비 +8% YoY, getdeploying)·H100 Vast.ai 최저 ~$1.49/h — 붕괴 아닌 firming. Tier0 최선행 신호 정상, 수요 변곡 조기경보 미발동. Vast.ai API 자동 갱신 지속' },
      { date: '2026-07-28', value: 3.21, note: '[정기 점검] 07-28 시장 스냅샷 — H100 온디맨드 중앙값 ~$3.46/h(범위 $1.49~6.98)·H200 코호트 중앙값 ~$4.11/h(범위 $2.30~13.78, getdeploying). 직전 $3.82 대비 정합, 붕괴 없음. 실측 바스켓은 Vast.ai API 자동 갱신 지속' },
      { date: '2026-08-11', value: 3.21, note: '[정기 점검] 08-11 시장 스냅샷 — H100 온디맨드 중앙값 ~$2.69/h(Jarvislabs, 범위 $1.49~6.98·스팟 ~$1.03~2.00)·H200 온디맨드 ~$3.99/h(Jarvislabs)·복수 공급사 중앙값 ~$4.38/h(스팟 ~$1.99, getdeploying). H200 중앙값 소폭 상승(~$4.11→~$4.38)·H100 공급사별 분산 큼 — firming/flat, 붕괴 없음. Tier0 최선행 조기경보 미발동' },
      { date: '2026-09-15', value: 3.21, note: '[정기 점검] 09-12 시장 스냅샷 — H100 온디맨드 마켓/부티크 ~$1.38~2.89/h·하이퍼스케일러 온디맨드 ~$11.68~12.29/h(중앙 ~$2.65~2.89)·H200은 H100 대비 +20~40% 프리미엄(Thundercompute·IntuitionLabs·GPUSmith). "시장 병목은 실리콘이 아니라 메모리"라는 진단 유지 — firming/flat, 붕괴 없음. Tier0 최선행 조기경보 미발동. 실측 바스켓은 Vast.ai API 자동 갱신 지속' },
      { date: '2026-09-22', value: 3.21, note: '[정기 점검] 09-22 시장 스냅샷 — H100 9월 온디맨드 중앙값 ~$3.25/GPU-h(범위 $1.49~6.98, Vast.ai 최저 ~$1.49)·H200 9월 온디맨드 중앙값 ~$4.40/GPU-h(Vast 최저 인스톡 $1.98, 09-16 08:00 UTC)(Thundercompute·IntuitionLabs·gpufinder). 직전 스냅샷 대비 firming(중앙값 상승)·붕괴 없음. Tier0 최선행 조기경보 미발동. 실측 바스켓은 Vast.ai API 자동 갱신 지속' },
    ],
    note: '실측 GPU 현물 임대가(Tier0 최선행). H100 SXM·H200 0.5:0.5 바스켓으로 유동성 안정화. 매일 자동 누적되어 추세 형성. 23년 ~$8 → 25~26년 $2~4. 2026-09 firming(H100 9월 온디맨드 중앙값 ~$3.25/h·H200 ~$4.40/h·메모리 병목).',
    inputType: 'number',
    hint: 'AI GPU 바스켓 중앙 임대가 $/GPU·h (Vast.ai 자동 갱신)',
    autoUpdateId: 'gpu_rental_h100_usd',
    autoUpdateSource: 'Vast.ai 공개 오퍼 API — H100 SXM·H200 바스켓 on-demand 중앙값 (실측, 프록시 아님)',
    autoUpdateIsProxy: false,
  },
  {
    id: 'gpu_supply_offers',
    name: 'GPU 현물 공급 (Vast.ai 가용 오퍼)',
    category: 'monthly',
    source: 'Vast.ai 공개 오퍼 API',
    unit: '건',
    unitLabel: '건 (가용 on-demand)',
    currentValue: 48,
    alertThreshold: 200,
    alertCondition: 'gte',
    alertDescription: '가용 AI GPU 오퍼 급증(>200) → 컴퓨트 현물 공급 완화·수요 둔화 신호 (추세로 해석)',
    scenarioSignals: ['D', 'C'],
    scenarioText: '현물 공급 완화 = 컴퓨트 수급 이완',
    warningThreshold: 120,
    status: 'normal',
    lastUpdated: '2026-06-02',
    history: [{ date: '2026-06-02', value: 48, note: 'Vast.ai H100 SXM/H200/NVL 가용 오퍼 합 (예시)' }],
    note: 'Vast 마켓 가용 오퍼 수(현물 공급 barometer, 노이즈). 절대값보다 추세 — 증가 = 공급 완화. 마켓플레이스 한정.',
    inputType: 'number',
    hint: 'Vast.ai 가용 AI GPU 오퍼 수 (자동 갱신)',
    autoUpdateId: 'gpu_supply_offers',
    autoUpdateSource: 'Vast.ai 공개 오퍼 API — 가용 on-demand 오퍼 수 (H100/H200, 실측)',
    autoUpdateIsProxy: false,
  },
  {
    id: 'ai_dc_credit_spread',
    name: 'AI-DC 파이낸싱 신용 스프레드 (6개월 변동)',
    category: 'monthly',
    source: 'Oracle·CoreWeave 채권, Blue Owl/Meta SPV, 사모신용',
    unit: 'bps',
    unitLabel: 'bps (6개월 변동)',
    currentValue: 40,
    alertThreshold: 150,
    alertCondition: 'gte',
    alertDescription: '+150bps 이상 확대 → AI-DC 조달 급랭·파이프라인 위축 선행 (착공 1~3분기 선행)',
    scenarioSignals: ['D', 'C'],
    scenarioText: '돈이 먼저 식는다 — capex·착공을 선행',
    warningThreshold: 75,
    status: 'normal',
    lastUpdated: '2026-09-22',
    history: [
      { date: '2026-06-02', value: 40, note: 'capex 부채·SPV·ABS 의존 확대, 스프레드 완만 (예시 판단값)' },
      { date: '2026-09-15', value: 40, note: '[정기 점검, 값 무변화·질적 경보] 광의 HY 스프레드 급확대 미관측이나 조달 구조 tell 실측: NVIDIA-OpenAI 오하이오 파이낸싱 개런티 $250B→~$105B 축소·Oracle YTD -24%·HSBC OpenAI +$207B 조달 갭. 경보 임계(+150bps) 여유, 미발동 — 조달 구조 경보로 격상 감시' },
      { date: '2026-09-22', value: 40, note: '[정기 점검, 값 무변화·질적 경보 심화] NVIDIA-OpenAI-Oracle $745B 순환 파이낸싱 루프가 "첫 스트레스 테스트(연준 금리)" 국면으로 보도(Yahoo Finance)·Oracle $43B 차입으로 마이너스 FCF 커버·OpenAI 2026 매출 ~$20B 대비 손실 ~$14B·AI 순환경제 버블 인덱스 73/100. 광의 HY 스프레드 자체 급확대는 여전히 미관측(경보 +150bps 여유)·CapEx 삭감 없음 → 조달 구조 경보 유지, 미발동. 송용호 관전 ②(FCF→대출→지분 매각→NVIDIA 금융업체화) 체인 실측 심화' },
    ],
    note: 'capex 상당분이 부채·SPV·ABS(Meta-Blue Owl, Oracle, CoreWeave). 스프레드 확대 = 신규 빌드 조달 위축. [2026-09-15] 관전 포인트 ② 체인의 실측 데이터 등장(웹 리서치) — NVIDIA가 OpenAI 오하이오 DC 파이낸싱 개런티를 $250B→<$120B(~$105B)로 축소(CNBC 08-17·Network World), Fortune(08-18) "보도 대비 $145B 낮아 칩 인공 수요 우려 신호"로 해석. Oracle 주가 YTD -24%(AI 버블 우려)·HSBC "OpenAI가 2030 $200B 매출을 내도 사업 유지에 +$207B 추가 조달 필요"·OpenAI-Oracle $300B/AWS $38B 다년 약정 대비 공급사 ~$96B 부채. = "FCF→대출→지분 매각→NVIDIA 금융업체화" 체인이 실측 국면 진입. 단 광의 HY 스프레드 자체는 급확대 미관측(경보 임계 +150bps와 여유)·CapEx 삭감 없음 → 조달 구조 경보로 격상 감시(demand_inflection_divergence·enterprise_ai_roi_proven 연동), 미발동. [2026-09-08] 다운턴 센싱 관전 포인트 ②(하이퍼스케일러 자금 구조) 축으로 승격(송용호 인터뷰 09-03) — 그동안은 자체 캐시플로우 투자였으나 FCF가 네거티브로 가면 순서대로 대출→지분 매각→NVIDIA가 금융업체처럼 자금을 대는 악성 체인 가능성. 이창수 "꼭짓점은 CapEx가 아니라 FCF" 프레임과 정합. 스프레드 확대는 이 체인의 실측 선행 신호 → demand_inflection_divergence와 함께 감시',
    inputType: 'number',
    hint: 'AI-DC 부채/ABS 신용 스프레드 6개월 변동(bps). 확대면 양수',
    autoUpdateId: 'ai_dc_credit_spread',
    autoUpdateSource: 'HYG 6개월 역행·듀레이션 환산 프록시 (Yahoo) — 광의 HY 신용·금리 영향 포함',
    autoUpdateIsProxy: true,
  },
  {
    id: 'dc_cancellation_count',
    name: 'AI DC 착공 취소·연기 건수 (분기)',
    category: 'quarterly',
    source: 'wiki/concepts/ai-datacenter-buildout.md · DCD 등',
    unit: '건',
    unitLabel: '건/분기',
    currentValue: 2,
    alertThreshold: 5,
    alertCondition: 'gte',
    alertDescription: '분기 5건+ → 파이프라인 위축 본격화 (착공 지표 내 선행 신호)',
    scenarioSignals: ['D', 'C'],
    scenarioText: '발표보다 빠른 신호 — 취소·연기 급증 = 변곡',
    warningThreshold: 3,
    status: 'normal',
    lastUpdated: '2026-06-02',
    history: [{ date: '2026-06-02', value: 2, note: 'Abilene 600MW 철회 · Stargate Norway OpenAI 이탈' }],
    note: '신규 착공보다 선행. 취소·연기·축소를 별도 카운트. [2026-09-08] 다운턴 센싱 관전 포인트 ③(전기) 병기(송용호 인터뷰 09-03) — 美 2027~2028 전력 쇼티지가 DC 건설 속도를 늦출 수 있고, 전력 수급 균형 전후로 저전력(파워 효율) 요구가 급증한다는 관측. 전력 병목發 착공 지연·취소는 이 카운트에 포착되며, 파워는 성능과 별개의 채택 차별화 축(송용호 "성능은 시스템이 흡수하므로 무기가 아니고 진짜 다른 축은 파워")으로 제품 전략에도 반영',
    inputType: 'number',
    hint: '분기 AI DC 착공 취소·연기·축소 건수 입력',
  },
  {
    id: 'spot_contract_spread',
    name: '스팟-계약가 괴리 (DRAM)',
    category: 'monthly',
    source: 'DRAMeXchange·TrendForce',
    unit: '%',
    unitLabel: '% (스팟−계약)',
    currentValue: 6,
    alertThreshold: -10,
    alertCondition: 'lte',
    alertDescription: '스팟이 계약 -10% 이하 → 하락 변곡 선행 (스팟이 계약을 주~월 선행)',
    scenarioSignals: ['D', 'C'],
    scenarioText: '스팟 롤오버 = 계약가 하락 조기경보',
    warningThreshold: -3,
    status: 'normal',
    lastUpdated: '2026-06-02',
    history: [{ date: '2026-06-02', value: 6, note: '부족으로 스팟>계약 (양수) (예시 판단값)' }],
    note: '부족기엔 스팟>계약(양수). 음전환·확대 = 조기경보',
    inputType: 'number',
    hint: '스팟가 대비 계약가 괴리 % (스팟이 높으면 양수)',
  },
  {
    id: 'memory_inventory_days',
    name: '메모리 재고 (DRAM+NAND, 주)',
    category: 'monthly',
    source: 'TrendForce·각사 IR',
    unit: '주',
    unitLabel: '주 (재고)',
    currentValue: 4,
    alertThreshold: 12,
    alertCondition: 'gte',
    alertDescription: '12주+ → 가격 하락 임박 (가격 하락 前 재고부터 쌓이는 고전적 최선행)',
    scenarioSignals: ['D', 'C'],
    scenarioText: '재고 급증 = 수요 둔화 조기 신호',
    warningThreshold: 8,
    status: 'normal',
    lastUpdated: '2026-06-02',
    history: [{ date: '2026-06-02', value: 4, note: '부족으로 낮은 재고 (예시 판단값)' }],
    note: '메모리사+고객(하이퍼스케일러·OEM)+채널 재고. 상승 전환 = 조기경보',
    inputType: 'number',
    hint: '메모리 재고 주수 입력 (현재 부족 ~4주)',
  },

  // ────────────────────────────────────────────
  // LTA→SCA 전환 추적 (2026-07-04 신설 — wiki/concepts/lta-to-sca-transition.md §5,
  // 기준선: sources/articles/sca-ewi-baseline-2026-07-04.md)
  // ────────────────────────────────────────────
  {
    id: 'competitor_sca_disclosures',
    name: '경쟁사 SCA형 계약 공시 (공동설계 포함, 분기)',
    category: 'quarterly',
    source: '각사 IR·보도 — wiki/concepts/lta-to-sca-transition.md',
    unit: '건',
    unitLabel: '건/분기',
    currentValue: 1,
    alertThreshold: 2,
    alertCondition: 'gte',
    alertDescription: '분기 2건+ → 3단계(SCA) 표준화 가속 — 개발실 전환(DT) 시급도 격상',
    scenarioSignals: ['B', 'A'],
    scenarioText: 'SCA가 계약 표준이 되는 속도 — 늦을수록 2nd source 고착 리스크',
    warningThreshold: 1,
    status: 'warning',
    lastUpdated: '2026-08-04',
    history: [
      { date: '2026-07-04', value: 1, note: '2026 Q2: Micron–Anthropic SCA(06-22) 1건. 누적 스톡: Micron SCA 16건·$100B 공시, SK hynix 커스텀 HBM 인증 3건(2025-06)' },
      { date: '2026-08-04', value: 1, note: '[스톡 정성 확인 — 이창수 부사장 08-03] 신규 공시 0건(값 무변화). 영업 수장 1차 확인: CSP·AI 프론티어끼리 원래 take-or-pay로 계약하던 관행이 메모리에도 처음 바인딩되며 산업 전반이 젠틀맨십 LTA→법적 구속(선수금·NTB/NTE) 체제로 표준화 중 — 계약 구조 락인(DF1 바닥 경직화) 논지의 1차 자료 재확인' },
    ],
    note: '공동설계 요소를 포함한 전략 계약 공시만 카운트 (단순 LTA 제외). 스톡(누적)은 note로 병기. [08-04] 이창수 1차 확인 — take-or-pay/NTB가 산업 표준으로 정착 중',
    inputType: 'number',
    hint: '분기 중 경쟁사(SK hynix·Micron)의 공동설계 포함 계약 공시 건수',
  },
  {
    id: 'samsung_codesign_contracts',
    name: 'Samsung 공동설계 조항 포함 계약 (공시)',
    category: 'quarterly',
    source: 'Samsung IR·보도 — wiki/strategies/dev-org-transformation.md',
    unit: '건',
    unitLabel: '건 (누적 공시)',
    currentValue: 0,
    alertThreshold: 1,
    alertCondition: 'gte',
    alertDescription: '첫 공시 1건+ = 개발실 전환 Phase 2 목표(SCA형 계약 수주) 달성 신호 (긍정 경보)',
    scenarioSignals: ['B'],
    scenarioText: '자사 전환 진도 — 0에 머무는 동안 경쟁사와 단계 격차 지속',
    warningThreshold: 0,
    status: 'warning',
    lastUpdated: '2026-08-04',
    history: [
      { date: '2026-07-04', value: 0, note: '공시 0건. 선행 신호: 커스텀 HBM 전담 2팀·엔지니어 250명 증원(Google·Meta·NVIDIA 타깃), Broadcom·AMD 커스텀 협의, 커스텀 HBM4E 설계 완료 목표 2026 중반' },
      { date: '2026-08-04', value: 0, note: '[1차 자료 확인 — 이창수 부사장 08-03] 공개 공시 기준 0건 유지. 단 영업 수장 1차 확인: 삼성이 이미 take-or-pay 멀티이어 다수 사인(열 곳 동시 진행·페이즈2 대기)·선수금 수백억 달러 규모 예치·NTB(하한)/NTE(상한) 가격 밴드·캐파 과반 훨씬 넘는 수준 바인딩 목표. 공동설계 조항(SCA)과는 다른 축(계약 바인딩 강도)이나, 다년 계약 락인 체질 전환이 실제 진행 중임을 확인 — 공시(공개)≠내부계약. 페이즈2는 중국 빅테크(바이트댄스·알리바바·텐센트)까지 확대' },
    ],
    note: 'Series H 파트너 지정은 자본 연계일 뿐 공동설계 계약 아님 — 0건 기준. 상승이 목표인 긍정 지표. [08-04] 이창수 1차 확인 — take-or-pay/NTB 멀티이어 바인딩은 진행 중이나 공개 SCA 공시와는 별개 축',
    inputType: 'number',
    hint: 'Samsung의 공동설계 조항 포함 계약 공시 누적 건수',
  },
  {
    id: 'custom_hbm_revenue_share',
    name: '커스텀 HBM 매출 비중 (업계)',
    category: 'quarterly',
    source: 'TrendForce·Counterpoint — sources/articles/sca-ewi-baseline-2026-07-04.md',
    unit: '%',
    unitLabel: '% (전체 HBM 매출 중)',
    currentValue: 0,
    alertThreshold: 30,
    alertCondition: 'gte',
    alertDescription: '30%+ → 범용→커스텀 역전 가속 — 범용 잔여 시장 고착(R2) 리스크 현실화 구간',
    scenarioSignals: ['B', 'E'],
    scenarioText: '커스텀 전환 속도 — HBM4E가 2027년 HBM 수요 ~40% 전망(TrendForce)',
    warningThreshold: 15,
    status: 'normal',
    lastUpdated: '2026-09-01',
    history: [
      { date: '2026-07-04', value: 0, note: '2026 매출은 표준 HBM3E/HBM4 중심 — 커스텀 매출 ~0%(설계 단계). 선행: HBM4E 2027년 ~40%·ASIC 수요 2026 +80%' },
      { date: '2026-08-04', value: 0, note: '[1차 자료 대비 — 인터뷰 2건] 값 무변화(현세대 커스텀 HBM 매출 ~0 유지)이나 방향은 층위가 갈림. 이창수(08-03): 커스텀 HBM4는 거의 다 캔슬 — 커머디티가 해마다 3/3E/4/4E로 앞서가는데 커스텀은 개발 2~3년 걸려 뒤늦게 나올 때 기술적으로도 열위, 지금 빅뱅 구간엔 커스텀 의미 없음 → 커머디티 기술 몰빵. 최장석(07-29): 반대로 zHBM(GPU 위 3D 수직 적층 커스텀)은 전 고객 요구·비표준이라도 타임투마켓 채택 늘어남·업체별 인터레이어 달라 하나의 커스텀 제품. 상충 아니라 층위(현세대 커스텀 HBM 퇴조 vs 차세대 zHBM 커스텀 부상, 2027+) — 이 지표는 zHBM 램프 시점부터 재정의 필요' },
      { date: '2026-09-01', value: 0, note: '[08-28 수집, 값 무변화·구조 신호 확정] 커스텀 HBM이 발표 단계로 가시화 — ① NVIDIA \'NVHBM\' 08-26 공식 발표: 메모리 컨트롤러를 base die로 이동, NVLink Fusion 파트너가 커스텀 PHY를 자기 설계에 통합. 첫 파트너 Amazon Annapurna Trainium4, Feynman 세대(2028) 첫 채택 GPU. 제조는 삼성·SK가 표준 구현으로 담당(고객이 base die 로직 설계 시 TSMC 파운드리 생산 가능). ② 삼성 zHBM Hot Chips 2026 스펙 공개(HBM4E 대비 전력효율 +70%·대역폭 2.3배, 상용시점 미공개). ③ SK는 하이브리드본딩을 HBM5(2029)로 연기, 삼성 3D 적층 vs SK 패키징 고도화 노선 분화. 현세대 커스텀 매출 여전히 ~0 유지, 본격 상승은 2027~2028 HBM4E/zHBM 램프부터 — 이 지표 재정의 트리거는 zHBM/NVHBM 양산 개시' },
    ],
    note: '커스텀(고객별 베이스다이·bespoke) HBM 매출 / 전체 HBM 매출. 2027 HBM4E 램프부터 본격 상승 예상. [08-04] 1차 자료: 현세대 커스텀 HBM4 퇴조(이창수) vs 차세대 zHBM 커스텀 부상(최장석) — DF3/제품믹스 축, 층위 구분 필요',
    inputType: 'number',
    hint: '업계 전체 HBM 매출 중 커스텀 비중 % (TrendForce 분기 리포트 기준)',
  },
]

// ────────────────────────────────────────────
// 시나리오 전환 트리거
// ────────────────────────────────────────────
// probabilityDelta: 각 시나리오 확률 변화 (활성화 시 적용, 합산 ~0 권장)
// df1Delta: AI 수요 축 이동 (-=버블, +=슈퍼사이클)
// df2Delta: 디커플링 축 이동 (-=공존, +=디커플링)
export const INITIAL_TRIGGERS = [
  {
    id: 'bigtech_capex_cut25',
    name: '빅테크 CapEx YoY 25%+ 삭감',
    condition: '1개사 이상에서 YoY -25% 이상 삭감 발표',
    targetScenarios: ['C', 'D'],
    immediateAction: 'HBM 신규 설비 집행 중단·소재 비축 즉시 실행',
    activated: false,
    activatedDate: null,
    note: '[2026-08-25] 미발동 — 08-25 웹 리서치 재확인: 빅테크 4사 2026 CapEx ~$725B(+77% YoY)·삭감(YoY -25%) 발표 0건, Q2 상향 가이던스 유지. 신규 삭감 신호 없음. [2026-08-11] 미발동 — Q2 실적 실현으로 정반대 방향 확정: MS·Meta·Amazon(07-29·07-30) 전원 상향, 삭감(YoY -25%) 발표 0건(Amazon $200B→~$220B·MS FY27 $255~260B·Meta 하한 $130~145B). 단 이창수 FCF 렌즈 실측 — 절대 삭감은 없으나 CapEx↑ + FCF 흑자→마이너스 반전(Meta -91%·Amazon TTM 마이너스)이 빅테크 내 차별화 선행 tell로 등장. [2026-08-04] 미발동 — 인터뷰 2건은 삭감 신호 아님. [2026-07-28] Alphabet 상향·SK하이닉스 사상 최대',
    probabilityDelta: { A: -4, B: -10, C: +8, D: +6, E: 0 },
    df1Delta: -2.5,
    df2Delta: 0,
  },
  {
    id: 'match_act_passed',
    name: 'MATCH 법안 하원 300표+ 통과',
    condition: '미 하원에서 300표 이상으로 MATCH 법안 통과',
    targetScenarios: ['A', 'C'],
    immediateAction: '시안 팹 이전 계획 즉시 발동',
    activated: false,
    activatedDate: null,
    note: '[2026-09-22] 미발동 유지 — Trump-Xi 정상회담 09-24 워싱턴 개최 확정(2일 후). MATCH 법안은 의회에서 진전(동맹국이 150일 내 정렬 실패 시 미 관할 확대·장비 판매 차단 조항)했고 베이징이 정상회담 당일 비판을 격화했으나, 하원 300표+ 본회의 통과 발동 조건은 미충족. 정상회담이 완화·라이선스 언어 완화로 귀결되면 MATCH 동력 약화, 결렬 시 통과 압력 확대 — 09-24 결과가 DF2 방향을 결정하는 최우선 추적 이벤트. [2026-09-15] 미발동 유지 — 09월 미중 정상회담이 실제 개최 국면에 진입하고 MATCH 법안(H.R.8170)이 가장 절차적으로 진전된 기술 파일로 정상회담 의제에 상정됨(베이징 비판 격화). 그러나 하원 300표+ 본회의 통과라는 발동 조건은 미충족 — 정상회담이 완화(거래·조달 허용)로 귀결되면 오히려 MATCH 동력이 약화될 수도, 결렬 시 통과 압력이 커질 수도 있는 양방향 분기점. 09-08이 지목한 "9월 정상회담=다음 분기점"의 결과가 DF2 방향을 결정 → 결과 확정까지 최우선 추적. [2026-09-01] 미발동 — H.R.8170 04-22 위원회 보고 후 본회의 표결 movement 여전히 없음(08-28 수집 재확인). 08-28~09-01 창의 DF2 활동은 입법이 아니라 행정·정치 경로: 애플–CXMT 인증 진전·Schumer 서한 답변시한(08-21) 경과·9월 시진핑 방미 정상회담 관측. MATCH 통과는 DF2 디커플링 급변 트리거로 계속 추적. [2026-08-25] 미발동 — H.R.8170 04-22 위원회 보고 후 본회의 표결 movement 없음(congress.gov, 08-25 확인). DF2 방향 결정 주 분기점으로 계속 추적. [2026-07-28] 미발동 — H.R.8170 위원회 통과 단계 유지, 07-21~28 창 내 본회의 표결 movement 없음(congress.gov). DF2 방향 결정 주 분기점으로 계속 추적',
    probabilityDelta: { A: +10, B: -8, C: +5, D: -7, E: 0 },
    df1Delta: 0,
    df2Delta: +2.5,
  },
  {
    id: 'samsung_hbm4_no_nvidia',
    name: '삼성 HBM4 NVIDIA 탑재 미발표 (2026년 말)',
    condition: '2026년 12월 31일까지 NVIDIA 공식 탑재 확인서 미확보',
    targetScenarios: [],
    immediateAction: 'HBM4 결함 원인 분석팀·TSMC 위탁 검토 착수',
    activated: false,
    activatedDate: null,
    note: '',
    deadline: '2026-12-31',
    probabilityDelta: { A: 0, B: -5, C: +3, D: +2, E: 0 },
    df1Delta: -1.0,
    df2Delta: 0,
  },
  {
    id: 'xian_license_rejected',
    name: '시안 팹 라이선스 거절 또는 3개월 단기 갱신',
    condition: 'BIS 라이선스 거절 또는 3개월 이하 단기 갱신',
    targetScenarios: ['A', 'C'],
    immediateAction: '시안 팹 단계적 축소 Plan B 즉시 실행',
    activated: false,
    activatedDate: null,
    note: '',
    probabilityDelta: { A: +8, B: -7, C: +5, D: -6, E: 0 },
    df1Delta: 0,
    df2Delta: +2.0,
  },
  {
    id: 'google_amazon_3ddram_patent',
    name: '구글·아마존 커스텀 ASIC CXL/3D DRAM 특허 출원 확인',
    condition: '구글 또는 아마존의 CXL/3D DRAM 인터페이스 특허 출원 공개',
    targetScenarios: ['E'],
    immediateAction: '3D DRAM 조직 독립 사업부 격상·HBM5 투자 50% 축소 검토',
    activated: false,
    activatedDate: null,
    note: '',
    probabilityDelta: { A: -2, B: -3, C: -1, D: -2, E: +8 },
    df1Delta: 0,
    df2Delta: 0,
  },
  {
    id: 'hbm_price_drop30',
    name: 'HBM 현물 가격 6개월 내 30%+ 하락',
    condition: 'DRAM Exchange 기준 6개월 누적 -30% 이상',
    targetScenarios: ['C', 'D'],
    immediateAction: 'HBM 생산 속도 조절·재고 관리 긴급 가이드라인 발동',
    activated: false,
    activatedDate: null,
    note: '',
    probabilityDelta: { A: -4, B: -8, C: +7, D: +5, E: 0 },
    df1Delta: -1.5,
    df2Delta: 0,
  },
  {
    id: 'bigtech_ai_revenue_500b',
    name: '빅테크 AI 직접 매출 합산 5,000억 달러 돌파 (2027년)',
    condition: '2027년 연간 기준 빅테크 4사 AI 직접 매출 합산 $500B 돌파',
    targetScenarios: ['B'],
    immediateAction: 'Main Bet 전략 최대 가속·텍사스 2기 조기 착공 확정',
    activated: false,
    activatedDate: null,
    note: '',
    isPositive: true,
    probabilityDelta: { A: 0, B: +8, C: -3, D: -5, E: 0 },
    df1Delta: +1.5,
    df2Delta: -1.0,
  },
  {
    id: 'imec_3ddram_power50',
    name: 'IMEC/마이크론 3D DRAM PoC 전력 50% 개선 입증',
    condition: 'HBM3E 대비 전력 효율 50%+ 개선 PoC 논문/발표 확인',
    targetScenarios: ['E'],
    immediateAction: '3D DRAM M&A 펀드 즉시 집행·IMEC 협약 체결',
    activated: false,
    activatedDate: null,
    note: '',
    probabilityDelta: { A: -2, B: -3, C: -1, D: -2, E: +8 },
    df1Delta: 0,
    df2Delta: 0,
  },

  // ── B "AI 르네상스" 확인 트리거 ────────────────────────────────────────────
  {
    id: 'samsung_hbm4_nvidia_confirmed',
    name: '삼성 HBM4 NVIDIA 공식 탑재 확정',
    condition: 'NVIDIA Rubin/Blackwell Ultra 플랫폼에 삼성 HBM4 공식 공급 계약 확인서 확보',
    targetScenarios: ['B'],
    immediateAction: 'Main Bet 전략 전면 가속 · HBM4E·HBM5 투자 확대 · 1c nm 전환 가속',
    activated: false,
    activatedDate: null,
    note: '',
    isPositive: true,
    note: '[2026-09-22] 미충족 유지 — 삼성 NVIDIA향 HBM4는 유상 평가 단계 유지(2월 양산 시작·NVIDIA/AMD 최종 인증 통과했으나 volume-production order 미확정). 신규 실측: NVIDIA가 신규 16-Hi HBM4 Q4 2026 납품 요청 — SK하이닉스 16층 볼륨·Micron 샘플·삼성은 16-Hi를 skip(삼성 12-hi HBM4는 최종 인증 단계). Micron HBM 캐파 2배 증설. SK 락인 심화, 인증→볼륨 발주 전환이 발동 조건, 09-30 Micron 실적·연말이 시간 압박 관문. [2026-09-15] 미충족 유지 — 삼성 HBM4는 NVIDIA/AMD 최종 인증(11.7Gbps/pin, 최소 10Gbps 상회)을 통과했으나 볼륨(양산) 발주 전환은 여전히 미확보, NVIDIA향 매출은 유상 평가 단계 수준으로 보도(직전 실측 mid-July 이후 반전 부재). SK하이닉스 NVIDIA HBM4 ~2/3 공급·Micron HBM4 12-hi 볼륨 출하(Vera Rubin 정렬)·Q4 FY26 $50B 가이던스·10만 wpm 램프로 경쟁사 락인은 심화. 인증→볼륨 발주 전환이 발동 조건, 09-30 Micron 실적·연말이 시간 압박 관문. [2026-09-01] 미충족 유지 — 볼륨 발주(공식 공급 계약 확인서) 여전히 미확보. 08-26 NVIDIA NVHBM 발표는 삼성·SK가 커스텀 HBM 표준 구현을 제조하는 구조 신호이나 첫 파트너는 Amazon Trainium4·첫 채택은 Feynman 세대(2028)로 현 Rubin HBM4 볼륨 발주 확정과는 별개. NVIDIA Rubin Ultra HBM 구성 축소 검토(공급 타이트발)도 삼성 배분 확대와 무관. 인증→볼륨 발주 전환이 여전히 발동 조건. [2026-08-25] 미충족 유지 — 08-25 웹 리서치: 볼륨 발주 전환 확인서 여전히 미확보(직전 실측 mid-July). 삼성 레거시 백엔드 베트남 이전 검토(TechTimes 08-15)는 캐파 언락 준비 신호이지 볼륨 계약 확정 아님. [2026-07-28] 미충족 유지 — 07-21~28 창 내 볼륨 발주 전환 보도 없음(07-15 Digitimes·07-17 Winbuzzer 이후 반전 부재). 인증(qualification)은 06-05 통과했으나 이 트리거는 "공식 공급(볼륨) 계약 확인서" 기준 — NVIDIA향 HBM4 매출은 여전히 유상 평가용 샘플 수준. 인증→볼륨 발주 전환이 발동 조건',
    probabilityDelta: { A: -3, B: +12, C: -5, D: -4, E: 0 },
    df1Delta: 1.5,
    df2Delta: -1.5,
  },
  {
    id: 'us_china_semiconductor_deal',
    name: '미중 반도체 무역 협상 타결 (범용 DRAM·NAND 거래 재개)',
    condition: '미중 양국 협상으로 범용 DRAM/NAND 중국 수출 공식 재개 합의 또는 포괄적 라이선스 발급',
    targetScenarios: ['B', 'D'],
    immediateAction: '시안 팹 운영 정상화 · 중국 영업팀 즉시 재가동 · 中 전용 메모리 SKU 출시',
    activated: false,
    activatedDate: null,
    note: '[2026-09-22] 미발동 — Trump-Xi 정상회담 09-24 워싱턴 개최 확정(2일 후, 무역·기술·타이완 3T 의제). 시장은 건설적 결과를 선반영(반도체·아시아 기술주·위안·홍콩 강세)하고 Busan 휴전 연장이 최선 기대이나, 범용 DRAM/NAND 수출 공식 재개 합의·포괄 라이선스 등 실현된 타결은 09-24 이후 확정 → 미발동. 베이징은 미 규칙 집행 연기를 압박·워싱턴은 희토류 정상화를 요구. 완화·라이선스 언어 완화 시 이 트리거(공존 강화·DF2 하향) 발동 방향, 결렬·MATCH 진전 시 match_act_passed(디커플링) 방향. DF2 최상위 양방향 분기점으로 결과 대기. [2026-09-15] 미발동 — 9월 미중 정상회담이 실제 개최 국면(무역·AI·수출통제·타이완 의제, 칩 장비 파일이 가장 절차 진전). 범용 DRAM/NAND 수출 공식 재개 합의·포괄 라이선스 등 실현된 타결은 미확정 → 미발동. 정상회담이 완화로 귀결되면 이 트리거(공존 강화·DF2 하향) 발동 방향, 결렬·MATCH 진전 시 match_act_passed(디커플링) 방향. DF2 최상위 양방향 분기점으로 결과 대기',
    isPositive: true,
    probabilityDelta: { A: -8, B: +9, C: -5, D: +4, E: 0 },
    df1Delta: 0,
    df2Delta: -3.5,
  },
  {
    id: 'apple_cxmt_approved',
    name: '애플 CXMT DRAM 미 행정부 승인·양산 채택 (중국 내수용)',
    condition: '미 행정부가 애플의 CXMT DRAM 조달을 승인(사전 양해)하고 애플이 중국 내수용 기기에 양산 채택 확정',
    targetScenarios: ['B', 'D'],
    immediateAction: '관리된 공존 리트머스 확인 — 中 CXMT 잠식 대비 범용 DRAM 원가 우위(1c nm) 가속 + 과점 균열 대비 SCA 락인·고부가(HBM) 편중 강화',
    activated: false,
    activatedDate: null,
    note: 'DF2 리트머스 (FT 2026-07-08). [2026-09-22] 미발동 유지 — Trump-Xi 정상회담 09-24 워싱턴 개최 확정(2일 후)에도 애플 CXMT 조달에 대한 미 행정부 공식 승인·양산 채택 확정은 09-22 현재 미보도(승인/차단 리트머스 미해결). 정상회담이 반도체 라이선스 언어를 완화하면 승인 쪽, 결렬·MATCH 진전이면 차단 쪽 — 09-24 결과가 이 좁은 리트머스의 상위 분기점. [2026-09-15] 미발동 유지 — 9월 미중 정상회담 개최 국면에서도 애플 CXMT 조달에 대한 미 행정부 공식 승인·양산 채택 확정 미보도(승인/차단 리트머스 미해결). 정상회담 결과(완화 vs MATCH 진전)가 이 좁은 리트머스의 상위 분기점. CXMT $42억 상하이 IPO 추진은 CXMT 캐파·자금 확충 신호이나 애플 채택과는 별개. [2026-09-01] 미발동 — 인증(qualifying) 단계 진전(애플 iPhone·MacBook 라인 검증 통과·CXMT DDR5 수율 >90%)했으나 미 행정부 승인·양산 채택 미확정: Schumer 초당파 서한 답변시한(08-21) 경과·백악관 사인오프 대기·CXMT Entity List 미등재. [저신뢰] 9월 시진핑 방미 정상회담 후 조달 허용 통지 관측(wccftech) — 실현 시 발동. 승인·채택 = 관리된 공존 확정 + 삼성·SK 과점 균열·애플 price leverage. 반대로 차단·무산(YMTC 2022 전철) 시 A·C 디커플링 신호. us_china_semiconductor_deal보다 좁은·약한 공존 신호',
    isPositive: false,
    probabilityDelta: { A: -3, B: +3, C: -1, D: +1, E: 0 },
    df1Delta: 0,
    df2Delta: -1.5,
  },
  {
    id: 'enterprise_ai_roi_proven',
    name: '엔터프라이즈 AI ROI 가시화 (주요 산업 생산성 입증)',
    condition: '맥킨지·가트너·IDC 등 리서치에서 AI 도입 기업의 생산성 +20%+ 효과 대규모 입증 발표',
    targetScenarios: ['B', 'A'],
    immediateAction: 'HBM 슈퍼사이클 2라운드 대비 · CapEx 사이클 가속 · 신흥 AI 시장 영업 확대',
    activated: false,
    activatedDate: null,
    note: '[2026-09-22] 미발동 유지 — 09-15~09-22 창에 AI 도입 생산성 +20%+ 대규모 입증 발표 없음(상방 미충족). 반대편(하락 선행) 관전 ① 신호는 실측 심화: NVIDIA-OpenAI-Oracle $745B 순환 파이낸싱 루프가 첫 스트레스 테스트(연준 금리)·Oracle $43B 차입·OpenAI 2026 매출 ~$20B 대비 손실 ~$14B·버블 인덱스 73/100 = "수익화 한계·순환 파이낸싱" 프레임 실측. 아직 OpenAI/하이퍼스케일러 붕괴(다운턴 트리거)는 미실현 — 조달 구조 경보 수준(ai_dc_credit_spread·demand_inflection_divergence 연동). [2026-09-15] 미발동 유지 — 09-08~09-15 창에 AI 도입 생산성 +20%+ 대규모 입증 발표 없음(상방 미충족). 반대편(하락 선행) 관전 ① 신호는 오히려 실측 강화: NVIDIA의 OpenAI 오하이오 DC 파이낸싱 개런티 $250B→~$105B 축소를 Fortune(08-18)이 "칩 인공 수요 우려 신호"로 해석·Oracle YTD -24%·HSBC "OpenAI 2030 $200B 매출에도 +$207B 조달 갭"은 송용호가 지목한 "수익화 한계·순환 파이낸싱" 프레임의 실측. 아직 OpenAI/하이퍼스케일러 붕괴(다운턴 트리거)는 미실현 — 조달 구조 경보 수준(ai_dc_credit_spread·demand_inflection_divergence 연동). [2026-09-08] 다운턴 센싱 관전 포인트 ①(AI 기술 ROI 진보 지속성)의 반대편 하락 선행 신호로 병기(송용호 인터뷰 09-03) — 이 트리거는 ROI 가시화(상방)이나, 새 모델이 이전 대비 유의미한 진보를 못 보이는 시점이 위험(모델+에이전트+하네스+루프 엔지니어링으로 합산 성능 진보를 계속 만들어야 투자가 이어짐). OpenAI의 광고 시장 진입(스스로 "마지막 카드"라던)과 IPO 시도는 수익화 한계 인식 신호이고, OpenAI 또는 하이퍼스케일러 중 하나가 무너지면 그때가 다운턴 트리거 — 특히 OpenAI IPO 실패는 큰 사건. 09-08 현재 대규모 생산성 입증 발표 없음(미발동)',
    isPositive: true,
    probabilityDelta: { A: +2, B: +11, C: -8, D: -5, E: 0 },
    df1Delta: 2.0,
    df2Delta: 0,
  },
  {
    id: 'bigtech_capex_2027_sustained',
    name: '빅테크 4사 2027년 AI CapEx $5,000억+ 유지 확인',
    condition: '2027년 분기 실적 발표에서 4사 합산 AI CapEx $500B 이상 연간 가이던스 제시',
    targetScenarios: ['B'],
    immediateAction: 'HBM5 조기 양산 결정 · 텍사스 2기 조기 착공 확정 · 소재 장기 계약 확대',
    activated: false,
    activatedDate: null,
    note: '[2026-08-11] 미충족이나 선행 상방 강화 — Q2 실적에서 Microsoft가 FY2027 CapEx를 $255~260B로 상향(단독으로 2027 4사 합산 $500B+ 경로의 큰 축). 4사 정식 2027 가이던스 제시 시 충족. Amazon도 2026 $220B로 상향해 2027 기저 상승. 단 이창수 "꼭짓점은 FCF"라 FCF 다이버전스(Meta -91%·Amazon TTM 마이너스) 병행 관전. [2026-08-04] 미충족·take-or-pay 멀티이어 바인딩이 계약적 뒷받침. [2026-07-28] Alphabet 상향·Cloud 백로그 $514B',
    isPositive: true,
    probabilityDelta: { A: 0, B: +8, C: -4, D: -4, E: 0 },
    df1Delta: 1.5,
    df2Delta: 0,
  },
  // ── 신규 트리거 (2026-05-06 추가) ────────────────────────────────────────
  {
    id: 'taylor_phase2_delayed',
    name: '텍사스 테일러 2단계(HBM) 발표 2027년 이후 연기',
    condition: '2026년 Q4 마감 미준수 + 2027년 이후로 발표 연기',
    targetScenarios: ['A', 'C'],
    immediateAction: '미국 합작투자(JV) 모델 검토 (샌디스크+키옥시아 모델) · 미국 OSAT 파트너십 협상',
    activated: false,
    activatedDate: null,
    note: 'Decision 8. 마이크론 Idaho ID2 + 인텔 Ohio 팹이 미국 HBM 표준 거점 고착화 위험',
    probabilityDelta: { A: -2, B: -5, C: +3, D: +1, E: 0 },
    df1Delta: 0,
    df2Delta: +0.5,
  },
  {
    id: 'cxmt_hbm4_production',
    name: 'CXMT HBM4 양산 진입 (2028년 이전)',
    condition: 'CXMT가 HBM4 12층 양산 단계 진입 — 화웨이 및 중국 AI 하드웨어 공급',
    targetScenarios: ['D', 'C'],
    immediateAction: '1c nm DDR5 원가 우위 사수 가속 · CXMT 우회 공급망 차단 · 중국 외 SKU 분리 정책',
    activated: false,
    activatedDate: null,
    note: '현재 CXMT는 HBM3 양산 중. HBM3E → HBM4 진입 시 글로벌 메모리 시장 구도 재편',
    probabilityDelta: { A: 0, B: -5, C: +3, D: +5, E: 0 },
    df1Delta: 0,
    df2Delta: 0,
  },
  {
    id: 'mna_target_undervalued',
    name: '차세대 메모리 스타트업 EV/EBITDA 5배 이하 6개월 지속 (Disney-Marvel 모델)',
    condition: '3D DRAM·CXL SW·FDP·어드밴스드 패키징 영역 주요 스타트업 EV/EBITDA 5배 이하 2분기 연속',
    targetScenarios: ['C', 'D'],
    immediateAction: 'M&A 펀드 5,000억 원 즉시 집행 권한 발동 · 5개월 내 종결 PMI 팀 가동',
    activated: false,
    activatedDate: null,
    note: 'Decision 9. Option L-4. 호황기에 펀드 적립, 다운사이클 진입 시 자동 발동 — Disney-Marvel·ExxonMobil-Pioneer 모델 차용',
    isPositive: true,
    probabilityDelta: { A: 0, B: +1, C: 0, D: +2, E: +3 },
    df1Delta: 0,
    df2Delta: 0,
  },
  {
    id: 'samsung_hbm_capacity_sold_out_2027',
    name: '2027년 HBM 캐파 Sold Out 12개월 사전 확정',
    condition: '2027년 HBM 캐파 전량 LTA로 12개월 사전 약정 완료 (NVIDIA + 미국계 빅테크 + ASIC 고객)',
    targetScenarios: ['B'],
    immediateAction: 'HBM5 양산 일정 가속 · HBM6 R&D 투자 확정 · 텍사스 2단계 즉시 추진',
    activated: false,
    activatedDate: null,
    note: '2026년 캐파는 이미 Sold Out. 2027년 사전 확정 시 시나리오 B 강력 가속',
    isPositive: true,
    probabilityDelta: { A: -2, B: +6, C: -3, D: -1, E: 0 },
    df1Delta: 1.0,
    df2Delta: 0,
  },
  {
    id: 'microsoft_capex_cut',
    name: 'Microsoft AI CapEx YoY -20% 이하 발표',
    condition: 'Microsoft 분기 실적에서 다음 회계연도 AI CapEx YoY -20% 이상 감소 가이던스',
    targetScenarios: ['C', 'D'],
    immediateAction: 'HBM 신규 설비 집행 일시 중단 · 재고 일수 모니터링 강화 · RS-5 다운사이클 EWI 발동',
    activated: false,
    activatedDate: null,
    note: 'Microsoft가 메모리 가격 영향 $25B 직접 인정한 만큼, MS의 capex 조정이 가장 강력한 다운턴 신호',
    probabilityDelta: { A: -3, B: -8, C: +6, D: +5, E: 0 },
    df1Delta: -2.0,
    df2Delta: 0,
  },

  {
    id: 'samsung_hbm_share_40pct',
    name: '삼성 HBM 시장 점유율 40%+ 회복',
    condition: 'TrendForce/Counterpoint 기준 삼성 HBM 분기 점유율 40% 이상 2분기 연속 확인',
    targetScenarios: ['B'],
    immediateAction: '기술 추격 완료 선언 · HBM4E 프리미엄 단가 전략 전환 · 점유율 50% 목표 로드맵 수립',
    activated: false,
    activatedDate: null,
    note: '',
    isPositive: true,
    probabilityDelta: { A: 0, B: +7, C: -3, D: -4, E: 0 },
    df1Delta: 0.5,
    df2Delta: 0,
  },

  // ── 수요 변곡 조기경보 트리거 (2026-06, demand-inflection-ewi.md) ──────────────
  // 인과 사슬 선행 신호 — 기존 capex-cut/price-drop 보다 작은 델타(더 이른·약한 신호)
  {
    id: 'gpu_rental_collapse',
    name: 'GPU 임대가 6개월 -35% 급락',
    condition: 'Vast.ai·ClusterMAX 기준 H100/H200 바스켓 임대가 6개월 누적 -35% 이상 급락',
    targetScenarios: ['D', 'C'],
    immediateAction: '수요 청산가 선행 약화 — RS-5 재고·캐파 규율 점검, HBM 증설 속도 재검토 (RS-9 발동)',
    activated: false,
    activatedDate: null,
    note: '인과 사슬 ① 최선행 신호 (선행 9~18개월). EWI gpu_rental_h100_usd 연동',
    probabilityDelta: { A: -2, B: -6, C: +3, D: +5, E: 0 },
    df1Delta: -1.5,
    df2Delta: 0,
  },
  {
    id: 'dc_construction_cancellations',
    name: 'AI DC 착공 취소·연기 분기 5건+',
    condition: 'DCD 등 기준 분기 5건 이상 신규 DC 착공 취소 또는 연기',
    targetScenarios: ['D', 'C'],
    immediateAction: '수요 파이프라인 둔화 — 신규 HBM 캐파 집행 보류, 소재 비축 재검토',
    activated: false,
    activatedDate: null,
    note: '인과 사슬 ② 돈 단계 (선행 6~12개월). EWI dc_cancellation_count 연동. 발표보다 빠른 신호',
    probabilityDelta: { A: -1, B: -5, C: +2, D: +4, E: 0 },
    df1Delta: -1.0,
    df2Delta: 0,
  },
  {
    id: 'demand_inflection_divergence',
    name: '수요 변곡 괴리 경보 (선행 꺾임 + 끈적 강세)',
    condition: '복합 위험 점수 경계(≥75) 또는 선행−끈적 괴리 2개 분기 연속 확대 (선행 악화·착공/메모리 여전히 강세)',
    targetScenarios: ['D'],
    immediateAction: '하락 전 대응 윈도우 — RS-9 프로토콜 발동, RS-1 캐파 동결 + RS-5 규율 즉시 적용',
    activated: false,
    activatedDate: null,
    note: '괴리 로직 = 하락 변곡 선행 윈도우. demand-inflection-ewi.md §3. [2026-09-22] 미발동 유지 — 3축 중 조달 축 tell 심화: NVIDIA-OpenAI-Oracle $745B 순환 파이낸싱 루프가 첫 스트레스 테스트(연준 금리)·Oracle $43B 차입·OpenAI 손실 ~$14B·버블 인덱스 73/100 = 순환 파이낸싱 경보. 그러나 (2)AI 최종수요(CapEx ~$725B·2027 >$1T·구조적 쇼티지 2027)·(3)GPU 생태계(H100 ~$3.25/h·H200 ~$4.40/h firming·메모리 병목)는 미꺼짐 → 3축 동시 꺾임 미충족, 조달 구조 경보 수준으로 미발동. [2026-09-15] 미발동 유지 — 3축(CSP FCF/조달·AI 최종수요·GPU 생태계) 중 조달 축에서 실측 tell 심화: NVIDIA-OpenAI 오하이오 파이낸싱 개런티 $250B→~$105B 축소·Oracle YTD -24%·HSBC "OpenAI +$207B 조달 갭"·공급사 ~$96B 부채 = 순환 파이낸싱 경보. 그러나 (2)AI 최종수요(토큰 소비·CapEx 상향·구조적 쇼티지)·(3)GPU 생태계(임대가 firming·메모리 병목)는 미꺼짐 → 3축 동시 꺾임 미충족, 조달 구조 경보 수준으로 미발동. 송용호 "가격 피크 26년·28~30 슬로우" 타이밍과 정합한 선행 감시 지속. [2026-09-08] 미발동 유지 — 송용호 다운턴 센싱 3대 관전 포인트(① AI ROI 진보 ② 하이퍼스케일러 자금 구조(FCF→대출→지분 매각→NVIDIA 금융업체화) ③ 전기)를 이 괴리 경보의 상위 프레임으로 편입(09-03 인터뷰). 세 축이 동시에 꺾일 때가 발동 조건 — 현재는 각 축이 정점 재확인·조달 경보 수준(선행 약화 아님)이라 미발동. 송용호도 "가격 피크 26년 가능성·28~30 슬로우"로 이 EWI의 하락 변곡 선행 감시 타이밍과 정합. [2026-08-11] 미발동 유지 — 단 이창수 3축 중 (1) CSP FCF 축이 Q2 실적에서 실측 등장: Meta FCF -91%→$784M·Amazon TTM FCF 마이너스 전환(~-$7.6B). 그러나 CapEx 자체 상승(4사 전원 상향·~+82% YoY)·핸드투마우스 재고 없음·Google Cloud 백로그 $514B·GPU 임대가 firming으로 (2)AI 최종수요·(3)GPU 생태계 미꺼짐 → 선행 약화 아닌 조달 구조 경보로 판정, 미발동. FCF 다이버전스를 정식 관전축으로 승격(향후 3축 동시 꺾임 시 발동). [2026-08-04] 미발동 — 3축 프레임 정합, 핸드투마우스로 선행 약화 신호 부재',
    probabilityDelta: { A: -2, B: -6, C: +2, D: +6, E: 0 },
    df1Delta: -1.5,
    df2Delta: 0,
  },
]

