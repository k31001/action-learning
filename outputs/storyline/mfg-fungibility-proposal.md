...
DRAM 쪽에서도 같은 수렴이 관측된다. ISSCC 2026에서 발표한 4F² COP DRAM은 셀 어레이 웨이퍼와 코어·페리 웨이퍼를 따로 만들어 하이브리드 본딩으로 접합하는 구조이고, 낸드가 검증한 기술 축이 DRAM 제품 안으로 들어온 지점이다[11]. 세 제품군이 같은 장비군 위에서 만난다는 이 글의 명제가 로직·디램·플래시 전 갈래에서 성립한다는 뜻이며, 그 위에서 제품 아키텍처를 어떻게 다시 그릴 것인가는 전략 2번의 몫이다[16].
...
- [16] 2차 저지선 전략 2번 「판을 옮긴다」 — 제품·아키텍처의 축(로직 진화 4단계, 본딩 분할 경계, 메모리–파운드리–로직 통합, zHBM 커스텀 플랫폼): outputs/storyline/dram-architecture-proposal.md
