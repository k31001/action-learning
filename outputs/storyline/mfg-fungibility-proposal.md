...
흡수 말고 회피하는 길도 이론상 있다. 공장을 갖지 않는 것이다. 일본 키엔스가 그 길을 끝까지 갔다 — 자체 공장 없이 위탁생산으로 제품을 만들어 고정비를 지지 않고, 그 대가로 10년 내내 영업이익률 50~56% 밴드를 벗어난 적이 없다[16]. 감가가 없으면 사이클이 손익을 흔들 지렛대도 없다. 우리에게 이 길은 닫혀 있다. 메모리는 팹이 곧 제품이고, 그 팹을 남에게 맡기는 순간 공정이 곧 경쟁력이라는 사업의 전제가 사라진다. 그러니 남은 선택지는 하나다. 고정비를 없앨 수 없다면, 그 고정비가 흔들릴 때 받아 낼 몸을 만드는 것.
...
DRAM 쪽에서도 같은 수렴이 관측된다. ISSCC 2026에서 발표한 4F² COP DRAM은 셀 어레이 웨이퍼와 코어·페리 웨이퍼를 따로 만들어 하이브리드 본딩으로 접합하는 구조이고, 낸드가 검증한 기술 축이 DRAM 제품 안으로 들어온 지점이다[11]. 세 제품군이 같은 장비군 위에서 만난다는 이 글의 명제가 로직·디램·플래시 전 갈래에서 성립한다는 뜻이며, 그 위에서 제품 아키텍처를 어떻게 다시 그릴 것인가는 전략 2번의 몫이다[17].
...
네 영역의 조치는 표 5에 놓았다. 관통하는 원리만 옮기면 이렇다. 설계에서는 차세대 로드맵의 승인 요건에 공통 기술 축 정렬 여부를 넣고 볼륨 티어에 N-1 여유를 스펙으로 박으며, 순서를 뒤집어 성능 목표보다 포맷과 사양 표준을 먼저 고정한다 — 디스플레이가 그 대가를 보여 줬다. 유리 기판이 10cm 커지면 생산 효율은 9% 오르지만 기존 세대의 장비는 더 이상 쓸 수 없게 된다[6]. 발주에서는 전환 가능성 등급을 사양서의 필수 항목으로 만들고 범용 장비군의 비중 하한을 티어별로 규정하되 EUV와 TSV와 본딩은 선단 티어로 격리한다. 시장은 이미 그렇게 팔고 있으므로 — 주요 장비사의 셀렉티브 식각 신제품은 낸드와 DRAM과 파운드리 로직 전반을 대상으로 명시한다[6] — 우리가 할 일은 그 범용성을 발주서에서 요구하는 것뿐이다.

**교차 퀄 상시화.** 6개월을 3개월로 줄이는 실제 수단은 여기에 있으므로 따로 적는다. 주력 제품을 둘 이상의 라인과 노드에서 미리 인증받아 둔다. 전환 시점에 퀄을 시작하면 창을 이미 넘긴다 — 2장에서 본 대로 퀄은 사외에 있고, 짧아도 수개월이며 길면 1년을 훌쩍 넘긴다. 전환 규범의 범위도 후공정과 테스트 프로그램과 퀄 패키지까지 넓힌다. 팹만 보면 병목을 못 푼다.

거버넌스에서는 투자 단위의 필수 속성에 전환 가능성 등급을 더하고 롤링 캐파 리뷰에 "이 투자로 전환 가능성이 늘었나 줄었나"를 심사 항목으로 신설한다. 둘 다 이미 있는 제도이므로 한 줄을 더하는 일이다[7].

**조직과 보상.** 앞의 네 영역은 규범을 바꾸는 일이고, 이 다섯 번째는 그 규범이 실제로 작동하게 만드는 일이다. 규범만으로는 부족한 이유가 있다. 제품 간 공용성은 언제나 누군가의 국소 최적을 깎아서 얻어지는데, 그 손해는 특정 조직에 귀속되고 이익은 회사 전체에 흩어진다. 그 비대칭을 그대로 두면 규범은 문서로만 남는다.

두 가지를 함께 건다. 하나는 **횡전개의 값을 재원으로 환산하는 것**이다. 고성능 DRAM에 파운드리의 핀펫과 HKMG를 적용하는 일, 차세대 낸드에 DRAM의 TSV를 옮겨 오는 일은 이미 벌어지고 있다[18]. 이것이 우연이 아니라 구조적으로 늘어나려면 절감된 값 — 개발 기간 단축분과 중복 투자 회피분 — 을 산출해 일부를 참여자 인센티브의 재원으로 돌려야 하고, 산정 기준에는 원 기술을 얼마나 그대로 가져다 썼는지를 넣는다. 바꿔 쓸수록 값이 줄고 그대로 쓸수록 커지는 구조여야 공통화가 실제로 일어난다. 다른 하나는 **사람이 건너갈 때의 처우**다. 파운드리나 연구소 인력의 메모리 파견은 대개 원 소속에서의 손해로 계산되는데, 파견 인센티브와 평가·승진 우대로 그 계산을 뒤집지 않으면 축 ①과 ②는 장비의 이야기로만 남는다.

조직 편제도 함께 바뀐다. DRAM에 본딩이 들어오면 낸드와 기술·라인을 공유할 수 있게 되는데, 이것을 각 제품 조직에 분산시켜 두면 공유는 일어나지 않는다. 개발에서는 두 제품의 본딩 공통기술을 한 조직이 맡고, 제조에서는 본딩 전용 모듈 팹과 전담 인력을 두어 관련 공정·설비·제조 기능을 하나로 편제한다[18]. 그래야 본딩에 관한 결정을 그 조직이 스스로 내린다. 축 ②가 말한 "제품은 다극으로, 공정은 단극에 가깝게"의 조직도 버전이다.
...
첫째, 파운드리와의 접점이다. HBM4 베이스다이가 이미 로직 팹의 산출물이고 평택 P4의 파운드리 예정 구역이 DRAM으로 돌려진 이상, 두 사업부의 설비 사양 표준을 한 문서에서 관리하면 전환 가능한 면이 넓어진다. 다만 여기서 멈춘다. 메모리와 로직을 제품 아키텍처 수준에서 통합하는 문제 — 본딩으로 갈라낸 층을 선단 로직에서 만드는 일과 그것이 만드는 경쟁 우위 — 는 이 글의 설비 축을 넘어서므로, 같은 저지선의 전략 2번이 본론으로 다룬다[17]. 여기서는 그 통합이 성립하려면 설비 사양 표준부터 한 문서에 있어야 한다는 사실만 남긴다. 둘째, 이미지센서다. DRAM과 CIS 공정의 약 80% 동일성은 우리가 이미 두 라인에서 실증했다[6]. 셋째, 감가 완료 설비의 두 번째 수명이다. 반도체 2차 시장의 본질이 거래가 아니라 검사·등급화·잔존수명 예측·보증에 있다는 위키의 판정은[15] 칩과 모듈을 대상으로 한 것이고, 장비로 확장하는 것은 아직 제안일 뿐이다. 그래도 경고는 분명하다 — 전환 불가능한 팹의 처분 경로는 매각뿐이었고, 그것도 원가 아래였다[5].
...
| **⑤ 조직과 보상**<br/>(규범을 작동시키는 동력) | · 횡전개로 절감된 값(개발기간 단축·중복투자 회피)을 산출해 **참여자 인센티브 재원으로 환산**하는 펀드 조성, 마일스톤 단위 지급<br/>· 산정 기준에 **원 기술 유용도**를 넣는다 — 바꿔 쓸수록 값이 줄고 그대로 쓸수록 커지는 구조<br/>· 파운드리·연구소 인력의 메모리 파견에 **파견 인센티브와 평가·승진 우대**를 사업부 차원에서 부여<br/>· **본딩 공통기술 개발조직**(개발담당 직속 공정 아키텍처 전담팀) + **본딩 전용 모듈 팹·전담 인력** 편제 | · 횡전개 과제 수와 그로 인한 절감액 **(신규 계기판 · 기준치 0)**<br/>· 파견 인력 수와 파견 후 원소속 복귀율<br/>· 본딩 공정의 의사결정 소요 일수 — 분산 편제 대비 단축폭 | 이미 진행·검토 중인 횡전개 — 고성능 DRAM에 Foundry FinFET·HKMG, 차세대 NAND에 DRAM TSV[18] · 공용성의 손해는 특정 조직에 귀속되고 이익은 회사 전체에 흩어지므로, 보상이 없으면 규범은 문서로만 남는다 · 초호황기는 조직 신설·인력 재배치의 저항이 가장 낮은 시기[18] |
...
답은 축의 구분에 있다. 제품 수 축소는 파는 것의 축이고 공용성은 만드는 방법의 축이다. 선택과 집중의 원본을 보면 그 구분이 분명해진다. 키엔스의 창업자는 창업 당시 영위하던 영업이익률 20%대의 자동절단기 사업조차 1983년에 매각하고 센서에만 집중했다 — 마진이 낮으면 사업을 아예 접는다[16]. 우리는 그럴 수 없다. 메모리사업 자체를 접을 수 없으므로 선택과 집중은 사업이 아니라 제품 믹스에서 해야 하고, 제품 믹스를 옮기려면 설비가 그 이동을 받아 줘야 한다. 반대편 사례가 그 값을 말한다 — 키몬다와 엘피다는 가격 전쟁에서 발을 빼지 못한 채 캐파를 늘리다 무너졌다[6][16]. 접을 수 없는 회사가 옮기지도 못하면 남는 길은 그 둘이 간 길뿐이다. 그리고 제품 수를 줄일수록 남은 제품끼리 설비를 받쳐 줘야 할 필요는 오히려 커진다. 극이 적을수록 한 극이 무너질 때의 충격이 크기 때문이고, 낸드에 완충재가 없다는 2장의 진단이 정확히 그 이야기였다. 표 3이 그린 어려운 쌍들 — 범용 DRAM에서 낸드로, HBM에서 낸드로 가는 길에 근대 사례조차 없다는 사실 — 이 그 취약함의 지도다. 규범이 쓰는 자원은 설계 단계의 한 번이고, 받쳐 주지 못해 치르는 값은 다운턴 내내다. 정식 명제로 쓰면 이렇다. 제품은 다극으로, 공정은 단극에 가깝게.
...
- [18] 자원배분 두 축 — 인사·조직·보상 관점(사내 자료, 2026-08-12) — 횡전개 실사례(고성능 DRAM에 Foundry FinFET·HKMG, 차세대 NAND에 DRAM TSV)·Project Incentive Funding·파견 인력 전략적 처우·DRAM/NAND Bonding 공통기술 개발조직과 전용 Module FAB·초호황기의 낮은 조직 변화 저항: sources/raw-notes/hr-org-open-innovation-note-2026-08-12.md
- [16] 키엔스 벤치마크(사내 별첨 자료, 2026-08-12) — 팹리스 경영과 고정비 회피·10년 50~56% 영업이익률 밴드·1983년 자동절단기 매각·"메모리는 접을 수 없으니 제품믹스로 선택과 집중"·키몬다/엘피다 반면교사: sources/raw-notes/keyence-benchmark-note-2026-08-12.md. 키엔스 관련 수치는 참고 도서 기반의 자료 인용치이며 외부 검증은 하지 않았다
- [17] 2차 저지선 전략 2번 「판을 옮긴다」 — 제품·아키텍처의 축(로직 진화 4단계, 본딩 분할 경계, 메모리–파운드리–로직 통합, zHBM 커스텀 플랫폼): outputs/storyline/dram-architecture-proposal.md
