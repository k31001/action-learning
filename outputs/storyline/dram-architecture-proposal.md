...
그 뒤가 네 번이다. 클럭이 막히자 같은 소자를 여럿 놓았다. 평면 미세화가 막히자 소자를 세웠다 — 인텔의 22nm 핀펫이 2011년, 우리의 3nm GAA 최초 양산이 2022년 6월이다. 모놀리식 다이가 약 858mm²의 레티클 한계와 수율에 막히자 블록마다 최적 공정을 따로 쓰고 패키지에서 붙였다. AMD는 같은 코어 수에서 7nm 모놀리식 다이의 제조원가가 멀티다이의 두 배가 넘는다고 밝혔다. 범용 아키텍처의 효율이 막히자 문제에 맞춘 구조로 갔고, TPU v1은 동시대 GPU·CPU 대비 와트당 성능이 30~80배였다[7].
...
선택지는 표 2에 놓았다. 자르지 않는 모놀리식, 낸드가 이미 양산 중인 어레이와 페리의 절단, 우리가 시제품으로 도달한 DRAM 셀과 코어·페리의 절단, 비트셀과 디코딩 로직만 갈라 선단 로직에 올리는 imec의 연구 단계 경계, 그리고 메모리 바깥의 경계인 HBM 베이스다이와 zHBM 인터레이어다.
...
그렇다면 해자는 다른 곳에 있다. zHBM 하나를 만들려면 견적서에 네 줄이 필요하다. 하이브리드 본딩, 선단 로직 노드에서 만든 베이스다이와 커스텀 인터레이어, DRAM 셀과 스택, 가속기와 한 몸으로 만드는 선단 패키징이다[8]. 본딩만 있으면 한 줄이 찬다. 네 줄을 동시에 견적낼 수 있어야 제품이 된다.
