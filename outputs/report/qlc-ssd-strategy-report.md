...
| `outputs/presentation/qlc-ssd-strategy-outline.md` · `scripts/generate_qlc_ssd_strategy_pptx.py` · `qlc-ssd-strategy.pptx` | 6장 덱(v5.0: 문제 → 해법 사다리 → 신뢰성 → 역량 → 실행 → 보증·SLA) |

## PPT 압축 맵 (6장 덱, v5.0 스토리라인: ① 문제 → ② 해법 사다리 → ③ 신뢰성 → ④ 역량 → ⑤ 실행 → ⑥ 보증·SLA)
...
| 2 | ② 해법 사다리 · 왜 호스트인가 | 요구는 고정, 단품은 악화, 보상은 상위 계층으로: 남은 지렛대 WAF는 호스트에 있습니다 | 이관 매트릭스(내구성 행) + WAF 셀(범용·CacheLib 실측, KV 캐시 미실측 가설) + 지렛대 스트립(다이 세대·OP·SLC 캐시·보증연수·워크로드 재정의·호스트 배치) ([memory-solution-ladder-report.md](memory-solution-ladder-report.md) §2.5) |
| 3 | ③ 신뢰성 · SSD가 풀 문제 | 다이 수 8배 증가로 다이 고장률 요구가 8배 엄격해졌고, 이 축은 SSD 내부 해법으로 대응합니다 | 다이 수 128→1,024 · FFR ≤3% 상한 모델 · SSD 내부 해법 3(다이 패리티 / 여분 다이·감량 운영 / 텔레메트리 예측) + 호스트는 관측·수용만 (사다리 소스 §9) |
