...
> 이 표는 **보고서 전용**이다. 덱 v6.2에서 5장의 통과 조건 블록을 제거해 그림에 공간을 주었고, 질문이 나올 때 이 표로 답한다.
...
> 이 절도 **보고서 전용**이다. 덱 v6.3에서 5장의 판돈·비용 스트립을 제거해 두 트랙 그림에 공간을 주었다.
...
| 5 | ⑤ 실행 · 누구와 어디서 | 3단계 진입은 워크로드를 개방하는 고객에 FDE를 집중하고, 그 외는 업스트림·규격으로 협업합니다 | **두 트랙 그림(v6.3)** — 좌 삼성 개발실 / ① FDE 상주 → · 고객 선별·선례 카드 · ← ② 접근권 확보 / 우 고객 시스템 계층 + 하단 조직·인사·문화 (5장). 단계별 통과 조건(§5.4)과 판돈·비용(§5.5)은 덱에서 제외하고 보고서에만 유지 |
