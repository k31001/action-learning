...
|---|---|---|---|
| 초기 2018~23 | TB당 원가 · 랙당 용량 | 데이터셋 6배, HDD의 TB당 대역폭 붕괴 → 플래시 용량 계층(Meta 10 MB/s/TB, TLC 서버의 6배 바이트 밀도), 공급자는 1U에 1PB·TCO -47% | 읽기 중심이라 0.3~0.6 DWPD로 충분, 읽기 데이터를 QLC로 옮기면 전력도 감소 |
| 현재 2024~26 | TB당 와트 · 공급 확보 | 추론 서버의 전력·공간 병목 → 2024년 30EB(4배), 245TB 경쟁 | 여전히 읽기 중심, 쓰기 티어는 TLC |
| 향후 2027~30 | GPU당 캐시 용량 · 토큰당 비용 | KV 캐시가 HBM·DRAM을 넘쳐 SSD로, 프리필 재사용으로 GPU당 동시 사용자·TTFT 개선(NVMe 오프로드로 H100 한 장이 동시 사용자 10배 주장), 2027년 75~100EB | 쓰기 많은 캐시 → 오늘은 TLC 1~3 DWPD, QLC 정격과 10~40배 갭 |

불변은 "계산 옆의 가장 싼 바이트"이고, 변화는 그 자리가 쓰기 많은 티어로 옮겨간 것이다. 내구성은 미디어가 아니라 고객 시스템이 데이터를 어떻게 놓느냐(배치 표준·캐시 관리자 정책)로 풀리므로, 4장의 Phase 1·2·3과 6장의 FDE·SCA가 뒤따른다. 상세와 출처는 [qlc-ssd-market.md](../../wiki/concepts/qlc-ssd-market.md) §3.4. 발표 덱은 이 축으로 1~3장을 잇고 니어라인 HDD 대체 논의(§2.3)는 덱에서 제외한다(사용자 결정 2026-09-17).
...
| # | 킥커 | 액션 타이틀 | 본문 |
|---|---|---|---|
| 1 | 배경 · 고객은 왜 QLC를 원하는가 | 고객이 사는 것은 계산 옆의 가장 싼 바이트이고, 그 자리가 추론 캐시 티어로 옮겨갑니다 | 3기 스트립(고객이 산 것 · 왜 · 조건 · 삼성) → 이정표 타임라인 → 통합 그래프(EB·%·$B) + 키넘버 3 → 결론 "고객은 추론 캐시 티어에서도 QLC의 경제성을 원합니다. 문은 내구성이고, 디바이스만으로는 못 엽니다". 니어라인 HDD 대체 언급 없음(v3.3) |
| 2 | 요구사항과 역량 · 기술 전략 | 내구성은 고객 시스템에서 풀리므로, 디바이스에서 고객 시스템 안까지 세 단계로 올라가야 합니다 | 상: 갭 타일 4(숫자 + 한 줄) / 중·하: Phase 1·2·3 **스택 그림 확대**(고객 시스템 5계층에 층마다 구체 기술을 적고 우리가 닿는 층을 색으로: 응용 vLLM·SGLang·TRT-LLM / 관리자 KVBM·LMCache·Mooncake·FlexKV / I/O NIXL·GDS·io_uring·xNVMe / 커널·플랫폼 write streams·XFS·CMX / SSD RUH·2Tb QLC·NVMe KV; 트레이스·정책 화살표, FDE 상주 아이콘, 진행 바에 삼성 현 위치). 협업 기업 로고 행은 제거 |
| 3 | 실행 전략 · 고객 협업 | FDE로 고객 시스템 안에 들어가고, 전략적 협약으로 워크로드를 받습니다. 선례는 이미 있습니다 | 중앙 **두 트랙 그림**(삼성 개발실 ↔ 고객 시스템 스택, 위 화살표 "① FDE가 들어간다", 아래 화살표 "② SCA로 워크로드가 온다", 가운데 **선례 카드 2**: Palantir FDE(상주 엔지니어 그림 + 파급) · Micron↔Anthropic SCA(계약 4요소 블록 + 16건·$100B·$22B)) / 하: 조직·인사·문화 3축 타일(축당 3줄 — 조직: 자회사 / FDE Pod / 시스템 SW 조직 강화, 인사: 고객 시스템을 아는 시스템 SW 전문가 채용·양성 / 미주 현지 채용 확대 / 별도 보상·상주 로테이션, 문화: 오픈소스 생태계를 주도 / 메인테이너·커미터 배출·레퍼런스 프로젝트 운영 / 업스트림 우선·실측 공개) + 결론 밴드 = 3장 요약 한 문장("고객은 계산 옆의 가장 싼 바이트를 사려고 QLC를 택했고, 그 자리가 쓰기 많은 추론 캐시로 옮겨가 내구성을 고객 시스템 안에서 풀어야 하며, 그 길은 FDE와 전략적 협약, 그리고 이를 해낼 사람과 문화가 엽니다"). 전략·재무 축·3티어 타임라인·결정 요청은 덱에서 제외(본문 §6.2·§6.3 참조) |
