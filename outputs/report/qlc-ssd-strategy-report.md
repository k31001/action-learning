...
| `outputs/presentation/qlc-ssd-strategy-outline.md` · `scripts/generate_qlc_ssd_strategy_pptx.py` · `qlc-ssd-strategy.pptx` | 5장 덱(v3.10 스토리라인 통일) |

## PPT 압축 맵 (5장 덱, v3.10 스토리라인: ① 산식 → ② 시점 → ③ 수요 → ④ 역량 → ⑤ 실행)

덱의 헤더는 5장 스토리 레일(현재 장 강조), 각 장 결론 밴드 우측은 다음 장 포인터, 5장 결론 밴드는 5장 요약 체인이다. 리드는 앞 장과의 연결 문장(1장은 이 장의 질문, 4·5장은 인용).

| # | 킥커(스토리 레일) | 액션 타이틀 | 본문 |
|---|---|---|---|
| 1 | ① 산식 · 왜 호스트 협력인가 (v3.9 신설, v3.10 문안) | RBER 상승은 컨트롤러 ECC가 보상했고, P/E 감소의 보상 변수 WAF는 호스트 계층에 있습니다 (리드 = 이 장의 질문: QLC의 내구성 격차를 어느 계층이 해소하는가) | 네 지표 차트(P/E 100배↓ / RBER 10⁶배↑ vs UBER 고정·ECC 60배 / 정격 DWPD 17→0.41 vs 요구 1~3·유효 7~10 / WAF 3.22→1.03) + 산식 카드(항별 결정 주체) + 이관 구조의 반복 3행(NAND→SSD / SSD→호스트 / 호스트→고객 시스템 공동 설계 = 본 덱 2~5장) + 결론 "요구는 고정, 단품 지표는 악화, 격차는 상위 계층의 변수가 보상 · QLC의 호스트 협력은 산식의 귀결" → ② 시점. 상세: [memory-solution-ladder-report.md](memory-solution-ladder-report.md) §2.5 |
