...
| **조직** | 고객별 Co-Design Pod · **시스템 아키텍트·모델링 전문 조직 신설** · **Distinguished Engineer(스타 아키텍트) 트랙** · 워크로드 인텔리전스 · 기술 마케팅 승격 | 개발 엔지니어를 고객 아키텍트 옆으로, 스타를 공식화 |
...
**왜 지금 채용에 유리한가 (처우 급등 국면)** — 2026 반도체 특수로 메모리 임직원 처우가 급등했다(삼성 DS 특별 성과급으로 **메모리 직원 ~6억원 성과급 시대**, SK하이닉스 영업이익 10% 성과급 명문화·연봉 상한 철폐). 이 국면이 스타 영입 협상에 유리한 위치를 만든다: ①메모리 사상 최대 이익으로 **재원** 확보, ②업계 보상 상향 리셋으로 스타급 처우 제시의 **사내 형평성 마찰이 작아짐**(일반 직원도 이미 6억대), ③"반도체 특수"에서 핵심 인재 확보는 이사회·주주가 납득하는 **명분**. 동시에 SK하이닉스의 인재 poaching 압력이 커지는 국면이므로, DE 트랙은 영입뿐 아니라 **스타 이탈 방어(retention)** 수단이기도 하다.

**조직 형태** — 관리자 승진과 분리된 **이원 경력 경로(dual-ladder)**로 Distinguished Engineer / Technical Fellow를 임원급으로 대우(관리 보직 없이 최고 처우 도달). 호명 문화를 가동해 컨퍼런스 발표·기술 백서 저자·특허 대표발명자·고객 공동설계 리드로 **이름을 남기게** 한다.

## 5. 액션 플랜 — 3 Phase

### Phase 1 · 90일 (증명): 파일럿 고객 1사 Co-Design Pod 발족, 워크로드 랩 최소 구성 + 시스템 모델 v0.1(디바이스→서버 레벨, 실측 오차 검증), 선제 제안 1호 제출, KPI 개정안 설계
### Phase 2 · 1년 (제도화): Pod 3~5개 확대, **시스템 아키텍트·모델링 전문 조직 신설**(모델 v1.0 랙 레벨), **DE 1호 임명 + 스타 아키텍트 영입 1명**, 로드맵 교차 리뷰 정례화, PoA 연 4회, 개정 KPI 전면 적용, **SCA형 계약 1건 이상 수주**
...
| Distinguished Engineer(스타 아키텍트) 수 | 0 (트랙 없음) | 1명+ | 5~10명·대외 호명 정착 |
