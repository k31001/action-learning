...
인재 축(FDE 스타)이 "누가 고객 아키텍처 안으로 들어가는가"라면, 제품·기술 축은 "무엇을 들고 들어가는가"에 답한다. **당위성은 환경 변화에서 나온다**: 고객(하이퍼스케일러, enterprise SSD 수요 ~55%)의 스토리지 통제권이 완제품 구매 → 커스텀 펌웨어 → 자체 컨트롤러(AWS Nitro SSD, 2021) → **표준 주도(FDP TP4146, Meta·Google 주도·삼성 공동 비준 2023)·웨이퍼 직구매**로 일관 상승했고 이 흐름은 불가역이다. 교차점에서 검토한 선택지 4개(A 컴포넌트 후퇴 — 부가가치 상실 / B 풀커스텀 — 파편화·소싱 체질 부재(최장석 인터뷰) / C FDP HW-only — 가격 경쟁 회귀 / D 표준+시스템 SW 플랫폼) 중 **D만이 부가가치 방어·펌웨어 공통화·통제권 정합·차별화 지속성을 동시 충족**한다. **FDP SSD 공급자 → FDP 기반 Host–SSD 통합 솔루션 제공자**로 전환: FDP는 인터페이스일 뿐이므로 워크로드 데이터 수명을 RUH에 매핑·검증하는 **시스템 소프트웨어**(SDK·워크로드 플러그인·Workload Profiler·Emulator/Digital Twin)를 SSD와 함께 제공해 도입 장벽을 낮추고, 표준 워크로드 프로파일로 펌웨어 공통화와 고객별 최적화를 양립시키며, End-to-End 공동검증(시스템 성과 지표: WAF·p999·usable capacity·격리·전력)과 Binding 계약 내 기술협력 조항으로 Captive SSD 요구를 삼성 완제품 생태계 안에 수용한다. 오픈소스(기본 라이브러리·연동·적합성 테스트)와 차별화 영역(NAND/FTL 모델·정책 추천·예측 모델)의 경계를 설정해 lock-in 우려 없이 TCO 우위를 제공한다. *"Binding으로 수요를 확보하고, FDP로 제품을 표준화하며, 시스템 소프트웨어로 고객 워크로드를 연결한다."* 상세: [wiki/strategies/fdp-host-ssd-platform.md](../../wiki/strategies/fdp-host-ssd-platform.md).
