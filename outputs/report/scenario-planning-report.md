...
**상류 드라이버 트리 (2026-06-11 확장)**: 각 병목에 영향을 주는 상류 요소를 depth 1(중류)·depth 2(상류)로 분해해 선행시차만큼 먼저 인지한다 — 예: CAPEX ← 하이퍼스케일러 이익·FCF(d1, 2~4분기 선행) ← **AI 기업 매출·이익(OpenAI·Anthropic·xAI·Google)**(d2, 12~18개월 선행). 22개 드라이버의 가중 롤업으로 병목별 선행 압력(상류/중류)을 산출하고, 현재 지수와의 괴리로 조기경보(악화 예고·완화 예고·상류-중류 괴리)를 발동한다. 2026-06 판독: CAPEX는 중류(가이던스 강세)가 조용하나 상류(GPU 임대가·자금조달)가 먼저 악화(괴리 +11), 패키징은 상류(45)가 현재(72)보다 낮아 2027~28 완화 방향. 수요 변곡 EWI는 같은 탭의 서브탭으로 통합 — 하방 방향(EWI)과 상방 제약(병목)을 한 화면에서 감시한다.
