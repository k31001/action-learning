...
**작성일**: 2026년 5월 6일 (Q1 2026 실적 + 벤치마크 통합 업데이트) | **최종 데이터 갱신**: 2026년 6월 30일 (병목 모델 지수 재점검 + SK하이닉스 나스닥 상장·Q1 확정실적·전력망 규제 격상 반영)
...
| **4대 병목 제약지수 (2026-06-30)** | 전력 **72**(▲+2) · CAPEX **39**(▼-3) · 파운드리 **55**(▲+3) · 패키징 **65**(▼-3) | 전력 6연속 상향·구조 악화 고착. 파운드리는 NVIDIA 수요 재가속으로 완화 흐름 역전 |
| HBM 점유율 (Q1 2026 확정실적, Counterpoint) | SK 57~58% / Samsung 21~32% / Micron ~21% | 분기 매출 기준은 "4월 추정 35~40%"보다 낮음 — 방법론 차이, Q2 확정실적(8월)에서 재확인 |
| DRAM 매출 점유율 (Q1 2026 확정) | Samsung 38.6%($37.4B) / SK 28.8%($28.0B) / Micron 22.4%($21.7B), 시장 ~$97B(+260% YoY) | 매출은 삼성 1위, 마진은 SK 압도(OPM ~72%) |
| **SK하이닉스 나스닥 ADR 상장** | **2026-07-10 예정, $29~30B 추정** | 미국 자본시장 직접 접근 — 달러 조달 채널 다변화 |
| **스마트폰 출하 전망 (Counterpoint)** | 2026년 **-13.9% YoY, 10.8억 대** (역대 최대 하락폭) | 모바일 DRAM 가격 ~3배 폭등이 원인 — 메모리 호황의 실물 수요 파괴 경고 신호 |
| TSMC 5월 2026 매출 | NT$417B YoY +30.1% (단월 사상 최고), CoWoS 수급갭 20%→10% 축소 전망 | 패키징 병목 완화 추세 지속 |
| Micron 미국 캐파 | 10% → 40% (10년) $200B 약정 | 미국 메모리 자급 비중 상승 — 시나리오 A 강화 |
| CXMT 2026E 매출 | **$50B+ 전망**(ASP 효과 주도) — 단 캐파는 삼성·SK의 절반 수준(~350K WSPM), HBM 배정 ~2%뿐 | 매출 규모만으론 과대평가 위험 — 여전히 범용 DRAM 위협 |
...
**병목 레이더 (2026-06-10 신설, 2026-06-30 최신 — 대시보드 Bottleneck Model 탭)**
...
| 병목 | 현재 지수 (2026-06-30) | 직전(06-14) 대비 | 핵심 감시 트리거 | 충격 시 영향 (연환산) |
|---|---|---|---|---|
| 전력 | **72 Orange** — PJM 용량권고·FERC 60일 명령·NERC Level3 경보(8/3 기한) | ▲ +2 (6연속 상향) | 상위 허브 2+ reserve <8% · LMP P90 72h · 접속지연 >60일 | HBM -0.61EB (-21.1%) |
| 첨단 패키징 (CoWoS) | **65 Orange** — 수급갭 20%→10% 축소 전망 | ▼ -3 | qualified output -15% WoW 2주 · 대형 사이트 outage | HBM -0.59EB (-20.5%) |
| 선단 파운드리 | **55 Yellow** — NVIDIA 수요 재가속, 직전 완화 흐름 역전 | ▲ +3 | N2/A16 램프 1분기+ 지연 · High-NA 삽입 지연 | HBM -0.43EB (-14.9%) |
| CAPEX/ROI | **39 Green** — "DC 용량 취소" 루머 반박, NVIDIA 매출 컨센서스 +20% 상회 | ▼ -3 | capex 가이드 -15% · FCF/CapEx <0.8 · HY OAS 급등 | **HBM -0.91EB (-31.5%)** |
...
**2026-06-30 변동 해석**: 전력은 4대 병목 중 유일하게 6회 연속 상향(64→68→70→72)으로 구조적 악화가 고착되는 중. 파운드리는 직전(06-14) 점검의 완화 판단(Rubin 출하 22%로 하향)이 NVIDIA 수요 재가속 신호(SemiAnalysis: HBM4 공급 해소→2H FY27 매출 컨센서스 +20% 상회)로 **단일 점검 주기 만에 역전** — 4축 중 변동성이 가장 크다는 점을 보여주는 사례로, 다음 점검(2026 Q3 Rubin 초기 출하)에서 추세 확정 필요. CAPEX·패키징은 완화 추세가 지속 확인됐다.

**상류 드라이버 트리 (2026-06-11 확장, 2026-06-30 갱신)**: 각 병목에 영향을 주는 상류 요소를 depth 1(중류)·depth 2(상류)로 분해해 선행시차만큼 먼저 인지한다 — 예: CAPEX ← 하이퍼스케일러 이익·FCF(d1, 2~4분기 선행) ← **AI 기업 매출·이익(OpenAI·Anthropic·xAI·Google)**(d2, 12~18개월 선행). 22개 드라이버의 가중 롤업으로 병목별 선행 압력(상류/중류)을 산출하고, 현재 지수와의 괴리로 조기경보(악화 예고·완화 예고·상류-중류 괴리)를 발동한다. 2026-06-30 판독: CAPEX 상류(AI 기업 매출·이익) 드라이버가 '중립'에서 '완화'로 개선(NVIDIA 수요 재가속 확인), 전력 상류(계통 접속)는 '임계' 유지 — PJM·FERC·NERC 규제 격상이 단기 해소 가능성을 추가로 낮춤. 모델 구조 도식(상류→병목→min()→수급)이 탭 상단에 내장되어 있으며, 수요 변곡 EWI는 프레임 일관성을 위해 같은 탭의 별도 서브탭으로 분리 운영한다(모델 화면에는 비표출).
