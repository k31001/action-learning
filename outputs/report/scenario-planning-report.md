...
**작성일**: 2026년 5월 6일 (Q1 2026 실적 + 벤치마크 통합 업데이트) | **최종 데이터 갱신**: 2026년 9월 15일 (병목 모델 지수 정기 갱신 — 텍사스 ERCOT 신규 접속 전면 일시중단으로 전력이 최고 병목으로 부상)
...
| **4대 병목 제약지수 (2026-09-15, 이전 07-04 대비)** | 전력 **78**(▲+6) · CAPEX **40**(─0) · 파운드리 **52**(▲+2) · 패키징 **65**(▼-2) | 텍사스 ERCOT 신규 DC 접속 전면 일시중단(474GW 대상)으로 전력이 4대 병목 중 최고 지수로 부상(3분기 연속 상승). CAPEX는 가이던스 불변이나 NVIDIA-OpenAI 파이낸싱 축소로 조달 스트레스, 파운드리는 N2 램프 격차로 재긴장, 패키징은 CoWoS 갭 축소로 소폭 완화 |
...
#### 2.2.3 [정기점검] 2026-09-15 시장 업데이트 — 텍사스 그리드 접속 전면 일시중단·TSMC 8월 매출 사상최고·HBM 하이브리드본딩 HBM5 이연

약 10주 만의 병목 모델 정기 점검(2026-09-15, 이전 점검 2026-07-04 대비). 08~09월의 위키 갱신은 다운턴 트랙·내부 인터뷰·발표자료 중심이라 4대 병목 제약지수는 07-04 이후 미갱신 상태였음.

- **병목 모델 4대 제약지수 갱신**: 전력 72→**78**(▲+6, 텍사스 주지사가 ERCOT 신규 DC 접속을 감사 완료 시까지 전면 일시중단시킴 — 대상 474GW·1,800개 프로젝트. PJM도 신규 대형부하 규정을 FERC 제출해 "부하 자체 발전 조달" 원칙으로 전환. 4대 병목 중 최고 지수·3분기 연속 상승), CAPEX/ROI 40→**40**(─0, 빅4 합산 $725B·삭감 0건 불변이나 NVIDIA가 OpenAI 오하이오 DC 파이낸싱 보증을 $250B 검토액에서 최종 $105B로 축소해 조달 구조 스트레스 신규 포착), 파운드리 50→**52**(▲+2, TSMC 8월 매출 사상 최고(+53.3% YoY)로 수요는 견조하나 N2 현재 캐파(월 2만 장)와 연말 목표(월 10만 장) 격차로 램프 실행 리스크 재부상), 패키징 67→**65**(▼-2, CoWoS 수급 갭 20%→10% 축소 전망 재확인이나 SK하이닉스가 하이브리드본딩을 HBM4E도 건너뛰고 HBM5(~2029~30)로 이연 확정 + 775마이크론 두께 한계 신규 제기). 상세: [wiki/concepts/bottleneck-model-2030.md §종합 판독(2026-09-15)](../../wiki/concepts/bottleneck-model-2030.md).
- **전력이 CAPEX/ROI를 제치고 최고 병목으로 부상**: 07-04까지는 "큐 지연"이었으나 09-15는 텍사스·PJM 두 최대 시장에서 동시에 정부·규제기관이 신규 접속을 직접 제한하는 국면으로 전환 — 시나리오 B(Main Bet)의 수익성 전제(전력을 먼저 확보한 자가 승자)를 더 날카롭게 확인.
- **HBM 점유율**: 삼성 09-03·09-12 보도 기준 33~40%대로 회복 재확인(07-04 위키 서술 35~40%와 정합). 단 경로별 수치 편차(SK 57%·삼성 22%·Micron 21% 계열도 병존)가 잔존 — 다음 ingest에서 Counterpoint 원문 대조 필요. 삼성 HBM4 NVIDIA 볼륨 오더는 07-17 이후 업데이트 없음(미전환 추정).
- **DRAM 가격**: 엔터프라이즈 64GB DDR5 RDIMM이 Citi 전망 기준 2026 Q1 $873→Q4 ~$1,586(3분기 누적 +80%+), DDR4 스팟은 08-31 기준 $42.50 사상 최고 — 서버향 초강세가 지속되며 병목 모델의 "기준~높음" 가격 경로를 계속 지지.
...
**병목 레이더 (2026-06-10 신설, 2026-09-15 지수 갱신 — 대시보드 Bottleneck Model 탭)**
...
| 병목 | 현재 지수 (2026-09-15) | 핵심 감시 트리거 | 충격 시 영향 (연환산) |
|---|---|---|---|
| **전력** | **78 Red** — 텍사스 ERCOT 신규 DC 접속 전면 일시중단(474GW)·PJM 대형부하 신규 규정 | 상위 허브 2+ reserve <8% · LMP P90 72h · 접속지연 >60일 | HBM -0.61EB (-21.1%) |
| 첨단 패키징 (CoWoS) | 65 Orange — CoWoS 갭 축소(20%→10%)하나 HBM4 하이브리드본딩 HBM5로 이연 | qualified output -15% WoW 2주 · 대형 사이트 outage | HBM -0.59EB (-20.5%) |
| 선단 파운드리 | 52 Yellow — 매출 사상 최고, N2 램프 실행 격차(2만→10만 장/월) | N2/A16 램프 1분기+ 지연 · High-NA 삽입 지연 | HBM -0.43EB (-14.9%) |
| CAPEX/ROI | 40 Yellow — 가이던스 불변·삭감 0건, NVIDIA-OpenAI 파이낸싱 축소로 조달 스트레스 | capex 가이드 -15% · FCF/CapEx <0.8 · HY OAS 급등 | **HBM -0.91EB (-31.5%)** |

**전력이 4대 병목 중 유일하게 Red 밴드(75~85)에 진입 — 텍사스·PJM의 정부발 접속 제한이 병목 서열을 CAPEX/ROI 중심에서 전력 중심으로 재편.**
