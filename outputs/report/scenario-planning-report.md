...
**작성일**: 2026년 5월 6일 (Q1 2026 실적 + 벤치마크 통합 업데이트) | **최종 데이터 갱신**: 2026년 9월 8일 (병목 모델 정기 점검 — 전력 74·CAPEX 41·파운드리 51·패키징 65)
...
| **4대 병목 제약지수 (2026-09-08, 이전 07-04 대비)** | 전력 **74**(▲+2) · CAPEX **41**(▲+1) · 파운드리 **51**(▲+1) · 패키징 **65**(▼-2) | 전력 3개 분기 연속 재상승(ERCOT 접속 전면 동결 신규). CAPEX·파운드리 완화 정체(CoreWeave·Oracle 신용경색), 패키징만 완화 지속(ABF 신규 병목 제한) |
...
#### 2.2.3 [정기점검] 2026-09-08 시장 업데이트 — ERCOT 접속 전면 동결·신용경색 신호·CXMT DRAM 10%·HBM4 하이브리드본딩 지연 공식화

5개 병렬 리서치 에이전트(SemiAnalysis·Counterpoint·TechInsights 우선 수집)로 진행한 정기 점검(2026-09-08, 이전 점검 2026-07-04 대비).

- **병목 모델 4대 제약지수 갱신**: 전력 72→**74**(▲+2, 텍사스 ERCOT 데이터센터 신규 접속 승인 전면 동결 — 2026-08-03 주지사 지시 — 이 신규 정치·행정 리스크로 3개 분기 연속 상승), CAPEX/ROI 40→**41**(▲+1, 가이던스는 전원 추가 상향되나 Alphabet·Amazon FCF 사상 첫 마이너스·Oracle S&P BBB- 강등·CoreWeave CDS 디폴트확률 ~50%로 신용시장 첫 균열), 파운드리 50→**51**(▲+1, N2·ASML은 순항이나 NVIDIA Rubin 물량 전망 재상승으로 수요측 압력 재긴장), 패키징 67→**65**(▼-2, CoWoS 캐파·수율 개선 지속되나 SK하이닉스 HBM4E 하이브리드본딩 배제 공식화·ABF 기판 리드타임 배증으로 완화 폭 제한). **하방 위험 순서 변경: 전력 > CAPEX/ROI ≈ 패키징 > 파운드리** — 전력이 처음으로 CAPEX/ROI를 앞질렀다. 상세: [wiki/concepts/bottleneck-model-2030.md §종합 판독(2026-09-08)](../../wiki/concepts/bottleneck-model-2030.md).
- **HBM·DRAM 시장 재편**: 삼성이 HBM4 세계 최초 양산과 "골든 수율" 80%(2026-08-26)를 발표하며 HBM 매출 점유율이 21→**33%**로 반등(SK하이닉스 58→50%, Micron 18%). DRAM 매출 점유율은 삼성 39%·SK하이닉스 26%·Micron 25%·**CXMT 10%**(전년 4%)로, CXMT가 3사 90%+ 과점 체제를 사상 처음 무너뜨렸다. TrendForce는 2026년 메모리 TAM 전망을 $551.6B→**$889.3B**로 대폭 상향.
- **DRAM/NAND 수급 시차 분화(신규)**: DRAM은 **2028 Q2까지 지속 타이트**(삼성 자체 경고+UBS), NAND는 **2027 H2**로 먼저 균형/오버서플라이 전환 — 기존 "2027~28 CXMT/YMTC 오버서플라이" 단일 프레임을 제품별로 분리할 필요.
- **패키징 기술 경쟁축 분화**: SK하이닉스가 Hot Chips 2026(8/23)에서 HBM4E까지 하이브리드본딩 배제를 공식화(HBM5부터 적용, JEDEC 775㎛ 스택 한계)한 반면, 삼성은 하이브리드본딩을 고수하며 국내 전용 라인을 구축 중.
- **신용시장 — 처음으로 가이던스와 반대 방향**: CoreWeave 5년물 CDS 디폴트확률 ~50%, Oracle CDS 7년 최고(S&P BBB- 강등, $16.3B 부채딜에 은행 이탈→PIMCO 앵커), 하이퍼스케일러 IG채권 발행 YoY+1,300% — 4대 빅테크+Oracle의 CAPEX 가이던스 추가 상향과 상충.
- **반독점 소송 진행**: 담당판사 Noel Wise(N.D. California) 확정, Micron이 답변서에서 혐의 부인.
- **시나리오 확률·DF1·DF2**: 상기 신규 사실을 대조한 결과 확률(A26·B39·C8·D21·E6)·DF1(8.5)·DF2(0.5) **전부 유지** — 신용경색 신호는 기존에 지목된 CapEx-vs-FCF 다이버전스의 확대판(조달 경보이나 수요 붕괴 아님), CXMT 10%는 이미 반영된 추세의 재확인, ERCOT 동결은 병목 모델 별도 축(DF1·DF2 비대상)으로 판단.

> 1차 자료: [sources/articles/september-2026-market-update-2026-09-08.md](../../sources/articles/september-2026-market-update-2026-09-08.md)
