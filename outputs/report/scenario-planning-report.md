...
**작성일**: 2026년 5월 6일 (Q1 2026 실적 + 벤치마크 통합 업데이트) | **최종 데이터 갱신**: 2026년 9월 22일 (병목 모델 정기 점검 — 파운드리 급상승·패키징 완화)
...
- **Main Bet은 시나리오 B "AI 르네상스" (확률 39%, 2026-07-14 재평가)**. 5개 시나리오 중 시나리오 B만이 2035년 시장 $5,200억 규모로 가장 크고 삼성의 동서 균형 포지션이 최대 가치를 발휘하는 유일한 미래다. 그러나 자동 실현되지 않는다. HBM4E·HBM5 기술 1위 탈환, 동서 균형 공급망(텍사스+인도), 1c nm 원가 우위, 커스텀 AI 메모리 솔루션, 텍사스 2기 미국 현지 생산이 동시에 추진되어야 1번 자리를 회복할 수 있다.
...
| 빅테크 5사 AI CapEx 2026 | **$745~755B+**(MS $190B·Alphabet $180~190B·Amazon $200B·Meta $125~145B·Oracle ~$50B, 추가 상향) | Dell'Oro 2026 글로벌 DC CAPEX **$1조 돌파** 전망·JPMorgan 2030 누적 $5.5조. 메모리 가격 상승이 CAPEX 상승의 명시적 요인(MS $25B) |
| HBM4 NVIDIA Rubin 공급 (배정 세분화, 07-04) | SK하이닉스 60~70%·삼성 25~30%·Micron 잔여 | Micron 초기 빌드 일시 배제 후 2026-06-01 GTC 재확인(3사 전원) — 자격검증 굴곡 확인 |
...
| DRAM 가격 — 감속 조짐(TrendForce 2026-07-03, 최신) | Q3 2026 범용 DRAM 계약가 **+13~18% QoQ**(Q2 +58~63% 대비 대폭 감속) | 서버향은 견조하나 범용 축 첫 감속 — 수요 변곡 EWI 조기신호 후보 |
| **4대 병목 제약지수 (2026-09-22, 이전 07-04 대비)** | 전력 **75**(▲+3) · CAPEX **43**(▲+3) · 파운드리 **59**(▲+9) · 패키징 **62**(▼-5) | 병목 축이 패키징→전공정(N3)으로 이동 중(SemiAnalysis·TSMC CEO). 전력 5회 연속 재상승(PJM 용량경매 사상최고가). CAPEX는 09-14 AI 감속론 센티먼트로 완화 추세 반전 |
| HBM 점유율 (Counterpoint Q2 2026, 이전 Q1 대비) | SK **50%**(Q1 58%) · 삼성 **33%**(Q1 21~32%, 소스 간 편차) · Micron **18%** | 삼성 HBM3E 12-Hi NVIDIA 인증 09-14 통과(18개월 지연)·2026 HBM 완판. 삼성-SK 격차 37%p→17%p로 축소 |
| **DRAM 반독점 소송 — 기각신청 (신규)** | 2026-09-03 삼성·SK하이닉스·Micron 공동 기각신청 제출, 09-22 현재 법원 결정 미발표 | 선행소송 재탕·셔먼법 1조 개연성 부족 주장. 병목 실물 구조 불변, 규제 리스크는 계속 모니터링 |
| **CXMT 5세대(G5) DRAM 양산 (신규)** | 2026-09-20 EUV 없이 쿼드러플 패터닝으로 11.95nm 양산 진입. DRAM 점유율 7.6~9.5%로 급등(빅3 합산 첫 90% 미만) | 기존 수출통제 실효성 논쟁 촉발. HBM3e 캐파도 확대 중(2026말 ~3만 WSPM) — RS-2·RS-4 전제 모니터링 격상 |
| **SK하이닉스 나스닥 이중상장 (신규)** | Form F-1 수정 2026-06-30·티커 SKHY·목표조달액 ~$294억·거래개시 2026-07-10 | 역대 최대 ADR 상장 전망 — 경쟁사 자본 접근성 확대, RS-1·5·6 벤치마크 갱신 필요 |
| **DRAM 반독점 집단소송 (신규 리스크)** | 2026-06-25 N.D.Cal. 제소, Samsung·SK하이닉스·Micron 공동 피고 | 병목 실물 구조는 불변, 삼성 규제·평판·재무 리스크로 별도 모니터링 |
| TSMC 파운드리·패키징 | N2 2026-03 매출 기여 개시·ASML High-NA 2029로 연기(리스크 축소)·CoWoS NVIDIA 배정 60%·CoPoS 전면양산 2028~29 재확인 | 파운드리 완화, 패키징은 완화 속도 둔화 |
| Micron 미국 캐파 | 10% → 40% (10년) $200B 약정 | 미국 메모리 자급 비중 상승 — 시나리오 A 강화 |
| CXMT HBM | 2026년 말까지 월 3만 장(30k wspm) HBM 웨이퍼 배정 전망(SemiAnalysis) | 위협 시점 조기화 — 기존 로드맵보다 빠름 |
| 텍사스 테일러 CHIPS 보조금 | $4.745B 확정 (목표 $6~8B의 60~80%) | 2단계 추가 협상 필수 (Decision 8) |
| 한국 국가 반도체 생태계 계획 (신규, 2026-06-29) | 약 800조 원($518B), 삼성·SK하이닉스 각각 신규 팹 2개 | 국내 정책 지원 확대 — MB2·SA2 실행 여건 개선 |

> 최신 데이터 출처: [sources/articles/july-2026-market-update-2026-07-04.md](../../sources/articles/july-2026-market-update-2026-07-04.md) (SemiAnalysis·Counterpoint·TechInsights 등)
...
#### 2.2.1 [속보] 마이크론 FY26 Q3 실적 (2026-06-24 발표) — 슈퍼사이클 정점 실증

2026-06-24, 마이크론이 회계연도 3분기(FY26 Q3, **2026-05-28 종료**) 실적을 발표했다. 경쟁사 1차 실적은 메모리 슈퍼사이클의 강도와 가격 구조를 가장 빠르게 읽는 창(窓)이며, 이번 분기는 **모든 핵심 지표가 가이던스를 전면 상회한 "혁신적(transformational)" 분기**였다.

**손익 — 사상 최대, 가이던스 전면 상회**

| 항목 | FY26 Q3 (5월 종료) | FY26 Q2 (직전) | FY25 Q3 (전년) | 변동 |
|------|---|---|---|---|
| 매출 | **$41.46B** | $23.86B | $9.30B | **+74% QoQ · +346% YoY** |
| GAAP 매출총이익률 | **84.9%** (사상 최고) | — | — | 80% 첫 돌파 |
| GAAP 희석 EPS (순이익) | **$24.67** ($28.24B) | — | — | — |
| Non-GAAP 희석 EPS (순이익) | **$25.11** ($28.86B) | — | — | 컨센서스 $20.60 대비 **+$4.51** |

- 매출은 컨센서스(~$35.7B)를 **약 $5.8B 상회**, 직전 자체 가이던스 **$33.5B를 +24% 초과**. 발표 직후 시간외 주가 **+13.7%**(~$1,192, 52주 최고 근접).

**사업부·제품 — 가격이 성장의 압도적 동인**

| 부문 | 매출 | 핵심 |
|------|------|------|
| DRAM | **$31.3B** (전사 76%) | 비트 출하 +low-single-digit% QoQ에 그쳤으나 **ASP +low-60s% QoQ 급등** → 매출 성장의 99%가 가격 |
| NAND | **$9.9B** | QoQ 거의 2배 |
| 데이터센터 | 분기 **$25B 초과** | **연환산 런레이트 $100B 초과** (CEO) |
| HBM | **HBM4 누적 $1B 초과** | HBM4 12-high 램프가 HBM3E 12-high 대비 **~2배 빠름**, 2026년 HBM 캐파 **전량 완판**. HBM 점유율은 웨이퍼 잠식 회피 위해 **DRAM 점유율에 근접 관리** |

**Strategic Customer Agreements (SCA) — $100B 최소 계약 매출**

- **16개 SCA, 최소 계약 매출 ~$100B**(RPO 기준, 최소 물량 × 최소 가격). 현금 예치금 + 금융 약정 **$22B** 수령 예상.
- 양방향 take-or-pay 장기공급계약(LTA)의 산업 표준화 신호 — capex 정당화와 고객 공급 예측가능성을 동시에 확보하는 구조.

**자본배분·수급·가이던스**

- FY26 Q4 capex ~$10B, **FY2026 연간 capex ~$27B**(정부 인센티브 차감 순). FY2027 분기 capex는 Q4 수준 상회(증가분 절반 이상이 클린룸 선반입 건설 capex).
- DRAM·NAND **수급 타이트가 calendar 2027 이후까지 지속** 전망(직전 CEO "shortage well beyond 2026"과 정합).
- **Q4 FY26 가이던스**: 매출 **$50.0B ± $1.0B**, GAAP 매출총이익률 **~86%**, GAAP EPS **$30.73 ± $1.00** → 분기 $50B 진입 전망.

**삼성전자 전략 함의 (4가지)**

1. **슈퍼사이클 정점 확증 ↔ 언와인드 리스크 동시 누적**: 출하(+low-single-digit%)를 가격(+low-60s%)이 압도하는 성장 구조는 정점의 전형. RS-5(재무 규율) 격상 명분이자, HBM sold-out·LTA·선급금 정점이 역설적 언와인드 셋업이라는 RS-9 센싱 논리를 외부에서 실증한다.
2. **RS-8 LTA 정량화 명분**: 마이크론이 SCA $100B·16건·예치금 $22B를 공시 → 삼성도 동일 take-or-pay 구조를 정량화·공시해 capex 신뢰도를 높일 근거.
3. **HBM 점유율 절제 동조**: 마이크론도 HBM을 DRAM 점유율에 묶어 절제 → 경쟁사 동시 절제 환경 확인. 삼성 단독 절제로 점유율을 잃는 시나리오 우려 약화.
4. **MB2 동서 균형 차별점**: 마이크론 데이터센터 $100B 런레이트는 미국 집중. 삼성 평택·시안·테일러의 동서 분산 공급이 글로벌 하이퍼스케일러·중국 고객 BCP 선호와 정합하는 차별점 유지.

> 1차 자료: [sources/filings/micron-q3-fy26.md](../../sources/filings/micron-q3-fy26.md) · 위키: [wiki/entities/micron.md](../../wiki/entities/micron.md) [Update 2026-06-25]

#### 2.2.2 [정기점검] 2026-07-04 시장 업데이트 — 전력망 재악화·CAPEX 추가 상향·SK하이닉스 나스닥·반독점 소송

SemiAnalysis·Counterpoint·TechInsights를 우선 참조하고 PJM·ERCOT·TrendForce·Dell'Oro·JPMorgan 등으로 보강한 정기 점검(2026-07-04, 이전 점검 2026-06-14 대비).

- **병목 모델 4대 제약지수 갱신**: 전력 70→**72**(▲+2, ERCOT 큐 1년 만에 4배·PJM 2030년까지 15GW 구조적 부족·변압기 리드타임 5년으로 신규 최대 병목), CAPEX/ROI 42→**40**(▼-2, 빅5 가이던스 추가 상향·Dell'Oro $1조 돌파·JPMorgan $5.5조), 파운드리 52→**50**(▼-2, ASML High-NA 2029 연기·N2 램프 순항), 패키징 68→**67**(▼-1, NVIDIA 배정 확정·OSAT 외주 확대되나 CoPoS 양산 2028~29 재확인·HBM4 마이크로범프 유지 결정으로 완화 둔화). 상세: [wiki/concepts/bottleneck-model-2030.md §종합 판독(2026-07-04)](../../wiki/concepts/bottleneck-model-2030.md).
- **HBM 공급망 재편**: Vera Rubin HBM4 배정이 SK하이닉스 60~70%·삼성 25~30%·Micron 잔여로 세분화 확인. Micron이 검증·핀속도 이슈로 초기 빌드에서 일시 배제됐다가 2026-06-01 GTC 타이베이에서 3사 전원 공급 재확인 — 자격 검증 과정의 굴곡을 재확인.
- **SK하이닉스 나스닥 이중상장**: 2026-06-30 Form F-1 수정 제출, 티커 SKHY, 목표조달액 ~$294억, 거래개시 2026-07-10 예정 — 역대 최대 ADR 상장. 조달자금이 캐파·R&D에 투입될 경우 삼성의 재무 규율 우위(RS-5)가 상대적으로 축소될 수 있어 모니터링 대상.
- **DRAM 반독점 집단소송(신규 리스크)**: 2026-06-25 미국 N.D. California 제소, 삼성·SK하이닉스·Micron 공동 피고 — HBM 전환 명목 범용 DRAM 공급제한·가격담합 주장(4년간 ~700% 상승 인용). 병목 모델의 실물 수급 구조는 불변이나 규제·평판·재무 리스크로 별도 추적([wiki/concepts/dram-antitrust-litigation.md](../../wiki/concepts/dram-antitrust-litigation.md) 신설).
- **DRAM 가격 감속 조짐**: TrendForce(2026-07-03) Q3 2026 범용 DRAM 계약가 전망 +13~18% QoQ — Q2(+58~63%) 대비 대폭 감속. 서버향은 에이전틱 AI 수요로 견조하나, 범용 축의 첫 감속 신호로 수요 변곡 EWI 조기경보 후보에 추가.
- **한국 800조 원 반도체 생태계 계획**(2026-06-29): 삼성·SK하이닉스 각각 신규 팹 2개 계획 — 국내 정책 지원 확대.

> 1차 자료: [sources/articles/july-2026-market-update-2026-07-04.md](../../sources/articles/july-2026-market-update-2026-07-04.md)

#### 2.2.3 [정기점검] 2026-09-22 시장 업데이트 — 병목 축이 패키징에서 전공정(N3)으로 이동

SemiAnalysis·Counterpoint·TechInsights를 우선 참조(세 원문은 프록시 차단으로 2차 인용 경유)하고 TrendForce·TSMC IR·Counterpoint 등으로 보강한 정기 점검(2026-09-22, 이전 점검 2026-07-04 대비).

- **병목 모델 4대 제약지수 갱신 — 최대 변화는 파운드리(+9)**: 전력 72→**75**(▲+3, PJM 2026/27 용량경매 사상최고가 $329~333/MW-day·중전압 개폐장치 2028년까지 매진), CAPEX/ROI 40→**43**(▲+3, 하드 데이터는 완화 지속이나 09-14 Amodei·Altman發 "AI 개발 속도 완화" 발언 후 메모리주 급락(-6~7%)이 3개 사이클 연속 완화 추세를 반전), 파운드리 50→**59**(▲+9, SemiAnalysis·TSMC CEO C.C. Wei: 병목이 패키징→전공정 N3로 이동, 2026년 하반기 N3 가동률 100% 초과 전망·"공급이 수요를 오랫동안 못 따라간다"), 패키징 67→**62**(▼-5, TrendForce CoWoS 공급-수요 갭 20%→10% 축소·JEDEC HBM 패키지 높이한도 상향으로 하이브리드본딩 긴급성 완화). **파운드리·패키징 격차가 17→3으로 급접근** — 2026~27년 최전선 병목이 패키징에서 전공정으로 이동 중이라는 것이 이번 사이클의 핵심 구조 변화. 상세: [wiki/concepts/bottleneck-model-2030.md §종합 판독(2026-09-22)](../../wiki/concepts/bottleneck-model-2030.md).
- **HBM 점유율 급변동**: Counterpoint Q2 2026 기준 삼성 HBM 매출 점유율이 33%로 급등(SK하이닉스와의 격차 37%p→17%p로 축소), SK하이닉스 50%·Micron 18%. 삼성은 2026-09-14 주간 HBM3E 12-Hi NVIDIA 인증을 마침내 통과(칩 개발 완료 후 약 18개월)하고 2026년 HBM 생산이 완판됐다고 보도됐다.
- **CXMT 5세대(G5) DRAM 양산(신규)**: 2026-09-20 EUV 없이 쿼드러플 패터닝으로 11.95nm 하프피치 달성, DRAM 시장점유율이 7.6~9.5%로 급등(빅3 합산 점유율이 10여 년 만에 처음 90% 미만). 기존 수출통제 실효성 논쟁을 촉발하는 이벤트로, 후속 규제 대응 여부는 다음 사이클에서 확인 필요.
- **DRAM 반독점 소송 — 기각신청**: 2026-09-03 삼성·SK하이닉스·Micron 3사가 N.D. California 법원에 공동 기각신청을 제출(선행소송 재탕·셔먼법 1조 개연성 부족 주장). 09-22 현재 법원 결정은 미발표.
- **인디애나·뉴욕 미국 팹 진척**: SK하이닉스 인디애나 HBM 패키징 공장 착공식(2026-08-27, $4B+, 클린룸 2028-10 목표). Micron 뉴욕 메가팹 첫 콘크리트 타설이 예정보다 1개 분기 이상 앞당겨짐(2026-07-09 발표) — 양사 모두 미국 캐파 확충이 순항 중.
- **전략 변경 없음**: 같은 날 별도 수행된 시나리오 포지션 맵 정기 재평가가 DF1/DF2 축을 이미 검토해 "신규 실현 외부 거시 사실 0건 → 유지"로 판정했으며, 본 정기점검의 병목 축 이동·CXMT G5 양산도 Main Bet(B)·Robust 전략의 전제를 뒤집는 수준은 아니라고 판단해 전략 변경은 없다. CXMT의 첨단 영역 추격 속도는 모니터링 우선순위를 격상했다([wiki/entities/cxmt.md](../../wiki/entities/cxmt.md) 참조).

> 1차 자료: [sources/articles/september-2026-market-update-2026-09-22.md](../../sources/articles/september-2026-market-update-2026-09-22.md)
...
| **B: AI 르네상스** ⭐ | AI 지속 | 공존 | 39% | 동서 양쪽 시장 공략, HBM 1위 탈환 시 최대 성장 |
| **C: 기술 냉전** | AI 붕괴 | 디커플링 | 8% | 이중 충격, 사상 최대 손실 가능성 |
| **D: 조용한 재편** | AI 붕괴 | 공존 | 21% | 2022~2023년형 다운사이클 재현, 체질 개선 기회 |
...
> **[2026-07-28 정기 재평가 — 유지]** 07-21 이후 git log 변경분은 전부 실행/조직 계층(개발실 전환 요약 PPT·수주산업화 8개 전략 옵션 질의)으로 거시 축 무영향 → **확률 A26·B39·C8·D21·E6 전부 유지**. in-window 신호는 전부 정점 재확인: **Alphabet Q2(07-22) 2026 CapEx 가이던스 $180~190B→$195~205B 상향**·Cloud 백로그 QoQ +$50B→$514B(상방 확인)·**SK하이닉스 Q2(~07-28) 영업이익 ~$43.7B·OPM ~76% 사상 최대**(정점 확인)·GPU 임대가 firming(H100 ~$3.46·H200 ~$4.11) ([hyperscaler-q2-2026-capex-2026-07-28.md](../../sources/articles/hyperscaler-q2-2026-capex-2026-07-28.md)). 이미 DF1 정점(8.5)이므로 Alphabet 상향은 재확인이지 신규 상방 레그 아님. **절제**: Microsoft·Meta(07-29)·Amazon(07-30) Q2 CapEx 가이던스가 창 직후 발표 예정 — DF1 방향 핵심 확인 이벤트 다음 주 대기. DF2 무변화(MATCH 위원회 단계·본회의 표결 movement 없음·CXMT HBM 미확정·애플 CXMT 테스트 단계). EWI 5종 갱신·트리거 발동 0건.
...
**병목 레이더 (2026-06-10 신설, 2026-09-22 최신 — 대시보드 Bottleneck Model 탭)**
...
| 병목 | 현재 지수 (2026-09-22, 이전 07-04 대비) | 핵심 감시 트리거 | 충격 시 영향 (연환산) |
|---|---|---|---|
| 전력 | **75 Red**(▲+3) — PJM 용량경매 사상최고가·개폐장치 매진 | 상위 허브 2+ reserve <8% · LMP P90 72h · 접속지연 >60일 | HBM -0.61EB (-21.1%) |
| 첨단 패키징 (CoWoS) | **62 Orange**(▼-5) — 공급-수요 갭 20%→10% 축소 | qualified output -15% WoW 2주 · 대형 사이트 outage | HBM -0.59EB (-20.5%) |
| 선단 파운드리 | **59 Yellow**(▲+9) — 병목 축이 전공정(N3)으로 이동 중 | N2/A16 램프 1분기+ 지연 · N3 가동률 추가 상승 | HBM -0.43EB (-14.9%) |
| CAPEX/ROI | **43 Yellow**(▲+3) — 완화 추세 반전(09-14 센티먼트) | capex 가이드 -15% · FCF/CapEx <0.8 · HY OAS 급등 | **HBM -0.91EB (-31.5%)** |

09-22 사이클의 핵심 변화: 파운드리(+9)가 패키징(-5)에 근접하며 서열이 좁혀짐(격차 17→3) — 2026~27년 최전선 병목이 패키징에서 선단 파운드리(N3 웨이퍼)로 이동 중이라는 것이 SemiAnalysis·TSMC CEO 발언 기준 이번 사이클의 핵심 구조 변화. 전력은 06-13 이후 5회 연속 재상승, 이번에 처음 **Red** 밴드 진입.
...
**상류 드라이버 트리 (2026-06-11 확장)**: 각 병목에 영향을 주는 상류 요소를 depth 1(중류)·depth 2(상류)로 분해해 선행시차만큼 먼저 인지한다 — 예: CAPEX ← 하이퍼스케일러 이익·FCF(d1, 2~4분기 선행) ← **AI 기업 매출·이익(OpenAI·Anthropic·xAI·Google)**(d2, 12~18개월 선행). 22개 드라이버의 가중 롤업으로 병목별 선행 압력(상류/중류)을 산출하고, 현재 지수와의 괴리로 조기경보(악화 예고·완화 예고·상류-중류 괴리)를 발동한다. 모델 구조 도식(상류→병목→min()→수급)이 탭 상단에 내장되어 있으며, 수요 변곡 EWI는 프레임 일관성을 위해 같은 탭의 별도 서브탭으로 분리 운영한다(모델 화면에는 비표출).
...
### 0. 별도 트랙 — 다운턴 시나리오 플래닝 (SP-2)

> 이 보고서는 **SP-1**(Focal Issue: "AI 메모리 시대에 어떤 전략적 위치를 차지할 것인가", 축 = AI 수요 × 미중 지정학)의 합성물이다. 2026-08-15부터 위키에는 **두 번째 시나리오 플래닝 트랙 SP-2**가 병행 운영된다.

| 항목 | SP-2 |
|---|---|
| Focal Issue | 2027~2030 사이 도착할 메모리 다운턴에 대해, 지금 무엇을 준비하고 도착 후 무엇을 실행할 것인가 |
| 축 | **DF-D1 발원지**(수요 수축 주도 ↔ 공급 확대 주도) × **DF-D2 전개 속도**(급락형 ↔ 침식형) |
| 축의 의미 | 원인 축이 *무엇을 조절할지*를, 속도 축이 *언제 결정할지*를 결정한다 — 급락형은 준비의 문제, 침식형은 인지의 문제 |
| 시나리오 (조건부 확률) | DT-D 저가 잠식 26% · DT-B 긴 하산 24% · DT-C 동시 방류 22% · DT-A 급제동 20% · DT-E 판 갈이 8% |
| 산출물 | 대비 DP-1~7 · 대응 DR-1~6 · 감별 지표 DX-1~8 |
| 단일 소스 | [wiki/downturn/](../../wiki/downturn/README.md) |

**왜 별도 트랙인가**: 본 보고서 §5의 매트릭스에서 다운턴은 시나리오 C(8%)·D(21%)의 *결과*로만 등장한다. 그러나 2028~29 신규 캐파 동시 도래는 AI 수요가 유지되어도 실현되는 확정 사실이므로 **공급발 조정은 시나리오 A·B 안에서도 일어난다**. 다운턴은 본 매트릭스의 특정 사분면에 갇혀 있지 않으며, 따라서 다운턴을 축으로 다시 한 번 잘라야 대비·대응 전략이 제대로 분기한다.

**본 보고서의 전략과의 관계**: SP-2의 대비 전략은 본 보고서 §6의 Robust 전략을 다운턴 렌즈로 재배치한 것이다. 무후회 3종은 [DP-2 옵션형 캐파](../../wiki/downturn/preparation.md)(→ RS-1·RS-5), [DP-4 감별 EWI 배선](../../wiki/downturn/preparation.md)(→ RS-9·D15·D16), [DP-5 차세대 별동대](../../wiki/downturn/preparation.md)(→ D6·D13·SD-1)이며, 이는 [CMO 통합 매트릭스 §4.2](../../wiki/storyline/cmo-matrix.md)의 cause-robust 3종과 정확히 일치한다. **신규 항목은 [DP-1 계약 만기 사다리화](../../wiki/downturn/preparation.md)** — 계약 커버리지는 급락을 막는 것이 아니라 미루며, 만기가 한 시점에 몰리면 오히려 급락 발사 장치가 된다는 발견이다. 코드가 충돌하면 본 보고서(SP-1)의 정의가 우선한다.
