...
| **4대 병목 제약지수 (2026-09-01, 이전 07-04 대비)** | 전력 **76**(▲+4, Red 최초 진입) · CAPEX **38**(▼-2) · 파운드리 **48**(▼-2) · 패키징 **65**(▼-2) | 전력이 ERCOT 신규 DC 접속 절차 전면 중단(08-03)으로 4개 점검 연속 재상승·최초 Red 진입. CAPEX·파운드리·패키징 3개는 완화 지속(삼성 HBM4 골든수율 80% 등) |
| HBM 점유율 (Counterpoint Q1 2026 확정 vs 4월 잠정) | SK 58%(확정)/50~55%(잠정) · 삼성 32%(확정)/35~40%(잠정) | 확정·잠정 수치 차이 — 집계 시점 정합 필요, 다음 lint에서 해소 |
...
#### 2.2.3 [정기점검] 2026-09-01 시장 업데이트 — ERCOT 접속 전면 중단·TSMC 선단 램프 가속·삼성 HBM4 골든수율 80%

병목 모델 정기 점검(2026-09-01, 이전 점검 2026-07-04 대비). 08-25·08-28 정기 재평가(assessment)가 SP-1/SP-2 시나리오 확률·EWI를 이미 갱신했으므로, 본 절은 4대 병목(상방 제약) 축만 다룬다.

- **병목 모델 4대 제약지수 갱신**: 전력 72→**76**(▲+4, ERCOT가 2026-08-03 Abbott 주지사 지시로 신규 데이터센터 접속 절차를 전면 일시중단·감사 시한 12-10·474GW 신청 중 90%가 DC — 4개 점검 연속 재상승, 4대 병목 중 최초 Red 진입), CAPEX/ROI 40→**38**(▼-2, 빅4 Q2 실적 시즌 전원 추가 상향·합산 ~$745~750B·삭감 0건), 파운드리 50→**48**(▼-2, TSMC N3 18만 장 목표를 2~3개월 조기 달성 전망·N2 연말 10만 장 경로 순항), 패키징 67→**65**(▼-2, CoWoS 수급갭 20%→10% 축소 최초 정량 확인·삼성 HBM4 마이크로범프 수율 80% 골든수율 조기 달성). 상세: [wiki/concepts/bottleneck-model-2030.md §종합 판독(2026-09-01)](../../wiki/concepts/bottleneck-model-2030.md).
- **삼성 HBM4 골든수율**: 2026-02 양산 개시 후 약 6개월 만에 수율이 60%대에서 80%("골든수율")로 상승 — 업계 예상보다 약 4개월 조기. 연말 목표 85%로 상향, 3Q26 HBM4 매출 전분기 대비 3배+ 전망. 하이브리드본딩(16-Hi) 축은 여전히 시험수율 ~10% 수준으로 3사 전원 HBM4E/5E로 이연 — 마이크로범프 정규 양산 라인만의 개선.
- **전력 축 레짐 전환**: 07-04까지의 "큐 적체"(점증적 병목)에서 "접속 절차 자체의 정지"(규제 레짐 전환)로 성격이 악화 — 텍사스는 미국 내 최대 데이터센터 신설 허브 중 하나. BNEF는 최대 49.8GW 지연·비용 최대 $15억 추가로 추정.

> 1차 자료: [sources/articles/september-2026-market-update-2026-09-01.md](../../sources/articles/september-2026-market-update-2026-09-01.md)
...
| 병목 | 현재 지수 (2026-09-01) | 핵심 감시 트리거 | 충격 시 영향 (연환산) |
|---|---|---|---|
| 전력 | **76 Red** — ERCOT 신규 DC 접속 절차 전면 중단(08-03) | 상위 허브 2+ reserve <8% · LMP P90 72h · 접속지연 >60일 | HBM -0.61EB (-21.1%) |
| 첨단 패키징 (CoWoS) | 65 Orange — 수급갭 20%→10% 축소·삼성 HBM4 골든수율 80% | qualified output -15% WoW 2주 · 대형 사이트 outage | HBM -0.59EB (-20.5%) |
| 선단 파운드리 | 48 Yellow — N3 조기 램프·N2 순항 | N2/A16 램프 1분기+ 지연 · High-NA 삽입 지연 | HBM -0.43EB (-14.9%) |
| CAPEX/ROI | 38 Green — 빅4 Q2 실적 전원 상향, 삭감 0건 | capex 가이드 -15% · FCF/CapEx <0.8 · HY OAS 급등 | **HBM -0.91EB (-31.5%)** |

운영 원칙: 전력은 실시간 API(EIA·PJM·ERCOT) 감시가 가능한 유일한 축, 나머지는 공시·IR 이벤트 기반(단일 공식 원문도 Red 승격 가능). 복합 충격(전력 Red + CAPEX Orange 동시 1개월)은 HBM -38~-47%로 비상대책위 발동 사유. 충격별 즉시/중기/사전 대응 매뉴얼은 대시보드 탭에 내장. 전력이 4대 병목 중 최초로 Red 밴드에 진입(4개 점검 연속 재상승)한 반면 CAPEX·파운드리·패키징은 완화 지속 — 상세: [bottleneck-model-2030.md §종합 판독(2026-09-01)](../../wiki/concepts/bottleneck-model-2030.md).

**상류 드라이버 트리 (2026-06-11 확장, 2026-09-01 최신 판독)**: 각 병목에 영향을 주는 상류 요소를 depth 1(중류)·depth 2(상류)로 분해해 선행시차만큼 먼저 인지한다 — 예: CAPEX ← 하이퍼스케일러 이익·FCF(d1, 2~4분기 선행) ← **AI 기업 매출·이익(OpenAI·Anthropic·xAI·Google)**(d2, 12~18개월 선행). 22개 드라이버의 가중 롤업으로 병목별 선행 압력(상류/중류)을 산출하고, 현재 지수와의 괴리로 조기경보(악화 예고·완화 예고·상류-중류 괴리)를 발동한다. 2026-09 판독: 전력은 중류(계통 접속)가 ERCOT 접속 전면 중단으로 상류(변압기 리드타임 5년)만큼 나빠진 드문 사례(중류-상류 동반 악화), 패키징은 적층 수율(상류)이 삼성 HBM4 골든수율 80%로 부분 완화됐으나 하이브리드본딩 축(16-Hi, ~10%)은 그대로라 완전한 완화 예고는 아직 미발동. 모델 구조 도식(상류→병목→min()→수급)이 탭 상단에 내장되어 있으며, 수요 변곡 EWI는 프레임 일관성을 위해 같은 탭의 별도 서브탭으로 분리 운영한다(모델 화면에는 비표출).
