...
**작성일**: 2026년 5월 6일 (Q1 2026 실적 + 벤치마크 통합 업데이트) | **최종 데이터 갱신**: 2026년 7월 28일 (병목 모델 지수 정기 갱신 + CXMT 상하이 IPO 반영)
...
| **4대 병목 제약지수 (2026-07-28, 이전 07-04 대비)** | 전력 **75**(▲+3) · CAPEX **44**(▲+4) · 파운드리 **54**(▲+4) · 패키징 **64**(▼-3) | PJM 6.8GW 예비력 부족 확정(강제감축 규정 발동 예정)·Oracle 신용등급 하향·삼성 파운드리 브로드컴 $200B 수주로 대만집중 분산·HBM4 하이브리드본딩 추가 지연 |
| **CXMT 상하이 IPO (신규, 2026-07-27)** | 시총 **$489B**(중국 A주 1위)·¥57.92B 조달·서방 마더보드 3사 DDR5 인증 vs 미 하원 안보조사 요구 | DF2 상반 신호 동시 발생 — 상업적 정상화(공존)와 정치적 역풍(디커플링) 병존, 조사 조치화 여부가 다음 리트머스 |
...
#### 2.2.3 [정기점검] 2026-07-28 시장 업데이트 — 병목 4대 지수 갱신·CXMT 상하이 IPO

SemiAnalysis·Counterpoint·TechInsights를 우선 참조하고 Utility Dive·Bloomberg·Forbes·DigiTimes·TechTimes 등으로 보강한 정기 점검(2026-07-28, 이전 점검 2026-07-04 대비).

- **병목 모델 4대 제약지수 갱신**: 전력 72→**75**(▲+3, PJM 용량경매 FERC 상한 도달·2년 연속 예비력 미달로 6.8GW 부족 확정·2027-06부터 50MW+ DC 강제감축 규정 FERC 제출 예정·ERCOT 큐 438GW·변압기 리드타임 2031년까지 확대), CAPEX/ROI 40→**44**(▲+4, TSMC·Micron 가이던스 재상향은 완화 요인이나 Oracle 신용등급 BBB→BBB- 하향·하이퍼스케일러 회사채 수요 급랭·부외부채 재평가($120B→최대 $1.65조)로 신용 리스크 신규 부상), 파운드리 50→**54**(▲+4, TSMC CAPEX $60~64B 대폭 상향+ASML High-NA 연기 재확인에 더해 삼성 파운드리 브로드컴 ~$200B 수주·Anthropic 2nm MOU로 대만 집중 리스크의 실질적 분산 시작), 패키징 67→**64**(▼-3, HBM4 하이브리드본딩이 열/냉각 이슈로 추가 지연되며 JEDEC이 HBM4E 세대까지 핀피치 완화 검토 — "패키징이 진짜 병목"이라는 업계 프레이밍 확산). 전력·CAPEX·파운드리 3개 지수가 동시 상향되고 패키징만 추가 완화 — 07-04 시점 트렌드가 일부 반전(파운드리는 삼성 실질 분산으로 완화 전환, CAPEX·전력은 신용·설비 리스크 실현으로 악화). 상세: [wiki/concepts/bottleneck-model-2030.md §종합 판독(2026-07-28)](../../wiki/concepts/bottleneck-model-2030.md).
- **CXMT 상하이 STAR Market 상장(신규, 2026-07-27)**: 공모 ¥57.92B(~$8.56B, 2026년 아시아 최대 IPO), 첫날 +466~472% 급등해 시가총액 **$489B**로 중국 A주 1위 등극. 조달 자금은 DDR5·HBM 캐파 확장에 투입, 2026년 말 목표 캐파가 기존 30만 WSPM에서 **35만 WSPM**으로 상향 전망(DigiTimes). 같은 주 서방 마더보드 3사(GIGABYTE·MSI·ASUS)가 CXMT DDR5를 전원 공식 인증한 반면, 상장 당일 미 하원 초당파 의원들이 CXMT 국가안보 조사·구매 전면 차단을 요구 — **DF2에 상반된 신호가 동시 발생**한 최초 사례. 조사가 실제 입법·블랙리스트로 이어지는지가 다음 리트머스(신규 EWI 트리거 `cxmt_security_probe_action`). 상세: [wiki/entities/cxmt.md §Update 2026-07-28](../../wiki/entities/cxmt.md).
- **DF1·DF2 재평가**: 07-21~28 창의 물질적 신규 소스(Alphabet CapEx 상향·SK하이닉스 사상 최대 실적)는 모두 DF1 정점 재확인 방향으로 DF1 8.5 유지. CXMT IPO의 상반 신호는 방향이 반대이고 안보 조사가 아직 조치화 전 단계라 DF2 0.5 유지 — 단 신규 모니터링 항목으로 추가. 시나리오 확률(A26·B39·C8·D21·E6)도 거시 축 무변화로 유지.

> 1차 자료: [sources/articles/bottleneck-model-update-2026-07-28.md](../../sources/articles/bottleneck-model-update-2026-07-28.md), [sources/articles/cxmt-shanghai-ipo-2026-07-27.md](../../sources/articles/cxmt-shanghai-ipo-2026-07-27.md)
...
| 병목 | 현재 지수 (2026-07-28) | 핵심 감시 트리거 | 충격 시 영향 (연환산) |
|---|---|---|---|
| 전력 | **75 Red** — PJM 6.8GW 부족 확정·강제감축 규정 예정 | 상위 허브 2+ reserve <8% · LMP P90 72h · 접속지연 >60일 | HBM -0.61EB (-21.1%) |
| 첨단 패키징 (CoWoS) | **64 Orange** — 완판 유지, 하이브리드본딩 추가 지연 | qualified output -15% WoW 2주 · 대형 사이트 outage | HBM -0.59EB (-20.5%) |
| 선단 파운드리 | 54 Yellow — 삼성 분산 수주로 완화, N2·High-NA 순항 | N2/A16 램프 1분기+ 지연 · High-NA 삽입 지연 | HBM -0.43EB (-14.9%) |
| CAPEX/ROI | 44 Yellow — 가이던스 강세, Oracle 신용등급 하향 신규 리스크 | capex 가이드 -15% · FCF/CapEx <0.8 · HY OAS 급등 | **HBM -0.91EB (-31.5%)** |
