...
**작성일**: 2026년 5월 6일 (Q1 2026 실적 + 벤치마크 통합 업데이트) | **최종 데이터 갱신**: 2026년 7월 7일 (삼성 Q2 잠정실적 89.4조 원·SK하이닉스 나스닥 확정·병목 모델 지수 업데이트)
...
- **Main Bet은 시나리오 B "AI 르네상스" (확률 38%, 2026-07-07 재평가)**. 5개 시나리오 중 시나리오 B만이 2035년 시장 $5,200억 규모로 가장 크고 삼성의 동서 균형 포지션이 최대 가치를 발휘하는 유일한 미래다. 그러나 자동 실현되지 않는다. HBM4E·HBM5 기술 1위 탈환, 동서 균형 공급망(텍사스+인도), 1c nm 원가 우위, 커스텀 AI 메모리 솔루션, 텍사스 2기 미국 현지 생산이 동시에 추진되어야 1번 자리를 회복할 수 있다.
...
### 핵심 수치 한눈에 (2026년 7월 7일 최신)

| 항목 | 수치 | 시사점 |
|------|------|---------|
| **Samsung Q2 2026 잠정 가이던스 (2026-07-07, 오늘)** | **연결 영업이익 약 89.4조 원(사상 최대, 19배 YoY)**, 매출 약 171조 원 | 호황의 정점 최강 확증. 발표 당일 주가 -7%(재료 소진) |
| Samsung Q1 2026 메모리 매출 | $50.4B (+292% YoY), DS 영업이익 약 49배(53.7조 원) | 분기 연속 사상 최대 경신의 출발점 |
| **Micron Q3 FY26 실적 (2026-06-24)** | **매출 $41.46B (+74% QoQ·+346% YoY), 매출총이익률 84.9%, EPS $25.11** — 가이던스 $33.5B 전면 상회 | SCA 16건 $100B·예치금 $22B. **FY2027 EPS 컨센서스 $98→$155 상향**(07-06) |
| 빅테크 5사 AI CapEx 2026 | **$745~755B+**(MS $190B·Alphabet $180~190B·Amazon $200B·Meta $125~145B·Oracle ~$50B) | Oracle FY27 capex ~$90~95B로 상향 확정. Dell'Oro $1조 돌파·JPMorgan $5.5조 |
| **BIS(국제결제은행) AI 버블 경고 (신규, 06-28)** | AI 투자 붐을 과거 버블 붕괴와 명시적 비교, 사모신용 $3B→$40B 급증 지적 | 통화당국 최상위 기관의 첫 공식 경고 — Oracle 주가 -40%대. 실적과 센티먼트 괴리 확대 |
| HBM4 NVIDIA Rubin 공급 (배정 세분화) | SK하이닉스 60~70%·삼성 25~30%·Micron 잔여 | Micron 초기 빌드 일시 배제 후 GTC 재확인 — 자격검증 굴곡 확인 |
| **삼성 HBM4E 업계 최초** | **2026-05-29 샘플 출하**(3.6TB/s, SK하이닉스보다 6개월 선행) | HBM4E 세대 점유율 역전 가능성 열림. HBM5 목업도 공개 |
| **4대 병목 제약지수 (2026-07-07, 이전 07-04 대비)** | 전력 **73**(▲+1) · CAPEX **41**(▲+1) · 파운드리 **50**(변동없음) · 패키징 **66**(▼-1) | 전력·CAPEX 동반 소폭 재상승(절차완화·실적호조에도 구조격차·버블경고가 상쇄). 패키징만 완화(SK하이닉스 신규 자본배분) |
| HBM 점유율 (Counterpoint Q1 2026 확정 vs 4월 잠정) | SK 58%(확정)/50~55%(잠정) · 삼성 32%(확정)/35~40%(잠정) | 확정·잠정 수치 차이 — 집계 시점 정합 필요, 다음 lint에서 해소 |
| **SK하이닉스 나스닥 상장 확정** | 조달목표 **$28.1~28.2B로 하향 확정**(45.45조→43.14조 원), 가격확정 7/9·거래개시 7/10, 코너스톤 최대 $7B | 자금용처 최초 공개(패키징팹 $12.4B·EUV $7.8B) — 경쟁사 자본조달 채널 격차 재확인 |
...
> 최신 데이터 출처: [sources/articles/july-2026-market-update-2026-07-07.md](../../sources/articles/july-2026-market-update-2026-07-07.md) (SemiAnalysis·Counterpoint·TechInsights 등)
...
#### 2.2.3 [정기점검] 2026-07-07 시장 업데이트 — 삼성 Q2 잠정실적 89.4조 원·SK하이닉스 나스닥 확정·BIS 버블 경고

3개 병렬 리서치 에이전트로 SemiAnalysis·Counterpoint·TechInsights 우선 수집(대부분 직접 fetch 403 → 검색 스니펫 경유) + FERC·ERCOT·BIS 등으로 보강한 정기 점검(2026-07-07, 이전 점검 2026-07-04 대비).

- **삼성전자 2026 Q2 잠정 가이던스 (오늘 발표)**: 연결 매출 약 171조 원(컨센서스 172.18조 원 소폭 하회), **연결 영업이익 약 89.4조 원 — 사상 최대, 전년 동기 대비 약 19배**. 발표 당일 주가는 "재료 소진"으로 약 -7% 하락. 확정 실적·컨퍼런스콜은 2026-07-30.
- **병목 모델 4대 제약지수 갱신**: 전력 72→**73**(▲+1, FERC EIT·ERCOT Batch Zero 절차 완화에도 SemiAnalysis가 미국 신규 DC 전력수요 21GW(2026)→84GW(2030) vs 그리드 연 15GW 증설로 구조적 격차 재확인), CAPEX/ROI 40→**41**(▲+1, BIS 연차보고서의 AI 버블 공식 경고·Oracle 주가 -40%대 신규 리스크 vs 삼성 실적·SK하이닉스 신규 팹 자금배분 등 펀더멘털은 견조), 파운드리 **50**(변동없음, TSMC 6월 매출 미발표 정보공백), 패키징 67→**66**(▼-1, SK하이닉스 나스닥 조달자금 중 $12.4B을 청주 P&T7 패키징팹에 배분 확정). 상세: [wiki/concepts/bottleneck-model-2030.md §종합 판독(2026-07-07)](../../wiki/concepts/bottleneck-model-2030.md).
- **SK하이닉스 나스닥 ADR 확정**: 조달목표 45.45조→43.14조 원(~$28.1~28.2B)으로 하향 확정(KOSPI 하락 반영). 코너스톤 투자자(Baillie Gifford·Coatue·Situational Awareness Partners) 최대 $7B. 가격확정 7/9·거래개시 7/10. 조달자금 용처 최초 공개(용인 Fab1 $20.2B·청주 패키징팹 $12.4B·ASML EUV $7.8B).
- **BIS(국제결제은행) 버블 경고**: 통화당국 최상위 기관이 연차보고서(06-28)에서 AI 투자 붐을 과거 버블 붕괴와 명시적으로 비교 — 기존 애널리스트 수준 거품론보다 질적으로 격상된 신규 리스크 신호. 같은 주 삼성 실적은 정반대로 사상 최고 — 센티먼트-펀더멘털 괴리 확대.
- **Micron FY2027 EPS 컨센서스**: $98→$155/주 상향(16건 다년 HBM 계약 근거, 기존 SCA $100B/16건과 계약군 동일성은 교차검증 필요). 삼성·SK하이닉스 합산 $2조 캐파 증설 공약이 Micron 투자자에게 "공급과잉 경고"로 해석되는 보도 병존.

> 1차 자료: [sources/articles/july-2026-market-update-2026-07-07.md](../../sources/articles/july-2026-market-update-2026-07-07.md)
...
|---------|-----|-----|------|-------------|
| **A: 황금 요새** | AI 지속 | 디커플링 | 27% | 시안 팹 상실, HBM 서방 듀오폴리로 고마진-고리스크 |
| **B: AI 르네상스** ⭐ | AI 지속 | 공존 | 38% | 동서 양쪽 시장 공략, HBM 1위 탈환 시 최대 성장 |
...
> **[2026-07-07 재평가]** LTA→SCA 계약 체제 확립(Micron–Anthropic SCA 06-22·SCA 16건 $100B 공시 제도화·Stargate LOI 글로벌 DRAM 40%)으로 다년 계약 락인 → 근단기 순환 붕괴 확률 추가 축소. 감속(범용 DRAM Q3 +13~18%)은 비AI-락인 축 집중·서버/HBM 견조. **B 37→38·C 9→8**, A·D·E 불변. 합계 100% (A27·B38·C8·D21·E6). DF1 8.5 정점·DF2 1.0 유지. 단일 소스: `wiki/scenarios/scenario-matrix.md` + 대시보드 Planning 탭.
...
| 병목 | 현재 지수 (2026-07-07) | 핵심 감시 트리거 | 충격 시 영향 (연환산) |
|---|---|---|---|
| 전력 | **73 Orange** — 절차완화(EIT·Batch Zero)에도 21→84GW 구조격차 재확인 | 상위 허브 2+ reserve <8% · LMP P90 72h · 접속지연 >60일 | HBM -0.61EB (-21.1%) |
| 첨단 패키징 (CoWoS) | **66 Orange** — SK하이닉스 신규 패키징팹 자금배분 확정, 완화 지속 | qualified output -15% WoW 2주 · 대형 사이트 outage | HBM -0.59EB (-20.5%) |
| 선단 파운드리 | 50 Yellow — TSMC 6월 매출 미발표, 정보공백 | N2/A16 램프 1분기+ 지연 · High-NA 삽입 지연 | HBM -0.43EB (-14.9%) |
| CAPEX/ROI | 41 Yellow — BIS 버블 경고 신규 vs 실적·투자 견조 | capex 가이드 -15% · FCF/CapEx <0.8 · HY OAS 급등 | **HBM -0.91EB (-31.5%)** |
...
**상류 드라이버 트리 (2026-06-11 확장, 2026-07-07 최신 판독)**: 각 병목에 영향을 주는 상류 요소를 depth 1(중류)·depth 2(상류)로 분해해 선행시차만큼 먼저 인지한다 — 예: CAPEX ← 하이퍼스케일러 이익·FCF(d1, 2~4분기 선행) ← **AI 기업 매출·이익(OpenAI·Anthropic·xAI·Google)**(d2, 12~18개월 선행). 22개 드라이버의 가중 롤업으로 병목별 선행 압력(상류/중류)을 산출하고, 현재 지수와의 괴리로 조기경보(악화 예고·완화 예고·상류-중류 괴리)를 발동한다. 2026-07-07 판독: CAPEX는 BIS 버블 경고로 자금조달(financing) 드라이버가 '긴장' 유지되나 가이던스·실적(capex_guide) 드라이버는 '완화'로 대조, 전력은 계통접속(interconnect) 드라이버가 '임계' 유지(SemiAnalysis 21→84GW로 재확인). 모델 구조 도식(상류→병목→min()→수급)이 탭 상단에 내장되어 있으며, 수요 변곡 EWI는 프레임 일관성을 위해 같은 탭의 별도 서브탭으로 분리 운영한다(모델 화면에는 비표출).
