...
**작성일**: 2026년 5월 6일 (Q1 2026 실적 + 벤치마크 통합 업데이트) | **최종 데이터 갱신**: 2026년 7월 14일 (SK하이닉스 나스닥 급락·삼성 Q2 잠정실적·병목 모델 지수 재갱신)
...
| **4대 병목 제약지수 (2026-07-14, 이전 07-04 대비)** | 전력 **73**(▲+1) · CAPEX **39**(▼-1) · 파운드리 **49**(▼-1) · 패키징 **65**(▼-2) | 전력 3개 분기 연속 재상승. 패키징이 가장 뚜렷한 완화(TSMC CoWoS 수급갭 20%→10% 축소). CAPEX는 빅4 견조하나 SK하이닉스 신용도 회의로 완화 속도 둔화 |
| HBM 점유율 (Counterpoint Q1 2026 확정 vs 4월 잠정) | SK 58%(확정)/50~55%(잠정) · 삼성 32%(확정)/35~40%(잠정) | 확정·잠정 수치 차이 — 집계 시점 정합 필요, 다음 lint에서 해소 |
| **SK하이닉스 나스닥 상장 후 사상 최대폭 급락 (신규)** | 07-10 ADR 상장($26.5B 조달) 후 07-13 서울 -15.37%(사상 최대)·코스피 -8.95%·서킷브레이커 | ADR-서울 차익거래+애널리스트 이익추정 하향(2026·27 -9%·-11%)+지정학 복합. AI 수요 냉각 아님 — DF1 불변, HBM 수익성 전환 신뢰는 모니터링 |
| **삼성 Q2 2026 잠정 영업이익 (신규)** | 약 89.4조 원(YoY 약 19배), 매출 171조 원 — 정식 실적 07-30 | ROI 실현 재확인. 세부 부문별 실적(HBM P&L 공개 여부)은 07-30 확인 |
| **DRAM 반독점 집단소송** | 2026-06-25 N.D.Cal. 제소, 원고 17명·700% 가격상승 주장. 기각신청 수개월 내 예상 | 병목 실물 구조는 불변, 삼성 규제·평판·재무 리스크로 별도 모니터링 |
...
#### 2.2.3 [정기점검] 2026-07-14 시장 업데이트 — SK하이닉스 나스닥 급락·삼성 Q2 잠정실적·CXMT 인증 진전

SemiAnalysis·Counterpoint·TechInsights 우선 수집 + CNBC·Bloomberg·Reuters·FT·TrendForce로 보강한 정기 점검(2026-07-14, 이전 점검 2026-07-04 대비).

- **SK하이닉스 나스닥 상장 후 사상 최대폭 급락**: 2026-07-10 나스닥 ADR 상장($26.5B 조달, 2014 알리바바 이후 외국기업 최대) 사흘 만인 2026-07-13, 서울 주가가 **-15.37%**(창사 이래 최대 낙폭) 급락하고 코스피 지수가 **-8.95%**(669.01p) 폭락해 20분간 서킷브레이커가 발동됐다(코스피 시총 약 546조 원 하루 증발). 원인은 ① ADR-서울 ~37% 프리미엄을 노린 국경 간 차익거래, ② 한국투자증권의 SK하이닉스 2026·2027 영업이익 추정치 하향(각각 -9%·-11%), ③ 미국-이란 지정학 충격(호르무즈 봉쇄 우려·유가 +9%)의 복합이다. 복수 매체는 "AI 수요의 갑작스런 냉각이 아니다"라고 평가하며, 빅4 CY2026 CAPEX 가이던스($725B)도 흔들리지 않았다 — **DF1(AI 수요) 8.5 정점 유지**로 판정하되, HBM 계약의 실제 수익성 전환에 대한 자본시장의 첫 회의 신호로 별도 모니터링([wiki/entities/sk-hynix.md §Update 2026-07-14](../../wiki/entities/sk-hynix.md), [wiki/driving-forces/key-drivers.md](../../wiki/driving-forces/key-drivers.md)).
- **병목 모델 4대 제약지수 갱신**: 전력 72→**73**(▲+1, Gartner 2026 DC 전력수요 1,000TWh 돌파·IEA 계획 프로젝트 20% 지연위험), CAPEX/ROI 40→**39**(▼-1, 빅4 $725B 재확인+삼성 Q2 19배가 SK하이닉스發 신뢰 균열을 일부 상쇄), 파운드리 50→**49**(▼-1, CoWoS 갭 축소 정합, TSMC Q2 실적은 07-16 발표 예정으로 미반영), 패키징 67→**65**(▼-2, TrendForce CoWoS 수급갭 20%→10% 축소 + 2027 캐파 20만 WPM 이상 신규 목표). 상세: [wiki/concepts/bottleneck-model-2030.md §종합 판독(2026-07-14)](../../wiki/concepts/bottleneck-model-2030.md).
- **삼성전자 Q2 2026 잠정 실적**: 연결 영업이익 약 89.4조 원(전년 동기 대비 약 19배), 매출 171조 원. 메모리 ASP QoQ +40~60% 추정. 정식 사업부문별 실적은 2026-07-30 발표([wiki/entities/samsung.md §Update 2026-07-14](../../wiki/entities/samsung.md)).
- **Counterpoint Q1 2026 DRAM 확정 데이터**: 글로벌 DRAM 매출 사상 최고 $97B(QoQ +80%·YoY +260%), 매출 점유율 삼성 38%·SK하이닉스 29%·Micron 22% — 위키 기존 TrendForce 계열(Q3 2025)과 집계 기준 차이로 병기.
- **CXMT 1260H — "제외 후 재등재" 확인 + 애플 대응 소폭 진전**: CXMT·YMTC가 1260H 리스트에서 한때 제외됐다가 2026년 6월 최신판에서 재등재된 사실이 확인됐다. 애플의 CXMT 대응은 "기술 검증"에서 정식 공급사 승인에 선행하는 "기술 인증" 쪽으로 소폭 진전했으나, 미 행정부 승인이라는 결정적 리트머스는 여전히 미해결 — DF2 포지션(1.0→0.5, 2026-07-14 기존 재평가) 변경 없음.
- **DRAM 반독점 집단소송 세부**: 원고 17명(개인+소기업), 4년간 최대 700% 가격 상승을 근거로 제시. Micron은 혐의 부인, 기각신청(motion to dismiss)이 향후 수개월 내 제출될 것으로 예상.
...
**병목 레이더 (2026-06-10 신설, 2026-07-14 갱신 — 대시보드 Bottleneck Model 탭)**
...
| 병목 | 현재 지수 (2026-07-14) | 핵심 감시 트리거 | 충격 시 영향 (연환산) |
|---|---|---|---|
| 전력 | **73 Orange** — 그리드 접속 큐·변압기 리드타임 5년, 3개 분기 연속 재상승 | 상위 허브 2+ reserve <8% · LMP P90 72h · 접속지연 >60일 | HBM -0.61EB (-21.1%) |
| 첨단 패키징 (CoWoS) | **65 Orange** — CoWoS 수급갭 20%→10% 축소, 완화 진행 중 | qualified output -15% WoW 2주 · 대형 사이트 outage | HBM -0.59EB (-20.5%) |
| 선단 파운드리 | **49 Yellow** — N2·High-NA 순항, CoWoS 갭 축소와 정합 | N2/A16 램프 1분기+ 지연 · High-NA 삽입 지연 | HBM -0.43EB (-14.9%) |
| CAPEX/ROI | **39 Yellow** — 빅4 가이던스 견조, SK하이닉스發 신뢰 균열 모니터링 | capex 가이드 -15% · FCF/CapEx <0.8 · HY OAS 급등 | **HBM -0.91EB (-31.5%)** |
