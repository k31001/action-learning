...
## 슬라이드 구성 (v3.3 2026-09-17 — 시각 중심 개편: 글 최소, 그림·도형으로 논증)
...
| # | 킥커 | 액션 타이틀 | 레이아웃 |
|---|---|---|---|
| 1 | QLC eSSD 전략 · 배경: 고객은 왜 QLC를 원하는가 | 고객이 사는 것은 계산 옆의 가장 싼 바이트이고, 그 자리가 추론 캐시 티어로 옮겨갑니다 | **3기 스트립(v3.3)**(초기·현재·향후: "고객이 산 것"을 큰 키워드로 — TB당 원가·랙당 용량 / TB당 와트·공급 확보 / GPU당 캐시 용량·토큰당 비용 — 그 아래 **왜**(데이터 6배·1U 1PB·TCO -47% / 추론 서버 전력·공간 병목 / KV 캐시가 HBM을 넘침·GPU당 사용자 10배)·**조건**(읽기 중심 0.3~0.6 DWPD / 쓰기 티어는 TLC / 쓰기 많은 캐시 → 오늘은 TLC 1~3 DWPD)·삼성 한 줄씩) → **이정표 타임라인**(2022-12 배치 표준 비준 · 2023-07 61TB · 2024 30EB · 2026-05 245TB · 2027 KV 75~100EB) → 통합 그래프(EB·%·$B) + 키넘버 스티커 3(10배·정체·350EB) → 결론 "고객은 추론 캐시 티어에서도 QLC의 경제성을 원합니다. 문은 내구성이고, 디바이스만으로는 못 엽니다"(니어라인 HDD 언급 제거) |
| 2 | 요구사항과 역량 · 기술 전략 | 내구성은 고객 시스템에서 풀리므로, 디바이스에서 고객 시스템 안까지 세 단계로 올라가야 합니다 (리드: 고객이 원하는 QLC 경제성과 캐시의 쓰기 사이 10~40배 갭은 배치 표준과 호스트 SW가 메운다; Phase 산출물 줄은 "고객이 얻는 것": 정격 내 QLC 원가·전력 / 자기 워크로드의 수명 보증 / 시스템 위의 TCO 보증) | 갭 타일 4(숫자 + 한 줄) → **Phase 1·2·3 스택 그림(v3 확대)**: 고객 시스템 5계층을 세 번 그리고 **층마다 구체 기술**을 적는다(응용 vLLM·SGLang·TensorRT-LLM / 관리자 Dynamo KVBM·LMCache·Mooncake·FlexKV / I/O NIXL·GPUDirect Storage·io_uring·SPDK·xNVMe / 커널·플랫폼 Linux write streams·XFS·f2fs·CMX(DOCA Memos) / SSD RUH 200+·2Tb QLC·NVMe KV·텔레메트리). Phase별로 우리가 닿는 층을 색으로(파랑 = 우리 코드·제품, 연파랑 = 관측·이해, 흰색 = 고객 영역)하고 그 층에서 우리가 하는 일을 기술로 적는다. Phase 2 트레이스↓·정책↑ 화살표, Phase 3 FDE 상주 아이콘·메인라인 머지·공용 TCO 모델. 각 Phase 아래 산출물 한 줄. **진행 바 + 삼성 현 위치**. 협업 기업 행은 제거(v3) |
| 3 | 실행 전략 · 고객 협업 | FDE로 고객 시스템 안에 들어가고, 전략적 협약으로 워크로드를 받습니다. 선례는 이미 있습니다 | **두 트랙 그림(v3)**: 좌 「삼성 개발실」(KV-ready QLC·FDE·업스트림 코드·공급/자본) ↔ 우 「고객 시스템」 스택(캐시 관리자에 FDE 아이콘, I/O·커널 머지, SSD 삼성 QLC, 하단 로고). 가운데 위 화살표 "① FDE가 고객 시스템 안으로 들어간다", 아래 화살표 "② 전략적 협약(SCA)으로 워크로드가 온다 · 창은 2027 상반기". 두 화살표 사이 **선례 카드 2**: Palantir FDE(엔지니어 3명 → 고객 현장 그림, 코드명 Delta·성과 평가, 파급: 락인·640%·Anthropic/OpenAI GTM 채택) · Micron↔Anthropic SCA 2026-06(공동 설계·다년 공급·운영 통합·자본 4블록, 16건·$100B·$22B, 삼성·SK 계약엔 공동 설계 조항 없음 → 우리가 제안). → **조직·인사·문화 3축 타일(v3.2, 축당 3줄)** — 조직(실리콘밸리 소프트웨어 자회사 · FDE Pod 고객 상주 · 시스템 소프트웨어 조직 강화) / 인사(**고객 시스템을 아는 시스템 SW 전문가 채용·양성** · 미주 현지 채용 확대·고객과 같은 시간대 · 별도 보상·상주 로테이션) / 문화(**오픈소스 생태계를 주도하는 문화** · 메인테이너·커미터 배출·레퍼런스 프로젝트 운영 · 업스트림 우선·KV 실측 최초 공개) → **결론 밴드 = 3장 요약 한 문장**(v3.3: "고객은 계산 옆의 가장 싼 바이트를 사려고 QLC를 택했고, 그 자리가 쓰기 많은 추론 캐시로 옮겨가 내구성을 고객 시스템 안에서 풀어야 하며, 그 길은 FDE와 전략적 협약, 그리고 이를 해낼 사람과 문화가 엽니다"). 전략·재무 타일, 90일·1년·3년 핀, 결정 요청은 v3.2에서 제거(내용은 위키 §2.1·§2.5·§3) |
