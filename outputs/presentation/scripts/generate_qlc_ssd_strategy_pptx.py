"""QLC eSSD 전략 — 6장 덱 생성 v5.1 (2026-09-19 순서 재편: 문제 → 신뢰성 → 해법 사다리 → 역량 → 실행 → 보증·SLA).

스토리라인(v5.1): ① 문제(왜 지금, 무엇이 필요한가: 다운턴 교훈 + AI 수요로 대용량·높은 DWPD로 이동하는 요구 → 고용량화의 두 축) →
  ② 신뢰성(첫째 축 다이 8배·고장률: SSD 내부 해법으로 닫힌다) → ③ 해법 사다리(둘째 축 DWPD: SSD 안에서 닫히지 않았고 남은 지렛대 WAF는 호스트에 →
...
S3 해법 사다리(solution_ladder_slide 공용, 이관 매트릭스): 열 = 요구 → 단품 → SSD 계층 → 호스트·시스템 계층, 행 = 내구성 축 + 지렛대 스트립
...
notes(s, "2장은 신뢰성 축, 고용량화가 낳는 첫째 축입니다. 3장의 내구성 축과 구조는 같지만 해결 주체가 다릅니다. 요구는 JESD218 기능 고장률 FFR 3% 이하로 고정인데, SSD당 NAND 다이 수는 S3700 800GB 128개(2012)에서 Kioxia LC9 245TB 1,024개(2025, 2Tb 다이 32단 스택 32패키지)로 8배 늘었습니다. 보호가 없으면 SSD 고장률은 다이 수 N에 비례하므로, 같은 FFR을 지키기 위한 다이 고장률 상한은 1−0.97^(1/N)로 128다이에서 2.4×10⁻⁴, 1,024다이에서 3.0×10⁻⁵입니다. 8배 엄격해진 것입니다. 칩 면적이 커질수록 수율이 떨어지는 포아송 모델과 같은 구조이며, 다이 고장률 개선은 기술적 한계에 접근하고 있습니다. "
      ...
      "호스트·플랫폼은 관측과 수용만 합니다. SSD가 노출한 지표를 관측하고(OCP SMART C0), 감량 운영을 플랫폼이 수용하며(Microsoft Hyrax), 드라이브 간 소거 부호가 SSD 단위 고장을 흡수합니다. 다이 고장의 해결 주체는 SSD이고 호스트 협력은 규격에 한정됩니다. 삼성 과제는 1,024다이급에서 패리티·예비 비율을 FFR과 용량 오버헤드 사이에서 재최적화하고, FIP 감량 정책과 텔레메트리 필드를 OCP에 규격으로 제안하며, 다이 고장 예측 모델을 6장의 수명 보증과 연결하는 것입니다. 결론: 이 축은 SSD 안에서 닫히고 호스트에는 지표만 제공합니다. 그러나 둘째 축 DWPD는 SSD 안에서 닫히지 않았고, 어느 계층이 닫을 수 있는지가 3장입니다. 근거는 소스 §9 F41~F47, 고장률 상한은 독립 고장 모델입니다.")
...
_ladder.build(_sys.modules[__name__], page_no=3, kicker=3, next_step=4,
              lead="신뢰성과 달리 DWPD 격차는 SSD 안에서 닫히지 않았습니다. SSD 단독 최적화는 QoS·성능은 개선했으나 WAF는 낮추지 못했습니다.",
              band_main="요구는 고정, 단품은 악화, 격차는 상위 계층이 보상해 왔고, 남은 지렛대 WAF는 SSD 밖 호스트에 있습니다.\n"
                        "다이 세대·OP·보증연수와 병행하되, 이 지렛대는 고객 협업과 새로운 역량을 요구합니다",
              notes_tail="신뢰성 축(다이 수·고장률)은 2장에서 보였듯 SSD 안에서 닫히지만, DWPD 축의 남은 지렛대는 SSD 밖 호스트가 쥐고 있습니다. 그래서 고객 협업과 새로운 역량이 필요하고, 그 역량이 4장입니다.")
