...
const F = '맑은 고딕';
const RAD = 0.08;
const TOTAL = 20;   // 표지 + 본문 12 + 2차 저지선 6 + 별첨 1
...
// ════════════════════════════════════════════════════════════════════════
{
  ...
  tx(s, '2차 저지선 상세 — S14 ~ S19', { x: 6.17, y: 5.58, w: 6.5, h: 0.62, fontSize: 20, bold: true, color: C.NAVY, align: 'center', valign: 'middle' });
...
// ════════════════════════════════════════════════════════════════════════
// S14 — 2차 저지선 섹션 도입 (골격만 · 내용은 작성자가 채운다)
// ════════════════════════════════════════════════════════════════════════
{
  const s = blankSlide(14, '제2 저지선 — 무엇을 지키는가');
  s.addNotes('내용 미작성 — 2차 저지선 섹션 도입. 원문 §4의 2차 방어 골격(제품: 램프업 속도와 수율 / 투자: "투자 셰어 < 빗 셰어 < 프로핏 셰어"의 순서를 지키는 규율 / 오퍼레이션) 세 축을 놓는 자리.');
}
...
// ════════════════════════════════════════════════════════════════════════
{
  ...
  // ── 우: 세 개의 축 + 경계 ─────────────────────────────────────────
  const axes = [
    { k: '축 ① 세대 연장성 · 시간축', b: '공통 기술 축(W2W 본딩·HAR 식각·ALD) 위에 로드맵 정렬 — 4F² COP DRAM으로 로직·디램·플래시 전 갈래 수렴' },
    { k: '축 ② 제품 간 동일성 · 제품축', b: '제품은 다극으로, 공정은 단극에 가깝게 — 공통성을 설계 규범으로 고정' },
    { k: '축 ③ N-1 설계 · 설비 세대축', b: '볼륨·로우엔드는 1~2세대 이전 노드로도 생산 가능하게' },
  ];
  axes.forEach((a, i) => {
    const y = 2.12 + i * 1.26;
  ...
  tx(s, '경계 — 선단은 그대로 간다: N-1은 바벨의 아래쪽 끝만', { x: 5.19, y: 5.86, w: 7.26, h: 0.46, fontSize: 20, bold: true, color: C.NAVY, valign: 'middle' });
  ...
  s.addNotes('명제 — 종합반도체는 제품 포트폴리오만이 아니라 양산 체제가 종합반도체여야 한다. 문제는 판가가 아니라 감가다: 2023년 DS 감가 29.4조가 2028E 44.5조·2029E 45.9조로 오면서 손익 붕괴선이 매출 66.6조에서 104조로 1.5배 올라간다. 같은 폭의 가격 붕괴가 오면 적자는 -14.88조에서 약 -31조로 깊어진다(반사실 추정, 밴드 -28~-35조). 지금 CAPEX를 급제동해도 최소 +13조는 이미 확정 — 파고는 이미 출발했다. 그러나 같은 곡선의 뒷면에서 2028년 말까지 취득원가 196조(메모리 귀속 108~137조)의 설비가 감가를 마친다. 위기와 기회를 가르는 것은 하나 — 그 설비로 무엇을 만들 수 있는가. 세 축은 서로 직교한다(시간축·제품축·설비 세대축). 경계: 선단 리더십은 불변이며 1c nm·HBM4E·EUV 목표는 조정하지 않는다. 값: 전환 본체를 6→3개월로 줄이면 전환 1회당 약 1조·다운턴 1회당 2~3조, N-1 라인의 비트당 원가 우위는 다운턴 25~35%(호황 11~21%). 네 실행 영역(설계 규범·설비 구매 사양·교차 퀄 상시화·투자 거버넌스) 어디에도 증액 요구가 없다 — 예산이 아니라 규범과 문서 양식의 변경이다. 첫 실행 항목은 목표치가 아니라 측정: 지금 우리 팹의 전환 가능 캐파 비율을 아무도 답할 수 없다.');
}
...
// ════════════════════════════════════════════════════════════════════════
{
  const s = contentSlide(17, '따라잡히는 축 위에 서 있다',
  ...
  rect(s, 0.67, 3.38, 5.66, 1.10, C.NAVY);
  tx(s, '가운데 — 삼성 DRAM 점유', { x: 0.89, y: 3.50, w: 5.22, h: 0.30, fontSize: 20, bold: true, color: C.INV_SUB });
  tx(s, '41% → 32.6%', { x: 0.89, y: 3.84, w: 3.4, h: 0.46, fontSize: 28, bold: true, color: C.INV });
  tx(s, '’24 → ’25 3Q', { x: 4.30, y: 3.92, w: 1.81, h: 0.30, fontSize: 20, color: C.INV_MUTED, align: 'right' });
  arrow(s, 3.50, 4.52, 3.50, 4.76, { color: C.BLUE70, width: 2 });
...
// ════════════════════════════════════════════════════════════════════════
{
  ...
  tx(s, '본딩만으론 한 줄', { x: 0.89, y: 5.40, w: 3.66, h: 0.30, fontSize: 20, color: C.MUTED });

  // ── 우: 세 개의 제안 ──────────────────────────────────────────────
  [
    { k: '제안 ① 웨이퍼 본딩은 필연이다', b: '원가가 아니라 시간을 산다 — 셀(4F² 다이 +20%) · 라인(열예산) · 개발(두 흐름 독립)' },
    { k: '제안 ② zHBM은 주문 생산으로 온다', b: '제품은 다품종으로, 설계는 단일 플랫폼으로 — 코어 공통 · 인터레이어 IP화' },
...
// ════════════════════════════════════════════════════════════════════════
{
  const s = blankSlide(19, '전략 3번 · (제목 미정)');
  s.addNotes('내용 미작성 — 2차 저지선의 후속 전략 자리.');
}

// ════════════════════════════════════════════════════════════════════════
// S20 — 별첨 · DS 전사로 넓히면 (파운드리 접점 포함)
// ════════════════════════════════════════════════════════════════════════
{
  const s = contentSlide(20, '별첨 · DS 전사로 넓히면',
