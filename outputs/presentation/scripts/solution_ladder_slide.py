...
def build(ns, page_no=1, kicker="메모리 해법 사다리 · 이관 매트릭스", next_step=None):
    ...
    heads = [("요구", "고객이 정한다 · 고정", "req"),
             ...
             ("호스트 · 시스템 계층의 보상", "3단계 공동 설계 (2022~, 본 덱 2~5장)", "host")]
    ...
    tb(s, x + PAD, R1_Y + 0.60, COL_W - 2 * PAD, 0.22, [("배치 표준 · 캐시 관리자 정책 · FDE·SCA 공동 설계 = 본 덱 2~5장", 8.5, False, GRAY)])
    ...
    notes(s, "1장은 이관 매트릭스입니다. 열은 이관 단계이고 행은 두 축입니다. 요구는 고객이 정하고 고정입니다. 단품 지표는 셀과 다이의 지표이며 세대마다 악화됩니다. 보상은 먼저 SSD 계층에서, 다음은 호스트·시스템 계층에서 이뤄집니다. 두 행 사이의 산식 DWPD = P/E × (1+OP) ÷ WAF ÷ 365 × 년은 각 항의 결정 주체가 열과 대응함을 보입니다. P/E는 셀, OP는 SSD, WAF는 호스트·앱, 년과 요구 DWPD는 고객입니다. "
          ...
          "결론: 두 축 모두 요구는 고정이고 단품은 악화되며 보상은 상위 계층으로 이관됩니다. QLC로 요구 DWPD를 충족하는 경로는 호스트 시스템 공동 설계뿐이며 이는 산식의 귀결입니다. 그 규격이 언제 정의되는지가 2장입니다. 근거 등급은 보고서 부록 A F28~F47에 있습니다.")
