...
v1.4/v3.12(2026-09-18): 병렬 축 스트립(SSD당 다이 수 128→1,024 · FFR ≤3% 고정 · 다이 RAID·FIP · 상위 계층) 추가, 차트 높이 6.4→5.5in. 근거 소스 §9 F41~F47.
...
def build(ns, page_no=1, kicker="메모리 해법 사다리 · P/E · BER · DWPD · WAF", next_step=None):
    ...
    rows = [("1단계  NAND → SSD  (1991~)", "UBER 요구 충족", True,
            ...
            ("2단계  SSD 단독 워크로드 최적화  (2014~2019)", "DWPD 요구 미충족 · 부분 성공", False,
             [("셀 · SSD", "P/E 100배 ↓", "정격 DWPD 0.4", ""), ("SSD · 드라이버", "QoS · 성능 개선", "추정 배치 · 스트림 · IOD → WAF ≈ 3", "part"),
    ...
    for title, status, ok, chips in rows:
        ...
        for i, (who, what, sub, st) in enumerate(chips):
            ...
            tb(s, cx + 0.1, cy + 0.50, cw3 - 0.2, 0.22, [(sub, 8.75, False, c_sub)], spacing=1.0)
            if i < 2:
    ...
    # ---- 하단: 병렬 축 — SSD당 다이 수와 다이 고장률 (v1.4, 소스 §9 F41~F47) ----
    Y_S, H_S = C_Y + C_H + 0.16, 9.42 - 0.12 - (C_Y + C_H + 0.16)
    rect(s, MX, Y_S, CW, H_S, fill=WHITE, line=LINE, line_w=0.75)
    rect(s, MX, Y_S, 0.06, H_S, fill=BLUE_T1)
    tb(s, MX + 0.24, Y_S + 0.10, 2.0, 0.24, [("병렬 축 · 다이 고장률", 11.25, True, BLUE)])
    tb(s, MX + 0.24, Y_S + 0.36, 2.0, 0.40, [("요구 FFR ≤ 3% 고정", 9.75, True, INK), ("JESD218 · 다이 수는 8배 ↑", 9.0, False, GRAY)], spacing=1.0)
    # 미니 막대: SSD당 NAND 다이 수(환산)
    BX, BW = MX + 2.35, 3.15
    tb(s, BX, Y_S + 0.08, BW, 0.2, [("SSD당 NAND 다이 수 (용량 ÷ 다이 밀도 환산)", 8.75, False, GRAY_2)])
    bars = [("'12", "128"), ("'18", "144"), ("'21", "256"), ("'23", "512"), ("'25", "1,024")]
    bmax, bh_max = 1024, H_S - 0.66
    bw_ = 0.34
    gap_ = (BW - 5 * bw_) / 4
    base = Y_S + 0.30 + bh_max
    for i, (yr, val) in enumerate(bars):
        v = float(val.replace(",", ""))
        h_ = max(0.05, bh_max * v / bmax)
        x_ = BX + i * (bw_ + gap_)
        rect(s, x_, base - h_, bw_, h_, fill=BLUE if i == 4 else BLUE_T2)
        tb(s, x_ - 0.2, base + 0.02, bw_ + 0.4, 0.18, [(yr + "  " + val, 8.25, i == 4, INK if i == 4 else GRAY)], align=PP_ALIGN.CENTER)
    # 칩 3 + 화살표: 다이 → SSD 내부(현재 해법) → 상위 계층(한계 시, 점선)
    CX_ = BX + BW + 0.30
    aw_, ag_ = 0.24, 0.08
    cwid = (RIGHT - 0.22 - CX_ - 2 * (aw_ + 2 * ag_)) / 3
    chips = [("다이 (단품)", "다이 수 8배 ↑ → 다이 고장률 8배 ↓ 필요", "SSD 고장률 ≈ N × 다이 고장률 · 칩 면적↔수율과 같은 원리 · 다이 고장률 개선은 기술적 한계", "none"),
             ("SSD 내부 (현재 해법)", "다이 RAID · 여분 다이 · 다이 은퇴", "Micron RAIN(XOR 패리티 다이) · 삼성 Fail-in-Place(PM1733, 감량 운영) · 패리티·예비 용량 오버헤드 ↑", "part"),
             ("상위 계층 (SSD 단독 한계 시)", "감량 운영 수용 · 호스트 소거 부호", "OCP SMART XOR 복구 카운트(호스트 관측) · 플랫폼 fail-in-place(MS Hyrax) · 드라이브 간 소거 부호", "next")]
    cy_ = Y_S + 0.07
    ch_ = H_S - 0.14
    x_ = CX_
    for i, (who, what, sub, st) in enumerate(chips):
        if st == "part":
            rect(s, x_, cy_, cwid, ch_, fill=BLUE_T2)
            c1, c2, c3 = INK, INK, GRAY
        elif st == "next":
            sp = rect(s, x_, cy_, cwid, ch_, fill=TINT, line=BLUE, line_w=1.0)
            sp.line.dash_style = MSO_LINE_DASH_STYLE.DASH
            c1, c2, c3 = BLUE, INK, GRAY
        else:
            rect(s, x_, cy_, cwid, ch_, fill=WHITE, line=LINE, line_w=0.75)
            c1, c2, c3 = GRAY_2, INK, GRAY
        tb(s, x_ + 0.12, cy_ + 0.04, cwid - 0.24, 0.18, [(who, 8.75, False, c1)])
        tb(s, x_ + 0.12, cy_ + 0.21, cwid - 0.24, 0.22, [(what, 11.25, True, c2)])
        tb(s, x_ + 0.12, cy_ + 0.43, cwid - 0.24, ch_ - 0.45, [(sub, 8.25, False, c3)], spacing=1.0)
        if i < 2:
            rect(s, x_ + cwid + ag_, cy_ + ch_ / 2 - 0.10, aw_, 0.2, fill=BLUE if i == 0 else BLUE_T2, shape=MSO_SHAPE.RIGHT_ARROW)
        x_ += cwid + aw_ + 2 * ag_
    ...
    notes(s, "1장은 QLC의 내구성 격차를 어느 계층이 해소하는가를 네 지표로 답합니다. 왼쪽 차트는 인과 순서입니다. ① 셀의 한계: 비트/셀을 늘릴수록 셀이 견디는 P/E 사이클이 SLC 30K~100K에서 QLC 100~1K로 100배 감소했습니다. "
          ...
          "오른쪽 위는 산식과 항별 결정 주체입니다. DWPD = P/E × (1+OP) ÷ (WAF × 365 × 년)에서 P/E는 셀, OP는 디바이스, 년과 요구 DWPD는 고객이 결정합니다. P/E가 100배 감소했으므로 남은 보상 변수는 WAF뿐입니다. 오른쪽 아래는 이관의 3단계입니다. 1단계 ECC는 UBER 요구를 충족해 완결됐고, 2단계 SSD 단독 최적화는 QoS·성능을 개선했으나 WAF를 낮추지 못해 DWPD 요구를 충족하지 못했으며, 3단계 호스트 시스템 공동 설계가 WAF를 1로 낮춰 DWPD 요구를 충족합니다. 본 덱 2~5장이 3단계입니다. 결론: QLC로 요구 DWPD를 충족하는 경로는 호스트 시스템 공동 설계뿐이며, 이는 산식의 귀결입니다. 그 규격이 언제 정의되는지가 2장입니다. "
          "하단 스트립은 병렬 축입니다. 내구성 축과 별개로 신뢰성 축에서도 같은 구조가 진행 중입니다. 요구는 JESD218 기능 고장률 FFR 3% 이하로 고정인데, SSD당 NAND 다이 수는 S3700 800GB 128개(2012)에서 Kioxia LC9 245TB 1,024개(2025, 2Tb 다이 32단 스택 32패키지)로 8배 늘었습니다. 보호가 없으면 SSD 고장률은 다이 수에 비례하므로, 칩 면적이 커질수록 수율이 떨어지는 것과 같은 원리로 다이 고장률을 8배 낮추거나 다이 고장을 허용해야 합니다. 다이 고장률 개선이 기술적 한계에 닿자 해법은 SSD 계층으로 이관됐습니다. 다이 RAID(Micron RAIN, 슈퍼페이지 XOR 패리티 다이), 여분 다이, 다이 은퇴와 감량 운영(삼성 Fail-in-Place PM1733: 다이 고장 시 데이터 재배치 후 4~8GB 감량 운영)입니다. 이 해법들은 패리티와 예비 용량 오버헤드를 키우므로 다이 수가 계속 늘면 SSD 단독으로는 부족해질 수 있고, 상위 계층은 이미 준비 중입니다. OCP SMART의 XOR 복구 카운트로 호스트가 다이 복구를 관측하고, Microsoft Hyrax처럼 플랫폼이 감량 운영을 수용하며, 드라이브 간 소거 부호가 SSD 고장을 흡수합니다. 근거 등급은 보고서 부록 A F28~F47에 있습니다.")
