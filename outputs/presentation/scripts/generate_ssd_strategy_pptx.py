...
# =========================================================
# S0. 요약 — 무엇을 · 왜 · 어떻게 (의사결정용, 배경 제외)
# =========================================================
s = prs.slides.add_slide(BLANK)
header(s, "삼성 SSD 전략적 방향성 · 요약",
       "커스텀 요구를 표준(FDP)으로 흡수하는 세 번째 비중 전환을 시작합니다",
       "이 장은 무엇을, 왜, 어떻게만 요약합니다. 배경과 근거는 본편 4장에 있습니다.")
cards0 = [
    ("무엇을 합니까",
     "FDP로 판매되는 SSD 비중을 늘립니다",
     "낸드를 SSD로(2005), SSD를 서버 SSD로(2017)에 이은 세 번째 비중 전환입니다. FDP 표준 제품 위에 시스템 소프트웨어 생태계를 얹어, 고객별 커스텀 개발 없이 워크로드 최적화를 제공합니다."),
    ("왜 지금입니까",
     "최대 고객이 자체 SSD로 이동 중이고, 지렛대는 지금뿐입니다",
     "대형 클라우드 사업자가 낸드 웨이퍼를 직접 사서(1년 새 +246%) 자체 SSD를 만듭니다. 하강기 완충재인 SSD 사업을 지키려면, 고객이 물량을 아쉬워하는 공급자 우위 국면에 워크로드 협력을 얻어야 합니다."),
    ("어떻게 합니까",
     "물량을 주고 워크로드를 받고, 생태계로 잠급니다",
     "장기 물량과 워크로드 협력을 한 계약으로 묶고(1순위 구글, 선례 마이크론과 앤트로픽), FDP 제품은 기존 조직으로, 생태계는 오픈소스 조직 증원으로 갑니다. 리눅스 커널 6.16의 FDP 수용으로 생태계의 문이 열렸습니다."),
]
for (lx, (label, key, body)) in zip((MX, 7.04, 13.29), cards0):
    card(s, lx, 2.95, 5.92, 4.72, label, key, body, tint=True, key_h=1.02)
band(s, 7.95, 2.20, "결정 요청",
     "1순위 고객 제휴 패키지 추진과 오픈소스 생태계 조직 증원, 두 가지를 요청드립니다",
     sub="성과 지표: 고객 시스템에서 실제 활성화된 FDP 용량 · 자체 설계 계획에서 완제품으로 돌아온 물량")
footer(s, "출처: 삼성 SSD 전략적 방향성 보고서 v1.1 (2026년 8월) · 상세 근거는 본편 4장", 1)
notes(s, "단독 열람용 요약입니다. 무엇을: FDP로 판매되는 SSD 비중을 늘리는 세 번째 비중 전환. 왜: 최대 고객의 자체 SSD 이동과 하강기 완충재 방어, 그리고 공급자 우위라는 지렛대의 시한. 어떻게: 물량과 워크로드를 교환하는 제휴 패키지, 제품(기존 조직)과 생태계(오픈소스 증원)의 두 트랙. 결정 요청 두 가지와 성과 지표를 하단에 명시했습니다.")
