...
# =========================================================
# S0b. 한 장 스토리 — 일곱 걸음 (①솔루션 완충 ②90% ③서버 집중 ④29% ⑤캡티브 11% ⑥FDP ⑦효과)
# =========================================================
s = prs.slides.add_slide(BLANK)
header(s, "삼성 SSD 전략적 방향성 · 한 장 스토리",
       "솔루션으로 사이클을 완충해 왔듯, 자체 SSD 시장을 FDP로 흡수합니다",
       "낸드의 솔루션화에서 서버 집중, 자체 SSD 대응까지 일곱 걸음을 한 장으로 요약합니다.")

SB_Y, SB_H = 2.95, 5.60
# 열 1 — 지켜온 방법: 솔루션화 (①·②)
rect(s, MX, SB_Y, 5.92, SB_H, fill=WHITE, line=LINE, line_w=0.75)
a_ix = MX + 0.36
tb(s, a_ix, SB_Y + 0.30, 5.20, 0.36, [("지켜온 방법 · 낸드의 솔루션화", 20.25, True, BLUE)])
tb(s, a_ix, SB_Y + 0.76, 5.20, 0.80,
   [("① 단품과 웨이퍼는 사이클을 그대로 맞고, 솔루션이 완충합니다", 21.75, True, INK)])
tb(s, a_ix, SB_Y + 1.72, 5.20, 0.32, [("② 솔루션 판매 낸드 비중 (비트 기준)", 18.75, True, INK)])
for (bi, (yr, val, frac, bfill, vcol, vbold)) in enumerate([
        ("2015", "약 60%ᵉ", 0.60, BLUE_T2, GRAY, False),
        ("2020", "약 80%ᵉ", 0.80, BLUE_T2, GRAY, False),
        ("2025", "90% 이상", 0.95, BLUE, BLUE, True)]):
    by = SB_Y + 2.12 + bi * 0.50
    tb(s, a_ix, by + 0.01, 1.05, 0.30, [(yr, 18, False, GRAY)])
    rect(s, a_ix + 1.15, by, 2.55 * frac, 0.30, fill=bfill)
    tb(s, a_ix + 1.15 + 2.55 * frac + 0.12, by + 0.01, 1.45, 0.30, [(val, 18.75, vbold, vcol)])
tb(s, a_ix, SB_Y + 3.66, 5.20, 0.30, [("연도별 값은 추정ᵉ · [사내 확인]", 18, False, GRAY)])
tb(s, a_ix, SB_Y + 4.10, 5.20, 1.10,
   [("낸드 매출에서 솔루션이 차지하는 비중이 90% 이상까지 올라, 단품 가격 사이클에 대한 직접 노출이 줄었습니다.", 18.75, False, GRAY)])

# 열 2 — 현재: 서버 집중과 남은 문제 (③·④·⑤)
b_x = 7.04
rect(s, b_x, SB_Y, 5.92, SB_H, fill=WHITE, line=LINE, line_w=0.75)
b_ix = b_x + 0.36
tb(s, b_ix, SB_Y + 0.30, 5.20, 0.36, [("현재 · 서버 집중과 남은 문제", 20.25, True, BLUE)])
tb(s, b_ix, SB_Y + 0.76, 5.20, 0.80,
   [("③ 작년부터 서버 SSD로 이익과 전환 비용을 함께 키우고 있습니다", 21.75, True, INK)])
tb(s, b_ix, SB_Y + 1.72, 5.20, 0.68, [("약 29%", 39, True, BLUE)])
tb(s, b_ix, SB_Y + 2.44, 5.20, 0.62,
   [("④ 서버 SSD 점유율 (사내 기준 [확인]) · 제고 필요", 18.75, False, GRAY)])
tb(s, b_ix, SB_Y + 3.14, 5.20, 0.32, [("⑤ 자체 설계 SSD의 잠식 (2027 전망ᵉ)", 18.75, True, INK)])
rect(s, b_ix, SB_Y + 3.52, 5.20 * 0.89, 0.32, fill=LINE)
rect(s, b_ix + 5.20 * 0.89, SB_Y + 3.52, 5.20 * 0.11, 0.32, fill=GRAY)
tb(s, b_ix, SB_Y + 3.90, 2.5, 0.30, [("완제품 89%", 18, False, GRAY)])
tb(s, b_ix + 2.70, SB_Y + 3.90, 2.5, 0.30, [("자체 SSD 11%", 18.75, True, INK)], align=PP_ALIGN.RIGHT)
tb(s, b_ix, SB_Y + 4.32, 5.20, 1.15,
   [("일부 고객은 낸드를 사서 자체 SSD를 만들며, 2027년 약 100EB, 서버 SSD의 11% 규모에 이릅니다 [사내 확인].", 18.75, False, GRAY)])

# 열 3 — 다음 수: FDP로 흡수 (⑥)
c_x = 13.29
rect(s, c_x, SB_Y, 5.92, SB_H, fill=TINT)
d_ix = c_x + 0.36
tb(s, d_ix, SB_Y + 0.30, 5.20, 0.36, [("다음 수 · FDP로 흡수", 20.25, True, BLUE)])
tb(s, d_ix, SB_Y + 0.76, 5.20, 0.80,
   [("⑥ 자체 개발의 이유는 최적화와 비용, FDP가 둘 다 채웁니다", 21.75, True, INK)])
tb(s, d_ix, SB_Y + 1.72, 5.20, 1.45,
   [("FDP는 고객(구글·메타)이 함께 설계한 배치 표준입니다. WAF와 수명을 개선해 비용을 줄이고, 제품은 공통으로 유지한 채 최적화는 소프트웨어로 제공합니다.", 18.75, False, GRAY)])
tb(s, d_ix, SB_Y + 3.30, 5.20, 0.32, [("기대 효과", 18.75, True, INK)])
tb(s, d_ix, SB_Y + 3.70, 5.20, 0.34, [("점유율 상승", 20.25, True, BLUE)])
tb(s, d_ix, SB_Y + 4.06, 5.20, 0.32, [("자체 SSD 11% 시장의 흡수분만큼 +α", 18.75, False, GRAY)])
tb(s, d_ix, SB_Y + 4.50, 5.20, 0.34, [("전환 비용 상승", 20.25, True, BLUE)])
tb(s, d_ix, SB_Y + 4.86, 5.20, 0.32, [("플랫폼과 생태계 결합이 고객을 잠급니다", 18.75, False, GRAY)])

# 결론 스트립 (⑦)
rect(s, MX, 8.80, CW, 0.72, fill=BLUE)
tb(s, MX + 0.50, 8.80, 1.3, 0.72, [("결론", 18.75, False, WHITE)], anchor=MSO_ANCHOR.MIDDLE)
tb(s, MX + 1.90, 8.80, 16.5, 0.72,
   [("⑦ 자체 SSD 시장을 FDP로 확보하면 점유율은 오르고, 전환 비용이 높아져 다운턴을 잘 맞을 수 있습니다", 21.75, True, WHITE)],
   anchor=MSO_ANCHOR.MIDDLE)
footer(s, "출처: 점유율·솔루션 비중·자체 SSD 규모는 사내 기준 [확인 필요] · 외부 집계는 본편 참조", 2)
...
footer(s, "출처: 스토리지 솔루션 사업사 리서치 · SSD 시장 전환기 리서치 · 기업용 SSD 2026년 1분기 집계 (2026년 8월)", 3)
...
footer(s, "출처: Captive SSD 리서치 · 가트너 재인용 검증, 기업 저장 인프라 기준 (2026년 8월)", 4)
...
footer(s, "출처: FDP 플랫폼 전략 · 구글 Captive와 FDP 팩트체크 · FDP 오픈소스 생태계 리서치 (2026년 8월)", 5)
...
band(s, 9.02, 1.28, "결론",
     ...
     main_size=24.0)
footer(s, "출처: 삼성 SSD 전략적 방향성 보고서 v1.1 · 마이크론 앤트로픽 계약 공시 (2026년 8월)", 6)
