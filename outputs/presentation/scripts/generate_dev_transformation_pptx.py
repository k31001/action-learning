...
설계 원칙: 여백 최소화 + 각진(sharp) 카드 + 얇은 헤어라인 그리드 + 본문 9pt.
디자인: IBM Carbon — Blue-60 #0f62fe(단일 액센트) · Yellow-30 #f1c21b · Red-60 #da1e28 ·
        Green-50 #24a148 · Gray-100 #161616 · Gray-70 #525252(2차 텍스트) ·
        Gray-20 #e0e0e0(border-subtle) · Gray-10 #f4f4f4(layer-01) · Blue-10 #edf5ff ·
        IBM Plex Sans(KR) + IBM Plex Mono(숫자). 팔레트 토큰은 `generate_carbon_pptx.py`와 동일.
...
def carbon_header(slide, kicker, title):
    ...
    add_rect(slide, MARGIN, Inches(0.3), Inches(0.055), Inches(0.62), fill=THEME['samsung_blue'])
    add_text(slide, MARGIN + Inches(0.18), Inches(0.28), Inches(11), Inches(0.24),
             kicker, font=FONT_MO, size=10, bold=True, color=THEME['samsung_blue'])
    add_text(slide, MARGIN + Inches(0.18), Inches(0.5), Inches(12), Inches(0.44),
             title, font=FONT_KO, size=19, bold=True, color=THEME['dark_text'])
...
def sect(slide, x, y, w, text):
    """섹션 소제목 — Carbon: 소형 대문자 느낌의 볼드 라벨."""
    add_text(slide, x, y, w, Inches(0.22), text, font=FONT_KO, size=9.5, bold=True, color=THEME['dark_text'])


def sowhat(slide, y, text, label='SO WHAT'):
    """Carbon notification--warning 스타일 배너."""
    add_rect(slide, MARGIN, y, CW, Inches(0.42), fill=THEME['amber_light'])
    add_rect(slide, MARGIN, y, Inches(0.055), Inches(0.42), fill=THEME['amber'])
    add_text(slide, MARGIN + Inches(0.18), y + Inches(0.09), Inches(1.4), Inches(0.24),
             label, font=FONT_MO, size=9, bold=True, color=THEME['dark_text'])
    add_text(slide, MARGIN + Inches(1.5), y + Inches(0.06), CW - Inches(1.7), Inches(0.3),
             text, font=FONT_KO, size=9.5, bold=True, color=THEME['dark_text'], valign='middle')
...
def build_slide_1(prs):
    ...
    qb_y = Inches(1.18)
    add_rect(slide, MARGIN, qb_y, CW, Inches(0.6), fill=THEME['soft_blue_bg'])
    add_rect(slide, MARGIN, qb_y, Inches(0.055), Inches(0.6), fill=THEME['samsung_blue'])
    add_text(slide, MARGIN + Inches(0.2), qb_y + Inches(0.07), CW - Inches(0.4), Inches(0.28),
             '“고객의 아키텍처 안으로 들어가 수요를 함께 설계하는 기업이 이긴다.” — 칩을 많이 파는 기업이 아니라.',
             font=FONT_KO, size=11, bold=True, color=THEME['dark_text'])
    add_text(slide, MARGIN + Inches(0.2), qb_y + Inches(0.35), CW - Inches(0.4), Inches(0.22),
             '신문섭, Bain & Company (2026-06-18 인터뷰)', font=FONT_KO, size=9, color=THEME['gray_caption'])
    ...
    for t, d, cap, c in stages:
        ...
        add_text(slide, lx + Inches(0.18), sy + Inches(0.08), lw - Inches(0.32), Inches(0.24),
                 t, font=FONT_KO, size=10.5, bold=True, color=c)
        add_text(slide, lx + Inches(0.18), sy + Inches(0.34), lw - Inches(0.32), Inches(0.34),
                 d, font=FONT_KO, size=9, color=THEME['dark_text'], line_spacing=1.0)
        add_text(slide, lx + Inches(0.18), sy + Inches(0.66), lw - Inches(0.32), Inches(0.22),
                 '요구 역량  ' + cap, font=FONT_KO, size=9, bold=True, color=THEME['gray_caption'])
        sy = sy + h + Inches(0.1)
    ...
    for name, desc, tag in comps:
        ...
        add_text(slide, rx + Inches(0.15), cy, Inches(1.5), h, name, font=FONT_KO, size=9.5, bold=True,
                 color=THEME['amber'] if isnew else THEME['gray_caption'], valign='middle')
        add_text(slide, rx + Inches(1.7), cy, rw - Inches(2.7), h, desc, font=FONT_KO, size=9,
                 color=THEME['dark_text'], valign='middle')
        add_rect(slide, rx + rw - Inches(0.94), cy + Inches(0.09), Inches(0.82), Inches(0.26),
                 fill=THEME['amber_light'] if isnew else THEME['soft_white_bg'])
        add_text(slide, rx + rw - Inches(0.94), cy + Inches(0.12), Inches(0.82), Inches(0.2), tag,
                 font=FONT_KO, size=7.5, bold=True, color=THEME['dark_text'] if isnew else THEME['gray_caption'], align='center')
        cy = cy + h + Inches(0.06)
    ...
    for i, (v, l) in enumerate(nums):
        ...
        add_rect(slide, x, ny, nw, Inches(0.6), fill=THEME['dark_text'])
        add_text(slide, x + Inches(0.13), ny + Inches(0.06), nw - Inches(0.26), Inches(0.32),
                 v, font=FONT_MO, size=16, bold=True, color=THEME['white'])
        add_text(slide, x + Inches(0.13), ny + Inches(0.39), nw - Inches(0.26), Inches(0.18),
                 l, font=FONT_KO, size=8, color=GRAY_20)

    # 하단 타임라인 (좌우 전폭)
    tl_y = Inches(5.62)
    add_text(slide, MARGIN, tl_y - Inches(0.24), Inches(11), Inches(0.22),
             '체질 전환을 요구하는 사건의 누적 — 일회성이 아니라 구조 전환', font=FONT_KO, size=9.5, bold=True, color=THEME['dark_text'])
    events = [
    ...
    for i, (d, t, hot) in enumerate(events):
        ...
        add_text(slide, x + Inches(0.1), tl_y + Inches(0.05), ew - Inches(0.2), Inches(0.18),
                 d, font=FONT_MO, size=8.5, bold=True, color=THEME['amber'] if hot else THEME['samsung_blue'])
        add_text(slide, x + Inches(0.1), tl_y + Inches(0.24), ew - Inches(0.2), Inches(0.24),
                 t, font=FONT_KO, size=8, color=THEME['white'] if hot else THEME['dark_text'], line_spacing=0.95)
...
def build_slide_2(prs):
    ...
    # 헤더행
    hy = colY + Inches(0.26)
    add_rect(slide, lx, hy, lw, Inches(0.26), fill=THEME['dark_text'])
    add_text(slide, lx + Inches(0.12), hy + Inches(0.03), Inches(1.5), Inches(0.2), '차원', font=FONT_KO, size=8.5, bold=True, color=THEME['white'])
    add_text(slide, lx + Inches(1.65), hy + Inches(0.03), Inches(2.4), Inches(0.2), 'As-Is · 수주 이행자', font=FONT_KO, size=8.5, bold=True, color=GRAY_20)
    add_text(slide, lx + Inches(4.15), hy + Inches(0.03), Inches(2.5), Inches(0.2), 'To-Be · 기술 파트너', font=FONT_KO, size=8.5, bold=True, color=RGBColor(0x78,0xA9,0xFF))
    rows = [
    ...
    for i, (dim, a, b) in enumerate(rows):
        ...
        add_text(slide, lx + Inches(0.12), ry, Inches(1.5), h, dim, font=FONT_KO, size=9, bold=True, color=THEME['dark_text'], valign='middle')
        add_text(slide, lx + Inches(1.65), ry, Inches(2.4), h, a, font=FONT_KO, size=9, color=THEME['gray_caption'], valign='middle')
        add_text(slide, lx + Inches(4.02), ry, Inches(0.25), h, '→', font=FONT_MO, size=10, bold=True, color=THEME['samsung_blue'], valign='middle')
        add_text(slide, lx + Inches(4.15), ry, Inches(2.5), h, b, font=FONT_KO, size=9, bold=True, color=THEME['dark_text'], valign='middle')
        ry = ry + h
    ...
    add_text(slide, rx + Inches(0.18), fy + Inches(0.1), rw - Inches(0.34), Inches(0.44),
             '“Delta” — 고객사에 상주하는 엔지니어. Anthropic·OpenAI가 엔터프라이즈 GTM 전략으로 채택(Palantir 640% 동력).',
             font=FONT_KO, size=9, color=THEME['gray_caption'], line_spacing=1.0)
    maps = [
    ...
    my = fy + Inches(0.6)
    for f, d in maps:
        add_text(slide, rx + Inches(0.18), my, Inches(2.75), Inches(0.22), '· ' + f, font=FONT_KO, size=9, color=THEME['gray_caption'])
        add_text(slide, rx + Inches(2.9), my, Inches(0.25), Inches(0.22), '→', font=FONT_MO, size=9, bold=True, color=THEME['samsung_blue'])
        add_text(slide, rx + Inches(3.15), my, Inches(2.1), Inches(0.22), d, font=FONT_KO, size=9, bold=True, color=THEME['dark_text'])
        my = my + Inches(0.27)
    add_text(slide, rx + Inches(0.18), my + Inches(0.02), rw - Inches(0.34), Inches(0.4),
             '메모리 변형: FDE(상주) + 시스템 아키텍트·모델링(성능·파워 정량화) — 모델을 무기로 들고 들어간다.',
             font=FONT_KO, size=8.5, italic=True, color=THEME['samsung_blue'], line_spacing=1.0)

    # 우하: 리스크 ↔ 이점
    ry3 = fy + fh + Inches(0.12)
    half = (rw - Inches(0.12)) / 2
    add_rect(slide, rx, ry3, half, Inches(1.78), fill=RED_10, line=THEME['light_gray'])
    add_rect(slide, rx, ry3, half, Inches(0.24), fill=THEME['red_alert'])
    add_text(slide, rx + Inches(0.12), ry3 + Inches(0.03), half - Inches(0.2), Inches(0.2), '안 하면 · 리스크', font=FONT_KO, size=8.5, bold=True, color=THEME['white'])
    for i, t in enumerate(['SCA 수주 배제', '커스텀 전환기 고착', '2nd source·가격력 상실', '미래 기술 선점 실패']):
        add_text(slide, rx + Inches(0.12), ry3 + Inches(0.32) + Inches(0.35) * i, half - Inches(0.22), Inches(0.32),
                 '· ' + t, font=FONT_KO, size=8.5, color=THEME['dark_text'])
    bx = rx + half + Inches(0.12)
    add_rect(slide, bx, ry3, half, Inches(1.78), fill=GREEN_10, line=THEME['light_gray'])
    add_rect(slide, bx, ry3, half, Inches(0.24), fill=THEME['green_pos'])
    add_text(slide, bx + Inches(0.12), ry3 + Inches(0.03), half - Inches(0.2), Inches(0.2), '하면 · 이점', font=FONT_KO, size=8.5, bold=True, color=THEME['white'])
    for i, t in enumerate(['지속 매출(락인+선급금)', '수익률 프리미엄', '미래 기술 선점', '자본 연계 옵션']):
        add_text(slide, bx + Inches(0.12), ry3 + Inches(0.32) + Inches(0.35) * i, half - Inches(0.22), Inches(0.32),
                 '· ' + t, font=FONT_KO, size=8.5, color=THEME['dark_text'])
...
def build_slide_3(prs):
    ...
    for i, (name, c, items) in enumerate(axes):
        ...
        add_rect(slide, x, cy, cw, Inches(0.3), fill=c)
        add_text(slide, x + Inches(0.12), cy + Inches(0.05), cw - Inches(0.24), Inches(0.22),
                 name, font=FONT_KO, size=9.5, bold=True, color=THEME['white'])
        iy = cy + Inches(0.4)
        for it in items:
            add_text(slide, x + Inches(0.12), iy, cw - Inches(0.22), Inches(0.34),
                     '· ' + it, font=FONT_KO, size=9, color=THEME['dark_text'], line_spacing=0.98)
            iy = iy + Inches(0.36)
    ...
    for i, (t, c, items) in enumerate(phases):
        ...
        add_text(slide, x + Inches(0.16), pcy + Inches(0.07), pw - Inches(0.3), Inches(0.24),
                 t, font=FONT_KO, size=9.5, bold=True, color=c)
        iy = pcy + Inches(0.36)
        for it in items:
            add_text(slide, x + Inches(0.16), iy, pw - Inches(0.3), Inches(0.3),
                     '· ' + it, font=FONT_KO, size=8.5, color=THEME['dark_text'], line_spacing=0.98)
            iy = iy + Inches(0.29)
    ...
    for i, (l, v) in enumerate(kpis):
        ...
        add_text(slide, x + Inches(0.1), kcy + Inches(0.07), kw - Inches(0.2), Inches(0.2),
                 l, font=FONT_KO, size=8, color=THEME['gray_caption'])
        add_text(slide, x + Inches(0.1), kcy + Inches(0.28), kw - Inches(0.2), Inches(0.26),
                 v, font=FONT_KO, size=8.5, bold=True, color=THEME['samsung_blue'])
...
def main():
    ...
    print('Palette: IBM Carbon · Fonts: IBM Plex Sans KR + IBM Plex Mono · 3 dense slides · body 9pt')
