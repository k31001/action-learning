# -*- coding: utf-8 -*-
"""메모리 해법 사다리 — 1장 슬라이드 v1.1 (네 지표 P/E · BER · DWPD · WAF로 "단품 → SSD → 호스트" 이관을 설명).

좌: 네 지표 차트(assets/solution_metrics_chart.png — generate_solution_metrics_chart.py, 2×2 인과 순서)
우: DWPD 산식 카드(항별 소유자, WAF = 남은 지렛대) + 두 단계의 대칭 도식(1단계 BER→ECC / 2단계 DWPD→WAF)
하: 결론 밴드
v1.0(계층 사다리 차트 assets/solution_ladder_chart.png)은 보고서 1장 그림으로 유지.
...
from deck_kit import (ASSETS, BLANK, BLUE, BLUE_T1, BLUE_T2, CW, GRAY, GRAY_2, HERE, INK, LINE, MX, RIGHT, TINT, WHITE,
...
header(s, "메모리 해법 사다리 · P/E · BER · DWPD · WAF",
...
band(s, 9.42, 0.80, "결론",
     ...
     main_size=18)
footer(s, "출처: JESD218(UBER·보존), ATP·Kioxia(DWPD 산식·WAF 3), Intel X25-E·S3700·P4510 및 Solidigm P5316 사양(정격 DWPD), Mielke·Cai(RBER), WD ECC/DSP 백서(LDPC 120bit/KB), CacheLib FDP(WAF) · RBER 대표값·환산 DWPD 등급은 보고서 부록 A F28~F35", 1)
