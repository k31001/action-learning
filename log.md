...
## [2026-09-03] ingest | 인터뷰 6번째 추가 — 송용호 부사장(AX/PI센터장) (v2.40.0 → v2.40.1)
- **무엇**: 2026-09-03 진행된 송용호 부사장(AX/PI센터장·前 솔루션개발실장, 녹취록 참석자 2, 약 79분) 내부 인터뷰를 sources 층에 녹취록 원본으로 보존하고, 대시보드 "인터뷰" 메뉴에 앞선 인터뷰와 동일 형식(Executive Summary 포함 16개 섹션·핵심 인용문 4개·다운턴 센싱 3대 관전 포인트 표)으로 구조화해 추가. 패치 v2.40.1.
...
- **dashboard (v2.40.1, 패치 = 콘텐츠 추가, 동일 탭·스키마)**: `data/interviews.js` INTERVIEWS 최상단에 송용호 인터뷰 추가(16개 섹션·keyQuotes 4). `data/updates.js` v2.40.1 항목. `src/version.js` v2.40.1. `npm run build` 검증.
