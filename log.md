...
## [2026-08-08] build | CMO Matrix 탭 신설 — 1~4차 다운턴 통합 매트릭스 + 필터 (v2.39.0)
...
**대시보드(마이너 v2.39.0 — 신규 페이지)**: 최상위 탭 **CMO Matrix** 신설 — `data/cmoMatrix.js`(엔트리·축 정의)와 `components/CMOMatrix.jsx`(필터 UI). 필터: 다운턴·국면·제품·관점·판정 다중 선택 + 본문 검색 + 프리셋 4종(효과 있었던 것만 ◎·실패 ✕·대비 국면·다음 다운턴). 뷰 토글: 매트릭스(제품×관점 그리드, 셀에 엔트리 칩)와 리스트(전체 필드 표). 엔트리 클릭 시 상세 패널 — 4차 항목은 원인 3종별 결과 분기, 실수 항목은 조기 신호·차단 장치 표시. `App.jsx` 탭 등록, index.md·CLAUDE.md 매핑 표 갱신, storyline-cmo.md에서 통합 버전 안내 링크. `npm run build` 검증.
