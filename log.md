...
## [2026-09-23] build | QLC 덱 v7.1 — 1장 ②구획에 "AI 메모리 수요가 HBM에서 스토리지로" 추가
- 사용자 지시: "슬라이드1에서 현재 ai 메모리 수요가 hbm에서 kv cache offloading을 위한 스토리지로 이동하고 있다는 내용과 근거가 추가되어야해"
- 1장 ②구획을 "구매 기준의 이동"에서 **"같은 수요가 스토리지로"**로 재설계. 네 블록 — (a) 구매 기준 3국면을 가로 압축 체인으로(세로 3.18in → 0.70in) (b) **G1 GPU HBM → G2 CPU DRAM → G3 NVMe SSD 스필 도해**(각 단의 성격·한계, 사이에 "넘친다"·"다시 넘친다", G3만 파란 테두리 강조) (c) 시장 신호 블록 (d) 그래서 SSD에 오는 요구 = DWPD 1~3
- **메커니즘 근거**: NVIDIA Dynamo KV Block Manager가 실제 구현한 4단 위계(G1 GPU HBM · G2 CPU DRAM · G3 로컬/풀 NVMe SSD · G4 원격) 🟡. KVBM은 "SSD 수명 연장을 위해 빈도 ≥2 블록만 CPU→디스크" 필터를 기본 활성 — 오프로드 설계자가 이미 SSD 내구성을 고려한다는 증거. NVIDIA는 CMX로 이 계층을 Vera Rubin 플랫폼 구성요소로 규정
- **시장 근거**: TrendForce 2026-08-18이 이 현상을 그대로 **"Vera Rubin의 HBM → NAND 스필오버"**라고 적었고 CMX가 6월 저점 대비 TLC 현물가 반등을 견인했다고 밝혔다 🟡. CMX NAND 소요 2026 35EB → 2027 100EB+ 🟡(서울경제 2026-07-20), SanDisk KV 캐시 2027 75~100EB ✅와 독립 경로 수렴. 삼성 V-NAND 캐파 약 60%를 V10 CMX 대응에 배정 🟡. 추론 수요 CAGR 86% 대 학습 16% 🟡(Kioxia)
- **표기 규율 — 대체가 아니라 스필오버**: "HBM 수요가 줄어 스토리지로 옮겨 갔다"로 읽히면 2026년 데이터(삼성 2026-Q2 HBM 점유 33%, +12%p · 2026-02 최초 상용 HBM4)에 반박당한다. 이동한 것은 총량이 아니라 **증분**이다. 도해 캡션에 "HBM 수요가 준 것이 아니라 넘친 것이 내려왔습니다"를 고정하고 발표 노트에서 한 문단으로 다시 못박았다
- **함께 적은 반대 신호**: CMX 타깃으로 벤더가 지명한 드라이브는 전부 TLC · SK하이닉스·Kioxia의 SLC 기반 AI SSD 개발 보도(2025-12) · DeepSeek V4.1-Flash의 KV 풋프린트 1/8 주장 · CMX 100EB에 KV 캐시 외 컨텍스트 저장 포함 가능 ⚠️. QLC를 이 계층에 넣은 공개 사례는 SanDisk FMS 2026 1건(DWPD 미공개)
- 레이아웃: 열 폭 ① 7.30 → 6.60, ② 5.50 → 6.30, ③ 5.38 → 5.04. 리드·하단 밴드·출처 줄·발표 노트 동반 갱신(밴드/출처가 각각 한 줄씩 넘쳐 문안 축약)
- 신설 위키 `wiki/concepts/hbm-to-storage-spillover.md`, 보고서 §1.2 신설(기존 1.2~1.4 → 1.3~1.5 재번호) + 부록 A **A-75~A-80** + 자산 맵 + PPT 압축 맵 v7.1, 기획서 v7.1 절, index.md, 역링크 2건
- 렌더 QA 통과(7페이지). 지식 그래프 재생성(노드 114·엣지 618, 고아 0). 대시보드 v2.46.43 + updates 엔트리, `npm run build` 통과
