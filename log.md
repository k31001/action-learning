...
## [2026-08-25] ingest | 스케줄 재실행 — Hot Chips 2026(삼성 zHBM vs SK i-HBM) 신규 확인분만 반영, 전면 재점검은 생략 (v2.46.12)

**상황**: 직전 항목(v2.46.11, 커밋 52fc7a1)과 동일한 정기점검 스케줄 프롬프트가 같은 날 20:25 UTC — 직전 완료 직후 짧은 간격 — 재실행됨. 외부 시장 데이터(실적·가격·CapEx 가이던스 등)가 수십 분 내 실질 변화할 가능성은 낮다고 판단해 **4-에이전트 전면 리서치를 재실행하지 않고**, 가벼운 확인 검색(2건)으로 그 사이 발생한 진짜 신규 이벤트만 스크리닝했다.

**발견**: **Hot Chips 2026**(학회 발표 2026-08-11경, 매체 보도는 08-24~25에 집중돼 직전 점검의 리서치 창에는 포착되지 않음) — 삼성·SK하이닉스가 "다음 HBM 병목"에 대해 서로 다른 기술 노선을 제시한 정황. 삼성은 **zHBM**(베이스다이 로직화: 메모리 1c 노드 + 로직 4nm, 표준 HBM4e 대비 전력효율 +70%·대역폭 +230% 주장)으로 08-04~06 FMS 2026 목업의 첫 정량 성능 수치를 공개. SK하이닉스는 **i-HBM**(패키징·열관리: CoWoS-S/L·Intel EMIB 비교, die-to-die PHY 냉각재로 열저항 -30%+ 목표)에 집중하며, 하이브리드본딩을 HBM4E가 아닌 **HBM5로 공식 순연**(775마이크론 높이 한계 문제의식과 함께) — 이는 08-25 정기점검(bottleneck-model-update-2026-08-25.md)에서 이미 확인한 "업계 전반 마이크로범프 유지" 판단의 SK하이닉스측 1차 근거 재확인. 신규 소스: `sources/raw-notes/hot-chips-2026-hbm-architecture-2026-08-25.md`.

**반영**: `wiki/concepts/hbm-roadmap.md`에 [Update 2026-08-25] Hot Chips 2026 섹션 신설(크리스 밀러 "아키텍처 계층 경쟁" 논지의 실증 사례로 연결) + `wiki/entities/samsung.md`·`sk-hynix.md`에 각사 관점 교차참조 절 추가. Bottleneck Model 지수·시장 점유율·병목 4대 축은 변경 없음(30분 내 실질 변화 없다고 판단, 재산정 생략).

**마이너 v2.46.12**(새 데이터 카테고리 — HBM 아키텍처 기술 노선 분기, 앞선 시장 지표 갱신과는 별개 층위). index.md 소스 등록, 지식그래프 재생성, `npm run build` 검증. **건너뜀 사유**: bottleneck-model-2030.md 재갱신 불필요(순수 기술 아키텍처 뉴스로 병목 4대 축의 자원량·제약지수와 무관), outputs/report·PPTX 무변경(숫자 변경 없는 기술 동향 1건은 리포트 재구성 임계치 미달), 시나리오·전략 확률축 무변경(같은 날 assessment 항목의 "유지" 결론과 정합).
