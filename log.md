...
- **건너뜀**: wiki·outputs·기타 dashboard 데이터는 본 인터뷰가 분석 페이지 수치를 직접 바꾸지 않으므로 미변경(신문섭 인터뷰와 동일 스코프). 인터뷰 인사이트의 위키 환원(HBM↔DDR 캐파 상쇄·GHBM·커스텀 체질)은 후속 ingest에서 검토.
