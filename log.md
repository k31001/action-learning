...
## [2026-09-01] ingest+build | 병목 모델 정기 갱신 — 전력 76(Red 최초 진입)·CAPEX 38·파운드리 48·패키징 65 (v2.46.12)
- **무엇**: 2026-07-04 이후 병목 모델 4대 제약지수를 웹 리서치(SemiAnalysis·Counterpoint·TechInsights 우선 탐색 + TrendForce·Holland & Knight·Utility Dive 등 보강)로 재점검·갱신. 신규 소스 1건 인제스트: `sources/articles/september-2026-market-update-2026-09-01.md`(ERCOT 접속 전면 중단·TSMC N2/N3 램프·CoWoS 갭 축소·삼성 HBM4 골든수율 80%, 방법·한계·URL 명시). `wiki/concepts/bottleneck-model-2030.md`에 "종합 판독(2026-09-01)" 절 신설(제약지수 표·핵심 신규 신호·병목별 서사, 07-04 절 뒤에 추가) — 전력 72→**76**(▲+4)·CAPEX 40→**38**(▼-2)·파운드리 50→**48**(▼-2)·패키징 67→**65**(▼-2).
- **왜**: 사용자 지시(병목 모델 정기 갱신, 변동폭 명시). 핵심 트리거: ERCOT가 2026-08-03 텍사스 주지사 지시로 신규 데이터센터 접속 절차를 전면 일시중단(감사 완료 시한 12-10, 신청 474GW 중 90%가 DC, BNEF 추정 최대 49.8GW 지연·$15억 비용) — 07-04까지의 "큐 적체"(점증적 병목)에서 "접속 절차 정지"(레짐 전환)로 성격이 악화돼 전력이 4대 병목 중 최초로 Red 밴드(75~85) 진입·4개 점검 연속 재상승. 반면 CAPEX(빅4 Q2 실적 전원 상향·삭감 0건)·파운드리(TSMC N3 18만 장 목표 2~3개월 조기 달성)·패키징(CoWoS 수급갭 20%→10% 축소·삼성 HBM4 마이크로범프 수율 80% 골든수율 예상보다 ~4개월 조기 달성, 단 하이브리드본딩 16-Hi는 여전히 ~10%로 HBM4E/5E 이연)은 완화 지속 — 자본·공급망 제약은 점진 해소, 전력만 구조적 악화.
- **영향받은 페이지**: `wiki/concepts/bottleneck-model-2030.md`(frontmatter last_reviewed·sources 갱신 + §종합 판독 2026-09-01 신설) · `index.md`(신규 소스 1건 등록) · `dashboard/src/data/bottleneckModel.js`(MODEL_ASOF·PREV_MODEL_ASOF·PREV_INDICES·BOTTLENECKS 4종 currentIndex/indexNote·DRIVERS_ASOF·드라이버 5종 note/trend 갱신 — 단일 소스 = wiki 페이지) · `dashboard/src/data/updates.js`(build 엔트리 신설) · `dashboard/src/version.js`(v2.46.11→v2.46.12 패치) · `dashboard/src/data/knowledgeGraph.js`(재생성, orphans=0) · `outputs/report/scenario-planning-report.md`(핵심 수치 표·§7.3 병목 레이더 표·§2.2.3 정기점검 절 신설로 09-01 수치 반영, 07-04/06-06 시점 잔존 수치 정합) · `outputs/presentation/slide-outline.md`(09-01 데이터 갱신 노트 추가, 구조 변경 없어 PPTX 미재생성 — 07-04와 동일 방침).
- **건너뜀**: SP-1/SP-2 시나리오 확률·EWI는 이번 갱신에 포함하지 않음 — 같은 날짜의 별도 assessment 로그(아래)가 이미 다뤘고 재점검 결과 무이동 확정. `wiki/entities/samsung.md`·`cxmt.md`·`ymtc.md` 등 개체 페이지는 이번 리서치로 확인된 사실(삼성 HBM4 80% 수율 등)이 병목 모델 범위 내 서사이자 아직 삼성 공식 IR 미확인 잠정치라 별도 개체 페이지 갱신은 보류(다음 Q3 실적 발표 시 정식 반영 예정) — 전략 페이지(`wiki/strategies/**`)는 이번 리서치로 기존 판단을 뒤집는 신규 사실이 없어 수정 없음.
