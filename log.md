...
## [2026-09-15] ingest | 병목 모델 정기 점검(약 10주 만) — 텍사스 ERCOT 신규 접속 전면 일시중단으로 전력이 최고 병목 부상 (v2.46.18)
- **무엇**: `wiki/concepts/bottleneck-model-2030.md`의 4대 병목 제약지수가 2026-07-04 이후 미갱신 상태(08~09월은 다운턴 트랙·인터뷰·발표자료 중심 갱신)임을 확인하고, "@data 최신 데이터 검색·Bottleneck Model 갱신" 사용자 지시에 따라 SemiAnalysis·Counterpoint·TechInsights 우선 탐색 + TSMC IR·PJM/ERCOT 1차 공시·Fortune·CNBC·Tom's Hardware로 보강한 정기 점검을 수행. 신규 소스 `sources/articles/september-2026-market-update-2026-09-15.md` 1건 등재.
- **제약지수 변동(07-04→09-15)**: 전력 72→**78**(▲+6) · CAPEX/ROI 40→**40**(─0) · 파운드리 50→**52**(▲+2) · 패키징 67→**65**(▼-2).
- **핵심 신호**: ① 전력 — 텍사스 주지사가 2026-08-03/13 ERCOT 신규 데이터센터 접속을 감사 완료 시까지 전면 일시중단(1,800개 프로젝트·474GW 대상), PJM도 신규 대형부하(50MW+) 규정을 FERC에 제출(부하 자체 발전 조달 원칙, 발효희망 10-12) — "큐 지연"에서 "정부발 접속 동결"로 질적 악화, 4대 병목 중 최고 지수로 CAPEX/ROI를 추월(3분기 연속 상승). ② CAPEX/ROI — 빅4 합산 $725B(+77% YoY)·삭감 0건 불변이나, NVIDIA가 OpenAI 오하이오 DC 파이낸싱 보증을 최대 검토액 $250B에서 최종 $105B(초기 5GW만 백스톱, 08-18 서명)로 축소·Oracle 주가 YTD -24%·HSBC "OpenAI 2030 +$207B 추가조달 필요" — 수요 붕괴 아닌 조달 구조 스트레스로 지수는 유지하되 모니터링 격상. ③ 파운드리 — TSMC 8월 매출 NT$514.81B(+53.3% YoY) 사상 최고이나 N2 현재 캐파(월 2만 장) vs 연말 목표(월 10만 장) 격차가 뚜렷해 램프 실행 리스크 반영. ④ 패키징 — CoWoS 수급 갭 20%→10% 축소 전망(TrendForce) 재확인해 완화 지속이나, SK하이닉스가 하이브리드본딩을 HBM4E도 건너뛰고 HBM5(~2029~30)로 이연 공식화(Hot Chips 2026) + 적층 775마이크론 두께 한계 신규 제기. ⑤ 삼성 HBM 점유율 33~40%대 회복 재확인(경로별 수치 편차 잔존, 다음 ingest 이월), 엔터프라이즈 DDR5 RDIMM·DDR4 스팟 가격 사상 최고 재확인.
- **왜**: 사용자 스케줄 작업 지시 — 최신 데이터를 검색해 보고서·대시보드·발표자료에 반영하고, Bottleneck Model 숫자를 갱신하며 지난 업데이트 대비 변동폭을 표현.
- **영향받은 페이지**: `wiki/concepts/bottleneck-model-2030.md`(§2 전력 서술·§5 종합 판독 2026-09-15 신설·출처)·`wiki/concepts/energy-constraints.md`·`wiki/concepts/ai-capex.md`·`wiki/entities/tsmc.md`·`wiki/concepts/hbm-market.md`·`wiki/concepts/price-trends.md`(각 [Update 2026-09-15] 절 추가)·`index.md`(병목 모델 요약 줄·신규 소스 1건 등재)·`dashboard/src/data/bottleneckModel.js`(MODEL_ASOF·PREV_INDICES·4개 currentIndex/indexNote·DRIVERS_ASOF·interconnect/node_ramp/stack_yield 노트)·`dashboard/src/data/updates.js`(ingest 엔트리 신설)·`dashboard/src/version.js`(v2.46.17→v2.46.18)·`outputs/report/scenario-planning-report.md`(§2.2.3 신설·핵심 수치·병목 레이더 표 갱신)·`outputs/presentation/slide-outline.md`(09-15 데이터 갱신 노트 추가). `dashboard/src/data/knowledgeGraph.js`(tsmc.md↔bottleneck-model-2030.md·energy-constraints.md↔bottleneck-model-2030.md 신규 링크 반영해 재생성 — orphans 0·edges 570).
- **전략 리뷰**: 전력 병목 급상승(78, Red 밴드 최초 진입)·파운드리 재긴장은 기존 [bottleneck-model-2030.md §6 전략 함의](../wiki/concepts/bottleneck-model-2030.md)("전력을 먼저 확보 → CoWoS·선단 로직 장기 잠금 → ROI 흔들려도 capex 지속")를 강화할 뿐 새 전략 축을 요구하지 않음 — RS-1(옵션형 캐파)·RS-9(수요 변곡 센싱)의 우선순위 재확인으로 충분하다고 판단, 신규 RS/D 추가·삭제 없음.
- **건너뜀**: PPTX 본체 재생성은 범위 외(슬라이드 구조 변경 없음, 노트만 추가 — 07-04 선례와 동일 처리). HBM 점유율 수치 계열 간 정합(Counterpoint 원문 대조)은 다음 ingest로 이월.
