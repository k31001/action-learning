...
## [2026-09-22] assessment | 시나리오 포지션 맵·확률 정기 재평가 — 유지 (DF1 8.5·DF2 0.5, A26·B39·C8·D21·E6) + Trump-Xi 09-24 정상회담 임박·순환 파이낸싱·GPU 임대가 EWI·트리거 반영 (v2.46.41)
- **무엇**: 직전 09-15 재평가 이후 git log 변경분을 파악(v2.46.18~v2.46.40, 42개 커밋 — 전부 QLC eSSD 덱/보고서/위키 내부 제품믹스·발표 층)하고 09-22 창의 in-window 신호를 웹 리서치로 대조 → 포지션 맵(DF1 8.5·DF2 0.5)·확률(A26·B39·C8·D21·E6) 전부 **유지**로 확정, 발동 트리거 0건. 수집 데이터를 EWI·트리거 노트/이력으로 반영하고 업데이트 내역 메뉴(type: assessment '포지션·확률' 필터)에 2026-09-22 엔트리 신설.
- **왜**: 09-15 이후 커밋은 전부 QLC eSSD 전략 스트림(덱 v3.1~v6.3·보고서 v1.0~v2.0·위키·발표 층)으로 **내부 제품믹스(DF3/NAND)·발표 층**이라 신규 실현된 외부 거시 사실이 repo에 유입된 바 없음. 거시 축은 "실현된 외부 사실"이 움직인다는 일관 방법론상 **축 무이동**. 웹 리서치 in-window 신호는 전부 정점 재확인·후기순환 조달 tell: 빅테크 4사 2026 CapEx ~$725B(+77% YoY·삭감 0건)·2027 전망 상향 유지(JPM·Goldman·Evercore·BofA >$1T — 36.5% 성장만으로 $1조 도달)·DRAM Q3 +13~18%·NAND +10~15%·소비자 DRAM 부족 DDR2까지 확대·구조적 쇼티지 2027 지속·GPU 임대가 firming(H100 9월 온디맨드 중앙값 ~$3.25/GPU-h·H200 ~$4.40/GPU-h·붕괴 없음)·Micron Q4 FY26 실적 09-30이 다음 확인점. **후기순환 조달·파이낸싱 tell 심화(수요 붕괴 아님)**: NVIDIA-OpenAI-Oracle $745B 순환 파이낸싱 루프가 첫 스트레스 테스트(연준 금리)·Oracle $43B 차입으로 마이너스 FCF 커버·OpenAI 2026 매출 ~$20B 대비 손실 ~$14B·AI 순환경제 버블 인덱스 73/100·NVIDIA $105B OpenAI 베팅(CFO Kress "once-in-a-generation 플랫폼 전환" 방어) = 송용호 관전 ②(NVIDIA 금융업체화)·①(AI ROI 한계)의 실측 심화. CapEx 자체는 상향·재고 없음이라 조달 구조 경보로 EWI 반영(축 무이동). DF2: **Trump-Xi 정상회담 09-24 워싱턴 개최 확정**(2일 후, 3T 무역·기술·타이완 의제·시장 건설적 결과 선반영·Busan 휴전 연장 최선 기대·MATCH 법안 의회 진전 150일 동맹 정렬 조항·베이징 비판 격화) — 09-15가 지목한 "9월 정상회담=다음 분기점"의 날짜가 09-24로 확정·임박했으나 타결·라이선스 언어·MATCH 통과·신규 통제 등 실현 정책 사실은 09-24 이후 확정 → 리트머스 미해결로 DF2 0.5 유지, 양방향 트리거(us_china_semiconductor_deal / match_act_passed) 대기. 삼성 HBM4 볼륨 발주 미전환·NVIDIA 16-Hi HBM4 Q4 요청에 삼성 skip(SK 볼륨·Micron 샘플·캐파 2배)은 B 시간 리스크.
- **영향받은 페이지**: `wiki/driving-forces/key-drivers.md`(DF1·DF2 09-22 현재 위치)·`wiki/scenarios/scenario-matrix.md`(확률 추정 09-22 평가 기준)·`dashboard/src/data/indicators.js`(INITIAL_QUADRANT_POSITIONS current·SCENARIOS 확률 주석·EWI bigtech_capex_growth·gpu_rental_h100_usd·ai_dc_credit_spread·samsung_hbm4_rubin_share·cxmt_apple_qualification·트리거 match_act_passed·us_china_semiconductor_deal·apple_cxmt_approved·samsung_hbm4_nvidia_confirmed·enterprise_ai_roi_proven·demand_inflection_divergence)·`dashboard/src/data/scenarioPlanning.js`(DF1·DF2 currentPosition·SCENARIOS_DATA A·B 주석)·`dashboard/src/data/updates.js`(assessment 엔트리 신설 — 업데이트 내역 메뉴)·`dashboard/src/version.js`(v2.46.40→v2.46.41 패치). 위키 링크 무변경이라 knowledgeGraph 재생성 불필요.

## [2026-09-15] assessment | 시나리오 포지션 맵·확률 정기 재평가 — 유지 (DF1 8.5·DF2 0.5, A26·B39·C8·D21·E6) + NVIDIA-OpenAI 파이낸싱·미중 정상회담·GPU 임대가 EWI·트리거 반영 (v2.46.17)
- **무엇**: 직전 09-08 재평가 이후 git log 변경분을 파악(커밋 0건 — 신규 ingest·외부 소스 0건)하고 09-08~09-15 창의 in-window 신호를 웹 리서치로 대조 → 포지션 맵(DF1 8.5·DF2 0.5)·확률(A26·B39·C8·D21·E6) 전부 **유지**로 확정, 발동 트리거 0건. 수집 데이터를 EWI·트리거 노트/이력으로 반영하고 업데이트 내역 메뉴(type: assessment '포지션·확률' 필터)에 2026-09-15 엔트리 신설.
- **왜**: 09-08 이후 **커밋 0건**이라 신규 실현된 외부 거시 사실이 repo에 유입된 바 없음. 거시 축은 "실현된 외부 사실"이 움직인다는 일관 방법론상 **축 무이동**. 웹 리서치 in-window 신호는 전부 정점 재확인·후기순환 조달 tell: 빅테크 4사 2026 CapEx ~$725B(+77% YoY·삭감 0건)·2027 전망 상향(JP모건 ~$1T·Goldman $1.01T)·DRAM Q3 +13~18%·NAND +10~15%·구조적 쇼티지 2027 지속·GPU 임대가 firming(H100 온디맨드 ~$2.65~2.89/h·H200 +20~40%·"메모리 병목")·Micron Q4 FY26 가이던스 $50B·HBM 10만 wpm 램프(09-30 실적이 다음 확인점). **후기순환 조달·파이낸싱 tell 심화(수요 붕괴 아님)**: NVIDIA의 OpenAI 오하이오 DC 파이낸싱 개런티 $250B→~$105B(<$120B) 축소(CNBC 08-17·Fortune 08-18 "보도 대비 $145B↓·칩 인공 수요 우려")·Oracle 주가 YTD -24%·HSBC "OpenAI 2030 $200B 매출에도 +$207B 조달 필요" = 송용호 관전 ②(NVIDIA 금융업체화)·①(AI ROI 한계)의 실측 보강. CapEx 자체는 상향·핸드투마우스 재고 없음이라 조달 구조 경보로 EWI 반영(축 무이동). DF2: **9월 미중 정상회담이 실제 개최 국면**(MATCH이 가장 절차 진전된 기술 파일로 의제·베이징 비판 격화) — 09-08이 지목한 "9월 정상회담=다음 분기점"이 열렸으나 타결·MATCH 통과·신규 통제 등 실현 정책 사실 미확정 → 리트머스 미해결로 DF2 0.5 유지, 결과에 따라 양방향 트리거(us_china_semiconductor_deal / match_act_passed) 대기. 애플–CXMT 승인/차단 미확정·CXMT $42억 상하이 IPO(증설 자금)는 시나리오 D 배경 재확인.
- **영향받은 페이지**: `wiki/driving-forces/key-drivers.md`(DF1·DF2 09-15 현재 위치)·`wiki/scenarios/scenario-matrix.md`(확률 추정 09-15 평가 기준)·`dashboard/src/data/indicators.js`(INITIAL_QUADRANT_POSITIONS current·SCENARIOS 확률 주석·EWI ai_dc_credit_spread·bigtech_capex_growth·gpu_rental_h100_usd·gpu_rental_price_trend·cxmt_dram_share·트리거 enterprise_ai_roi_proven·demand_inflection_divergence·match_act_passed·us_china_semiconductor_deal·apple_cxmt_approved·samsung_hbm4_nvidia_confirmed)·`dashboard/src/data/scenarioPlanning.js`(DF1·DF2 currentPosition·SCENARIOS A·B 주석)·`dashboard/src/data/updates.js`(assessment 엔트리 신설 — 업데이트 내역 메뉴)·`dashboard/src/version.js`(v2.46.16→v2.46.17 패치). 위키 링크 무변경이라 knowledgeGraph 재생성 불필요.

## [2026-09-08] assessment | 시나리오 포지션 맵·확률 정기 재평가 — 유지 (DF1 8.5·DF2 0.5, A26·B39·C8·D21·E6) + 송용호 인터뷰 3대 관전 포인트 EWI·트리거 반영 (v2.46.16)
- **무엇**: 직전 09-01 재평가 이후 git log 변경분을 계층 분류하고 09-02~03 수집 소스로 대조 → 포지션 맵(DF1 8.5·DF2 0.5)·확률(A26·B39·C8·D21·E6) 전부 **유지**로 확정, 발동 트리거 0건. 송용호 인터뷰의 다운턴 센싱 3대 관전 포인트·중국 가격 통제 프레임·QLC eSSD/FDP 제품믹스 데이터를 EWI 7종·트리거 노트로 반영.
- **왜**: 변경분은 전부 ① 내부 인터뷰(송용호 부사장 AX/PI센터장 09-03 — 다운턴 시뮬레이션·기본기(품질·원가)·조직 체질 층위) ② 제품믹스 리서치(대용량 QLC eSSD 벤더별 타임라인·FDP RUH 갭 09-02 — DF3/NAND) ⑤ 발표 층(향후 계획 덱·내부 인터뷰 인용문 덱·FDP 컨설팅 덱)으로 **신규 실현된 외부 거시 사실 0건**. 거시 축은 "실현된 외부 사실"이 움직인다는 일관 방법론상 내부 전문가 프레이밍·제품믹스·발표 층으로 분류 → 축 무이동. 송용호 자체 관측(가격 피크 26년 가능성·28~30 슬로우 가능·단 AI 수요는 아직 갈 길 멀어 존재)은 이미 DF1 8.5 후기순환 정점·D 시나리오 2027~2028 조정 타이밍과 정합 → 정점 재확인. 다운턴 센싱 3대 관전 포인트(① AI 기술 ROI 진보 지속성 — OpenAI 광고 진입·IPO는 한계 인식 신호 ② 하이퍼스케일러 자금 구조 — FCF 마이너스→대출→지분 매각→NVIDIA 금융업체화 ③ 전기 — 美 27~28 쇼티지가 DC 건설 속도 지연 가능)와 중국 만성적 공급과잉·가격 통제 프레임(5% 오버서플라이면 가격 하락하는 시장에서 중국 점유 10%·소비 50%, 뒤에 중국 정부)은 시나리오 상대 확률이 아니라 하락 변곡 선행 EWI·트리거로 반영. QLC eSSD 타임라인(245TB 첫 상용 출하 Micron 2026-05·삼성 용량 리더십 후발·CMX 타깃 200+ FDP 쓰기 스트림·7~10+ DWPD vs 현행 RUH 2~8개·AI 서버 $187B/서버가치 65%)은 DF3/NAND 제품믹스 신호. DF2: MATCH 본회의 movement 없음·애플–CXMT 리트머스 미해결·신규 수출통제 없음(9월 시진핑 방미 정상회담이 다음 분기점) → 유지.
- **영향받은 페이지**: `wiki/driving-forces/key-drivers.md`(DF1·DF2 09-08 현재 위치)·`wiki/scenarios/scenario-matrix.md`(확률 추정 09-08 평가 기준)·`dashboard/src/data/indicators.js`(INITIAL_QUADRANT_POSITIONS current·SCENARIOS 주석·EWI 7종 enterprise_ai_roi_proven·ai_dc_credit_spread·dc_cancellation_count·demand_inflection_divergence·cxmt_asp_gap·enterprise_ssd_tam·nvidia_storage_next_partner)·`dashboard/src/data/scenarioPlanning.js`(DF1·DF2 currentPosition·SCENARIOS_DATA A·B 주석)·`dashboard/src/data/updates.js`(assessment 엔트리 신설 — 업데이트 내역 메뉴)·`dashboard/src/version.js`(v2.46.15→v2.46.16 패치). 위키 링크 무변경이라 knowledgeGraph 재생성 불필요.
...
- **wiki**: samsung-impact.md §5.1c 신설(슬라이드 2p 단일 소스 — 지출·창출·체크리스트·종합). 수치는 전부 기존 소스 인용(ai-capex, hyperscaler-q2-2026-actuals, hyperscaler-q2-2026-capex, ai-compute-economics-gap).
- **건너뜀**: dashboard 무변경(version bump 생략), 신규 소스 없음(기존 소스 재인용), index.md 무변경(신규 페이지 없음).

## [2026-08-26] ingest+build | 영향 슬라이드 v5.2 — 27~28E CAPEX 리서치 반영 + 문제의 전략 방향 정렬
- **무엇**: 7차 피드백 — ① 발표자 노트 v5.1 정합 확인(부문별 CAPEX·과잉 유지 가정 전부 반영돼 있었음) ② **2027~28 CAPEX 웹 리서치 인제스트**: TrendForce 2026-07-30 보도(DRAM 신규 캐파 램프 2027 H2·산출 기여 2028, 2027 DRAM 타이트/NAND 완화 분기), Kioxia FY26~28 연평균 ¥4,700억(+66% vs FY25, TrendForce News 06-03), Kioxia–SanDisk 2026 합산 +40%(06-01), YMTC 캐파 26~27 약 2배, DRAM 장비 매출 2027 +7.8% → 신규 소스 `sources/articles/memory-capex-outlook-2027-2028-2026-08-26.md`(수집 방법·한계 명시 — 원문 2건 프록시 차단, 검색 요약 기반). 슬라이드 27E를 과잉 유지 가정에서 **리서치 기반 추정 $81Bᵉ**(DRAM 58.2=54.0×1.078 프록시 + NAND 23.0)로 교체, 28E는 27E 유지 가정. 각주·발표자 노트에 산출 방법 병기.
- **③ 문제 정의의 전략 방향 정렬**(비명시적): 검토 중인 후보 방향(전환 가능한 몸·FDP 생태계 서버 락인·zHBM 차세대 집중)으로 자연스럽게 이어지도록 — 2열 문제를 "밀려난 수요는 SSD로 온다: 서버 고객을 묶어둘 표준·SW 고리의 확보"로 교체(FDP 방향 암시, 명칭은 노트에서만 예시로 언급). 1·4열(CIS 전환처)·5열(zHBM·별동대)은 기존 문구가 이미 해당 방향을 가리켜 유지. 발표자 노트에 "문제와 대응 방향의 연결" 단락 추가 — "결론을 못박지 말고 문제가 가리키는 방향으로만 열어 둘 것" 발표 지침 포함.
- **wiki**: samsung-impact.md §5.1b(27~28E 추정 근거)·§5.4(2열 문제), memory-capex-history.md §3(2027E~28E 전망 행) 동반 갱신. index.md sources 등록.
- **건너뜀**: dashboard — 무변경.

## [2026-08-25] build | 영향 슬라이드 v5.1 — 막대 색 통일 + CAPEX 차트 부문별 확장
- **무엇**: 6차 피드백 — ① 수요 막대의 2025(기준) 막대가 범례 색과 달랐던 것(옅은 톤 사용)을 범례와 동일한 본색으로 통일(두 막대 모두 세그먼트 흰 경계선, 라벨·연도 표기로 구분) ② CAPEX 몰리는 경우 차트를 3사 합산 3개년에서 **부문별 누적 4개년**으로 확장 — DRAM 3사(진한 회색) + NAND 6사(옅은 회색) 스택, 2025 $67.6B → 2026E $76.2B(TrendForce·각사 IR) → **27E·28E는 과잉 유지 가정**(공표 전망치 부재 — 시나리오 정의 그대로 2026E 수준 지속, 옅은 색·점선·ᵉ 표기로 실측/계획과 구분). 미니 범례·각주·발표자 노트 동반 갱신.
- **건너뜀**: dashboard — 무변경. 2027~28 공표 전망치 웹 리서치 — 기존 위키 데이터셋(TrendForce 2026E까지)과 정합 유지 위해 가정 표기 방식 선택, 실제 전망치 소스 확보 시 교체.

## [2026-08-25] build | 영향 슬라이드 v5 — 실측 구성·공급 전망 차트·발표자 노트
- **무엇**: 5차 피드백 반영 — ① 수요 막대 기준을 개념 배분에서 **2025 실측 매출 구성**(DRAM $1,657억 중 HBM $340억·NAND $697억, TrendForce·Yole → 지수 14/56/30)으로 교체(시나리오 막대는 실측 기준에 방향 가정 적용, 각주 명시. 서버/범용 분리 실측치 부재로 DRAM 단일 세그먼트) ② 공급 두 케이스는 수요 막대 제거하고 **공급 전망 차트**로 — CAPEX 몰림: 3사 CAPEX 합산 2024 $54B→2025 $64B→2026E $95B(각사 IR, 리드타임 2~3년 → 2028~29 캐파 도래) / 후발 잠식: CXMT 캐파 11%→15%E(2028)·매출 8%→14%E(2027E) ③ **발표자 노트 신설** — 트랙 맥락·다섯 갈래 상세 해설·막대/차트 계산 근거·문제 정의 배경·출처 목록.
- **wiki**: samsung-impact.md §5.1b(v5 데이터 기반) 추가.
- **검증**: LibreOffice 렌더 검수(차트 제목-막대 라벨 겹침 수정), 노트 포함 저장 확인.
- **건너뜀**: dashboard — 무변경.

## [2026-08-25] build | 영향 슬라이드 v4 — 누적 막대 · 구체 팩트 · "~는 경우" 표기
- **무엇**: 4차 피드백 반영 — ① 시나리오 코드(DT-x)·이름 슬라이드에서 제거, 하위 원인을 "~는 경우"로("투자 자금이 끊기는 경우" 등 5종) ② **제품 수요를 누적 막대 지수 개념도로**(현재 총수요=100, §5.3 방향 표 지수화 — 배분·폭은 방향 표현용, 각주 명기. 수요발 68·88, 공급발 100 유지, 전환발 98에 점선 "차세대 이동분" 세그먼트) ③ **공급 두 케이스 구체화** — CAPEX 몰림: Micron 아이다호·SK 용인·국내 신규 팹 2028~29 동시 가동(착공 확정) / 후발 잠식: CXMT 캐파 11%→15%E(2028)·HBM3 월 6만장·YMTC Xtacking ④ 수요 케이스에도 팩트 라인(Meta FCF -91%·4사 CapEx ~$750B / KV 캐시 오프로드 동시 사용자 10배·CXL 풀링) ⑤ **문제 정의 제품 구체화**(HBM4 증설분+50%·CIS 공용 80%·eSSD 38.2%·2023 감산 6개월 지연·DDR4/LPDDR4 철수·zHBM·4F² 등) ⑥ 색 보강 — 제품 4색 블루 램프 + 그룹 밴드 옅은 틴트(모노톤 v3가 횡하다는 피드백).
- **수치 원칙**: 슬라이드의 모든 수치·기업·기술명은 위키 기인용 사실만 사용. 막대 배분은 개념도임을 각주에 명시(실측 아님).
- **wiki**: samsung-impact.md §5에 v4 노트 추가, §5.4 문제 표를 제품·데이터 구체화 버전으로 교체(소스 링크 포함).
- **검증**: LibreOffice 렌더 4회 반복 검수(문제 텍스트 줄바꿈 3줄 고정), em-dash 0.
- **건너뜀**: dashboard — 무변경.

## [2026-08-25] build | 영향 슬라이드 v3 — 원인 중심 재구성 (S/W·속도 축 제거, 모노톤)
- **무엇**: 사용자 피드백 반영 3차 개정 — ① 상단 S/W 장단점 나열 제거 ② 속도(급락/침식)·기간 요소 제거(수요발=단기전·공급발=장기전 패턴 확인됨, downturn-history 기준) ③ **원인 계통을 주인공으로**: 수요발 2갈래(투자 자금이 끊긴다=조달 경색 DT-A / 필요량이 줄어든다=원단위 감소 DT-B) × 공급발 2갈래(기존 3사가 쏟아낸다=증설 DT-C / 신흥이 파고든다=보조금 DT-D) + 전환발(제품 정의가 바뀐다 DT-E). 기존 5개 시나리오가 원인 기준으로 정확히 1:1 분할됨을 확인 — DF-D1의 금융/기술 경로와 공급발 주체 구분이 그대로 하위 분기.
- **신규 요소**: 시나리오별 **가상 헤드라인** 한 문장(수치 미포함, "(가상)" 명기) + **제품별 수요/공급 화살표 그리드**(▼▼~▲▲ 5단계, 움직이는 쪽 열 음영 — 수요발은 수요 열만, 공급발은 공급 열만 움직여 열 자체가 원인 감별이 됨) + **풀어야 할 문제** 5문(전략 자체가 아닌 문제 정의 뉘앙스).
- **색감**: 사용자 요청("최대한 단조롭게")에 따라 적/녹 전면 제거 — 그레이스케일 + 블루 4곳(타이틀 리드·괘선·문제 틱·종합 라벨)만.
- **wiki**: `samsung-impact.md`에 §5(원인 계통·가상 헤드라인·제품 파급 표·문제 정의) 추가 — 슬라이드 v3의 단일 소스. §1~4 S/W 진단은 분석 층으로 유지.
- **파일**: `generate_downturn_impact_slide.py` v3 재작성, PPTX 재생성. LibreOffice 렌더 검수.
- **건너뜀**: dashboard — 무변경(버전 bump 없음).
...
- **파일**: `outputs/presentation/scripts/generate_downturn_impact_slide.py` v2 재작성, PPTX 재생성.
- **검증**: LibreOffice 렌더 검수 2회(힌트 줄 정렬 수정 포함), em-dash 0·확률 미표기 원칙 유지.
- **건너뜀**: wiki·dashboard — 콘텐츠 판정 무변경(시각 압축만)이라 위키 원본이 그대로 단일 소스, 버전 bump 없음.

## [2026-08-25] build | PPTX 전체에서 시나리오 확률 표기 제거 — 발표 층과 분석 층의 분리
- **무엇**: 사용자 지시("PPT만 확률 전부 걷어내고 대시보드는 놔두고")에 따라 발표 산출물(PPTX) 전체에서 시나리오 확률 수치를 제거. 추정 정밀도가 낮은 숫자가 발표 층의 신뢰를 깎는다는 판단. **위키·대시보드·보고서의 확률은 유지** — 분석 층(불확실성 추정 포함)과 발표 층(정성 표기)의 역할 분리.
- **대체 표기**: Main Bet은 "최유력 시나리오"로 정성 표기. 매트릭스 사분면·와일드카드·Side Bet의 확률 칩은 제거(시장 규모 등 나머지 수치는 유지).
- **수정 파일**: `slide-outline.md`(슬라이드 14·15 + 2026-08-25 표기 변경 노트), `scripts/generate_pptx.py`(매트릭스 차트 라벨 5곳·본문 3곳·SO WHAT 1곳 + matplotlib 국문 폴백에 Noto Sans CJK KR 추가), `scripts/generate_team_alignment_pptx.py`(1곳). 재생성: 본편·apple-hig·carbon·team-alignment 4개 덱 + assets 차트. **carbon-design 덱은 생성 스크립트가 없어 바이너리 직접 수술**(확률 칩 텍스트 10곳 제거·빈 칩 도형 4개 삭제·Main Bet 배너를 판정/최유력 스탯으로 대체).
- **재현성 복구**: `template.pptx`가 `*.pptx` gitignore에 걸려 레포에 없어서 프레시 클론에서 덱 재생성이 불가능했다(이번에 발견). 기존 산출 덱에서 슬라이드·관계를 제거해 템플릿을 복원하고 화이트리스트에 추가해 커밋 — 이후 환경에서도 `scripts/generate_pptx.py`가 바로 동작.
- **검증**: 전 PPTX(다운로드 미러 포함) 텍스트 전수 스캔으로 시나리오 확률 패턴 0건 확인, LibreOffice 렌더로 본편 s14·carbon-design s14~16 시각 검수(원본과 비교해 LO 렌더 한계와 수술 영향 분리 확인).
- **건너뜀**: dashboard — 사용자 지시로 무변경(버전 bump 없음). outputs/report — 보고서는 발표 층이 아니므로 확률 유지.
- **병합 확장 (main 병합 시)**: 병합 중 유입된 업스트림 신규 덱 3종 점검 — `downturn-review.pptx`(SP-2 복기 덱)에 조건부 확률(KPI 타일·요약 표·사분면 차트 버블 라벨·리드의 한계 확률·발표자 노트)이 있어 소스(`generate_downturn_review_pptx.py`·`generate_downturn_assets.py`·outline) 수정 후 재생성. 판정 타일은 정성값(최속 전개·최장 전개·감산 유효·최유력·매뉴얼 무효)으로 대체. `ssd-strategy`·`storyline-overview`는 확률 없음 확인. 전 PPTX(노트 포함) 재스캔 0건. `s10 "확률·영향력 종합 1개 선택"`은 와일드카드 선정 방법론 서술(수치 아님)이라 유지.

## [2026-08-25] build | 시나리오별 삼성 영향 진단 슬라이드 1장 + samsung-impact 위키 신설 (v2.42.0 → v2.42.1)
- **무엇**: 사용자 요청("각 시나리오에 대한 삼성전자 영향성 슬라이드 1장. 강점·약점 기반으로 취약점 보완과 강점 활용의 힌트까지만, 전략은 별도 예정이므로 제외, 확률 제거, 시나리오별 중요 분야 통찰 제시")에 따라 SP-2 진단 층 산출물 제작. 위키 헌법에 따라 단일 소스 `wiki/downturn/samsung-impact.md` 신설 후 슬라이드를 합성.
- **wiki**: samsung-impact.md — 강점 4코드(S1 재무 요새·S2 풀라인업·S3 IDM 결합·S4 세대 선행) × 약점 4코드(W1 인증 열위·W2 원가 열위·W3 판단 지연·W4 커밋 관성)를 고정 좌표계로, 시나리오×제품 노출 매트릭스(직격/압박/제한/방어·수혜) + 시나리오별 진단 4요소(승부처·찔리는 곳→보완 힌트·살리는 곳→활용 힌트·전략적 중요 분야). 종합 통찰: **약점은 W3로 수렴(A·B·C·D 공통), 강점 발화는 유형별 분화(급락 S1·침식 S3·공급발 S4·수요발 침식 S2) → 감별이 강점 활용의 전제**. 확률·전략 코드(DP/DR)는 역할 경계상 의도적 미표기(확률은 사용자 지시 — 추정 정밀도 낮음). index.md 2건 등록.
- **outputs**: `outputs/presentation/downturn-scenario-impact.pptx` (1장, 16:9) + 생성 스크립트 `scripts/generate_downturn_impact_slide.py` (python-pptx, `.venv/bin/python`으로 재생성). samsung-memory-ppt-design-skill 규율 적용 — 화이트+Samsung Blue 단일 액센트, 적/녹은 노출 등급 의미 전달에만, 액션 타이틀, em-dash 0, 각주 외 10pt 미만 0, 문서등급 플레이스홀더. `.gitignore`에 최종 산출물 화이트리스트 추가.
- **dashboard (v2.42.1, 패치 = 생성 데이터 갱신)**: 신규 위키 페이지 링크 반영 위해 지식그래프 재생성(`node scripts/build-knowledge-graph.mjs`). updates.js 항목 추가. UI 컴포넌트 변경 없음.
- **검증**: LibreOffice 렌더로 슬라이드 시각 검수 2회(종합 밴드 오버플로 수정), `npm run build` 통과.
- **건너뜀**: outputs/report·기존 25장 slide-outline — SP-1 보고서 서사와 별개의 SP-2 단독 장이므로 구조 무변경. 대시보드 SP-2 탭에 진단 뷰 추가는 미실시(요청 범위 밖 — 필요 시 후속).
## [2026-08-19] build | 한 장 스토리 슬라이드 신설 (일곱 걸음) — 덱 6장

- **요청**: 사용자 구술 스토리라인 7단계 — ① 단품·웨이퍼는 사이클 직격, SSD가 다운턴 대비 ② 솔루션 판매 낸드 비중 90%+ ③ 작년부터 서버 SSD 집중(이익+전환 비용) ④ 서버 점유율 약 29%, 제고 필요 ⑤ 고객 자체 SSD 2027년 ~100EB = 서버 SSD 11% ⑥ 원인(최적화·비용)을 FDP로 충족 ⑦ FDP로 흡수 시 점유율·전환 비용 동시 상승. "데이터는 조사해 채우고 모르면 임시로, 나중에 교체."
- **구현(S0b, 2페이지 삽입)**: 3열(솔루션화 비중 추이 바 60ᵉ→80ᵉ→90%+ / 현재 — 29% 빅넘버·캡티브 11% 누적 바 / FDP — 원인 충족·기대 효과 2종) + ⑦ 결론 스트립. ①~⑦ 번호를 요소에 배선해 스토리 추적 가능. 총 6장, 푸터 재번호.
- **데이터 처리**: 90%·29%·100EB/11%는 사내 수치로 [사내 확인] 표기, 2015·2020 솔루션 비중은 임시 추정ᵉ. **기준 충돌 주의**: 사내 29% vs 외부 매출 기준 1Q26 38.2% — 기준 차이(용량/수량/기간 추정)를 스피커 노트에 명시, 발표 시 기준 명기 필요.
- **QA**: 지오메트리 1차 1건(누적 바 라벨 박스 겹침) → 0건, OOXML validate 통과. outline·index 동기. **건너뜀**: 보고서 본문 무변경(스토리는 기존 논증의 재배열 + 사내 수치 — 수치 확정 시 위키·보고서 반영 검토), dashboard·KG 무변경.
...
## [2026-08-22] build | 다운턴 복기 덱 개편 — DT16 경계 재분류·개조식 톤·다음 다운턴 시나리오 6장·발표자 노트 (v2.46.4)

요청: ① DT16 제외("애매") ② 문체를 명사형 개조식으로("~했다" 서술형이 AI스러움) ③ 다음 다운턴 시나리오를 같은 맥락(발원×속도·역사 통찰)으로 분석한 본편 1장 + 시나리오별 별첨 ④ (추가) 발표자 노트에 함축 표현 보충 설명.

**위키 갱신**: `downturn-history.md` — 제목·요약을 "본편 4건 + 경계 1건"으로 재분류(DT16 = 전체 +2%ᵉ·DRAM 단독, §2에 제외 사유 명시), §4 패턴 1 문구 정합, **§5 「다음 다운턴과의 연결 — 역사 대응물 매핑」 신설**: DT-A=DT19형(단 조달 경색)·DT-B=역사에 없음(경험칙 예외)·DT-C=DT08형(감가 정점=낙폭 확대)·DT-D=DT12형(단 퇴출 함수 부재)·DT-E=DT12 전환 심판대, 각각 계승 문법·함정 + 역사 통찰 3접점(발원=기간의 적용·낙폭 확대의 연장·직전 성공=함정의 다음 회차). scenario-matrix·preparation·response-playbook 인용.

**덱 전면 개편 (6장 → 11장)**: S1 과거 4건 요약(DT16 제거: 타임라인 음영·산점도·표에서 제외, 밴드 문구 정합) + **S2 다음 다운턴 신설**(시나리오 사분면 차트 신규 — DT-A~D 버블·확률·역사 대응물·B→A 전이 화살표·현재 위치·DT-E 노트 / 시나리오×역사×유효 대응×치명 실수 5행 표 / "지금 닫히는 창" DP-1·2·6 유효기간 3타일 / 무후회 밴드 4종) + Appendix 과거 4장 + **Appendix 시나리오 5장 신설**(KPI·역사와의 연결·감별 신호 DX·트리거/대응 문법/대비 체크 DP). 전 장 문체를 명사형 개조식으로 전환, **전 11장에 발표자 노트**(용어·읽는 법·핵심 논리·예상 질문). validate PASS·렌더 QA(사분면 라벨 재배치 1회전).

**패치 v2.46.4** (KG 재생성 — downturn-history↔scenario/preparation/response 링크 추가). index.md 3곳 갱신, outline 재작성, `npm run build` 검증. **건너뜀 사유**: scenario-matrix 등 SP-2 페이지 본문 무변경(매핑은 downturn-history §5에 신설 — 기존 페이지 확률·전략 불변), dashboard SP-2 데이터셋 무변경, 위키 산문 문체는 유지(개조식 전환은 덱 한정 — 위키는 서술형이 표준).

## [2026-08-24] ingest+build | S1 통합(D+N) 전환 + NAND 단독 슬라이드 신설 (v2.46.5)

요청: ① S1 산점도(최악 분기/지속기간)와 표의 대응·결과를 DRAM 단독이 아닌 D+N 통합으로 ② S1 포맷의 NAND 단독 자료.

**소스 신설**: `nand-quarterly-addendum-2026-08-24.md` — 통합 QoQ 산출의 공백이던 4Q08 NAND -19.3%(DRAMeXchange, ASP -32%·빗 +18%)·4Q22 -25%(TrendForce/evertiq) 웹 검증.

**위키 갱신**: `downturn-history.md` §2 — 속도 지표를 **전체 메모리(D+N) 통합 기준**으로 승격: 검증된 D·N QoQ의 연간 매출 비중 가중 평균 산출표(4Q08 -30%ᵉ·4Q10 -14%ᵉ·4Q18 -18%ᵉ·4Q22 -29%ᵉ, 오차 ±1~2%p·4Q10 N 보합 가정 명시) + 비교표 최악 분기 열을 통합ᵉ/D 병기로. 새 통찰: 순간 낙차 최대는 여전히 DT08(-30%ᵉ 금융위기), DT23은 근접(-29%ᵉ)하나 총낙폭·누적 속도(3분기 -58%ᵉ)는 최대.

**덱 갱신 (11장 → 12장)**: S1 산점도를 통합 QoQ로 교체(축·주석·버블=전체 낙폭), 표 대응·결과를 D/N 통합 관점으로("N: SanDisk 시도"·"N: 도시바 30% 감산"·"N 먼저 회복"·"삼성은 NAND 중심" 등). **S2 NAND 단독 뷰 신설**(S1 포맷): NAND 단독 타임라인(하강 연도 강조·DT12 가격 위기·DT16 비동행 주석) + D vs N 낙폭 비교 차트(동조화 진행 시각화, DT16 +14% 비동행 포함) + NAND 관점 5행 표 + NAND 밴드 4종 + 발표자 노트. 이후 장 번호 +1. validate PASS·렌더 QA.

**패치 v2.46.5** (KG 재생성). index.md·outline 갱신, `npm run build` 검증. **건너뜀 사유**: 시나리오 장(S3~) 무변경(통합 전환은 역사 파트 한정), dashboard 데이터셋 무변경, DRAM 단독 수치는 삭제가 아니라 병기(위키·산점도 주석·표 sc2)로 유지.

## [2026-08-24] build | DRAM 단독 뷰 슬라이드 신설 — 3페이지 삽입, 13장 (v2.46.6)

요청: "DRAM만 따로 슬라이드1 포맷으로 만들어 세 번째 페이지에."

**덱 갱신 (12장 → 13장)**: S3 DRAM 단독 뷰 신설(통합 → NAND → **DRAM** → 시나리오 순) — DRAM 단독 타임라인(**DT16 포함 5개 창**: DRAM 렌즈에서는 경계 사례가 정식 조정으로 성립, 비강조 그레이·하강 블루) + DRAM 5점 산점도(분기 검증치 5건 전부 보유: 4Q08 -36·4Q10 -20·4Q15 -9.1·4Q18 -18.3·4Q22 -32.5 — 세 렌즈 중 유일하게 전건 검증) + DRAM 관점 5행 표 + 밴드 4종("진앙은 항상 DRAM"·"퇴출의 무대"·"DT16 = D 단독"·"다음도 진앙은 D: DT-C 캐파·DT-D CXMT") + 발표자 노트(1장 통합 산점도와의 차이 해설 포함). 이후 장 번호 +1(다음 다운턴 4, Appendix 5~13). 자산 2종 신규(dram_timeline·dram_scatter). validate PASS·렌더 QA.

**패치 v2.46.6** (KG 재생성). outline·index.md 갱신, `npm run build` 검증. **건너뜀 사유**: 위키 무변경(DRAM 열 데이터·DT16 D 단독 성립은 §1·§2에 기존 문서화 — 덱은 재배치만), 소스 무변경, dashboard 데이터셋 무변경.

## [2026-08-24] build | 타임라인 음영 발원별 색상화 + 발원·원인 라벨

요청: "다운턴 음영을 수요발/공급발에 따라 색을 다르게, 글도 살짝, 원인도 한두 단어."
...
## [2026-09-02] ingest+build | 배경 슬라이드 ①·② 개편: 컬럼 재배치·QLC 타임라인·RUH 갭 (v2.46.11)

요청: ① 낙폭+원인 한 컬럼 통합, CapEx+AI 인프라 투자 그래프 2컬럼, 대용량 QLC eSSD 용량×출시시점 벤더 비교 3컬럼 ② write-intensive 특성 제거, 현행 FDP RUH vs 요구 200개 그래프, ScaleFlux 팩트체크·출처 명시. **리서치·소스 등재**: qlc-essd-timeline-fdp-ruh-2026-09.md — QLC 타임라인(Solidigm 61TB '23.07 선행→122TB '24.11, 삼성 BM1743 61TB '24.07 후발·122TB FMS'24 전시-미출하, Micron 6600 ION 245TB '26.05 첫 출하, Kioxia LC9 '25 샘플링), FDP RUH 현행 2~8 vs ScaleFlux 200+(NVIDIA CMX 타깃, "NVIDIA 공식 스펙 200개 요구"는 미확인으로 표기 주의 명시), ScaleFlux 7~10+ DWPD 달성 확인(StorageReview '26.07.30), AI 서버 시장 '23 $50B(23%)→'24 $187B(65%, TrendForce). wiki §2.5(QLC 경쟁 시계열·재배분 정량)·§2.6(RUH 갭 항목 신설) 갱신. 배경① 3컬럼 재구성(낙폭+eSSD 점유/CapEx+AI 서버 가치 바/QLC 타임라인 도트 차트), 배경② RUH 갭 바(2~8 슬리버 vs 200+)·ScaleFlux 실증 박스. 렌더 QA 3회(원인 타이틀 축약·'24 줄바꿈·도트 간격). DT_DOWNLOADS desc 갱신, version v2.46.11(패치), knowledge graph 재생성(orphans 0), dashboard build 통과. report/slide-outline은 개발실 전환 덱 전용 생성기 범위라 미해당.
...
## [2026-09-03] ingest | 인터뷰 6번째 추가 — 송용호 부사장(AX/PI센터장) (v2.46.13 → v2.46.14)
- **무엇**: 2026-09-03 진행된 송용호 부사장(AX/PI센터장·前 솔루션개발실장, 녹취록 참석자 2, 약 79분) 내부 인터뷰를 sources 층에 녹취록 원본으로 보존하고, 대시보드 "인터뷰" 메뉴에 앞선 인터뷰와 동일 형식(Executive Summary 포함 16개 섹션·핵심 인용문 4개·다운턴 센싱 3대 관전 포인트 표)으로 구조화해 추가. 패치 v2.46.14 (원격 main v2.46.13 병합 후 재번호).
- **왜**: 사용자 지시 — 사내 임원·전문가 인터뷰 시리즈(신문섭 → 최장석 → 이창수 → **송용호**)의 네 번째 내부 인터뷰. 과제팀이 스토리라인·2차 저지선 아이디어 6건을 정리한 뒤 AX/PI 관점의 자문을 받은 자리.
- **핵심 논지**: (1) 과제 프레임에 빠진 두 키워드 = **품질·원가**. 다음 다운턴의 첫 장면은 대규모 RMA(고객의 학습 효과 — 재고 대신 반품), 빌미는 품질, 멀티 벤더 하위 정리. 멀티이어 계약은 경쟁사가 먼저 깨고 가격을 맞춰주면 100% 안전장치가 아님. (2) 성능은 시스템 계층이 흡수하므로 채택을 가르는 무기가 아니고, 파워는 상쇄 불가한 다른 축. (3) 다운턴 센싱 3대 관전 포인트 — AI 기술이 ROI를 상각할 만큼 계속 진보하는가(OpenAI 광고 진입·IPO는 위험 신호) / 하이퍼스케일러 자금 구조(FCF 마이너스 → 대출 → 지분 매각 → NVIDIA 금융업체화) / 전기(미국 27~28년 쇼티지 가능성, 수급 균형 전후 저전력 요구 급증). (4) 중국 — AI가 증폭한 불균형에서 흑자로 실력을 키우는 정의 차세, 5% 오버서플라이 임계 vs 점유 10%·소비 50% → "중국 공산당이 시장가를 정하는" 만성적 공급과잉 시대. (5) 대응 — 새 시장 도망은 힘듦, 플랫폼 제품 위의 최저 원가 커스터마이제이션, AI 활용은 선택지가 아닌 기본. (6) 조직 체질 — 실행력이 뛰어나 사전 준비를 안 하는 회사(테크노드 갭·캐시플로우 뒷배가 사라짐), 단 한 번도 고객 지향적이지 않았던 회사의 CRM 변화. (7) 니어라인 SSD·PBSSD의 실체는 원가·파워, 사이클 무관 시드 유지 원칙. (8) 보고서 조언 — 다음 세대 리더의 생각, 과감하게 "2035년 만성적 공급과잉 시대" 화두.
- **sources**: `raw-notes/song-yongho-ax-pi-interview-2026-09-03.md` 신설(참석자 구성·과제 맥락 헤더 + ASR 정정 대조 + 자동 전사 녹취록 전문, 원본·불변). `index.md` raw-notes 섹션 갱신.
- **dashboard (v2.46.14, 패치 = 콘텐츠 추가, 동일 탭·스키마)**: `data/interviews.js` INTERVIEWS 최상단에 송용호 인터뷰 추가(16개 섹션·keyQuotes 4). `data/updates.js` v2.46.14 항목. `src/version.js` v2.46.14. `npm run build` 검증.
- **건너뜀**: wiki·outputs·기타 dashboard 데이터는 본 인터뷰가 분석 페이지 수치를 직접 바꾸지 않으므로 미변경(앞선 인터뷰들과 동일 스코프). 인터뷰 인사이트의 위키 환원 후보 — 2차 저지선에 품질·원가 축, EWI 전기 축, 중국 "가격 결정권 이전" 프레임(시나리오 C·D 내러티브), 멀티이어 한계 조건(lta-to-sca-transition) — 는 후속 ingest에서 검토.

## [2026-09-03] ingest | 용어 정정 — 리얼라인 SSD → 니어라인(Nearline) SSD (송용호 인터뷰) (v2.46.14 → v2.46.15)
- **무엇**: 송용호 인터뷰의 SSD 제품 컨셉 용어를 자동 전사 오인식("리얼라이·리얼하게 SSP·리얼라인")에서 **니어라인(Nearline) SSD**로 일괄 정정(`interviews.js` 섹션 14 제목·본문·태그·시사점, 소스 헤더 정정표, `index.md`·`updates.js`·`log.md` 표기). 녹취 전문은 원본 불변으로 그대로 둠. 패치 v2.46.15.
...
## [2026-09-03] build | 내부 인터뷰 인용문 슬라이드 v2 — 인용문 전용 포맷으로 재작성
- **무엇**: `internal-interview-quotes.pptx`를 레포의 기존 인용문 전용 포맷을 승계해 다시 만듦. 참조: `generate_pptx.py` Focal Issue(대형 인용부호 + 인용문 블록)·클로징(큰 인용문 + 출처 한 줄), `generate_dev_transformation_summary.cjs`(인용 카드 + "이름, 출처(일자)" 출처 라인).
- **왜**: 사용자 피드백 — "인용문 전용 슬라이드 포맷이 많이 있다, 그걸 참조해서 다시". v1은 3단 비교 카드에 프레임 라벨·해설·과제 반영 문단이 붙어 인용문이 주인공이 아니었다.
- **v2 문법**: 컬럼마다 대형 인용부호(72pt, Samsung Blue) → 인용문 16pt 볼드(주인공) → 헤어라인 → 이니셜 원 + 화자명·직책·인터뷰 일자. 카드 채움·번호·프레임 라벨·과제 반영 문단 제거(과제 반영은 발표자 노트로 이동). 컬럼 간 세로 헤어라인, 리드는 한 줄.
- **검증**: LibreOffice 렌더 2회(인용문-출처 간 여백 과다 → 인용 영역 3.05→2.45in, 직책·일자 줄 분리로 각주 충돌 해소). em-dash 0, 문서등급 플레이스홀더 유지. 대시보드·위키 무변경.

## [2026-09-06] build | 향후 계획 2장 덱 신설 — 전략 3 FDP 4주 계획 + 4대 전략 종합 타임라인

요청: 경영진 보고 후 남은 기간(9/7~10/30)의 향후 계획 슬라이드. 추석 연휴·발표자료 준비 기간을 빼고 검토 4주로 압축. 세 검토 축은 대화로 확정 — ① 현실 인식(인터뷰: 사내 FDP 기술·고객 관계 현황, 보고서 사내 주장·추정치 확정) ② AI 워크로드(조사·기술검토: KV Cache 오프로드 등에서 FDP가 통하는가·난이도·차별화 여지, 해자 판단은 이 축으로 흡수) ③ 실행 전략(전략구상: 고객 접근·협력 구조 + 개발실 조직·개발 체계 준비안). 인터뷰는 1차(2주차 현황)·2차(4주차 결론 검증)로 세 축 공용.
**신규**: `outputs/presentation/future-plan-outline.md`(기획서·일정 전제·축별 주차 표) + `scripts/generate_future_plan_pptx.py` → `future-plan.pptx` 2장(미커밋). 슬라이드 1 = 3축×4주 타임라인 + 산출물 열 + 수렴 밴드, 슬라이드 2 = 4대 전략 종합(전략 1·2·4 점선 플레이스홀더, 전략 3 FDP 3레인). 디자인 시스템은 `ssd-strategy.pptx` 승계. 렌더 검증은 PowerPoint PDF 내보내기 → PNG로 확인.
**건너뜀 사유**: 위키·보고서 본문 무변경(계획 슬라이드는 부록 D 과제의 일정 배치일 뿐 전략 수치 불변), dashboard 무변경(version bump 없음), index.md 무변경(outputs는 인덱스 대상 아님).

## [2026-09-06] build | 향후 계획 덱 v2 — 키워드형 압축·인터뷰 1회(9월 말)·추석 열 제거

피드백: "글이 많다. 추석 표기 불요. 인터뷰는 9월 말 한 번. 어떤 문제를 풀기 위해 무엇을 할지 핵심 키워드만. 2번째 슬라이드는 나머지 전략 자리를 고려해 공간을 줄일 것."
**반영**: `generate_future_plan_pptx.py` 전면 재작성 — 4주 열(9/7·9/14·9/21·9/29) 동일 폭, 3·4주 경계에 세로 분기선 + ◆ "담당임원 인터뷰 · 9월 말"(앞 3주 조사·설계 / 4주차 확정·결론), 셀은 키워드 1~2줄, 좌측 라벨은 축명·문제 한 줄·방법 태그. 슬라이드 2는 4행 동일 높이(1.42in), 전략 3 FDP 셀에 ①②③ 세 줄 키워드. `future-plan-outline.md` 일정 전제·구성 표 v2 갱신. 위키·dashboard 무변경(사유: 계획 슬라이드 전용).

## [2026-09-06] build | 향후 계획 덱 v3 — 간트형(행=축·열=주·막대=기간·하단 범례)으로 재구성

피드백: 사용자가 참고 이미지(월 열 × 단계 행 간트, 화살 막대, 하단 색 점 범례) 제시 — "이런 형태로는 안될까?"
**반영**: `generate_future_plan_pptx.py` v3 — 8주 열(9/7~10/26) 교대 음영 + 10/12~ 발표자료 준비 틴트, 오각형 막대에 키워드, 행 색은 Samsung Blue + 그레이 계열(슬레이트·블루·중간 회색·블루 틴트)로 스킬 팔레트 준수, 3·4주 경계 인터뷰 분기선·◆, 하단 범례 4열(문제 한 줄 + 산출물). 슬라이드 2는 전략 1·2·4 점선 플레이스홀더 막대 + 전략 3 네 막대. 렌더 검증(PowerPoint PDF → PNG). `future-plan-outline.md` 구성 표 v3 갱신. 위키·dashboard 무변경.

## [2026-09-06] build | 향후 계획 덱 v4 — "무엇을·왜" 중심 개편 (세 열 논증 + 4전략 표)

피드백: "일정표 형태가 되니 깊이가 없어 보인다. 무엇을 왜 하는지가 드러나게 개편, 글자수는 늘려도 됨."
**반영**: `generate_future_plan_pptx.py` v4 — 슬라이드 1은 세 검토 축을 세 열 카드로 세워 질문 → 왜(v1.1의 남은 약점: 사내 근거의 급·AI 워크로드 효과의 조건부성·실행안 미검증) → 무엇을(불릿 3) → 결과(산출물·보고서 반영) 구조. 일정은 하단 한 줄 스트립(9/7 · 9월 말 인터뷰 · 10월 초 v1.2 · 10/12 발표자료)으로 강등. 슬라이드 2는 4전략 × (왜·무엇을·산출물) 표, 전략 3만 채움. 근거 문장은 비판적 검토서(F-2·T-1·T-2·S-5)와 fdp-host-ssd-platform §4.5·4.6·2.6에서 가져옴. 렌더 검증 후 텍스트 길이 2회 조정(넘침 제거). `future-plan-outline.md` 구성 표 v4. 위키·dashboard 무변경.

## [2026-09-06] build | 향후 계획 덱 v5 — 슬라이드 2를 상단 핀 타임라인 + 하단 전략별 4열로 개편
...
## [2026-09-17] ingest+build | QLC eSSD 전략 보고서·위키 3페이지·3장 덱 신설 (v2.46.17 → v2.46.18)

- **무엇**: 브리프 피드백 10건(자매편·QLC eSSD 한정·니어라인 HDD 대체 제외·KV cache 캐시 티어 타깃·2030 시계·외부 공개 정보만·Phase 1 디바이스/2 워크로드 최적화/3 co-design·투자 상한 없음·고객 실명·로고 허용·디자인 중시)을 반영해 리서치 → 모델 → 위키 → 보고서 → 그래프 → 덱 체인을 완주.
- **sources 4건**: `qlc-essd-history-2022-background-2026-09.md`(2018~2026 타임라인, 2022 준비기 논증, 2022 vs 2026 비교표) · `qlc-essd-market-size-forecast-data-2026-09.md`(TrendForce 분기 eSSD 매출 2022~2Q26 $37.59B, QLC 30EB(2024) 앵커, DC NAND 295→909EB, $/TB 인덱스, KV cache 2027 75~100EB·CMX 100EB) · `kv-cache-qlc-tech-stack-vendor-capability-2026-09.md`(5계층 스택, 벤더 9사 Phase 역량, QLC 0.075~0.6 vs TLC 1~3 DWPD, CacheLib WAF 3.22→1.03 ✅) · `execution-benchmarks-sw-capability-customer-collab-2026-09.md`(SK hynix AI Company $10B, Solidigm 반전, Astera↔Pliops $70M/60명, SV 보상 격차 1.5~1.8배, 인수 후보·협업 메커니즘·재무 수단). **제약 고지**: 세션 프록시가 외부 원문 열람을 차단해 대부분 🟡(검색 인용) 등급, ✅는 TrendForce 실측·GitHub 확인·CacheLib 문서에 한정 — 후속 lint에서 1차 페이지 재확인 권고.
- **wiki 3건 신설**: `concepts/qlc-ssd-market.md`(3기 비교표·2022 배경·2022~2030 모델: QLC 53→550EB, 비중 20→55%, 매출 정상화 기준선 $32→37B·쇼티지 상단 $65B, KV 티어 QLC 조건부 175EB, 가정표·민감도·삼성 몫 시나리오) · `strategies/qlc-workload-capability-phases.md`(Phase 정의·요구 역량·삼성 현황·갭·참여 스택 6순위·협업 기업 10) · `strategies/qlc-execution-strategy.md`(5축: 전략·조직·인사·문화·재무, 볼드 3티어 90일/1년/3년, 가성비 판정, 층별 협업 제안·유인 설계 6·시퀀싱·KPI·리스크). 기존 `fdp-host-ssd-platform`·`dev-org-transformation`·`ssd-ufs-market`·`nand-process-transition`에 Update 절 추가. FDP 표기는 정의 1회 후 "배치 표준"으로 자제(사용자 결정).
...
## [2026-09-17] build | QLC 덱 v3 — 슬라이드 2 계층별 기술 명시(협업 기업 행 제거), 슬라이드 3 FDE·SCA 두 트랙 + 선례 카드

피드백: "슬라이드 2에서 협업 기업 부분을 빼고 Phase 1·2·3를 키워 각 레이어별 구체 기술을 표현 / 슬라이드 3에 전략적 협약으로 워크로드를 공유받는 전략을 넣고 Micron↔Anthropic SCA를 선례로, 파견 엔지니어는 FDE로 표현하고 Palantir 선례와 파급력을 간략히, 그림으로."
- **S2**: 협업 기업 로고 행 제거로 확보한 공간에 Phase 스택을 5.06in 높이로 확대. 새 헬퍼 `stack2()`로 각 층에 이름 + 구체 기술 두 줄(응용 vLLM·SGLang·TensorRT-LLM / 관리자 Dynamo KVBM·LMCache·Mooncake·FlexKV / I/O NIXL·GPUDirect Storage·io_uring·SPDK·xNVMe / 커널·플랫폼 Linux 6.16 write streams·XFS·f2fs·CMX(DOCA Memos) / SSD RUH 200+·2Tb QLC·NVMe KV·텔레메트리·액체냉각). Phase별로 우리가 닿는 층에서 하는 일을 기술로 명시(Phase 2: io_uring·GDS 백엔드 write stream 부착, RUH 정책·WAF·유효 DWPD 실측; Phase 3: 캐시 관리자 4종 플러그인 메인라인 머지, NIXL·xNVMe 기본 백엔드, DOCA Memos↔배치 표준 공동 정의, 공용 TCO 모델). 상주 아이콘 라벨을 "FDE 상주"로.
- **S3**: 액션 타이틀 "FDE로 고객 시스템 안에 들어가고, 전략적 협약으로 워크로드를 받습니다. 선례는 이미 있습니다". 두 화살표(① FDE가 들어간다 / ② 전략적 협약(SCA)으로 워크로드가 온다) 사이에 **선례 카드 2**: Palantir FDE(엔지니어 3명 → 고객 현장 미니 그림, 코드명 Delta·성과 평가, 파급: 고객 락인·640% 주가 수익률의 원천으로 회자·Anthropic/OpenAI GTM 채택) · Micron↔Anthropic SCA 2026-06(공동 설계·다년 공급·운영 통합·자본 4블록, SCA 16건·최소 매출 약 $100B·예치금 $22B, 삼성·SK 계약엔 공동 설계 조항 없음 → 우리가 제안). 대상 로고는 고객 시스템 박스 하단으로. 5축·핀·결정 요청 문구를 FDE·SCA 용어로 통일.
- **위키·보고서 동기화**: `qlc-execution-strategy.md` §2.0에 "두 목표의 선례와 파급"(FDE = Palantir, SCA = Micron↔Anthropic) 추가, 보고서 §6.0·PPT 압축 맵 v3, `qlc-ssd-strategy-outline.md` v3 구성표. 렌더 QA 2회(Phase 2 주석 열 폭·선례 카드 넘침·핀 텍스트 줄바꿈 수정).
- **건너뜀**: 대시보드 무변경(버전 bump 없음), 그래프·모델 무변경.

## [2026-09-17] build | QLC 덱 v3.1 — 슬라이드 3 인사·문화 축 보강(시스템 SW 전문가 채용·양성, 미주 현지 채용, 오픈소스 생태계 주도)

피드백: "슬라이드 3 문화에 오픈소스 생태계를 주도하는 기업 문화가 필요하고, 내부 인재와 기술은 고객의 시스템을 잘 이해하는 시스템 소프트웨어 전문가 채용·양성으로 해당 조직 강화가 필요하며, 미주 고객 협업을 위한 현지 채용 증대도 필요."
- **S3**: 5축 타일을 축당 3줄로 확장(F_H 0.96→1.10). 조직 = SV 자회사 / FDE Pod 상주 / **시스템 SW 조직 강화**, 인사 = **시스템 SW 전문가 채용·양성 / 기준: 고객 시스템 이해 / 미주 현지 채용 확대·별도 보상**, 문화 = **오픈소스 생태계를 주도 / 메인테이너·커미터 배출** / 업스트림 우선·실측 공개. 결정 요청 ①에 "미주 현지 채용 승인" 추가(밴드 19pt), 삼성 개발실 박스 하단 문구·발표자 노트 갱신. 렌더 QA 2회(밴드 줄바꿈 수정).
- **위키** `qlc-execution-strategy.md`: 3차 재점검 주석. §2.2에 "강화할 조직은 시스템 소프트웨어 조직" 문단(배치 표준 OSS 자산은 5사 중 최다이나 KV 스택 연결 0 — kv-cache 소스 §3·§4). §2.3 인사 축을 여섯 장치로 재편 — ① 시스템 SW 전문가 채용(기준 = 고객 시스템 이해, MSL 직무 정의 재작성) ② 미주 현지 채용 증대(자회사 정원 70%+ `[사내 확인]`, MSL San Jose 거점, Solidigm 공동 CEO·SK hynix AI Company 선례) ③ 내부 양성 트랙(업스트림 PR 1건 수료 조건) ④ 별도 보상 ⑤ 앵커+상주 로테이션 ⑥ 호명되는 전문가. §2.4 문화 축을 "오픈소스 생태계를 주도"로 격상(레퍼런스 프로젝트 운영·메인테이너 배출·SNIA/OCP 의장·기여 시간 보장, Intel OTC 조건). 3티어에 I-8·I-9·Y-8·L-6, 가성비 2행, KPI(현지 채용 비율·메인테이너 수 등), 리스크 2행 추가.
- **보고서·기획서·인덱스**: 보고서 Executive Summary 넷째·§6.2 조직/인사/문화·§6.3·§6.5·PPT 압축 맵 동기화, `qlc-ssd-strategy-outline.md` v3.1, `index.md` 두 항목 요약 갱신, `dev-org-transformation.md` Update 절에 한 줄.
- **건너뜀**: 그래프·모델 무변경. 대시보드는 지식 그래프 재생성 결과에 따라 결정(아래 항목 참조).
- **대시보드**: 지식 그래프 재생성으로 `knowledgeGraph.js` 노드 요약 2줄 변경(노드 107·엣지 585·orphans 0 유지) → `version.js` v2.46.18→**v2.46.19**(패치), `updates.js` 항목 추가, `npm run build` 통과.

## [2026-09-17] build | QLC 덱 v3.2 — 슬라이드 3 전략·재무 타일과 타임라인 제거, 결론 밴드를 3장 요약 한 문장으로

피드백: "슬라이드 3에서 전략·재무 부분과 하단 타임라인을 제거하고, 최하단 파란 배경 글은 슬라이드 3개를 한 문장으로 요약하는 메시지로 대체."
- **S3**: 5축 타일 → **조직·인사·문화 3축 타일**(폭 1/3, F_H 1.34, 축당 3줄 — 조직: 실리콘밸리 소프트웨어 자회사 / FDE Pod 고객 상주 / 시스템 소프트웨어 조직 강화, 인사: 고객 시스템을 아는 시스템 SW 전문가 채용·양성 / 미주 현지 채용 확대·고객과 같은 시간대 / 별도 보상·상주 로테이션, 문화: 오픈소스 생태계를 주도하는 문화 / 메인테이너·커미터 배출·레퍼런스 프로젝트 운영 / 업스트림 우선·KV 실측 최초 공개). 90일·1년·3년 핀과 결정 요청 밴드 제거. 결론 밴드(19pt 두 줄): "니어라인 HDD 대체가 아니라 추론 캐시 티어를 QLC로 가져가려면 디바이스에서 고객 시스템 안까지 세 단계를 올라가야 하며, 그 길은 FDE와 전략적 협약, 그리고 이를 해낼 사람과 문화가 엽니다"(1장·2장·3장 순). 리드 "조직·인사·문화가 그 길을 엽니다", 노트 갱신. `band()`에 `\n` 의미 단위 줄바꿈 지원(FDE/와 분리 방지).
- **동기화**: `qlc-ssd-strategy-outline.md` v3.2, 보고서 PPT 압축 맵, `index.md` 덱 항목. **위키는 무변경** — 실행 전략 페이지의 5축·3티어·결정 요청은 그대로 유지하고 덱만 3축으로 압축(전략·재무·타임라인은 §2.1·§2.5·§3 참조).
- **건너뜀**: 대시보드 무변경(위키 링크 변경 없음 → 지식 그래프 동일, 버전 bump 없음), 그래프·모델 무변경.

## [2026-09-17] build | QLC 덱 v3.3 — "고객은 왜 QLC를 원하는가"를 3장 관통 축으로, 니어라인 HDD 언급 제거

피드백: "슬라이드 전반에 고객이 왜 QLC SSD를 원하는지에 대한 통찰이 빠져 있다. 분석해서 배경부터 결론까지 일관되게 잇고, 니어라인 이야기는 중요하지 않으니 빼라."
- **분석(위키)**: `qlc-ssd-market.md` §3.4 「고객이 QLC를 원하는 이유 — 세 국면을 관통하는 불변과 변화」 신설. 불변 = 고객이 사는 것은 비트가 아니라 **계산 옆에 둔 가장 싼 바이트(TB당 원가·전력·공간)**(초기: 데이터 6배·HDD TB당 대역폭 붕괴 → 플래시 용량 계층, 1U 1PB·TCO -47%, 읽기 중심이라 0.3~0.6 DWPD로 충분 / 현재: 추론 서버 전력·공간 병목 → 30EB·245TB / 향후: KV 캐시가 HBM·DRAM을 넘쳐 SSD로, GPU당 동시 사용자 10배·TTFT). 변화 = 가장 빨리 크는 수요가 처음으로 **쓰기 많은 티어**에 있어 내구성이 문이 됐고, 내구성은 미디어가 아니라 고객 시스템의 데이터 배치로 풀린다 → Phase 1·2·3 → FDE·SCA. 출처: 연혁 소스 §2.2·§2.3, 시장 소스 §2.1·§4, kv-cache 소스 §1·§3, 오프로드 생태계 소스.
- **S1**: 킥커 "배경: 고객은 왜 QLC를 원하는가", 타이틀 "고객이 사는 것은 계산 옆의 가장 싼 바이트이고, 그 자리가 추론 캐시 티어로 옮겨갑니다", 3기 스트립을 **고객이 산 것(키워드) · 왜 · 조건(내구성) · 삼성** 4행으로(E_H 2.02→2.34, 타임라인 5.44, 그래프 C_Y 6.16·C_H 3.14), 결론 "고객은 추론 캐시 티어에서도 QLC의 경제성을 원합니다. 문은 내구성이고, 디바이스만으로는 못 엽니다". 니어라인 HDD 대체 문구 제거.
- **S2**: 타이틀 "내구성은 고객 시스템에서 풀리므로, 디바이스에서 고객 시스템 안까지 세 단계로 올라가야 합니다", 리드에 10~40배 갭의 원인(고객이 원하는 QLC 경제성 vs 캐시의 쓰기), Phase 산출물 줄을 **고객이 얻는 것**(정격 내 QLC 원가·전력 / 자기 워크로드의 수명 보증 / 시스템 위의 TCO 보증)으로.
- **S3**: 결론 문장 "고객은 계산 옆의 가장 싼 바이트를 사려고 QLC를 택했고, 그 자리가 쓰기 많은 추론 캐시로 옮겨가 내구성을 고객 시스템 안에서 풀어야 하며, 그 길은 FDE와 전략적 협약, 그리고 이를 해낼 사람과 문화가 엽니다". 세 장 노트 갱신.
- **동기화**: 보고서 Executive Summary 둘째·§2.4 신설·PPT 압축 맵, `qlc-ssd-strategy-outline.md` v3.3, `index.md`. 니어라인 HDD 대체 범위 결정(사용자 2026-09-17 피드백 3)은 위키 §3.1·보고서 §2.3에 유지하고 덱에서만 제외.
- **대시보드**: 지식 그래프 재생성으로 `knowledgeGraph.js` 노드 요약 변경(노드 107·엣지 585 유지) → `version.js` v2.46.19→**v2.46.20**(패치), `updates.js` 항목 추가, `npm run build` 통과. 그래프·모델 무변경.

## [2026-09-17] build | QLC 덱 v3.4 — 용어를 "용량 계층의 TB당 TCO"로 통일, 신문섭·송용호 인터뷰 인용 4건 삽입(전략 신빙성)

피드백: "'계산 옆의 싼 바이트'를 더 오피셜하고 직관적인 기술 용어로(추천 조합 적용). 신문섭 파트너·송용호 센터장 인터뷰 중 고객 시스템으로 들어가야 한다·고객 관계의 중요성·고객이 우리 제품을 어떻게 쓰는지 알아야 한다는 취지의 인용을 슬라이드 적절한 곳에. 늘어난 만큼 다른 텍스트는 줄여도 된다."
- **용어**: 불변 = **용량 계층(capacity tier)의 TB당 TCO**($/TB·W/TB·TB/U — Meta capacity tier·byte density, Solidigm 1U 1PB TCO 논거의 단위), 향후 국면 = **GPU당 컨텍스트 용량·토큰당 비용**(NVIDIA context memory storage 프레임). "계산 옆의 가장 싼 바이트"는 쉬운 풀이로만 남김. 1장 TCO(드라이브·랙 단위)와 2·3장 TCO 보증(시스템 단위)의 층위 차이를 스토리로 명시. `qlc-ssd-market.md` §3.4 용어 문단, 보고서 §0·§2.4, 덱 S1 타이틀·리드·3기 키워드·결론, S3 결론 문장.
- **인용(덱)**: `quote()` 헬퍼 신설. S1 스티커 박스 하단 — 송용호 "부품이 어떻게 쓰일지는 시스템을 설계하는 사람 마음에 있다. 그걸 알았으면 HBM을 진작 준비했을 것이다"(기존 "이익은 비트가 아니라…" 문장 삭제, 키넘버 행 축소). S2 리드 — 신문섭 "승부는 칩을 많이 파는 기업이 아니라, 고객의 아키텍처 안으로 들어가 수요를 함께 설계하는 기업이 가져간다"(기존 갭 설명 리드 삭제, 갭 타일이 대신). S3 리드 — 송용호 "단 한 번도 고객 지향적인 적이 없었다. 진짜 고객 지향이 뭔지 이해하고, 그것을 위한 전략이 필요한 시점이 이미 됐다"(기존 선례 리드 삭제, 선례 카드가 대신). S3 삼성 개발실 박스 하단 — 송용호 "고객의 집에 가서 저녁을 같이 먹는 것, 삼성은 상상할 수 없는 일이지만 SK하이닉스는 한다"(관계 = 워크로드 접근권; 기존 5축 요약 문구 삭제). 노트 3장에 인용 해설.
...
## [2026-09-18] build | QLC 덱 v3.6 — 슬라이드 1·2 문장을 기술 전문 공식 용어로 개편

피드백: "전반적으로 문장의 표현을 기술 전문성을 가진 공식적인 용어로 개편(슬라이드 1·2)."
- **1장**: 킥커 "다운턴 복기에서 도출한 교훈", 타이틀 "다운턴 극복의 결정 요인은 고객 요구 적중이었고, 요구는 고객 시스템 내부에서 먼저 관측됐습니다", 리드 "DT19·DT23 두 다운턴과 QLC eSSD 연혁을 동일 시간축에 배치해 도출한 세 교훈이 이후 세 장의 논리적 전제". 타임라인 라벨: 재고 조정 국면·매출 낙폭 / 최대 낙폭·진앙 / 차기 전환점(e)·가격 정상화 국면 / HBM 조직 축소 / 데이터 배치 표준 비준 / 61.44TB 출시·삼성 대비 12개월 선행 / 비트 출하 30EB / 데이터 배치 규격 미정의 / 비트 550EB·매출 정체. 교훈 카드: 수요 센싱("수요 신호는 발주보다 약 2년 선행하여 고객의 규격·표준·소스 코드에 나타난다")·고객 협업("요구사항 정의 과정에 참여한 공급자가 적중했다")·의사결정 시점("직전 다운턴의 의사결정이 다음 다운턴의 초기 조건을 결정한다"), 행 라벨 역사·현재. 결론 "요구사항은 다운턴 국면에서 정의되었고, 정의에 참여한 공급자가 선점했습니다. 차기 요구사항은 지금 고객 코드에서 정의되고 있습니다".
- **2장**: 킥커 "고객의 QLC 채택 동인", 타이틀 "고객의 구매 기준은 용량 계층의 TB당 TCO이며, 해당 계층은 추론 캐시 계층으로 이동합니다", 리드 "…차기 KV 캐시 계층은 쓰기 집약적이라 내구성이 제약 조건". 카드 라벨 구매 기준·동인·조건·삼성(학습 데이터셋 6배·1U당 1PB·5년 TCO -47% / 읽기 중심 워크로드 0.3~0.6 DWPD 충족 / 전력·공간 제약 / 쓰기 계층은 TLC / KV 캐시 SSD 오프로드·GPU당 동시 사용자 10배 / 쓰기 집약 캐시 / 시스템 SW 연계 공백), 스티커 "모델의 핵심 수치", 결론 "고객은 추론 캐시 계층에서도 QLC의 TB당 TCO를 요구합니다. 제약 조건은 내구성이며, 디바이스 단독으로는 해소되지 않습니다". 노트 2장 전면 개편. 렌더 QA(2022-12 라벨 3줄 넘침 → 2줄, 교훈 카드 행 간격 0.70→0.76).
- **동기화**: 보고서 PPT 압축 맵 1·2행, 기획서 v3.6. 위키·그래프·모델·대시보드 무변경(덱 문체 개편만, 지식 그래프 동일 → 버전 bump 없음). 3·4장은 범위 외로 "캐시 티어" 표기 유지.

## [2026-09-18] build | QLC 덱 v3.7 — 슬라이드 1 교훈 카드 3을 텍스트 대신 도식으로

피드백: "슬라이드 1의 시각적 표현이 부족. 교훈 1·2·3의 텍스트를 줄이고 시각적으로 표현."
- **교훈 1 수요 센싱**: 두 레인 선행 그림 — 고객 규격·코드 레인(2022-12 배치 표준 비준 ●)과 발주·비트 출하 레인(2024 30EB ●) 사이 화살표 "≈2년 선행", 2026 슬롯은 빈 원(캐시 관리자 코드 규격 미정의 = 선점 구간).
- **교훈 2 고객 협업**: 61TB QLC 출시 시점 막대(OCP 2022-10 예고 기준 Solidigm 9개월 채움 · 규격 정의 참여 vs 삼성 21개월 테두리 · 12개월 후발) + HBM4 규격 정의 아이콘(SK hynix↔NVIDIA 두 사람 연결 = 공동 정의→주도권 vs 삼성 회색 한 사람 = 후발).
- **교훈 3 의사결정 시점**: 결정→초기 조건 연쇄 2열 3행(DT19 무감산 성공 경험→DT23 국면 오판 / DT19 HBM 조직 축소→DT23 회복기 주도권 상실 / 2026 고객 시스템 진입→2027H2 TCO 보증 수익원, 3행은 파랑 강조).
- 역사·현재 텍스트 행 삭제, 한 줄 명제 축약, "→ n장" 링크 유지. 헬퍼 `lesson_visual_1/2/3`. 렌더 QA 3회(날짜 라벨 줄바꿈·HBM 아이콘 행 하단 침범 → 라벨 축약·카드 2 압축).
- **동기화**: 기획서 v3.7, 보고서 PPT 압축 맵 1행. 위키·그래프·모델·대시보드 무변경(지식 그래프 동일 → 버전 bump 없음).
- (후속) 3장 노트의 "캐시 티어"를 "캐시 계층"으로 통일(덱 전 장 용어 일치). 4장 전체 렌더 재점검 후 main 반영.

## [2026-09-18] build | QLC 덱 v3.8 — 슬라이드 1 교훈 카드를 키 숫자 + 도식으로(문장 제거), 타임라인 압축

피드백(재요청): "슬라이드 1의 시각적 표현이 부족. 교훈 1·2·3의 텍스트를 줄이고 시각적으로."
...
## [2026-09-18] build | QLC 덱 v3.10 — 5장 스토리라인 통일(산식 → 시점 → 수요 → 역량 → 실행), 공식 용어, 시각 스토리 장치

피드백: "1장부터 5장까지 논리의 흐름이 자연스럽게 이어지도록 표현을 수정하고, 모호하고 비유적인 표현은 구체적·전문적 용어로. 불필요한 표현·내용은 과감히 삭제하고 빈 공간은 스토리라인을 시각적으로 표현하는 데 써라."
- **스토리 스파인**: ① 산식(왜 호스트 협력인가) → ② 시점(왜 지금인가) → ③ 수요(고객의 요구는 무엇인가) → ④ 역량(어떻게 해소하는가) → ⑤ 실행(누가 어떻게 실행하는가). 각 장의 결론이 다음 장의 질문이 되도록 리드를 연결 문장으로(2장 "1장의 보상 변수 WAF는 고객이 정의하는 배치 규격", 3장 "2장이 지목한 차기 요구사항의 내용", 3장 결론에 "1장의 산식대로" 참조, 4장 결론 "3단계 진입 수단이 다음 장").
- **시각 장치 3종**(`generate_qlc_ssd_strategy_pptx.py`): 헤더 킥커 → `story_rail()`(번호 원 + 장 이름 + 질문, 밑줄 진행 바) / 결론 밴드 우측 `band(next_step=n)` 다음 장 포인터 / 5장 결론 밴드 → `band_chain()` 5장 요약 체인. 2장 교훈 카드 하단 링크는 레일과 같은 번호 원 칩으로. `deck_kit.py` 재추출(마커 `# ==== S1`).
- **용어 치환**: 버텼고→보상했고, 지킬 차례→보상 변수, 풀리므로·올라가야→해소·역량 확장, 들어가고·받습니다→진입·접근권 확보, 지렛대→변수, 갭→격차, 접점 공백→접점 부재, 수단은 있음→해소 수단 실증, Phase 제목(배치 표준 디바이스 확보 / 워크로드 실측 기반 최적화 / 고객 시스템 내 공동 설계), 고객이 얻는 것→고객 보증 범위, 우리→삼성, 선례는 이미 있습니다→두 수단 모두 업계 선례. 차트 패널 제목(`generate_solution_metrics_chart.py`)도 공식 용어(셀 단독 보정 불가 · 정격 DWPD 하락 · 남은 변수 WAF).
- **삭제·재배치**: 1장 산식·구조 카드의 설명 문장 2개(밴드와 중복) → 구조 카드를 3행으로(3단계 호스트→고객 시스템 공동 설계 = 본 덱 2~5장, `solution_ladder_slide.py` v1.2·단독 덱 동반 갱신) / 2장 3기 띠(3장 스트립과 중복) → 교훈 도식 영역 확대 / 4장 범례 행 → Phase 1 옆, Phase 1 옆 문구 삭제 → 결론 밴드 신설 / 5장 Palantir 카드 문장 축약, 한 문장 결론은 노트로 이동. 렌더 QA(NanumGothic 5장 + 단독 1장): BER 패널 제목 축약, 포인터 폭 3.4in·12.5pt, 4장 타일 폭 조정.
- **동기화**: 기획서 v3.10(구성표·헬퍼), 보고서 PPT 압축 맵(스토리 레일 표기), 해법 사다리 보고서 부록 B v1.2, 위키 `qlc-ssd-market.md` §3.4 덱 표기 원칙(스토리 스파인·시각 장치)·`solution-ladder-component-to-system.md` 산출물 줄, `index.md` 3항목. 지식 그래프·대시보드는 아래 항목.
- **대시보드**: 지식 그래프 재생성(위키 §3.4·산출물 줄 문구 갱신, 노드 108·엣지 597·mutual 111) → `version.js` v2.46.25→**v2.46.26**(패치), `updates.js` 항목, `npm run build` 통과.

## [2026-09-18] ingest+build | 해법 사다리 v1.3 / QLC 덱 v3.11 — 1장 이관의 3단계 정정(ECC 완결 → SSD 단독 최적화 부분 성공 → 호스트 공동 설계 유일)

피드백: "1단계는 ECC가 맞고, 2단계는 SSD가 워크로드에 최적화하려는 시도를 했으나 고객 워크로드를 완벽히 감지할 수 없어 QoS·성능 개선 효과는 있었지만 WAF를 크게 줄이지 못했다. 그래서 3단계 호스트 시스템 co-design으로 WAF를 1에 가깝게 만드는 것이 QLC로 요구 DWPD를 만족하는 유일한 방법이다."
- **수집·팩트체크**(소스 §8, F36~F40): Multi-streamed SSD(삼성 HotStorage'14, NVMe Streams 채택 미미 🟡) · AutoStream(삼성 SYSTOR'17, 런타임 워크로드 감지 자동 스트림, 평가 워크로드 WAF −60%·성능 +237%까지 — 워크로드 의존 🟡) · FTL 핫/콜드 분리(특허 US20160139812A1·US11068197·ASA-FTL, 추정 정확도 한계, 실 캐시 워크로드 WAF 3.22 = 디바이스 단독 ≈3 🟡) · NVMe IO Determinism/NVM Sets/PLM(NVMe 1.4 2019, 테일 지연 QoS 제거·WAF 무관 🟡) · FDP 전제(삼성 블로그: 데이터 생애주기는 호스트가 안다 ✅). 원문 열람 차단 유지, WebSearch 요약 기반.
- **위키**: `solution-ladder-component-to-system.md` §2.5 "두 단계의 대칭" → **이관의 3단계 표**(1 ECC 완결 / 2 SSD 단독 워크로드 최적화 2014~2019 부분 성공 / 3 호스트 시스템 공동 설계 2022~ = 유일한 경로). `qlc-workload-capability-phases.md` QLC 미디어 관리 행의 "SLC 캐시·GC 정책으로 WAF ≈1 유지"를 "배치 표준 적용 시 WAF ≈1(디바이스 단독은 ≈3)"로 정정(모순 제거).
- **슬라이드**(`solution_ladder_slide.py` v1.3, 단독 덱·QLC 덱 1장 공용): 구조 카드를 이관의 3단계로 재구성(행별 충족/미충족 상태, 칩 who/what/sub 3줄, 2단계 중간 칩은 연파랑 = 부분 성공), 타이틀 "ECC가 RBER을 보상했듯 P/E 감소는 WAF로 보상해야 하며, 이는 호스트 공동 설계로만 가능합니다", 리드에 2단계 사실, 결론 "QLC로 요구 DWPD를 충족하는 경로는 호스트 시스템 공동 설계뿐". 차트 ④ 패널(`generate_solution_metrics_chart.py`): ② SSD 단독 최적화 브래킷(WAF ≈3 유지) vs ③ 호스트 공동 설계 표기. QLC 덱 5장 요약 체인 1번 칩 갱신. 렌더 QA: 3단계 행 제목 축약, 화살표·라벨 위치 조정.
- **보고서·문서**: 해법 사다리 보고서 §2.5 3단계 서술·부록 A F36~F40·부록 B v1.3, QLC 보고서 PPT 압축 맵 1행, 기획서 v3.11, `index.md` 5항목.
- **대시보드**: 지식 그래프 재생성(위키 §2.5·phases 행 문구 갱신) → `version.js` v2.46.26→**v2.46.27**(패치), `updates.js` 항목, `npm run build` 통과.

## [2026-09-18] ingest+build | 해법 사다리 v1.4 / QLC 덱 v3.12 — 1장에 병렬 축(SSD당 다이 수·다이 고장률) 추가

피드백: "SSD에 들어가는 NAND 다이 수의 경향성도 봐야 한다. 다이 수가 늘수록 다이 고장으로 함께 버려야 할 다이가 늘어, SSD 고장률을 일정 수준으로 유지하려면 다이 고장률을 계속 낮춰야 하는 부담이 생겼고 기술적 한계에 봉착 — 칩 면적↔수율과 같은 원리. SSD 내부 RAID·여분 다이 교체 해법이 발전하는 방향이고, SSD 수준에서 해결하지 못하면 상위 계층의 도움이 필요할 수 있다. 첫 슬라이드에 포함."
- **수집·팩트체크**(소스 §9, F41~F47): JESD218 FFR ≤ 3% 클라이언트·엔터프라이즈 공통(✅) · SSD당 다이 수 환산 — S3700 800GB 16패키지×8다이×64Gb = 128(✅ 분해 리뷰), P4510 8TB ≈144·P5316 30.72TB ≈256·61.44TB ≈512(⚠️ 용량÷다이 밀도), Kioxia LC9 245.76TB = 2Tb BiCS8 32다이 스택 × 32패키지 = 1,024(✅), Micron 6600 ION 245TB 2026-05 출하 · 고장 모델 (1−p)^N ≈ 수율 Y = e^(−AD)(✅ 모델) · 다이 패리티(Cai 외 arXiv 1808.04016 슈퍼페이지 XOR 패리티 다이 ✅, Micron RAIN 🟡) · 삼성 Fail-in-Place(PM1733/1735 2019, 다이 고장 시 재배치·플레인 4GB/다이 8GB 감량 ✅) · OCP Datacenter NVMe SSD 사양 SMART C0 XOR 복구 카운트(🟡) · Microsoft Hyrax 플랫폼 fail-in-place(✅, SSD 적용은 해석 ⚠️). OCP 사양의 별도 "fail-in-place" 요구 조항은 검색으로 확인하지 못해 주장하지 않음.
- **위키** `solution-ladder-component-to-system.md` §2.5: 병렬 축 문단 + 내구성 축·신뢰성 축 비교표(요구 고정 / 단품 악화 / SSD 계층 해법 / 상위 계층).
- **슬라이드**(`solution_ladder_slide.py` v1.4): 차트 높이 6.4→5.4in(글자 배율 1.15로 보정, `generate_solution_metrics_chart.py` FS), 하단 전폭 **병렬 축 스트립** — 라벨(요구 FFR ≤3% 고정) + 미니 막대(다이 수 128→144→256→512→1,024) + 칩 3(다이 → SSD 내부·현재 해법(연파랑) → 상위 계층·한계 시(점선)). 3단계 카드는 차트 하단에 맞춰 축소(칩 0.74in). 결론 1행에 FFR·다이 수 추가, 출처 축약. 렌더 QA: 밴드 3줄 넘침·스트립 보조문·출처 줄 정리.
- **보고서·문서**: 해법 사다리 보고서 §2.5 병렬 축 문단·부록 A F41~F47·부록 B v1.4(스트립 행), QLC 보고서 PPT 압축 맵, 기획서 v3.12, `index.md` 5항목.
- **대시보드**: 지식 그래프 재생성(위키 §2.5 병렬 축 문구) → `version.js` v2.46.27→**v2.46.28**(패치), `updates.js` 항목, `npm run build` 통과.

## [2026-09-18] build | QLC 덱 v3.13 / 해법 사다리 v2.0 — 1장 레이아웃 zero-base 재설계(이관 매트릭스)

피드백: "슬라이드 1 레이아웃을 제로 베이스에서 다시 만들자."
- **진단**: v3.12의 1장은 2×2 차트·산식 카드·3단계 카드·병렬 축 스트립·밴드 등 8개 블록이 병렬 배치돼 읽는 순서가 없고, 두 축(내구성·신뢰성)이 시각적으로 대응하지 않았다.
- **재설계**(`solution_ladder_slide.py` v2.0): 페이지 구조 자체가 논지가 되는 **이관 매트릭스** — 열 = 이관 단계(요구·고정 → 단품 지표·악화 → SSD 계층의 보상·1단계 ECC 완결·2단계 단독 최적화 부분 성공 → 호스트·시스템 계층의 보상·3단계 공동 설계·본 덱), 행 = 내구성 축 / 신뢰성 축, 행 사이 = DWPD 산식 칩(항별 결정 주체가 열과 대응). 셀마다 미니 차트 1개 또는 빅넘버·타일 1개. 헤더 스토리 레일·결론 밴드·다음 장 포인터는 유지.
- **셀 미니 차트** `scripts/generate_ladder_cells.py` → `assets/ladder_cell_{component,dwpd,waf,dies}.png`: 셀 크기와 1:1 figsize로 그려 확대·축소 없이 배치(글자 = 실제 pt). 기존 2×2 차트 `solution_metrics_chart.png`는 보고서 그림으로 유지.
...
## [2026-09-19] build | QLC 덱 v4.0 — 4장 순서 재편(교훈·문제 제기 → 해법 사다리 → 역량 → 실행)

피드백: "첫 슬라이드는 다운턴에서 배운 교훈으로 시작해 고객의 니즈가 AI 수요로 대용량·높은 DWPD로 이동하고 있어 솔루션이 필요하다는 문제 제기. 두 번째는 지금까지 솔루션의 경향(단품이 못 푼 문제는 상위 제품이 풀었고, 이번 문제 WAF는 SSD 수준에서 풀기 어렵다) → 호스트 시스템 수준 솔루션의 당위성. 세 번째부터 솔루션 제안과 실행 방안으로 마무리."
- **구조**: 5장 → **4장**. 1장 = 구 2장(다운턴 교훈) + 구 3장(고객의 QLC 채택 동인)을 한 장으로: 타임라인 + 교훈 카드 3(폭 4.15in로 축소, 도식 폰트·간격 재조정) + 우측 문제 제기 패널(요구의 이동 3국면 / 키 숫자 10배·350EB·10~40배 / "대용량 QLC로 1~3 DWPD를 충족할 해법이 필요" / 송용호 인용). 2장 = 해법 사다리 매트릭스(다음 장 → ③, 본 덱 3~4장 표기, 노트에 1장 문제 제기와의 연결 추가). 3장 = 역량, 4장 = 실행(요약 체인 4칩: 교훈·문제 / 해법 사다리 / 역량 / 실행).
- **문안**: 1장 타이틀 "다운턴의 교훈은 고객 요구 적중이고, AI 수요는 요구를 대용량·높은 DWPD로 이동시킵니다", 리드 "두 다운턴은 요구가 고객 시스템 안에서 먼저 관측됨을 보였고, 지금 그 요구는 추론 캐시 계층의 대용량·높은 DWPD입니다", 결론 "차기 요구는 추론 캐시 계층의 대용량·1~3 DWPD이며 QLC 정격 0.075~0.6으로는 충족되지 않습니다". 스토리 레일 4단계(교훈·문제 / 해법 사다리 / 역량 / 실행), 교훈 카드 연결 칩 ②③④.
- 구 3장의 수요 그래프(`qlc_demand_share_revenue_slide.png`)는 보고서 그림으로 유지, 슬라이드 코드는 git 이력(b4e3f5d~4ce6a1c). 렌더 QA(NanumGothic 4장): 패널 수직 예산·교훈 2 문구 조정.
- **동기화**: 기획서 v4.0(4장 구성표, 이전 5장 이력 보존), 보고서 PPT 압축 맵(4장, 이전 맵 보존), 위키 `qlc-ssd-market.md` §3.4 덱 표기 원칙·§3.5 제목, `index.md` 2항목, `deck_kit.py` 재추출(STORY 4단계). 지식 그래프·대시보드는 아래 항목.
- **대시보드**: 지식 그래프 재생성(위키 §3.4 문구) → `version.js` v2.46.29→**v2.46.30**(패치), `updates.js` 항목, `npm run build` 통과.

## [2026-09-19] build | QLC 덱 v4.1 — 1장 zero-base 재설계(시간축 = 논리축), 타이틀은 문제 제기

피드백: "슬라이드 1 레이아웃을 제로 베이스에서 다시 만들자. 제목에서 결론을 내면 재미가 없다 — 고용량 QLC에서 DWPD를 높이기 위한 솔루션이 필요한 상황이다 정도만 문제 제기."
- **재설계**: 상단 타임라인(2018~2030)의 세 구간과 아래 세 패널을 같은 폭으로 정렬해 과거 → 지금 → 문제로 읽히게 함. 교훈(2018~23) = 3행(키 숫자 2년·12개월·2027H2, 두 줄 설명, 소형 도식, 행 끝에 받는 장 번호 ②③④) / 지금·요구의 이동(2024~26) = from → to 3행(구매 기준·드라이브 용량·요구 DWPD) / 문제·해법이 필요한 상황(2027~30, 틴트) = 10~40배 격차 빅넘버 + 10배·350EB + 문제 박스("고용량 QLC에서 DWPD를 높일 해법이 필요합니다 · 어느 계층이 해결할 수 있는가 → 2장") + 송용호 인용.
- **문안**: 타이틀 "AI 추론 수요는 고용량 QLC에 높은 DWPD를 요구하며, 이를 충족할 해법이 필요한 상황입니다"(결론 아님), 리드 "두 다운턴의 교훈은 요구를 고객 시스템 안에서 먼저 관측하라는 것이었고, 지금 그 요구는 대용량과 높은 DWPD로 이동하고 있습니다", 밴드 라벨 "결론" → "문제". 렌더 QA 통과(숫자 열 폭 1.32in로 조정).
- **동기화**: 기획서 v4.1, 보고서 PPT 압축 맵 1행, `index.md`. 위키·대시보드 변경 없음(산출물만 변경, 버전 bump 생략). 구 v4.0 1장 코드는 git 이력(5675b5f).

## [2026-09-19] ingest+build | QLC 덱 v5.0 — 비판적 리뷰 반영: 6장 재구성, "유일한 경로" 철회·동급 비교, 신뢰성·보증 슬라이드 신설, FDE 선별·개발실 범위

피드백(리뷰 후): "'유일한 솔루션' 표현 수정, OP·보증연수·다이 세대 등 다른 축 언급 · 동급 비교로 과장 없이 · 고객의 QLC 요구 전제 유지 · CacheLib는 정합성 높은 실험으로 대체, 없으면 가설 명시 · 신뢰성 축은 SSD가 풀 문제라 별도 솔루션 슬라이드 · 1·2장 연결 · FDE 필요한 고객 선별해 효과 극대화 · 자회사 제외, 개발실 내부 범위 · 삼성 현 위치 조정 · 수명 보증·SLA 리스크 슬라이드 · 판돈은 락인 점유율, 비용은 개발실 자원 · 결정 요청 불요."
- **수집·팩트체크**(소스 §10, F48~F54): 동급 정격 DWPD(P5336 61TB 0.58 · 6550 ION 61TB 1.0 RDWPD · CM9 1/3 · LC9 245TB 0.3 ✅) → 격차 2~10배 · CHEOPS'25 KV 오프로드 I/O(읽기 2.0GiB/s vs 쓰기 11MiB/s ✅) · VLDB'26 NoWA(호스트 패턴으로 SSD WAF=1 ✅) · 삼성·NVMe FDP 범용 WAF ≈3→≈1(🟡) · **KV 캐시 FDP WAF 실측은 공개 문헌에 없음 → 가설로 명시(⚠️)** · 보증 관행 5년 또는 TBW·DWPD 선도달(✅) · OCP SMART C0로 호스트 WAF 산출(✅).
- **위키**: `solution-ladder-component-to-system.md` §2.5 3단계 행 "유일한 경로" 철회 → "남은 지렛대"; `qlc-ssd-market.md` §3.4 덱 표기 원칙 6장 스파인; `qlc-execution-strategy.md` §2.2 덱 범위 결정(개발실 내부 · FDE 선별 · 자회사 제외 · 판돈·비용); `qlc-workload-capability-phases.md` §6 현 위치 정직 표기.
- **덱 v5.0(6장)**: 1 문제(동급 비교 2~10배, 극단 40배 각주, 교훈 연결 칩 ②⑤④) / 2 해법 사다리(내구성 행만, 리드 연결, 3단계 셀 "요구 도달·조건부", WAF 셀 = 범용·CacheLib·KV 캐시 점선 미실측, 지렛대 스트립 6칩) / **3 신뢰성 신설**(다이 수 막대 + FFR 상한 모델 차트 + SSD 내부 해법 타일 3 + 호스트 관측 점선 + 삼성 과제) / 4 역량(격차 타일 갱신, 현 위치 Phase 1 진행 중·2 준비·3 미착수, 진행 바 60/15%) / 5 실행(고객 선별·협업 채널 표 4행 + FDE 상주 조건 3 + 단계별 통과 조건 90일·12개월·2027H1 + 개발실 내부 3축 + 판돈 캐시 계층 QLC 0→50%·비용 별도 투자 없음; 자회사·별도 보상·결정 요청 제외) / **6 보증·SLA 신설**(유효 DWPD = 정격 × 3/WAF 차트, 리스크 시나리오 3, 보증 설계 원칙 5, 6장 요약 체인). 스토리 레일 6단계, 포인터 폭 3.7in.
...
## [2026-09-19] build | QLC 덱 v5.1 — 2장 ↔ 3장 순서 교체 (신뢰성은 SSD 안에서, DWPD는 SSD 밖에서)
- 사용자 지시: "신뢰성 문제는 SSD 내부 해법으로 해결 가능하지만 DWPD 문제는 내부 해결이 안 되고 고객 협업과 새로운 역량이 필요하다"는 논리로 전개. 스파인을 ① 문제 → ② 신뢰성 → ③ 해법 사다리 → ④ 역량 → ⑤ 실행 → ⑥ 보증·SLA로 재편.
- 1장: 문제 카드에 "고용량화의 두 축 · 다이 8배 → 신뢰성(2장, SSD 내부) · DWPD 격차 → 내구성(3장)" 추가, 결론 밴드 "고용량(다이 8배)과 1~3 DWPD", 교훈 1 링크 → 3장.
- 2장(신뢰성): 타이틀 "…엄격해졌지만, 이 축은 SSD 내부 해법으로 닫힙니다", 리드 "고용량화의 첫째 축", 결론 밴드 2행 "이 축은 SSD 내부에서 닫히지만, 둘째 축 DWPD는 SSD 안에서 닫히지 않았습니다" → 3장 포인터.
- 3장(해법 사다리): 공용 빌더 v2.2에 lead·band_main·notes_tail 오버라이드 추가. 리드 "신뢰성과 달리 DWPD 격차는 SSD 안에서 닫히지 않았습니다…", 결론 밴드 "남은 지렛대 WAF는 SSD 밖 호스트에… 고객 협업과 새로운 역량을 요구합니다" → 4장. 단독 1장 덱(`memory-solution-ladder.pptx`)은 기본 문안 유지·재생성.
- 4장 노트 산식 참조 3장으로, 6장 요약 체인 순서 교체·원칙 ⑤ "감량 운영(2장 FIP 유사)". 스토리 레일 STORY·deck_kit 재추출.
- 동기화: 기획서 v5.1 절 신설(v5.0은 이력), 보고서 PPT 압축 맵 v5.1(2·3행 교체), 위키 qlc-ssd-market §3.4 스파인·solution-ladder 산출물 줄·사다리 보고서 부록 B v2.2, index. 대시보드 updates 항목 + 패치 v2.46.32. 수치·근거 변경 없음(순서·연결 문안만).

## [2026-09-19] build | QLC 덱 v5.2 — 제목 6개를 한 문단으로 개편, 비유 표현 제거
- 사용자 지시: "제목만 연결해도 완성된 논리적 흐름을 가진 문단이 되도록 제목을 전체적으로 개편. 비유적·모호한 표현은 없애고 기술적·전문적 용어 사용."
...
- 각 제목은 30pt 한 줄 폭에 맞춰 렌더 폰트 폭을 실측(NanumGothic Bold 46px, 상한 2,100px)해 다듬었고, 제목이 놓친 논거는 2·3장 리드가 받는다.
- 용어 통일: 지렛대 → 잔여 변수·격차 축소 변수, 닫힌다 → 충족·해소, 워크로드를 여는 → 개방하는, 리스크를 안으므로 → 수반하므로. 덱 본문·결론 밴드·공용 빌더 v2.3(title 오버라이드 추가)·단독 사다리 덱·위키 §2.5·사다리 보고서 부록 B까지 적용.
- 동기화: 기획서 v5.2 절, 보고서 PPT 압축 맵 제목 6개, 위키 qlc-ssd-market §3.4(제목 문단 명시)·solution-ladder, index. 대시보드 updates 항목 + 패치 v2.46.33. 수치·근거 변경 없음.

## [2026-09-19] build | QLC eSSD 전략 보고서 v2.0 — 덱 v5.0~v5.2 논리로 본문 재구성
- 사용자 지시: "지금까지 수정한 내용 기반으로 마크다운 보고서 다시 써주고 메인 반영." v1.0(브리프 질문 10개 답변 구조, 자회사·10~40배·티어)을 덱 6장 순서·제목으로 다시 썼다.
- 구조: 0 Executive Summary(제목 문단 + 장별 요지) → 1 문제(교훈 3 · 요구의 이동 · 동급 비교 2~10배 · 두 축) → 2 신뢰성(FFR 고정 vs 다이 8배 · 상한 모델 · SSD 내부 해법 3 · 호스트 관측 · 삼성 과제) → 3 내구성(이관 매트릭스 · ECC 완결 · 단독 최적화 부분 성공 · 3단계 근거 표 + KV 캐시 미실측 가설 · 격차 축소 변수 표) → 4 역량(격차 4 · Phase 표 · 현 위치 정직 · 산출물·기술 전략) → 5 실행(두 목표 · FDE 선별 표 · 개발실 내부 3축 · 통과 조건 · 판돈·비용 · 협업 기업 · KPI·리스크) → 6 보증·SLA(관행 · 유효 DWPD 표 · 리스크 3 · 원칙 5) → 7 수요·매출 모델(구 3장) → 부록 A(A-33~A-49 추가: F41~F54) · B 용어(잔여 변수·격차 축소 변수·유효 DWPD·FFR·FIP) · C 자산 맵 · D 덱 범위 밖 확장 옵션(자회사·별도 보상·SCA 자본·지분·볼드 안 → 위키 유지) · E 배경 상세(구 1.1~1.4·2.1·2.3) · PPT 압축 맵 유지.
- 용어: 티어 → 계층, 지렛대 → 잔여 변수·격차 축소 변수, "유일한 경로" 삭제. 수치 근거는 사다리 소스 §7~§10(F28~F54)과 기존 QLC 소스 4종.
- 덱 정합: 3장 2단계 셀 차트(`ladder_cell_dwpd.png`)의 격차 라벨 "10~40배" → "2~10배"(동급 비교와 일치, v5.0에서 누락됐던 잔존 표기). PPTX 2종 재생성.
- 동기화: index(보고서 항목 v2.0), 대시보드 updates 항목 + 패치 v2.46.34. 위키 본문은 변경 없음(보고서는 위키 합성 산출물).

## [2026-09-20] build | QLC 덱 시각화 강화판 v1.0 — 별도 PPT 7장, 텍스트 47%, 히어로 그림 7종(SVG/PNG)
- 사용자 지시: "핵심 내용은 유지하면서 텍스트는 절반 이하로, 남는 공간은 시각 요소 강화, 별도 PPT, 논리의 흐름이 쉽게 느껴지도록, SVG·PNG 그림 완성도가 핵심."
- 산출: `outputs/presentation/qlc-ssd-strategy-visual.pptx`(7장 = 요약 스토리 맵 + 1~6장). 생성기 `scripts/generate_qlc_ssd_strategy_visual_pptx.py`(deck_kit 재사용, 스토리 레일 장 이름만, 결론 밴드 1줄 + 다음 장 포인터), 그림 `scripts/generate_qlc_visual_figures.py`(SVG 헬퍼 + Chromium headless 3배 렌더, Noto Sans CJK KR, PIL 폰트 폭 실측으로 겹침 검사) → `assets/visual/qlc_vis_s{0..6}.svg|png`.
- 그림 7종: 스토리 맵(두 트랙 분기·2→6 점선·판돈/비용) / 문제(타임라인·교훈 3·요구 이동 3·동급 정격 막대 vs 요구 밴드·두 축 분기) / 신뢰성(128 vs 1,024 다이 픽토그램·고장률 상한·패리티·여분 다이·은퇴·텔레메트리 메커니즘) / 해법 사다리(3단 계단·DWPD 산식 결정 주체·WAF 실증 막대·격차 축소 변수 칩) / 역량(격차 배지·Phase 카드 진행 바·5계층·트레이스/정책 화살표) / 실행(고객 선별 2축 맵·내부 실행 3축·통과 조건·판돈/비용) / 보증(유효 DWPD 곡선 × WAF 밴드·보증선·리스크·5원칙 흐름).
- 텍스트 분량(공백 제외): 원본 v5.2 pptx 8,493자 + 차트 라벨 약 981자 = 9,474자 → 시각화판 pptx 1,617자 + 그림 라벨 2,841자 = 4,458자(47%). 제목 문단·수치·근거는 v5.2/보고서 v2.0과 동일, 새 수치 없음. 발표자 노트에 원본 설명 압축.
- 렌더 QA: soffice → PNG 7장 육안 검사(겹침·잘림 0). 동기화: 기획서 「시각화 강화판 v1.0」 절, 보고서 부록 C 행, 위키 §3.4 한 문장, index 2행, .gitignore 화이트리스트, 대시보드 updates 항목 + 패치 v2.46.35. 지식 그래프는 링크 변화 없음(재생성 생략).

## [2026-09-22] build | QLC 덱 시각화 강화판 v2.0 — 네이티브 도형 판(편집 가능), 그림은 4장만, 5장 실행은 두 트랙 화살표로 복귀
- 사용자 지적: "꼭 필요한 부분만 SVG·PNG로 해야 하는데 전체가 그림 파일이라 수정하기 어렵다. 실행 부분은 이전 화살표 버전이 더 낫다."
- `generate_qlc_ssd_strategy_visual_pptx.py` v2.0: v1.0 SVG 좌표계(1842 × 625 단위)를 그대로 쓰는 `Canvas` 어댑터로 박스·칩·번호 원·화살표(커넥터 + tailEnd)·표·막대·타임라인·계단·산식·5계층·Phase 카드를 python-pptx 도형·텍스트로 재작성. 그림은 `generate_qlc_visual_figures.py --parts`의 4장(다이 픽토그램 128·1,024, RBER 스파크라인, 유효 DWPD 곡선)만 삽입.
- 5장 실행: v4.0 두 트랙 화살표(삼성 개발실 → ① FDE 상주 → 고객 시스템 5계층 + FDE 아이콘·코드 머지·로고, ← ② 워크로드·규격 접근권·시한 2027H1) 사이에 고객 선별·채널 카드(AI 랩·NVIDIA = FDE / 하이퍼스케일러 = 업스트림·OCP / OEM = 레퍼런스)와 선례 카드(Palantir FDE · Micron↔Anthropic 협약, 삼성·SK 조항 부재 → 선제 제안), 아래에 조직·인사·문화 3축 · 통과 조건(90일·12개월·2027H1) · 판돈(0 → 50%)/비용(별도 투자 없음).
- 렌더 버그 수정: Chromium `--window-size`에 소수가 들어가면 무시돼 작은 그림이 잘림 → 정수 캐스팅 + 최소 창 900 × (h+400).
- 텍스트 분량: pptx 4,593자 + 곡선 차트 라벨 162자 = 4,755자(원본 9,474자 대비 50%). v1.0 전체 그림 7종은 참고용으로 유지. 동기화: 기획서 v2.0 절, 보고서 부록 C, 위키 §3.4, index, 대시보드 updates + 패치 v2.46.36.

## [2026-09-22] build | QLC 원본 덱 v6.0 — 교훈 3건 팩트체크 정정, 신뢰성 ppm 모델, 차트 확대, 보증에서 런타임 대응 기술로
- 사용자 지시 6건(시각화판은 동결, 원본만 개정): RUH 200 제거·요구 16 / 해법 사다리 그래프 확대 + WAF·DWPD 요구 추이 조사 / 신뢰성 ppm·256·512TB·여분 다이·RAID5·RAID6 비교·내부 해법 시각화 / HBM 표현 완화·2028 근거·배치 규격 용어·교훈 1·2 논리·구매 기준 SSD 관점·0.6 기준 격차·수요 추이 / 보증을 WAF 급등 감지·런타임 대응 기술로 전환
- 리서치 6건 수행 후 `sources/articles/qlc-v6-*.md` 6종 신설(신뢰성 ppm·구매 기준/DWPD 이력·WAF 실측·배치 핸들 개수·2028 전환점/수요 경로·규격 교훈 팩트체크). 프록시가 1차 도메인을 차단해 대부분 검색 인덱스 경유(🟡)이며 등급을 원장마다 명시
- 팩트체크로 세 건을 뒤집었다. ① 표준 비준은 선행 신호가 아니다 — ZNS는 2020-06 비준·같은 달 커널 패치에도 6년째 물량 0, FDP는 Meta 프로덕션 코드 머지(2024-01) 후 물량. ② OCP 조달 사양 기고자는 구매자 5사이고 SSD 벤더는 없다 — "규격 정의 참여자가 선점"은 주체가 틀렸다. ③ 차기 전환점은 2028(2027은 수급률 전환, 신증설 비트 도달은 2028)
- 2장 신뢰성: 요구를 OCP MTBF 2,000,000시간(5년 누적 21,662 ppm)으로 두고 다이 수 512→2,133, 보호 없음 43→10 ppm, 단일 패리티 79배·이중 패리티 395배·여분 다이 1,000배 이상 완화를 차트로. 현 수준 역산 200~2,700 ppm과 겹쳐 **512TB급은 이중 패리티 또는 여분 다이 필요** 판정. 내부 해법은 텍스트 타일에서 도해로 교체
- 3장: `solution_ladder_slide.py` v3.0으로 셀 차트를 3.95×2.75in(면적 약 3배)로 확대하고 정격 DWPD 57배↓ 대 허용 기입량 67배↑, WAF before→after 쌍(CacheLib 사용률 100%/50%·XD8·LLM KV 미실측)을 신설. 연도축 WAF 추이선은 사용률 교란 때문에 배제
- 6장: 조건부 보증 → 런타임 감지·대응 기술 확보로 축 이동(목표 감지 1h·회복 4h, 루프 4단계, 기술 요소 T1~T6, 무중단 FDP 재구성이 최대 공백). 보증은 TBW 기준 유지 + 대응 이력 부속으로 축소
- 동기화: 위키 3종 신설(ssd-die-reliability-ppm·essd-purchase-criteria-shift·waf-runtime-response) + 기존 5종 정정(RUH 200+ 전량 제거, 격차 2~5배, RS-9에 신호 판별 기준 추가), 보고서 v3.0, 기획서 v6.0 절, index, 지식 그래프 재생성, 대시보드 v2.46.37
- 시각화 강화판(v2.0)은 사용자 지시로 개정 대상에서 제외했다. 따라서 시각화판은 v5.2 내용 기준이며 원본과 수치가 다르다(의도된 차이)

## [2026-09-22] build | QLC 덱 v6.1 — 5장 실행을 두 트랙 그림 형식으로 복귀
- 사용자 지시: v3.x 4장 덱의 실행 슬라이드 이미지를 제시하며 "실행 슬라이드는 이 형식이 더 나아보여"
- 표 중심 레이아웃(고객 선별 5열 표 + 우측 3축 타일)을 폐기하고 그림 구도로 교체: 좌 삼성 개발실(제공하는 것 4 + 송용호 인용) / 중앙 ① FDE 상주 → 화살표 + 고객 선별·채널 카드 + 선례 카드(Palantir FDE 도해, Micron↔Anthropic SCA 4요소 그리드) + ← ② 접근권 확보 화살표 / 우 고객 시스템 4계층 + FDE 아이콘 + 고객 로고
- 하단은 조직·인사·문화 타일 3 + 단계별 통과 조건 3 + 판돈·비용으로 유지. v5.0에서 제외한 자회사·별도 보상·지분 참여는 복원하지 않았고, 개발실 제공 항목 네 번째는 v6.0의 WAF 급등 감지·런타임 대응으로 채움
- 화살표 도형 몸통 두께를 0.5 → 0.86으로 키워 텍스트가 몸통 안에 들어오도록 수정(기본값에서는 두 줄 중 아래 줄이 몸통 밖으로 나가 잘림)
- 렌더 QA 통과. 동기화: 기획서 v6.1 행, 보고서 PPT 압축 맵 5행, 대시보드 v2.46.38

## [2026-09-22] build | QLC 덱 v6.2 — 5장에서 단계별 통과 조건 블록 제거
- 사용자 지시: "단계별 통과 조건은 제거해줘"
- 5장 하단의 통과 조건 3단계(90일·12개월·2027H1) 블록을 덱에서 삭제. 확보한 공간으로 상단 두 트랙 그림을 3.86 → 4.10in로 키우고(선례 카드와 고객 선별 행에 여유), 판돈·비용을 전폭 스트립으로 재배치
- 통과 조건은 보고서 5장 4절과 위키에 유지하고 "보고서 전용" 표기를 달았다. 발표 노트에 질문 시 그 표로 답하도록 명시
- 렌더 QA 통과. 동기화: 기획서 v6.2 행, 보고서 PPT 압축 맵 5행·§5.4 주석, 대시보드 v2.46.39

## [2026-09-22] build | QLC 덱 v6.3 — 5장에서 판돈·비용 스트립 제거
- 사용자 지시: "판돈과 비용도 불필요해보인다"
- 하단에는 조직·인사·문화 타일만 남기고, 확보한 공간으로 상단 두 트랙 그림을 4.10 → 4.86in로 확대. 선례 카드(Palantir 상주 도해·Micron SCA 4요소 그리드)와 고객 시스템 스택, 고객 선별 행의 글자·도형을 함께 키웠다
...
## [2026-09-23] build | QLC 원본 덱 v7.0 — 교훈 하나로 통합, 배치 힌트 슬라이드 신설(6장 → 7장), 수율 상충·3장 압축·7장 시각화
- 사용자 피드백 5건 반영. 덱이 6장 → **7장**이 됐다
- **1장 전면 재설계**: 교훈 3건("수요 센싱 · 고객 협업 · 의사결정 시점")을 **하나**로 합쳤다 — "고객과 함께 수요를 설계하고 그 판단을 끊지 않은 쪽이 이겼다". 두 레인 HBM 타임라인(SK하이닉스 2013-12 AMD 공동 개발 → 2021-10 표준 선행 HBM3 → 2022-06 NVIDIA 공급 → 2025-06 DRAM 역전 → 2026-06 다년 파트너십 / 삼성 2019-03 최초 HBM2E → 2019 시장 과소평가 → 2024-04 팀 재구성 → 2025-09 12단 퀄 18개월 지연)에 **회복 카드 3장**(2025-Q4 DRAM 1위 탈환·2026-02-12 최초 상용 HBM4·2026-Q2 점유 33%)을 같은 슬라이드에 두어 반증을 함께 적었다. 이어 구매 기준 3국면(성능·$/TB → QoS·지연 꼬리 → 밀도·전력·공급)과 KV 캐시 카드, 그리고 "TLC는 1~3 DWPD를 만족하지만 고객은 비용·밀도 때문에 QLC를 원한다"는 **문제 제기로 마무리**(결론 없음)
- **2장 보강**: 수율 상충 사슬(선별 강화 → 수율 하락 → **원가 상승**)을 넣어 "다이 품질로는 못 푼다"를 세우고 보호 구조로 전환. 구체 수율 손실 폭은 `[사내 확인]` 표기
- **3장 압축**: 4열 이관 매트릭스와 격차 축소 변수 스트립을 덱에서 제거(`solution_ladder_slide.py` v4.0). DWPD 산식 칩(항별 결정 주체)·3단 논증 사슬만 남기고 차트 3종을 5.90in로 확대. 제거분은 보고서 §3.1·§3.4에 유지하고 덱 대조 주석을 달았다
- **4장 신설(배치 힌트)**: 호스트 VM·응용 → 인터페이스 → 8칸 NAND 블록 도해로 네 경우를 그렸다. ① 무배치 WAF **3.0**(✅ 실측) ② 멀티스트림 **1.8**(⚠️ 모델) ③ FDP 전면 **1.05**(✅ 실측) ④ 혼재 **2.2**(⚠️ 모델), QLC 정격 0.6 기준 유효 DWPD 0.6 / 1.0 / 1.7 / 0.8. 네 값을 하나의 연속 측정으로 그리지 않기 위해 각 경우에 등급 배지를 붙였다
- **7장 시각화**: 텍스트·박스 나열을 2×2 순환 루프(관측 → 감지·분류 → 런타임 대응 → 검증·롤백)와 기술 요소 6개 2열 행(확보 상태 칩)으로 재구성
- 리서치 2건: `sources/articles/qlc-v7-hbm-codesign-lesson-2026-09.md`(HBM 타임라인·반증 6건), `qlc-v7-placement-cases-waf-2026-09.md`(4경우 WAF 근거·CacheLib 폴백 코드·WARP Noisy RUH·공개 공백)
...
## [2026-09-23] build | QLC 덱 v7.2 — 1장 ②구획에 "KV 캐시가 HBM 용량을 넘어선다" 추가 (이동이 아니라 증설)
- 사용자 지시: "슬라이드1에서 현재 ai 메모리 수요가 hbm에서 kv cache offloading을 위한 스토리지로 이동하고 있다는 내용과 근거가 추가되어야해"
- 1장 ②구획을 "구매 기준의 이동"에서 **"HBM 위에 새 계층이 얹힌다"**로 재설계. 네 블록 — (a) 구매 기준 3국면 가로 압축 체인(세로 3.18in → 0.64in) (b) **G1 GPU HBM → G2 CPU DRAM → G3 NVMe SSD 도해** + "대체가 아니라 증설" 스트립 (c) 근거 블록 (d) 요구 DWPD 1~3. 열 폭 ① 7.30 → 6.60, ② 5.50 → 6.30, ③ 5.38 → 5.04
- **리서치가 초안의 사실 오류 2건을 잡았다.** 웹 리서치(`sources/articles/qlc-v7-hbm-to-storage-shift-2026-09.md`, 8절) 결과:
  - ① 초안은 G1을 "**용량이 고정된 계층**"으로 썼는데 **틀렸다.** GPU당 HBM은 같은 기간 커진다 — 96/192GB → 216/288GB(2026) → 384GB(Rubin Ultra 2027) 🟡. "80 → 288GB로 커졌지만 단일 사용자 128K 컨텍스트가 약 40GB·사용자 수에 비례"로 교체
  - ② 초안은 메커니즘 출처를 "**NVIDIA Dynamo KVBM의 4단 위계**"로 적었는데, **NVIDIA가 2026-09-18 Dynamo v1.5.0에서 KVBM을 deprecate**했다(v1.6.0 제거 목표, ✅ 릴리스 노트 원문). 디스크 티어는 유지되나 전용 매니저는 엔진 네이티브로 흡수 → **메인라인 PR 이력**으로 교체(LMCache #52 2024-08 → #773 2025-06 → Dynamo #3510 2025-10 → vLLM #40020 2026-05, 전부 ✅)
- **프레이밍 정정 — "이동"이 아니라 "증설"이다.** 사용자 요청 표현은 "HBM에서 스토리지로 이동"이었으나 공개 자료가 지지하지 않는다: 벤더 본인의 단어가 **"추가(additional) KV 캐시 저장소"**(SK하이닉스)이고, GPU당 HBM은 오히려 커지며, Micron의 계층 정의는 **HBM을 hot KV로 유지**하고, Kioxia가 "못 따라간다"고 지목한 것은 HBM이 아니라 **DRAM**이며, 컨텍스트 전용 가속기 Rubin CPX조차 128GB GDDR7 → **168GB HBM4로 재설계**됐다는 보도가 있다(⚠️ 애널리스트, NVIDIA 미확인). 내용은 요청대로 넣되 표현을 사실에 맞췄고, "대체가 아니라 증설" 스트립을 슬라이드에 고정했다
- **헤드라인 근거를 격상**: TrendForce의 "HBM→NAND 스필오버" 인용 대신 **HBM 1위 업체 본인의 진단** — SK하이닉스 2026-07-29 2분기 실적 콜 "KV 캐시를 저장할 HBM 용량이 한계에 도달하고 있다" + near-GPU storage 개발 공표 🟡
- 보조 근거: NAND 비트 출하 중 eSSD 비중 **1년 만에 26% → 48%** 🟡(Counterpoint Q2 2026), CMX NAND 2026 35EB → 2027 100EB+ 🟡, 삼성 V-NAND 캐파 약 60% CMX 배정 🟡, 추론 수요 CAGR 86% 🟡
- **덱 사용 금지 문장(부정 확인)**: "오프로드로 HBM 수요 N% 감소" · "KV 캐시의 N%가 SSD에 안착" · "AI NAND가 전체 NAND의 N%" · "NAND가 HBM보다 X배 빨리 성장"(금액 대 비트 불일치). 넷 다 근거 미확보. B200 HBM 용량도 180 대 192GB로 출처가 엇갈려 단일 수치 금지
- 신설 소스 1건, 위키 `wiki/concepts/hbm-to-storage-spillover.md` 신설, 보고서 v4.2 §1.2 신설(기존 1.2~1.4 → 1.3~1.5) + 부록 A **A-75~A-84** + 자산 맵 + PPT 압축 맵 v7.2, 기획서 v7.2 절, index.md, 역링크 2건
- 렌더 QA 통과(7페이지). 지식 그래프 재생성(노드 114·엣지 618, 고아 0). 대시보드 v2.46.44 + updates 엔트리, `npm run build` 통과
- ⚠️ 작업 중 `version.js`를 `open(p,"w").write(open(p).read())` 패턴으로 한 번 더 날렸다(2026-09-23 두 번째). `git checkout` 후 read→write 2단계로 복구. **이 패턴은 쓰지 말 것**

## [2026-09-23] build | QLC 덱 v7.3 — 1장 핵심 논리를 design-in 판별로 전환, 요구의 누적, 질문으로 마무리
- 사용자 지시: "현재는 HBM에서 시장을 작게봐서 졌고 이번에는 QLC를 놓치면 안된다에 가까운데 이러면 과거 삼성 의사결정에 대한 논쟁이 슬라이드의 중심이 될 위험이 있어"
- **①구획 명제 교체**: "기술이 없어서가 아니라 시장을 작게 봤다" → **"표준을 만족하는 기술을 가진 것만으로는 부족했고, 시장 전환 시점에 고객이 요구하는 사양을 먼저 맞춘 쪽이 design-in을 가져갔다"**. 특정 회사의 판단을 평가하지 않는 구조로 전환
- **시각 근거**: JEDEC HBM4 표준 **최대 8 Gbps** 대 NVIDIA 요구 **10~13 Gbps**를 막대 두 개로 그리고 그 사이를 "표준을 만족해도 닿지 않는 구간"으로 표시. 두 레인 HBM 타임라인은 제거하고 **중립 기록 카드 3장**으로 교체(2019-03 삼성 최초 HBM2E — 기술은 확보돼 있었다 / 2021-10 SK하이닉스 표준 제정 전 HBM3 완료 → 2022-06 NVIDIA 최초 공급 / 2026-02 삼성 최초 상용 HBM4 — 제품 개발은 이어졌다)
- **삭제한 표현과 이유** (덱·보고서·위키 전량):
  - "기술이 없어서가 아니라 **시장을 작게 봤기 때문**" — 사실이 아니라 **해석**. 내부 수요 판단의 근거를 공개 자료로 확인 불가
  - "**전담 조직을 후순위로**" / "조직 축소" — **팩트체크되지 않음**(단일 보도 계열 CNBC 2024-11-08) → **"전략적 우선순위가 약화되었다"**로 대체
  - "전담 팀 재구성 — **5년 공백**" — **사실이 아님**. 삼성 사업보고서 기준 해당 기간에도 HBM 제품 개발 지속. "공백" 표현 자체를 삭제
  - "HBM 성장률을 과대평가로 보고 자원을 범용 DRAM으로" — 단일 출처 계열의 내부 판단 서술, 삭제
- **②구획: 시대 구분 → 요구의 누적.** 사용자 지적대로 구매 기준 3국면은 **프레임워크이지 객관적 시대 구분이 아니어서** 반론 여지가 크다. 「기존 요구(용량당 비용·밀도·전력) → **+ 추가(내구성)**」 2블록 도식으로 교체. 증거는 **OCP 사양 개정 이력** — v2.5~v2.7이 QoS를 빼지 않고 텔레메트리·FDP·전력 측정을 추가했다(요구가 교체됐다면 빠진 항목이 있어야 하는데 없다)
- **추론 CAGR 86%/학습 16% 덱에서 삭제.** 출처(Kioxia Investor Day 2026-06-02)는 있으나 레포 원장 간 정의가 **CAGR**과 **"증분 DC 수요의 86%"**로 엇갈린다. 덱에는 "AI 수요의 중심이 추론으로 빠르게 확대되고 있습니다 · 데이터센터 NAND 295EB(2025) → 1,807EB(2028), Kioxia 전망"으로 출처와 함께 표기
- **QLC 정격을 단일 값에서 범위로 정정.** "현 QLC 0.6"의 0.6은 Solidigm P5336 값이며 **Kioxia LC9(245.76TB)은 0.3**이다. 정격은 용량·기준 블록의 함수(같은 Micron 6600 ION이 4K 0.075 / 16K 0.3 / 128K 순차 **1.0** — 13배 차이) → 덱은 **0.3~1.0**과 "245TB급은 0.3 수준"만 적고 단일 배수를 내세우지 않는다. 보고서 §1.4에 제품별 표와 격차 범위(61TB급 1~5배 / 245TB급 10배 / 극단 조합 40배는 사용 금지)
- **"QLC TB당 20~30% 저렴" 삭제 — 출처 없음.** 레포가 보유한 유일한 인용 가능 수치는 VDURA Flash Volatility Index의 TLC 대비 **−13~−20%**(🟡 현물 지수, 계약가 아님)다. 덱은 **방향성("낮다")만** 표기
- **③구획을 질문으로 마무리**: TLC(3 DWPD 급·61TB·비용 높다) 대 QLC(0.3~1.0·245TB급·비용 낮다) 대비 후 → **"고용량 QLC의 밀도·비용 이점을 유지하면서 AI 추론이 요구하는 내구성을 확보할 수 있는가?"**
- 액션 타이틀 교체: "표준 충족이 아니라 고객 사양이 design-in을 갈랐고, 그 사양에 지금 내구성이 더해지고 있습니다". 리드·밴드·출처 줄·발표 노트 전면 재작성
- 위키: `codesign-demand-lesson.md` 재작성(§4에 쓰지 않기로 한 표현 4종과 이유), `essd-purchase-criteria-shift.md` 제목·§1을 "요구는 누적된다"로 재작성, `qlc-ssd-market.md` 덱 표기 v7.3. 보고서 v4.3(§1.1~§1.4 재작성, PPT 압축 맵 v7.3), 기획서 v7.3 절, index.md 2건
- 렌더 QA 통과(7페이지). 지식 그래프 재생성(고아 0). 대시보드 v2.46.45 + updates 엔트리

## [2026-09-23] build | QLC 덱 v7.4 — 수치 검증: DWPD 범위 확정, DC NAND EB 오류 정정, Gartner 추론 역전 추가
- 웹 리서치 전수 조사(신설 원장 `sources/articles/qlc-v8-dwpd-price-inference-2026-09.md`, 6절) 결과로 v7.3의 숫자 셋을 고쳤다
- **① QLC 정격 0.3~1.0 → 245TB급 헤드라인 0.3~0.6.** 1.0은 **128K 순차 기준**이지 헤드라인이 아니었다. 전수: 삼성 BM1743 61TB **0.26** · Solidigm P5336 61TB 0.58(16KB 정렬)/122TB 0.6(32KB) · Micron 6550 ION 61TB **8개 기준 전부 공개**(1.00 순차 / 0.25 4K / **JESD219A 0.30**) · Kioxia LC9 245TB **0.3** · Micron 6600 ION 245TB **1.0/0.3/0.075**(128K·16K·4K, 13.3배) · **삼성 BM1773 245TB 0.6, >260PBW, 2026-09-18 출시**(신규 확인)
- **가장 중요한 발견은 값이 아니라 비교 불가**: 245TB급 세 제품 중 **둘(Kioxia LC9, 삼성 BM1773)이 측정 워크로드를 공개하지 않는다.** 같은 용량에서 0.3과 0.6이 2배 차이 나지만 셀·컨트롤러 차이인지 측정 기준 차이인지 분리할 수 없다 → 슬라이드 주석으로 고정
- **② 「DC NAND 295EB(2025) → 1,807EB(2028)」은 오류였다.** 1,807EB는 **전체 flash** CY28 수치이고 **데이터센터는 909EB**(CAGR 46%)다. 오류는 v7 원장 V-21에서 유입돼 위키·덱으로 전파됐다. `sources/`는 불변 계층이므로 v7 원장은 수정하지 않고 **v8 원장 §3-B가 정정을 보유**하며, 위키에 "인용 시 v8 수치를 쓴다"를 명시했다. 보고서 부록 A-10은 원래 맞게 적혀 있었다
- **③ 추론 CAGR 86% 판정 확정**: Kioxia Investor Day 2026-06-02 **슬라이드 15 스크립트 원문** "inference AI applications... estimated **CAGR of 86%**"(CY25~28, 학습 16%). 레포 3곳의 **"증분 DC 수요의 86%"는 오독**이었다. 다만 **86%는 성장률이지 점유율이 아니라** "추론이 학습보다 크다"를 직접 말하지 못한다
- **④ 추론 근거를 Gartner로 격상**: **Gartner 2026-08-10 — 2026년 AI 최적화 IaaS 지출의 55%가 추론, 추론 $23.3B가 학습 $19.0B를 사상 최초로 추월, 2027년 59%** ✅. 검산 23.3+19.0=42.3≈$42B 정합. 규모의 역전을 직접 말하는 유일한 1차 수치이므로 덱 메인으로 쓰고 Kioxia는 NAND 쪽 보조로 병기
- **⑤ 가격 판정 확정 — "20~30%"는 산술적으로 불가능**: TLC(3비트/셀) → QLC(4비트/셀)는 비트 +33%지만 **비트당 원가 절감의 천장은 −25%**(1−3/4)이고, 컨트롤러·DRAM·PCB가 공통 원가라 드라이브 $/TB 절감은 그보다 **작을 수밖에 없다**. Objective Analysis(Jim Handy) 사다리 50/33/25/20%와 정합. 유일한 공개 지수 VDURA(−13~20%)도 QLC/TLC 비율이 3개 시점 중 **2개가 정확히 0.8000**이라 모델 가정 의심. **TrendForce 포함 어떤 기관도 QLC/TLC $/TB를 분리 공개하지 않는다**
- **함께 기록한 반대 신호**: 전력 용량 기준으로는 **학습 5GW 대 추론 2GW**라는 집계(⚠️ 원 출처 미확인). TrendForce는 추론이 AI 서버 수요의 주 동인이 되는 시점을 **2029년**으로 봄 — Gartner(지출)와 분모가 달라 섞어 쓰면 안 됨
- **부정 확인**: "Solidigm D5-P5810" 존재하지 않음(D7-P5810은 SLC 50/65 DWPD) · "Kioxia LC8" 확인 불가 · SanDisk 256TB·SK하이닉스 PS1012·ScaleFlux QLC는 DWPD 미공개 · 국내 매체가 Gartner $42B를 "59조 달러"로 오역(재인용 금지)
- 보고서 v4.4(§1.4 전면 재작성, 부록 **A-85~A-94**, 자산 맵, PPT 압축 맵 v7.4), 기획서 v7.4 절, 위키 `hbm-to-storage-spillover.md` 정정 고지 ②와 §3 갱신, index.md. 렌더 QA 통과(7페이지). 대시보드 v2.46.46

## [2026-09-23] build | QLC 덱 v7.5 — 1장 교훈의 시점 오류 정정: "갈린 것은 규격이 아니라 양산 시점"
- 사용자 지적: "삼성은 2019년 업계 최초 HBM2E 발표하였지만 정작 2022년에 NVIDIA에 공급한 것은 하이닉스였어. 이 부분에 대한 원인을 분석하여 교훈을 다시 만들어야 될 것 같아. **현재의 내용은 HBM4의 고객 요구사항에 대한 부분으로 2022년 상황과는 맞지 않는** 부분이 있어"
- **지적이 정확했다.** v7.3~v7.4의 ①구획은 **JEDEC HBM4 최대 8 Gbps 대 NVIDIA 요구 10~13 Gbps** 막대 도식으로 2022년 design-in을 설명했다. **HBM4 표준(JESD270-4)은 2025-04 제정**이고 NVIDIA의 그 요구도 2025~2026년 사실이므로, **3년 뒤 근거로 과거를 해석**한 시점 오류였다
- 2019~2023 창 안으로 전수 재조사(신설 원장 `sources/articles/qlc-v9-hbm3-designin-causes-2026-09.md`, 9절 + 부록)
- **결정적 반직관 — 2022년에 규격 격차는 없었다.** 양사 HBM3 공표 스펙이 **6.4 Gbps/pin · 819 GB/s로 동일**하다(✅ 파생, 양사 PR·제품 페이지 대조). "삼성이 제시한 규격이 NVIDIA가 필요로 한 규격에 미달했다"는 **2021~2022년 공개 근거는 존재하지 않는다**
- **새 교훈**: "삼성은 2019년 3월 업계 최초로 HBM2E를 발표했다. 그러나 2022년 H100의 HBM3 공급사는 SK하이닉스였다. **갈린 것은 규격이 아니라 양산 시점**이었고, 구속력 있는 일정은 표준이 아니라 **고객의 제품 출시 일정**이었다"
- **인과 사슬(✅, 벤더 공식 PR과 JEDEC 일자만으로 성립 — 해석 없음)**: SK하이닉스 개발완료 **2021-10**(JEDEC JESD238 공표 2022-01-28보다 3개월 앞섬) → 양산·NVIDIA 공급 **2022-06-09**(퀄 통과 후) → **2022 Q3 H100 출하, HBM3 8단 16GB 단독 공급**. 삼성은 2021-10 Tech Day "HBM3 2022 Q2 출시" 로드맵만 있고 **2022년 중 개발완료·샘플·양산 공식 발표를 확보하지 못했으며**(부재의 기록), NVIDIA 공급망 진입은 **2023 Q4·1Znm·소량** → 약 **5개 분기** 공백
- **슬라이드**: 막대 도식 → **두 레인 타임라인**으로 교체. 스펙 동일 스트립 + 타임라인(세로 기준선 두 개 — **JEDEC 표준(회색)** 과 **고객 일정 H100 2022 Q3(파랑 강조)**, 삼성 레인에 「약 5개 분기」 공백 바) + 교훈 박스 + 과장 방지 주석 + MR-MUF 단서
- **MR-MUF는 지목되지만 확정되지 않는다**: 2022-04에 공개 용어로 정착했으나 인과 서술은 전부 2023년 이후 사후 해석이고, **수율 수치가 출처 간 정면 충돌**한다(TrendForce 2024-03 삼성 10~20% 대 SK 60~70% ↔ EE Times·SemiAnalysis SK 75~80% 대 삼성 60~65%, 벤더 확인값 없음) → **덱·본문에 수율 숫자 사용 금지**
- **비지지 8종(덱 금지)**: 공동 설계 · 표준 참여 차이 · "2022년 삼성 스펙 미달"(**2024년 HBM3E 건, 2년 시차 오류**) · "NVIDIA 요구 속도 미달"(**HBM4 시대, 창 밖 — v7.3이 범한 오류**) · "12단 수율"(**H100은 8단**, 12단 샘플 2023-04) · "SK가 투자를 더 했다"(**CapEx 2019~2020 2년 연속 감소** 16.0→13.9→10.1조) · 가격(창 안 데이터 없음) · "삼성이 밀려났다"(**2022년 점유율 40%**, 비었던 것은 소켓 한 세대)
- **단일 원인 서사 차단**: 마이크론은 HBM3를 **통째로 건너뛰고** HBM3E로 직행했다 → 단독 공급은 **두 벤더의 불참이 겹친 결과**이기도 하다. 2020-07 SK하이닉스 HBM2E 양산은 "8년 만의 엔비디아 공급망 재진입"이었으나 **당시 제1공급사는 여전히 삼성**이었고, 우선공급 계약 공개 기록은 결과 이후인 2023-12 → 인접성은 원인이 아니라 **결과를 굳힌 기제**
- **위키 개명**: `codesign-demand-lesson.md` → **`hbm3-designin-lesson.md`**. 리서치가 **공동 설계를 비지지**로 판정했으므로 파일명이 그 주장을 담고 있으면 안 된다. 참조 6곳(qlc-ssd-market·essd-purchase-criteria-shift·hbm-to-storage-spillover·solution-ladder·보고서·index) 링크 갱신
- **미확인 잔존**: 삼성 HBM3 "Icebolt" 양산 개시일, NVIDIA의 2021~2022 퀄 기준 수치(전면 미공개)
- 보고서 v4.5(§1.1 전면 재작성, 부록 **A-95~A-101**, 자산 맵, PPT 압축 맵 v7.5), 기획서 v7.5 절, index.md 2건. 렌더 QA 통과(7페이지). 대시보드 v2.46.47

## [2026-09-23] lint | 1장 슬라이드에서 조사 과정 언어 제거 — "없는 정보는 무시"
- 사용자 지시: "없는 정보는 무시하자"
- 앞선 v7.5 보고에서 남은 공백 2건(삼성 HBM3 "Icebolt" 양산 개시일, NVIDIA 2021~2022 퀄 기준 수치)을 거론했으나 **더 추적하지 않기로 했다**
- 더 중요한 것은 그 태도가 **슬라이드 문안에 새어 들어와 있었다**는 점이다. "확보되지 않음"·"공개된 바 없습니다"는 **우리가 무엇을 못 찾았는지**를 말할 뿐 세상에 대한 사실이 아니며, 임원 슬라이드에 들어갈 문장이 아니다. 3곳을 고쳤다:
  - ① 삼성 2021-10 마커 부제: "양산 공표는 확보되지 않음" → **"출시 시점 로드맵"**. 양산 부재는 타임라인의 **「약 5개 분기」 공백 바와 2023 Q4 진입** 사실이 이미 보여준다
  - ① MR-MUF 단서: "…2022년 당시의 1차 근거와 벤더가 확인한 수율은 공개된 바 없습니다" → **"다만 이는 2023년 이후의 사후 해석이며, 확정된 설명은 아닙니다"**. 과잉 주장을 막는 가드는 유지하고 못 찾은 것의 목록만 뺐다
  - ③ DWPD 주석: "두 제품 모두 측정 워크로드를 공개하지 않아 나란히 비교할 수 없습니다" → **"측정 기준이 제품마다 달라 단일 값으로 비교하지 않습니다"**. 같은 가드를 사실 서술로
- **서술 규율에 추가**: `qlc-ssd-market.md` 1장 규율 ⑥, `hbm3-designin-lesson.md` §7 — "조사 과정 언어를 슬라이드에 쓰지 않는다. 원장·보고서에만 두고 슬라이드에는 확인된 사실로 적는다"
- 원장(`sources/`)과 보고서의 부정 확인 절은 **그대로 유지**한다 — 거기는 기록이 있어야 할 자리다
- 렌더 QA 통과(7페이지). **대시보드 변경 없음**(덱 문안만 수정, `src/` 무변경) → 버전 bump 생략

## [2026-09-23] lint | 4장 배치 힌트 도해 — 수명 등급 3종이 실제로 보이도록 부호화 수정
- 사용자 지적: "응용에 따라 도형의 배경색을 파란색 계통으로 다르게 가져갔는데 눈에 잘 띄지 않아서 다른 계통의 색을 써주고, **3개 종류의 응용을 가정하였으나 실제로 보이는 것은 2개 종류만** 보이고 있어"
- **원인은 색이 아니라 부호화 설계였다.** `nand_block()`이 **수명 0(짧음)을 회색 점선 빈 칸으로 덮어써서** NAND 블록에는 중간·김 두 색만 나타났다. 범례는 3색을 보여 주는데 블록은 2색뿐 — **색(수명 등급)과 상태(유효/무효)가 한 채널에 섞여 있었다.** 두 축을 분리했다
- **고친 것 5건**:
  - 수명 램프: `#1428A0 / #3C5AC8 / #AAB8E8` → `#1428A0 / **#6E86DC** / **#CFD8F2**`. 앞의 두 색이 명도가 거의 같아 구분이 안 됐다
  - **이중 부호화 추가**: 색 + 글자 **S · M · L**(NAND 셀·응용 칩·범례 전부). 색맹·흑백 인쇄에서도 구분된다
  - **무효화 표현**: 회색 점선(등급 정보 소실) → **해당 등급 색의 점선 + 그 색 글자**. 어떤 등급이 죽었는지 보인다
  - **범례**: 실제 렌더 모습 그대로 4항목(S·M·L + 등급색 점선)으로 맞추고 중복된 "무효화된 페이지" 항목 제거
  - **④ 혼재 블록 재구성**: `[0,1,2,0,2,1,0,1]`은 ①과 똑같은 균등 혼합이라 두 경우가 구분되지 않았다 → `[2,2,0,2,0,2,1,0]`·`[2,0,2,1,2,0,2,2]`로 **L(힌트 없는 레거시 VM) 우세 + S·M 간섭** 구조로 바꿨다
- **브랜드 규칙과의 조정**: 사용자는 "다른 계통의 색"을 요청했으나 덱 규율은 **액센트를 Samsung Blue 하나로 고정**하고 무지개 팔레트를 금지한다(녹·적은 달성·리스크 의미에만). 수명 등급은 **순서형**이라 범주형 색상환보다 **단일 색상 명도 램프가 본래 옳은 표현**이기도 하다. 그래서 계통을 바꾸는 대신 **명도 간격을 벌리고 글자 레이블을 더해** 구분성을 확보했다
- **정합성 검산**: 「소거 대상 블록 복사 n/8칸」이 그 블록의 유효 칸 수와 일치하는지 전수 확인 — ① 5/8 ② 4/8 ③ 0/8 ④ 5/8 모두 일치
- **정합성 규칙을 위키에 고정**(`fdp-placement-mechanics.md` §6): 색 = 수명 등급만, 채움/점선 = 유효/무효. 두 축을 섞지 않는다. 8칸 블록은 원리 모식도이며 실측 레이아웃이 아니다
- 기획서 v7.6 절 추가. 렌더 QA 통과(7페이지). **대시보드 변경 없음**(덱 스크립트·문서만 수정) → 버전 bump 생략
