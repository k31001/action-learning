...
## [2026-08-12] build | 공통 개요편 PPT 18장 — 전략 1번 「전환할 수 있는 몸」 + 파운드리 별첨

- `outputs/storyline/mfg-fungibility-proposal.md`(main에서 병합)를 슬라이드 2장으로 압축해 17장 → 18장.
- **S15 · 전략 1번 「전환할 수 있는 몸」** (빈 슬라이드 채움): 좌측 = 같은 CAPEX 곡선의 두 얼굴(앞면 2029E 감가 45.9조·붕괴선 67조→104조·같은 붕괴 시 약 −31조 / 뒷면 감가 완료 설비 취득원가 196조·메모리 귀속 108~137조), 우측 = 세 축(세대 연장성·제품 간 동일성·N-1 설계) + 경계(선단 리더십 불변), 하단 = 값 3타일(전환 6→3개월 회당 1조 / N-1 원가 우위 25~35% / 증액 요구 없음). 원문 6개 장 중 1·3·4장의 결론만 슬라이드에 남기고 2장(각자도생 진단)·5장(반론 4건)·표 1~5는 발표자 노트로 이관.
- **S18 · 별첨 「DS 전사로 넓히면」**(신설, 마지막): 파운드리 접점(HBM4 베이스다이 = 로직 팹 산출물 · 평택 P4 파운드리 구역 → 1c DRAM 실증 · 설비 사양 표준 통합 관리)·이미지센서(DRAM↔CIS 약 80% 동일, 단 캐파 50% 감소·비용 최소 1조)·감가 완료 설비 2차 시장 + 하단 밴드(10nm 이하 로직·메모리 동시 양산 유일 — 구조적 자산이자 고비용 논쟁 대상). 원문 §4.1의 "옵션으로만 제시한다" 유보를 핵심 메시지로 그대로 유지.
- 2차 저지선 섹션 구조 조정: 기존 잠정 제목(제품/투자/오퍼레이션)은 전략 1번이 세 축을 모두 관통하므로 전략 번호 체계로 교체 — S14 섹션 도입(골격) · S15 전략 1번(작성 완료) · S16~S17 전략 2·3번(골격).
- 검증: OOXML validate 통과 · 신규 2장 렌더 검수 2라운드(카드 넘침·값 타일 줄바꿈·출처 2줄 수정) · 20pt 미만 런 0건 · 발표자 노트 18/18.
- main 병합(74b5645) 동반 — log.md 충돌은 양쪽 항목 보존으로 해소. 아웃라인 md·index.md 갱신. dashboard 미변경 → version bump 생략.

## [2026-08-12] ingest+build | 제품 믹스 비동기성 — 데이터 조사 → 위키 환원 → 슬라이드 (19장)

요청: 「전환할 수 있는 몸」의 당위성 슬라이드 — 제품 포트폴리오별 수요 변동의 비동기성을 과거·현재 데이터와 미래 시나리오로 시각화.

- **조사(웹)**: TrendForce·Yole 계열 수치 확보 → `sources/articles/memory-product-mix-history-2026-08.md` 신설. 핵심 실측 2건 — ① **2025년 DRAM +73%($907억→$1,657억) vs NAND +3.4%($674억→$697억)**, ② **2024년 4분기 DRAM +9.9% QoQ vs NAND −6.2% QoQ**(이듬해 1Q 추가 −20% 전망). 같은 분기 반대 방향은 단일 사이클 가설의 반증. HBM의 DRAM 매출 내 비중 2023 8% → 2024 20%(비트로는 5%) → 2026E 41%.
- **분석(위키)**: `wiki/concepts/product-mix-asynchrony.md` 신설. 전체 메모리 매출 대비 믹스를 계산 — HBM 4.6%(2023) → 30.1%(2026E) 6.5배, NAND 42.4% → 26.7% 반토막, 범용 DRAM 53→46→56→43% 진동. 2029년 3경로(①수요발 ②공급발 ③전환발, storyline-cmo §CMO-4)별 믹스는 **예시 가정**으로 명시(예측 아님) + 위키 시나리오 A~E 대응 표기.
- **슬라이드(S15, 전략 1번 바로 앞)**: 100% 누적 막대 7개 — 실적 4개 | 구분선 | 2029 시나리오 3개. 세그먼트 NAND·범용 DRAM·HBM·차세대, 10% 이상만 값 표기(2023 HBM 4.6%는 얇은 슬라이버 자체가 메시지). 하단 결론 2타일(HBM 5→30% / 2025 DRAM +73%·NAND +3%). 논리: 방향을 미리 고를 수 없으므로 방향 선택이 아니라 전환 속도를 산다 → 다음 장 전략 1번.
- 슬라이드 번호 이동: 전략 1번 S15→S16, 전략 2·3번 S16~17→S17~18, 별첨 S18→S19. TOTAL 19, S2 로드맵 배지 S14~S18로 갱신.
- 검증: OOXML validate 통과 · 렌더 검수(출처 2줄 넘침 1건 수정) · 20pt 미만 런 0건 · 발표자 노트 19/19. index.md(sources·concepts·outputs 3곳)·아웃라인 md 갱신. dashboard 미변경 → version bump 생략.

## [2026-08-12] build | 전략 2번 「판을 옮긴다」 + 당위성 슬라이드 + 전략 1 변경 반영 (20장)

- main 병합(6fb52f7) — `outputs/storyline/dram-architecture-proposal.md` 신규 + `mfg-fungibility-proposal.md` 개정 수신. log.md 충돌은 양쪽 항목 보존으로 해소.
- **S17 · 「따라잡히는 축 위에 서 있다」**(전략 2번 당위성, 신규): 좌 = 위아래 협공 3밴드(위 삼성 HBM 40%→17% / 가운데 삼성 DRAM 41%→32.6%, 2024→2025 3Q / 아래 CXMT 4.9%→8% · YMTC 5%→13%)와 수렴 화살표, 우 = 축의 대비(로직 4판 칩 2×2 — 멀티코어 ’04·FinFET ’11·칩렛 ’19·DSA ’15~ vs 메모리 한 축 — 6F² 2007~ 그대로·10년 밀도 2배 → 4F² COP 2026), 하단 3타일(20년 격차 6만 vs 100배 · 랙 전력 8kW→210kW · CXMT 상장 $86억). 원문 §1~§2 기반.
- **S18 · 「전략 2번 · 판을 옮긴다」**(신규): 좌 = 해자(zHBM 견적서 네 줄 — 본딩·선단 로직 노드·DRAM 셀/스택·선단 패키징 → 넷 다 되는 회사 1곳, 본딩만으론 한 줄), 우 = 세 제안(웨이퍼 본딩은 필연 / zHBM은 주문 생산 / 어디서 자를 것인가) + 경계(원가 목표 불변·로우엔드 방어는 전략 1의 N-1), 하단 값 3타일. §3 제품 3분류·§6 실행 4영역·반론 4건은 발표자 노트로 이관.
- **전략 1 변경 반영 2건**: ① S16 축 ① 본문에 4F² COP DRAM 수렴 추가("로직·디램·플래시 전 갈래") ② S20 별첨 카드 ①에 경계 명시("설비 사양 표준 통합까지 — 제품 아키텍처 수준의 통합은 전략 2번의 본론"). 신규 참고 [16]은 S18 신설로 반영.
- 슬라이드 번호 이동: 전략 3번 골격 S17→S19, 별첨 S19→S20. TOTAL 20, S2 로드맵 배지 S14~S19.
- 검증: OOXML validate 통과 · 렌더 검수 2라운드(줄바꿈 넘침 7건 수정) · 20pt 미만 런 0건 · 발표자 노트 20/20. 아웃라인 md·index.md 갱신. dashboard 미변경 → version bump 생략.

## [2026-08-12] build | 키엔스 벤치마크 반영 — 전략 1·2 슬라이드 갱신 (20장 유지)

- main 병합(fdac823) — 「키엔스 벤치마크 별첨 반영 — 전략 1·2 동시 보완」 수신. log.md 충돌은 양쪽 항목 보존으로 해소.
- **S17(전략 2 당위성) 본문 반영**: 하단에 키엔스 대비 밴드 신설 — "키엔스 OPM 10년 내내 50~56% — 삼성 DS 51.6%는 **2018년 최고 호황에서만**". 같은 숫자의 성질이 정반대라는 대비가 "셀·캐파·원가는 살 수 있는 축"이라는 핵심 메시지의 새 근거. 기존 3타일은 y 6.32로 내려 유지.
- **S18(전략 2) 본문 반영 2건**: ① 해자 카드에 "20년 분업 끝 — 통합이 자격" 추가(분업으로 갈라진 산업에서 통합 유지가 부담이었으나 이 판에서는 자격) ② 제안 ② 본문에 방향 전환 추가 — "받아 적는 주문이 아니라 먼저 제안하는 주문(IP가 곧 전환 비용)".
- **S16(전략 1)은 본문 무변경, 노트만 보강**: 키엔스의 팹리스 회피 경로와 1983년 저마진 사업 매각 사례는 "감가를 없앨 수 없으니 받아 낼 몸을 만든다"는 기존 핵심 메시지의 보강 근거일 뿐 새 논지가 아니므로, 20pt 하한 아래 슬라이드 밀도를 지키기 위해 발표자 노트로만 흡수(키몬다·엘피다 반대편 사례 포함).
- 참고번호 이동([16]→[17], 키엔스가 [16]으로 삽입)은 슬라이드가 각주 번호를 쓰지 않으므로 영향 없음.
- 검증: OOXML validate 통과 · S17·S18 렌더 검수 · 20pt 미만 런 0건 · 발표자 노트 20/20. 아웃라인 md 갱신. dashboard 미변경 → version bump 생략.

## [2026-08-12] build | 공통편 "두 겹" 프레임 반영 — 슬라이드 2장 신설 (22장)

- main 병합(66d445c) — 「과제팀 스토리라인 R3 반영 — 공통편에 "두 겹" 프레임 도입」 수신(common-overview.md +63줄, 신규 소스 memory-cycle-storyline-r3). log.md 충돌은 양쪽 항목 보존으로 해소.
- **신규 S6 「패배는 두 겹으로 온다」**(S5 뒤): 4행 표(회사 / 1겹 원가·기술 / 2겹 시간·자리 / 결과) — 키몬다 ✕✕ 퇴출 · 하이닉스 ✕✕/◎ 생존(부채비율 221%에도 채권단 출자전환 3년 → 플래시로 회복) · 엘피다 ✕/◎ 퇴출(공적자금 3년 받고도 회복 못 함) · 삼성 22~25 ✕✕ 생존(HBM3 양산 1년 반 지연 + 인증 실패, 커서 살았다). 결론 밴드 "밀린 칸은 그 칸에서 갚아야 한다" + 교훈 3타일. 20년을 관통하는 렌즈이므로 CMO 매트릭스 3장 앞에 배치.
- **신규 S15 「되돌릴 수 있는 것부터 버린다」**(세 개의 저지선 뒤): 가역성 3층(L1 미집행 CapEx·팹 용도 / L2 가동률·믹스·노드 / L3 사업 매각·라인 폐쇄) + SK 실증(가역은 과감 — M15X 낸드→DRAM 5.3조 / 비가역은 유지 — 솔리다임 안 팔고 24년 2분기 흑자) + 우리의 2022~23 반례 + 질문("사업이 틀렸나, 시점이 이른가"). 결론 밴드 "SK가 이긴 것은 처음부터 옳아서가 아니라 틀린 베팅을 3년 안에 되돌렸기 때문".
- **기존 슬라이드 갱신 4건**: ① S12 자연 실험 — SK 카드를 "3년 안에 되돌렸다(M15X 전환)"로 교체, DRAM 카드를 "순환 점유율은 되찾힌다 32.6%→38.5% / 유출 점유율은 안 돌아온다"로 전면 교체 ② S19 전략 2 당위성 — 가운데 밴드에 26.1Q 38.5% 회복 추가, 타일 "CXMT 상장 $86억" → "3사 94% → 실질 6사" ③ S11 재감사 노트에 소모전 4대 전제 붕괴·3층 원인·푸젠진화 대 CXMT 접근권 자연 실험 추가 ④ S2 로드맵 장별 범위 재계산(역사 S5~S10·재감사 S11~S12·처방 S13~S15, 배지 S16~S21).
- 슬라이드 번호 전면 이동(+2), TOTAL 22. 검증: OOXML validate 통과 · 신규 2장 + 갱신 2장 렌더 검수 3라운드(줄바꿈 넘침 6건 수정) · 20pt 미만 런 0건 · 발표자 노트 22/22. 아웃라인 md·index.md 갱신. dashboard 미변경 → version bump 생략.

## [2026-08-12] build | 3차 저지선 2전략 + 저지선별 한 장 요약 + 최종 결론 (26장)

- main 병합(9978d3b) — `outputs/storyline/ssd-fdp-proposal.md`·`open-innovation-proposal.md` 신규 + 전략 1·2 개정 수신. log.md 충돌은 양쪽 항목 보존으로 해소.
- **전략 1·2 개정 반영 2건**: ① S18(전략 1)에 실행 영역 ⑤ 「조직과 보상 — 증액 없음」 카드 신설(횡전개 인센티브 재원·파견 처우·본딩 전담조직), 값 타일을 「선단 불변·N-1은 아래쪽만 / 전환 6→3개월·회당 1조 / N-1 원가 우위 25~35%」로 교체 ② S20(전략 2) 제안 ①에 횡전개 실증 추가("이미 진행 중: DRAM에 FinFET·HKMG, 낸드에 TSV").
- **신규 S16 「제2 저지선 — 부가가치를 지킨다」**(2차 저지선 맨 앞): 저지선 위치 스트립 3구간(2차 활성) + 전략 2카드(성격/문제/축/값) + 결론 밴드("설비를 돌릴 수 있는 몸과, 돌릴 곳이 있는 로드맵은 한쪽만으로 성립하지 않는다").
- **신규 S21 「제3 저지선 — 대체 불가성을 만든다」**(3차 저지선 맨 앞): 같은 아키타입(3차 활성) + 전략 3·4 카드 + 밴드("계약은 물량을 지키고 통합은 갱신을 지킨다 — 그리고 자리는 한 번 차면 비지 않는다").
- **신규 S22 「통제권은 되돌아오지 않는다」**(전략 3번 당위성): 좌 = 통제 계층 4단 상승(완제품 → 커스텀 펌웨어 → 자체 컨트롤러 → 표준 주도+웨이퍼 직구매, 색 농도로 상승), 우 = 구글 이중 트랙(FDP 공동 설계 / Titanium SSD 240만 IOPS) + 네 동인 분해(TCO·공급 안보·보안/수직 통합은 닫혀 있고 ④ 워크로드 최적화만 열려 있음), 하단 = KV Cache · 실행 창 2027년 이전.
- **신규 S23 「전략 3번 · SSD의 CUDA」**: 좌 = 문(SCA) → 열쇠(FDP 플랫폼) → 사람(FDE) → 환류, 우 = 수익화 사다리 3단(1차 CUDA 모델이 본질 — FDP Enable 무료·하드웨어로 회수) + 지표 전환(출하량 아닌 활성화 용량), 하단 값 2타일(5년 누적 59억 달러·12~179억 / 하이퍼스케일러 내 28.1%→31.9%). §5.2 세 갈래 길·§6 산정 다이얼과 민감도는 발표자 노트로 이관.
- **신규 S24 「전략 4번 · 지금만 살 수 있는 것」**: 좌 = 세 투자 전략의 구분(가격 D9 / 기술 SE-1 / 자리 — 이 전략, 행사 시점은 지금뿐), 우 = 두 개의 시계(저전력 반도체 2~4년 ★★★ / 퀀텀·극저온 7~10년 이상 ★★☆)와 후보군, 하단 = 실행 3원칙. 후보 13곳 전체 표와 §6 「남겨 두는 것」은 노트로 이관.
- **신규 S25 「겨울이 오기 전에」**(최종 결론, 다크): 배경(호황은 환경이 줬다·다음 창 2028~29·FCF 변곡) → 문제(1차 저지선이 지키는 것은 물량뿐 + 세 구멍) → 답(2차 전략 1·2 / 3차 전략 3·4)을 한 장에. 닫는 문장 "겨울이 왔을 때 대체되지 않는 공급자만이 봄의 규칙을 쓴다".
- 슬라이드 번호 이동: 별첨 S22→S26. TOTAL 26, S2 로드맵 배지 「2·3차 저지선 — S16 ~ S25」.
- 검증: OOXML validate 통과 · 렌더 검수 2라운드(줄바꿈 넘침 18건 수정 — S16 카드 4건 포함) · 20pt 미만 런 0건 · 발표자 노트 26/26. 아웃라인 md·index.md 갱신. dashboard 미변경 → version bump 생략.
