...
## [2026-09-08] ingest | Bottleneck Model 2030 정기 점검 — 전력 74(▲2)·CAPEX 41(▲1)·파운드리 51(▲1)·패키징 65(▼2), ERCOT 접속 동결·신용경색 신호·CXMT DRAM 10% (v2.46.16 → v2.46.17)

- **무엇**: 5개 병렬 리서치 에이전트(전력/CAPEX·하이퍼스케일러/파운드리·패키징/HBM·DRAM·NAND 시장/AI 수요·중국 경쟁, SemiAnalysis·Counterpoint·TechInsights 우선 수집 — 3사 원문은 이번에도 egress 차단으로 2차 인용 경유)로 2026-07-04 이후 약 2개월 치 시장 데이터를 수집해 `sources/articles/september-2026-market-update-2026-09-08.md`로 저장. Bottleneck Model 2030의 4대 제약지수를 전력 72→**74**(▲2)·CAPEX/ROI 40→**41**(▲1)·파운드리 50→**51**(▲1)·패키징 67→**65**(▼2)로 갱신하고 변동폭·근거를 `wiki/concepts/bottleneck-model-2030.md` §종합 판독(2026-09-08)에 기록. 하방 위험 순서가 "CAPEX/ROI > 전력 ≈ 패키징 > 파운드리"에서 **"전력 > CAPEX/ROI ≈ 패키징 > 파운드리"로 처음 역전**.
- **왜**: 사용자 지시(스케줄 작업) — semianalysis.com·counterpointresearch.com·techinsights.com 등을 참조해 보고서·Bottleneck Model·대시보드·발표자료에 필요한 최신 데이터를 검색·반영하고, 지난 업데이트 대비 변동폭을 표현하라는 요청.
- **핵심 신호**: ① **전력** — 2026-08-03 텍사스 주지사 지시로 ERCOT 데이터센터 신규 접속 승인 전면 동결(감사 완료 목표 12-10) — 기존 "물리적 대기열" 병목 위에 **정치·행정 리스크**가 처음 추가됨, 3개 분기 연속 지수 상승(70→72→74)으로 4대 병목 중 유일하게 추세가 명확. PJM 2028/29 용량부족 6,831MW로 예측대로 확대. ② **CAPEX/ROI** — 4대 빅테크+Oracle 가이던스는 전원 추가 상향(MS FY27 $255~260B 등)됐으나 Alphabet·Amazon FCF 사상 첫 마이너스·Oracle S&P BBB- 강등(정크 한 단계 위)·CoreWeave CDS 5년물 디폴트확률 ~50%·하이퍼스케일러 IG채권 발행 YoY+1,300% — **가이던스와 신용시장이 처음으로 반대 방향**, 완화 추세(46→44→42→40) 정체로 전환. ③ **파운드리** — N2 램프·ASML 출하는 순항이나 NVIDIA Rubin 2026 유닛 전망이 KeyBanc 기준 170~180만 대로 재상승(4월 하향분 되돌림)·A16 2027 슬립 플래그로 수요측 압력 재긴장, 완화 추세(56→54→52→50) 정체. ④ **패키징** — CoWoS 캐파 목표 120~140K WPM·수급갭 20%→10%·수율 98~99%로 완화는 지속되나, SK하이닉스가 Hot Chips 2026(8/23)에서 **HBM4E까지 하이브리드본딩 배제를 공식화**(HBM5부터 적용, JEDEC 775㎛ 스택 한계 — 삼성은 반대로 하이브리드본딩 고수)하고 **ABF 기판 리드타임이 12주→26주로 배증**하며 신규 병목 벡터로 부상해 완화 폭 제한. ⑤ **시장 데이터** — 삼성 HBM4 골든수율 80%(2026-08-26) 발표로 HBM 매출점유율 21→33% 반등(SK하이닉스 58→50%, Micron 18%), DRAM 매출점유율 CXMT **10% 돌파**(전년 4%, 3사 90%+ 과점 사상 첫 붕괴), TrendForce 2026 메모리 TAM $551.6B→**$889.3B** 대폭 상향, **DRAM(2028 Q2까지 타이트) vs NAND(2027 H2 오버서플라이) 수급 시차 분화**(신규 인지), 반독점 소송 담당판사 Noel Wise 확정·Micron 답변서 혐의 부인.
- **시나리오·전략 영향 검토(사용자 지시 — 필요시 전략 추가/삭제 가능, 결과 기록)**: `wiki/scenarios/scenario-matrix.md`에 [2026-09-08 추가 재평가] 항목을 신설해 신규 외부 거시 사실을 DF1·DF2·확률(A26·B39·C8·D21·E6)과 대조 → **전부 유지**로 판정, 전략 추가/삭제 불필요로 결론. 근거: 신용경색 신호(CoreWeave·Oracle)는 08-11 재평가가 이미 지목한 CapEx-vs-FCF 다이버전스의 확대판(조달 경보이나 수요 붕괴 아님, EWI만 감시 강화) — Nvidia FQ2 $96.2B/FQ3 가이던스·Anthropic 첫 영업이익(ARR $47B, OpenAI 추월)이 오히려 지속 쪽 재확인. CXMT DRAM 10%는 이미 D21에 반영된 추세의 재확인. ERCOT 동결은 DF1·DF2가 아니라 병목 모델의 별도 축(제약지수)으로 분리 반영. DF2는 CXMT·YMTC의 국방부 1260H 리스트 제외 vs 상무부 Entity List·H200 심사 강화라는 **엇갈린 신호**로 리트머스 미해결 판정, 재배분 미실행(9월 미중 정상회담이 다음 분기점).
- **downturn SP-2 트랙 반영**: `wiki/downturn/differential-indicators.md` 현재 패널 상태를 2026-09-08로 갱신 — DX-1(FCF·조달) 경보 강화(Alphabet FCF 마이너스 신규·CoreWeave/Oracle CDS), DX-3(DC 취소·연기) 정상→주의 격상(2026 미국 DC 캐파 30~50% 이연), DX-5(CXMT·인증) DRAM 매출점유율 10% 반영. DRAM/NAND 수급 시차 분화를 신규 소단락으로 추가하고 DX-6·DX-8 제품별 분리 추적을 다음 갱신 검토 과제로 플래그.
- **영향받은 wiki 페이지(13개)**: `wiki/concepts/bottleneck-model-2030.md`(§종합 판독 신설 + 드라이버 6종 노트 갱신)·`hbm-market.md`·`dram-market-share.md`·`ai-capex.md`·`memory-market-overview.md`·`energy-constraints.md`·`ai-demand-sustainability.md`·`demand-inflection-ewi.md`·`dram-antitrust-litigation.md`·`wiki/entities/cxmt.md`·`china-competitors.md`·`samsung.md`·`sk-hynix.md`·`micron.md`·`wiki/downturn/differential-indicators.md`·`wiki/scenarios/scenario-matrix.md`(추가 재평가 항목).
- **dashboard (v2.46.16 → v2.46.17, 패치 = 데이터 갱신, 동일 탭·스키마)**: `data/bottleneckModel.js`(MODEL_ASOF·PREV_MODEL_ASOF·PREV_INDICES·4개 병목 currentIndex·indexNote·DRIVERS_ASOF·드라이버 6종 note/level/trend 갱신)·`data/indicators.js`(gpu_rental·ai_dc_credit_spread history 추가, DF1/DF2 current 노트에 [09-08 추가 재평가] 프리펜드)·`data/scenarioPlanning.js`(DF1·DF2 currentPosition에 동일 추가 재평가 프리펜드)·`data/updates.js`(신규 entry 최상단 추가)·`src/version.js`(v2.46.17). 지식 그래프 재생성(`node scripts/build-knowledge-graph.mjs`, 노드 104·엣지 576·orphans 0). `npm run build` 검증 통과.
- **outputs 동기화**: `outputs/report/scenario-planning-report.md` — 작성일 헤더·Executive Summary 표·§2.2.3 신설(전력·CAPEX·파운드리·패키징 델타 + 신용경색·CXMT·DRAM/NAND 시차 서술). `outputs/presentation/slide-outline.md` — 2026-09-08 데이터 갱신 노트 추가(슬라이드 구조 변경 없음, 확률·DF 전부 유지이므로 슬라이드 27 지표 값만 차기 우선 갱신 대상으로 플래그, PPTX 재생성은 보류). PPTX 본체는 미재생성(구조 변경 없어 불요).
- **건너뜀 사유**: `wiki/storyline/storyline.md`·`storyline-*.md`는 확률·DF 위치·전략 목록이 전부 "유지"로 확정되어 서사 갱신 불요(기존 컨벤션과 동일). PPTX 본체 재생성은 슬라이드 구조·논증 변경이 없어 생략.
