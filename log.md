...
## [2026-08-03] ingest | 크리스 밀러(Chip War 저자) 최근 인터뷰·기고 종합 수집 (v2.32.10)
...
대시보드: interviews.js에 외부 전문가 항목(크리스 밀러 종합, 공개 자료 기반) 추가 — 인터뷰 메뉴 4번째. updates.js ingest 항목 추가. version v2.32.9→v2.32.10(패치: 데이터 추가·기존 카테고리 내). 지식그래프 재생성(wiki 링크 신규)·npm run build 검증. 개발 브랜치 push (main 아님 — 세션 지정 브랜치).
