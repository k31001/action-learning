...
## [2026-08-04] ingest | 정기 외부 데이터 점검 — 빅4 CapEx 전원 상향(메모리 원가 귀속 2개 분기 연속) + Bottleneck Model 4대 지수 갱신·변동폭 표기

스케줄 작업(정기 자동 실행): SemiAnalysis·Counterpoint Research·TechInsights 및 기타 유용 사이트에서 최신 데이터 수집 → 위키·Bottleneck Model·대시보드·보고서·발표자료 반영. 5개 병렬 리서치 에이전트(하이퍼스케일러 실적·SemiAnalysis·Counterpoint/TechInsights·삼성 HBM4/가격·전력망+파운드리+패키징) 수행.

**핵심 발견**: 직전 재평가(07-28)에서 "다음 주 발표 예정, DF1 핵심 확인 이벤트"로 대기 중이던 Microsoft·Meta(07-29)·Amazon(07-30) Q2 2026 실적이 전원 발표. **빅4(Alphabet·MSFT·Meta·Amazon) 전원 CY2026 CapEx 가이던스 상향 또는 재확인 — 둔화 신호 0건**(합산 ~$725B→~$730~760B). **Amazon이 CapEx 상향분 $20B을 "메모리 가격 상승"에 명시 귀속**(직전분기 Microsoft $25B 귀속에 이어 2개 분기 연속) — 메모리=AI 지출 되먹임 구조를 하이퍼스케일러 1차 자료로 재확인. SK하이닉스 Q2 실적은 사상 최대(영업이익 ₩60.54조·OPM 76%)이나 컨센서스(~₩64.1조) 소폭 미스(HBM4 매출인식 시차 추정). 삼성 HBM4 볼륨 발주는 여전히 미전환. 전력망은 PJM이 2028/29 용량경매에서 목표 예비율(20%) 대비 14.4%만 확보해 이사회가 백스톱 경매·DC 우선감축안을 제안 — 계통 운영기관이 신뢰성 목표 미달을 스스로 공식화한 최초 신호. 변압기 리드타임 최대 5년→60개월로 재악화. HBM4 하이브리드본딩은 16-hi HBM4E로 채택 시점이 추가 후퇴. HBM 시장점유율 관련 상충 데이터(SK 62%·Micron 21%·삼성 17% vs 기존 기준선 SK 50~55%·삼성 35~40%·Micron 5~10%) 발견 — Counterpoint·TechInsights 원문 403 차단으로 1차 검증 불가, **기존 수치 유지하고 상충만 기록**(lint 대상).

**Bottleneck Model 2030 — 4대 지수 갱신(2026-07-04 대비 변동폭)**: 전력 **72→75(▲+3)**·CAPEX/ROI **40→37(▼−3, 최대 완화 폭)**·파운드리 **50→49(▼−1)**·패키징 **67→69(▲+2)**. CAPEX 완화(빅4 전원 상향+메모리원가 귀속)와 전력 악화(PJM 미달 공식화)가 동시 진행 — 하방 위험 1위가 전력(75)으로 근접, 격차 축소.

**신규 소스**: `sources/articles/august-2026-market-update-2026-08-04.md`(5개 병렬 리서치 통합, SemiAnalysis 뉴스레터 원문 403 차단으로 2차 인용 기반 — 원문 대조 권고 플래그).

**갱신 wiki 8종**: (1) `concepts/bottleneck-model-2030.md` — 종합판독(2026-08-04) 신규 절, 4대 지수 표+변동폭. (2) `concepts/ai-capex.md` — 빅4 실적 표(직전 가이던스 vs 확정치)+메모리원가 귀속 상세. (3) `concepts/energy-constraints.md` — PJM 용량경매 미달·변압기 60개월. (4) `driving-forces/key-drivers.md` — DF1·DF2 "외부 데이터 갱신" 항목 추가(정점 재확인). (5) `scenarios/scenario-matrix.md` — 확률 유지 근거에 외부 데이터 갱신 병기. (6) `entities/sk-hynix.md` — Q2 실적(사상최대·컨센서스 미스). (7) `entities/cxmt.md` — HBM3 지연 재확인+LPDDR6 샘플링+STAR Market IPO. (8) `entities/samsung.md` — 볼륨 발주 미전환 재확인+HBM 점유율 상충 플래그. (9) `concepts/dram-antitrust-litigation.md` — 기각신청 예정+2018년 선례.

**동기화 완료**: dashboard indicators.js(bigtech_capex_growth 77→80·samsung_hbm4_rubin_share·cxmt_hbm3_production 정성 갱신)·bottleneckModel.js(MODEL_ASOF 08-04, PREV 07-04, 4대 currentIndex+indexNote 갱신)·BottleneckModel.jsx(헤더 델타 표기 수정)·scenarioPlanning.js(DF1·DF2 currentPosition·시나리오 B summary)·updates.js(신규 entry v2.32.11)·version.js(v2.32.10→v2.32.11 패치). `cd dashboard && npm run build` 통과(경고 없음, 청크 크기 안내만). outputs/report/scenario-planning-report.md(최신 신호 스냅샷 표 2건·병목 레이더 표·작성일 갱신)·outputs/presentation/slide-outline.md(08-04 데이터 갱신 노트 추가, 확률 무변화로 슬라이드 본문·PPTX 재생성은 07-04 노트와 동일하게 다음 구조 개편으로 이연). 위키 링크 구조 무변경(신규 wiki-to-wiki 엣지 없음, source 인용만 추가) → 지식그래프 재생성 생략.
