...
> **이 페이지의 자리** — 세 문서가 역할을 나눈다. **이 페이지(서사)**: 다운턴별로 C·M·O를 정밀 분해하고 메커니즘의 이전 가능성을 심사한다. **[CMO 통합 매트릭스](cmo-matrix.md)(데이터)**: 같은 역사를 대비/대응 국면 × 제품 × 5관점으로 통일해 가로지르는 비교·필터를 가능하게 한다(대시보드 CMO Matrix 탭). **[SP-2 다운턴 시나리오 플래닝](../downturn/README.md)(미래)**: 이 페이지 §4의 "원인 3종"을 원인 축 × **속도 축**의 2×2로 전개해 대비(DP)·감별(DX)·대응(DR) 전략으로 분화한다. 이 페이지가 "과거의 어떤 교훈이 이전 가능한가"를 심사하면, SP-2는 "그 교훈을 어떤 형태의 하강에 어떻게 쓸 것인가"를 설계한다.
...
flowchart LR
    ...
    subgraph S19["2019 다운사이클 2018 Q4~19"]
        C19["C: 3강 과점 첫 다운턴<br/>재고 조정발 절벽 · 후발이 먼저 감산"] --> M19["M: 무임승차형 버티기 +<br/>기술 완주 · HBM팀 축소"] --> O19["O: 1Q20 점유 44.1% ·<br/>3차의 C를 생성"]
    end
    subgraph P3["다운사이클 2022~23"]
        C3["C: 3강 과점<br/>코로나 수요 절벽"] --> M3["M: 역사이클 캐파<br/>공식 재복제"] --> O3["O: 범용 승리 ·<br/>HBM 니치 상실"]
    end
    P1 --> P2 --> S19 --> P3 --> NOW["2026~28 맥락 전환<br/>메커니즘 감사 필요"]
```
...
## 2. 사례 분해 — 네 번의 다운턴, 네 개의 CMO 구성

> **범위 주기**: 관측 다운턴은 1차·2차·2019·3차 네 개다. 2018 Q4~2019 다운사이클은 [CMO 통합 매트릭스](cmo-matrix.md) 편입(대응 6건·대비 3건·P6)에 이어 이 절의 정밀 CMO 구성(CMO-2019)으로도 전개됐다. 번호 체계는 위키 관례를 따라 1·2·3차를 유지하고 2019는 연도로 표기한다.

### 요약 표 — 다운턴 4건의 CMO 구성 + 다음 다운턴 설계
...
| **CMO-2019** · 2019 다운사이클 (2018 Q4~2019) | 3강 과점 성립 후 첫 다운턴 · 서버·모바일 재고 조정발 절벽(DRAM -37~38%) · **후발 2사가 먼저 감산**(Micron 2019-03·SK 2019-07)+요카이치 정전 · 삼성은 정점 확장 직후(2017 CapEx 27.3조 최대·재고 고점·배당 3년 고정) | **무임승차형 버티기** — 무감산·CapEx 유지로 후발의 감산 위에서 버팀 + 기술 4축 세대 완주(1z·136단·Flashbolt·EUV), 동시에 **[배분] HBM팀 축소** | 1Q20 DRAM 점유 44.1%·2020 COVID 반등 흡수로 버티기 회수 / **HBM 축소가 3차 다운턴의 C(출발 위치)를 생성** | 버티기의 ◎는 경쟁사가 대신 감산해줄 때만 — 같은 M이 3차(전원 버티기)에서 ✕. 다운턴 대응의 성적표는 그 다운턴 안에서 끝나지 않는다 |
...
| **CMO-4 (설계)** · 다음 다운턴 (2028~29 창) | §3의 **8대 변화** — 절제 균형+CXMT · 계약 바닥 · 인증 게임 · 비동기 사이클 · CAPEX/ROI 진입 경로 · 추격자 출발 위치 · **계약 만기 구조** · **감가 정점 통과** | §5 액션 판정·§6 경쟁사 벤치마킹·§7의 감사 통과 메커니즘만 재조립 — 계약 바닥 선점(+만기 사다리화) · 역사이클 대상 교정 · 치킨게임 봉인 · 맥락 EWI | (목표) 매출 바닥이 계약으로 보장된 다운턴 · 다음 세대 인증 선점 · 절제 균형 유지 | 과거 공식의 복제가 아니라, 맥락 감사를 통과한 재조립 |
| **CMO-4의 형태 분해** · [SP-2](../downturn/README.md) | 원인 축(수요발↔공급발) × **속도 축**(급락형↔침식형) | 대비 **DP-1~7** → 감별 **DX-1~8** → 대응 **DR-1~6** | DT-A 급제동 20% · DT-B 긴 하산 24% · DT-C 동시 방류 22% · DT-D 저가 잠식 26% · DT-E 판 갈이 8% (**조건부 확률** — 도착을 전제로 한 형태별 배분) | 원인만으로는 처방이 갈리지 않는다 — **언제 결정할지**는 속도가 정한다 |
...
### CMO-2019 · 2019 다운사이클 (2018 Q4~2019) — 무임승차형 버티기와 시한폭탄 배분

- **Context**: 3강 과점 성립 후 처음 맞은 다운턴. 하이퍼스케일러·스마트폰 재고 조정발 수요 절벽(DRAM 2019 -37~38%·4Q18 매출 QoQ -18.3%)에, **후발 2사가 먼저 감산**(Micron 2019-03 웨이퍼 감축 최초 공식화 → SK 2019-07 NAND 투입 15%+ 축소)하고 요카이치 정전(2019-06)이 NAND 공급을 추가로 조였다. 삼성의 출발 위치는 정점 확장 직후 — 2017 반도체 CapEx 27.3조 사상 최대, 2018년 말 재고 고점, 배당 3년 고정 약정 ([samsung-2019-downturn-2017-2019-actions-2026-08-16.md](../../sources/articles/samsung-2019-downturn-2017-2019-actions-2026-08-16.md)).
- **Mechanism**: 1차의 소모전(퇴출 유도)과 다른 **무임승차형 버티기** — 경쟁사가 공급을 줄여주는 동안 1위가 무감산("인위적 웨이퍼 투입 감소 검토 안 함")·CapEx 유지(22.6조)로 버티며 빗 점유율을 흡수한다. 병행 메커니즘은 다운턴 중 기술 전환(1z DRAM·136단 V낸드·HBM2E Flashbolt·EUV 완주). 그리고 같은 해, HBM 전담팀 축소라는 배분 결정이 있었다.
- **Outcome**: 버티기는 회수됐다 — 4Q19 출하 증가로 매출 보합 방어, 1Q20 DRAM 점유 44.1%, 2020 COVID발 반등을 온전한 캐파로 흡수. 그러나 **HBM팀 축소는 2022~23 다운턴의 바꿀 수 없는 출발 위치(C)가 됐다** — 이 다운턴의 성적표에는 안 보였던 비용이 다음 다운턴에서 40%→17%로 청구됐다. 2019가 CMO 렌즈에 주는 고유 교훈: **다운턴 대응의 판정은 그 다운턴 안에서 완결되지 않는다** ([samsung-2019-downturn-2017-2019-actions-2026-08-16.md](../../sources/articles/samsung-2019-downturn-2017-2019-actions-2026-08-16.md)).
...
과거 CMO 구성의 발화 조건과 현재 맥락을 항목별로 대조하면 여덟 개의 구조 변화가 나온다 (#7·#8은 [SP-2](../downturn/README.md) 분석에서 추가된 변수다).
...
| 7 | **계약 만기 구조** | 현물 중심이라 만기 개념 자체가 없음 — 하강이 즉시 매출에 도달 | 커버리지가 하강을 **막는 것이 아니라 미룬다**. 만기가 2028~29에 몰려 있으면 그 시점에 시장가로 한꺼번에 재가격되어, **계약이 급락 방지 장치가 아니라 급락 발사 장치**가 된다 — 커버리지 총량이 아니라 **만기 집중도**가 형태를 결정 ([DP-1](../downturn/preparation.md), [DX-7](../downturn/differential-indicators.md)) |
| 8 | **원가 하한(감가 위치)** | 감가 구조가 사이클 논의의 변수로 다뤄지지 않음 | 삼성 기계장치 내용연수 5년·상각률 77.1%, 감가상각비의 85~93%가 기계장치 → **2028~29에 감가 완료 설비 풀이 최대**가 되면 원가 하한이 내려가고 가격이 더 깊이 갈 여지가 생긴다. 방어 자산이면서 동시에 **과잉의 연료** ([semiconductor-depreciation-cost-structure-2026-08.md](../../sources/articles/semiconductor-depreciation-cost-structure-2026-08.md)) |

## 4. 메커니즘 감사 — 아직 발화하는 것, 부러진 것, 새로 생긴 것

### 요약 표 — 메커니즘별 발화 판정

| 메커니즘 | 과거 발화 사례 | 2026~28 판정 | 판정 근거 (§3 맥락 변수) | 전략 배선 (SP-1) | 다운턴 배선 (SP-2) |
|---|---|---|---|---|---|
...
| 소모전 (치킨게임) | CMO-1·2 (퇴출 → 가격 급등) | **부러짐** | 3강 절제 균형에선 자해(#1) · CXMT는 "이윤 극대화 경기자" 전제 불충족 | 봉인 — RS-6·RS-2·MB-4로 대체 | **DR-2b** 소모전 봉인 = 명시적 **금지 옵션**. 치킨게임 4대 전제가 CXMT 상대로 전부 붕괴 ([memory-cycle-storyline-r3-2026-08-12.md](../../sources/raw-notes/memory-cycle-storyline-r3-2026-08-12.md)) |
...
| 계약적 매출 바닥 | **부분 선례** — 2005-11 Apple 선급 LTA(개별 계약) | **신규 = 정책화** | 메커니즘 자체는 사내 선례가 있고, 신규인 것은 **전사 커버리지 정책화**다. 발화 조건(공급 부족 호황 + 고객 물량 불안)이 지금만 성립(#2) | RS-8·RS-4·D12 | **DP-1** 만기 사다리화 — 커버리지 총량에 **만기 분산 KPI**를 더한다(#7). **DR-3** 계약 방어(가격 인하 대신 물량 이연) |
| SW 전환비용 락인 | 없음 (신규) | **신규** | CMX·SCADA·FDP 소프트웨어 층의 등장 | RS-3 | DP-5 감축 예외(컨트롤러·펌웨어·시스템 SW 인력) |
| 데이터 트리거 규율 | 없음 (신규) | **신규** | EWI 인프라·트리거-행동 배선 가용(#4·#5 감시) | RS-9·D15·D16 | **DP-4** 감별 EWI 배선 🔵 무후회 · **DX-1~8** 원인·속도 판별 · **DR-1** 30일 감별 프로토콜 |
...
**새로 생긴 메커니즘 (과거엔 존재하지 않았거나 제도로는 없었던 것)** — ① *계약적 매출 바닥*: take-or-pay·NTB·Participating Forward는 다운턴이 매출에 도달하기 전에 차단하는 메커니즘이다 ([lee-changsoo-memory-sales-interview-2026-08-03.md](../../sources/raw-notes/lee-changsoo-memory-sales-interview-2026-08-03.md), [rs8-structured-revenue-hedging.md](../strategies/invariant/rs8-structured-revenue-hedging.md)). **단 "과거 세 다운턴 어디에도 없었다"는 정리는 교정이 필요하다** — 2005-11 Apple NAND 장기공급계약(선급 총 $1.25B·5개사)에서 삼성은 이미 선급 LTA의 공급자 당사자였다 ([samsung-pre-downturn-preparation-2005-2022-2026-08-08.md](../../sources/articles/samsung-pre-downturn-preparation-2005-2022-2026-08-08.md), [CMO 통합 매트릭스 §5.5](cmo-matrix.md)). 당시는 고객 주도 개별 계약이었고 전사 커버리지 정책으로 제도화되지 않았다. 따라서 **신규인 것은 메커니즘이 아니라 그 제도화**이며, 이는 난이도의 성격을 바꾼다 — 없던 능력을 만드는 일이 아니라 20년 전 개별 계약으로 했던 것을 정책으로 굳히는 일이다. ② *소프트웨어 전환비용 락인*: CMX·SCADA·FDP 통합으로 고객 이탈 비용을 쌓는 메커니즘 ([rs3-customer-switching-cost.md](../strategies/invariant/rs3-customer-switching-cost.md)). ③ *데이터 트리거 규율*: 정점의 낙관과 바닥의 공포를 우회하는 EWI-행동 배선 ([rs9-demand-inflection-sensing.md](../strategies/invariant/rs9-demand-inflection-sensing.md)).
...
|---|---|---|---|---|---|---|
| P1 | 1차 (2005~07) | **CapEx 선제 삭감** (2007-01 반도체 -18%·메모리 -17%) — 투자 절제 | 네 대비기 중 유일한 호황 말기 브레이크. 단 직전까지 확대가 이어져 **늦었다** | △ | 성립 — 단 확정형 삭감은 되돌림도 어렵다 | [DP-2 옵션형 캐파](../downturn/preparation.md) — 삭감/확대의 이분법을 **단계 구조**로 대체 |
| P2 | 1차 (2005~07) | **양산 투입 이연** (50nm 개발 완료·양산은 2008로) — 전환 타이밍 절제 | 60nm 우위를 쥔 채 진입 → 2008 Q4 반도체 OPM -14%(경쟁사 -40% 이하) | **◎** | 성립 — **성공한 대비 2건 중 하나** | DP-2 · [RS-1](../strategies/invariant/rs1-options-based-capacity.md) — "개발은 선행, 투입은 지연"의 제도화 |
...
### 5.3 도출 원리 — 효과가 분명했던 액션의 공통 분모

판정 ◎ 액션(A1~A4)의 공통 분모는 하나다: **다운턴을 버티는 시간이 아니라 사용하는 시간으로 썼다** — 조직을 재편하고(A1), 싸진 자산을 사들이고(A2), 리드타임을 소화하고(A4), 그 전부를 가능케 하는 체력을 유지했다(A3). §7의 전략 순위는 이 공통 분모의 2026년 번역이다: **2순위(역사이클 대상 교정)는 A2·A3·A4의 직계 후손**이고, A5는 2023-04 감산 선회가 실증한 불발(✕)로서 폐기·대체되며(3순위), A6은 반면교사로 역이용된다(2순위의 R&D 하한·별동대). 주목할 것은 **1순위(계약 바닥)만이 위 대응 액션 목록에 없다**는 점이다 — A1~A6은 전부 "다운턴 도착 후"의 대응이지만, 계약 바닥은 도착 전에만 만들 수 있고 그 창이 지금 열려 있다 ([lee-changsoo-memory-sales-interview-2026-08-03.md](../../sources/raw-notes/lee-changsoo-memory-sales-interview-2026-08-03.md)). 다만 **"사내에 전례가 없다"는 뜻은 아니다** — 대비 국면까지 시야를 넓히면 2005-11 Apple 선급 LTA라는 선례가 있고(§5.1 P3), 없는 것은 그 개별 계약을 전사 정책으로 제도화한 경험이다.

### 5.4 다운턴별 M×C→O 매트릭스 — 삼성 관점

> 📊 **통합·필터 버전**: 아래 표들을 관측 4개 창(1·2차, 2019, 3차)+4차 예측 통합 데이터셋으로 재구성하고 대비/대응 국면과 제품(DRAM·NAND·SSD·UFS)축을 추가한 것이 [CMO 통합 매트릭스](cmo-matrix.md)다 (대시보드 **CMO Matrix 탭**). 이 절이 다운턴별 정밀 분석이라면, 통합 매트릭스는 가로지르는 비교와 필터를 위한 것이다.

위의 액션 판정 표(A1~A6)를 다운턴별로 펼친 것이 아래 매트릭스 4종이다. **행 = 삼성의 액션(M, 메커니즘명 병기)**, **열 = 그 다운턴의 맥락(C, 값 명시)**, **셀 = 그 맥락에서 그 액션이 만든 결과(O)**다. 모든 셀은 "C가 [값]이었으므로 M은 [발화/부분 발화/불발] → [관측 결과]" 템플릿을 따르며, 판정은 4등급 — **◎**(효과 분명: 수치 관측+시간 선행+C 경로 서술 가능) · **△**(부분·조건부) · **✕**(역효과·불발) · **—**(그 M×C 상호작용이 무의미해 비움) — 에 증거 첨자 ¹(1차 수치) ²(2차 서술) ³(추정, ◎³ 금지)를 붙인다. 같은 O가 여러 셀에 걸치면 primary 셀 1곳에만 완전 서술하고 나머지는 "↖(행×열 참조)"로 표시하며 색을 한 단계 낮춘다. 색은 대시보드(Storyline 탭 → CMO 렌즈)에서 셀 배경으로 구분되고, 위키에서는 이모지 — 🟩(◎ primary)·🟢(◎ 참조)·🟡(△)·🟥(✕) — 와 기호로 이중 부호화한다. 무행동도 배분 결정이므로 `[배분]` 태그로 행에 포함한다.
...
#### CMO-2019 · 2019 다운사이클 (2018 Q4~2019)

**M/C 판별 기준**: 2018 Q4 진입 시점의 소여(3강 과점 구조, 재고 조정발 수요 절벽, 후발의 감산 개시와 요카이치 정전, 정점 확장·재고 고점·배당 고정이라는 출발 위치) = C, 기간 중 선택(무감산 유지, CapEx 유지, 기술 세대 완주, HBM팀 축소, 비전 2030, 수출규제 대응) = M. 증거 등급 주의 — 이 표의 근거 소스는 원문 프록시 차단으로 전 항목 검색 요약 경유라 **첨자는 전부 ²(◎¹ 없음)**.

C 열 정의:
- **C19-a 경기자 구조** = 3강 과점 첫 다운턴 · **후발이 먼저 감산** (Micron 2019-03 웨이퍼 감축 최초 공식화 → SK 2019-07 NAND 15%+ 축소·M10 CIS 전환) + 요카이치 정전(2019-06, 3Q19 NAND 공급 ~17%↓)
- **C19-b 배분 규칙** = 캐파·원가 게임 유지 (인증 게임 이동 전 — HBM은 아직 니치)
- **C19-c 수요·거래 구조** = 서버·모바일 재고 조정발 절벽 (DRAM 2019 -37~38%·4Q18 QoQ -18.3%) + 일본 수출규제 외생 충격(2019-07)
- **C19-d 삼성 출발 위치** = 정점 확장 직후 (2017 CapEx 27.3조 최대·2018년 말 재고 고점·배당 3년 고정) · 반도체 영업이익 -69%(14.02조)

| M (액션 · 메커니즘) | C19-a 경기자 구조 = 후발이 먼저 감산·정전 | C19-b 배분 규칙 = 캐파·원가 게임 유지 | C19-c 수요·거래 = 재고 조정발 절벽·수출규제 | C19-d 출발 위치 = 정점 확장 직후·OP -69% |
|---|---|---|---|---|
| **무감산 공식 기조 유지** ("인위적 웨이퍼 투입 감소 검토 안 함" 2Q19 컨콜 + 12라인 R&D 용도 전환) — 메커니즘: 무임승차형 버티기 | 🟩**◎²** C가 "후발이 먼저 감산+정전"이었으므로 버티기 M은 무임승차로 발화 → 삼성만 캐파 유지, 4Q19 출하 증가로 매출 보합 방어·1Q20 DRAM 점유 44.1% [대조 ᵃ] | — | 🟢**◎²** ↖(무감산×C19-a 참조) — 절벽 국면에서 출하 확대가 가격 하락을 상쇄한 경로 | 🟡**△²** C가 이익 -69%였으므로 버티기 비용도 노출 — 부분 발화. 단 재무 요새와 배당 고정 약정 이행이 동시에 감당됨 |
| **CapEx 삭감 없이 유지** (2019 반도체 22.6조) — 메커니즘: 재무 요새 기반 역사이클 유지 | 🟢**◎²** ↖(CapEx×C19-d 참조) — 경쟁사 삭감 국면에서의 유지 자체가 점유 압박 병행 | — | — | 🟩**◎²** C가 정점 확장 직후·이익 급감이었음에도 유지 → Counterpoint "지속 투자가 2020 반등기 선도 지위 강화" — 2020 COVID 반등을 온전한 캐파로 흡수 [대조 ᵇ] |
| **기술 세대 전환 완주** (1z DRAM 2019-03·136단 V낸드 2019-08·HBM2E Flashbolt 2019-03·EUV D1x 준비) — 메커니즘: 다운턴 중 기술 전환 | — | 🟩**◎²** C가 원가=점유율 게임이었으므로 세대 선행 M 발화 → 2020~21 상승기 원가·세대 우위로 회수 [대조 ᶜ] | 🟢**◎²** ↖(기술 완주×C19-b 참조) — 회복기(2020 서버 수요) 방향과 정렬 | — |
| **[배분] HBM 전담팀 축소** (2019 — "니치" 판단·자원 범용 재배치) — 메커니즘: 주력 논리에 의한 니치 배제 | — | 🟥**✕²** C가 아직 캐파 게임이었으므로 축소 결정이 당시에는 합리적으로 보였다 — 그러나 이는 C의 미래 이동(인증 게임화)을 읽지 못한 배분 → 이 다운턴의 성적표에는 비용이 안 보였고, 3차 다운턴의 C(HBM 후순위 출발)로 청구됨: 40%→17%·33년 만의 역전. 같은 해 SK는 반대 방향 | — | — |
| **[배분] 반도체 비전 2030 선언** (2019-04-24 — 시스템반도체 133조·15,000명) — 메커니즘: 다운턴 중 포트폴리오 다각화 | — | — | — | 🟡**△²** C가 요새 체력이었으므로 절벽 한복판의 초장기 공표가 가능 — 파운드리 확장으로 계승(2021 171조). 단 메모리 사이클 대응 효과는 간접(2차의 5대 신수종 △과 같은 계열) |
| **수출규제 대응** (공급처 다변화·JSR-imec 벨기에 레지스트 우회·국산화 착수) — 메커니즘: 공급망 이원화 | — | — | 🟩**◎²** C에 지정학 외생 충격(2019-07)이 겹쳤으므로 이원화 M 발화 → 생산 차질 없이 통과, 소재 국산화(솔브레인 2020-01 등)로 구조화 — 공급망 리스크 관리 제도화의 계기 | — |

- ᵃ 동일 C에서 Micron은 감산 최초 공식화, SK는 투입 축소+라인 용도 전환, Kioxia/WD는 정전으로 비자발 축소 — 무감산 유지는 삼성 단독. 1위의 버티기가 후발의 감산 위에서 작동한 무임승차 구조.
- ᵇ 동일 C에서 Micron FY19 CapEx $10.5B→$9B 삭감·FY20 추가 축소 예고, SK 2020 CapEx 상당폭 축소 방향 — 유지에 가까운 집행은 삼성 단독.
- ᶜ 동일 C에서 1z 세계 최초·9x단 이상 V낸드 유일 생산 — 단 같은 해 HBM 조직 축소와 병존 (제품 로드맵과 조직 배선의 불일치).

> **고정 각주** — ① *반사실 한계 + 증거 등급*: "삼성도 감산했다면"의 대조군은 없다. 경쟁사 대조(ᵃ~ᶜ)로 보완하되, 근거 소스가 전부 검색 요약 경유라 모든 첨자가 ²이다 ([samsung-2019-downturn-2017-2019-actions-2026-08-16.md](../../sources/articles/samsung-2019-downturn-2017-2019-actions-2026-08-16.md) §미확인 15건 참조). ② *C 교호작용*: C19-a(후발 감산)와 C19-d(1위 체력)가 **결합해야** 무임승차가 발화한다 — 무감산의 ◎는 경쟁사 행동에 의존한 조건부 성공이며, 3차에서 같은 M이 ✕가 된 것은 이 결합(누군가 대신 줄여주는 구조)의 부재 때문이다. 또한 C19-b가 아직 캐파 게임이었다는 사실이 M4(HBM 축소)를 당시 합리적으로 보이게 했다 — **한 다운턴의 배분(M)이 다음 다운턴의 소여(C)가 되는 순환**이 이 표에서 가장 선명하다. ③ *채움률*: 10/24 = 42%.
>
> 출처 일괄: 무감산 워딩·CapEx 22.6조·기술 4축·HBM팀 축소·비전 2030·수출규제 대응·경쟁사 감산 순서·1Q20 44.1%는 [samsung-2019-downturn-2017-2019-actions-2026-08-16.md](../../sources/articles/samsung-2019-downturn-2017-2019-actions-2026-08-16.md), HBM 40%→17%·33년 만의 역전은 [dram-market-share.md](../concepts/dram-market-share.md).
...
> **[2026-08-15 이후] 이 원인 3종은 SP-2로 전개됐다** — 원인 축(수요발↔공급발)에 **속도 축**(급락형↔침식형)을 직교시켜 2×2 + 와일드카드로 재구성한 것이 [SP-2 시나리오 매트릭스](../downturn/scenario-matrix.md)다. 아래 표는 그 **가로축만 담은 초기 형태**로 읽어야 한다. 속도 축이 추가되면서 드러난 것은, 원인이 "무엇을 조절할지"를 정한다면 속도는 **"언제 결정할지"**를 정한다는 점이다 — 급락형은 준비의 문제(도착 전 배선이 결과의 전부), 침식형은 인지의 문제(시간은 있으나 위기감이 없어 결정이 미뤄진다)다. 아래 "시나리오 연결" 열의 확률은 **SP-1 무조건부 확률**이며, SP-2의 DT-A~E 확률은 도착을 전제로 한 **조건부** 값이라 서로 더하거나 비교할 수 없다.
...
- *무엇을 하자는 것인가*: "사겠다고 약속한 물량은 안 사가도 돈을 내는" take-or-pay 다년 계약, "이 가격 밑으로는 안 판다"는 가격 하한(NTB), 하한을 보장받는 대신 상승분의 절반을 나누는 Participating Forward를 계약 커버리지의 표준으로 만드는 것 — [RS-8 구조화 매출 헷지](../strategies/invariant/rs8-structured-revenue-hedging.md)와 [RS-4 고객 분산·장기계약](../strategies/invariant/rs4-customer-portfolio-diversification.md)이고, 실행 결정은 D12다. **여기에 [DP-1](../downturn/preparation.md)이 한 차원을 더한다 — 커버리지 *총량*만이 아니라 *만기 분산*을 KPI로 삼는 것**이다. 계약은 급락을 막는 것이 아니라 미루므로, 만기가 2028~29 한 시점에 몰리면 그때 시장가로 한꺼번에 재가격되어 커버리지가 두꺼울수록 낙차가 커진다. 만기 집중도(HHI) 산출 체계는 아직 **미측정**이며([DX-7](../downturn/differential-indicators.md)) 2026 Q4까지 세우는 것이 첫 실행 항목이다.
- *왜 1순위인가*: §5 대응 액션 목록의 유일한 공백이자 §6 B4(마이크론 계약·수요 선점 역순)가 이미 실증한 구조이며, 그 발화 조건(C)이 "공급 부족 호황 + 고객의 물량 불안"인데 이 조건은 지금만 성립한다 — 고객들이 스스로 선수금을 예치하고 다년 계약에 서명하는 국면은 다운턴이 시작되는 순간 끝난다 ([lee-changsoo-memory-sales-interview-2026-08-03.md](../../sources/raw-notes/lee-changsoo-memory-sales-interview-2026-08-03.md), [micron-q3-fy26.md](../../sources/filings/micron-q3-fy26.md)). 과거 공식이 "폭락을 체력으로 버틴다"였다면, 새 공식은 **"폭락이 매출에 도달하지 못하게 한다"**다.
...
**SP-2가 이 순위에 더하는 것 — 속도 축과 몸의 문제**

이 네 순위는 전부 **원인**을 다룬다. [SP-2](../downturn/README.md)가 추가한 속도 축은 여기에 두 가지를 얹는다.

1. **감별을 대응보다 앞에 놓는다** — 4순위(맥락 EWI)는 이 렌즈에서 마지막이지만, 다운턴이 실제로 도착하면 **첫 번째 액션**이 된다: 도착 판정 후 30일간은 비대칭 액션을 금지하고 원인·속도를 확정한 뒤 처방을 분기시키는 것이 [DR-1 감별 프로토콜](../downturn/response-playbook.md)이다. 이 렌즈의 언어로는 — **어떤 M을 발화시킬지 정하기 전에 C가 무엇인지부터 확정하라.** 판정서에는 반증 조건과 기각한 대안 가설을 반드시 함께 남긴다.
2. **몸을 바꿀 수 있는지가 새 변수다** — 감산이 무효인 수요발·침식형에서 남는 액션은 캐파의 **목적지 변경**이다. DRAM↔CIS 설비 공용률 80%(전환 시 캐파 -50%), 전환 리드타임 9~12개월이라는 실물 제약 때문에 이것은 도착 후에 시작할 수 없다 — 평시에 최소 1회 실증해두는 것이 [DP-6](../downturn/preparation.md)이고, 과거 선례는 오스틴 메모리→로직 전환($3.6B, 2010-06 착수 → 2012-08 완료)이다. **대비기에 착수했기에 다운턴 중에 완료할 수 있었다**는 것이 이 선례의 CMO적 교훈이다.

> 📊 대시보드에서 자세히: 전략 전체는 **Strategy 탭 → Robust Strategy / Core Strategy**, 17개 결정은 **Strategy 탭 → Decisions**, 트리거는 **EWI 탭**, 다운턴 대비·대응·감별은 **Scenario Planning 탭 → 다운턴 시나리오 플래닝(SP-2)**에서 볼 수 있다.
