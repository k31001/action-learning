...
3. **고객·생태계 출신 앵커 + 상주 로테이션**: 하이퍼스케일러 스토리지팀·NVIDIA DevTech·캐시 관리자(LMCache·FlexKV·Mooncake) 커미터 출신을 Co-Design Pod 앵커로 영입하고, 본사 펌웨어·FTL 엔지니어를 고객 상주로 6~12개월 로테이션한다. 스타트업 팀 인수(ScaleFlux acqui-hire)는 선택지에서 제외한다(사용자 결정 2026-09-17): 목표는 팀을 사는 것이 아니라 고객 시스템 안에 우리 사람이 들어가는 것이다. 시장 기준가만 참고로 남긴다(Astera↔Pliops 약 $70M/60명, 같은 소스 §2). Tensormesh(LMCache)는 NVIDIA·AMD·CoreWeave가 이미 지분을 가져 공동 투자·기여가 현실적이다.
...
| Y-2 | 고객·생태계 출신 앵커 영입 + 본사 엔지니어 상주 로테이션 1기 복귀(Phase 2 역량 내재화) | 영입·로테이션 |
