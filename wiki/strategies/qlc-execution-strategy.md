...
**안팎의 같은 진단** (2026-09-17 5차 피드백: 전략의 신빙성. 덱에 인용):
- **외부 — 신문섭(Bain & Company 파트너, 2026-06-18)**: "앞으로의 승부는 칩을 많이 파는 기업이 아니라, **고객의 아키텍처 안으로 들어가 수요를 함께 설계하는 기업**이 가져갈 것이다." 메모리 기업의 경쟁력은 범용 제품을 싸고 빠르게 만드는 능력에서 고객별 문제를 해결하는 능력(커스터마이제이션·하드웨어를 이해하는 임베디드 SW·고객과의 공동 설계)으로 이동한다 ([expert-interview-ai-infra-supercycle-2026-06-18.md](../../sources/raw-notes/expert-interview-ai-infra-supercycle-2026-06-18.md) §1·최종 메시지). → 목표 ②(들어간다)의 외부 진단. 덱 S2 리드.
- **내부 — 송용호(AX/PI센터장, 2026-09-03)**: "메모리는 부품이고, **이 부품이 어떻게 쓰일지는 시스템을 설계하는 사람 마음에 있다. 그걸 알았으면 HBM을 진작 준비했을 것이다.** 고객이 가려운 데를 조금이라도 빨리·정확하게 보려면 고객을 계속 만나고 친해져야 한다." → 목표 ①(워크로드를 받는다)의 이유. 덱 S1 스티커.
- 같은 인터뷰: "우리는 말로는 고객 지향을 하지만 솔직히 **단 한 번도 고객 지향적인 적이 없었다.** 진짜 고객 지향이 뭔지 이해하고, 고객 지향을 하기 위한 전략도 필요한 시점이 이미 됐다"(게임의 룰이 코디자인·코패키징으로 고객과 함께하는 룰로 바뀌었다는 과제팀 진단에 100% 동의) → 문화·인사 축의 근거. 덱 S3 리드. "**고객의 집에 가서 저녁을 같이 먹는 것 — 삼성은 상상할 수 없는 일이지만 SK하이닉스는 한다**" → 관계가 곧 워크로드 접근권, 미주 현지 채용·FDE 상주(§2.3)의 근거. 덱 S3 삼성 개발실 박스 ([song-yongho-ax-pi-interview-2026-09-03.md](../../sources/raw-notes/song-yongho-ax-pi-interview-2026-09-03.md) 1:03:19·1:11:42, 자동 전사 문구는 문맥 보정 — `dashboard/src/data/interviews.js`와 동일).
...
> **덱 범위 결정(2026-09-19)**: 발표 덱(v5.0)은 개발실 내부에서 실행 가능한 범위만 담는다 — 시스템 소프트웨어 조직 강화, 개발실 소속 Co-Design Pod(3~5명, 워크로드를 여는 고객 1~2사 선별), 미주 법인 협업·본사 엔지니어 상주 로테이션, 오픈소스 메인테이너 문화. 자회사 설립·별도 보상 체계·지분 참여·결정 요청은 위키에만 유지하고 덱에서는 제외한다. FDE 상주는 트레이스·KV 정책 접근을 허용하고 캐시 관리자 코드를 자체 운영하는 고객(AI 랩·NVIDIA 생태계 등)에 한정하고, 하이퍼스케일러는 OCP 규격·업스트림 채널, OEM·네오클라우드는 레퍼런스 스택으로 협업한다. 비용은 별도 투자가 아니라 개발실 개발 자원 투입이며, 판돈은 락인으로 얻는 추론 캐시 계층 점유율(2030년 350EB 중 QLC 조건부 상방 175EB, [qlc-ssd-market.md](../concepts/qlc-ssd-market.md) §4.3)이다.
