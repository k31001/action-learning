...
- **Distinguished Engineer(DE) 트랙 신설**: 시스템 아키텍트 최고 역량을 임원급으로 대우하는 별도 기술직 트랙 — 고객사와 대등하게 기술을 토론·주도하는 **스타 엔지니어**의 공식 자리. 관리자 승진 없이도 최고 처우에 도달하는 이원 경로. 상세·채용 논거는 [§4.6](#46-스타-엔지니어--distinguished-engineer-트랙-조직문화)
...
- 이 국면이 스타 영입 협상에 **유리한 위치**를 만든다:
  ...
  2. **정상화**: 업계 보상이 상향 리셋 → 일반 메모리 직원도 이미 6억대이므로, 스타 DE에게 최상위 밴드를 제시해도 사내 형평성 마찰이 과거보다 작다
  3. **명분**: "반도체 특수" 국면의 핵심 인재 확보·유지는 이사회·주주가 납득하는 투자
- **양면성 = 추가 시급도**: 동일 국면이 SK하이닉스의 삼성 인재 poaching 압력도 키운다 (메모리 6억 vs 비메모리 2억 격차·이직 우려). **DE 트랙은 영입뿐 아니라 이미 보유한 스타의 이탈 방어(retention) 수단**이기도 하다.

**(3) 조직 형태**
- 관리자 승진과 분리된 **이원 경력 경로(dual-ladder)**: Distinguished Engineer / Technical Fellow를 임원급으로 대우, 관리 보직 없이도 최고 처우 도달.
- DE의 역할: Co-Design Pod·시스템 모델링 조직의 기술 리더, 고객사 아키텍트의 카운터파트, 로드맵·표준을 자사 유리하게 드라이브하는 대외 얼굴 (기술 마케팅·백서의 저자).
- 호명 문화: 사내외에 개인을 드러냄 — 컨퍼런스 발표·기술 백서 저자·특허 대표발명자·고객 공동설계 리드로 **이름을 남기게** 한다.
...
| A5 | **Distinguished Engineer 트랙 설계** — 이원 경력 경로·처우 밴드·임명 기준. 처우 급등 국면 활용해 스타 영입/유지 협상 프레임 마련 | DE 트랙 제도안 | 개발실 + HR |
...
| B6 | **DE 1호 임명 + 스타 아키텍트 영입 1명** — 사내 발탁 + 외부 영입 병행, 호명 문화 가동(컨퍼런스·백서) | DE 임명·영입 |
...
| Distinguished Engineer(스타 아키텍트) 수 | 0 (트랙 없음) | 1명+ 임명·영입 | 5~10명 · 대외 호명 정착 |
