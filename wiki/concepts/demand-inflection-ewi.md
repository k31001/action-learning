...
- **자동 갱신 vs 수동**: `ai_dc_credit_spread`는 HYG(광의 HY 회사채 ETF) 6개월 역행·듀레이션 환산 프록시로 일 1회 자동 갱신(금리 영향 포함 — 광의 신용 프록시). **GPU 임대가는 자동 연동하지 않음** — CRWV(CoreWeave) 주가는 임대 *가격*이 아니라 *지분 가치*를 추종해(예: 6개월 +30%인데 실제 임대가는 하락) 핵심 선행 신호를 오인시킬 위험이 커 수동 유지(ClusterMAX/Vast.ai 큐레이션). 스팟-계약 괴리(DXI)·재고·취소 건수도 무료 실시간 피드 부재로 수동.
