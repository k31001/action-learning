...
- **자동 갱신 vs 수동**:
  - `gpu_rental_h100_usd` (Tier0 최선행) — **Vast.ai 공개 오퍼 API** 실측. H100 SXM on-demand 중앙값 $/GPU·h를 일 1회 자동 갱신, 매일 누적되어 추세 형성(실측 현물가, 프록시 아님). ※ CRWV(CoreWeave) 주가 프록시는 거부 — 임대 *가격*이 아니라 *지분 가치*를 추종(6개월 +30%인데 실제 임대가는 하락)해 핵심 선행 신호를 오인시킴.
