...
>
> 대시보드 미러: `dashboard/src/data/bottleneckModel.js` + `dashboard/src/components/BottleneckModel.jsx` (**Bottleneck Model 탭** — 2026-06-11부터 병목 모델·수요 변곡 EWI 서브탭의 통합 홈. 수요 EWI는 Data Viz 서브탭에서 이쪽으로 이동, §5 드라이버 트리가 두 축을 한 인과 구조로 연결).
...
**조기경보 규칙**: ① **악화 예고** — 상류(d2) 압력이 현재 제약지수보다 +10 이상 높으면 선행시차 후 병목 심화 예상. ② **완화 예고** — d2가 현재보다 −15 이상 낮으면 완화 방향. ③ **상류-중류 괴리** — d2−d1 ≥ +10이면 중류는 아직 조용하나 상류가 먼저 꺾인 상태(가장 이른 경보). 2026-06-11 기준: CAPEX ③(+11)·파운드리 ③(+33)·패키징 ②(−27) 발동.
...
- **패키징**: 유일하게 **상류가 현재보다 낮음**(d2 45 < 현재 72, −27) — 신규 사이트·기판 완화가 진행되면 2027~28 완화 방향 (**완화 예고**). 단 16-Hi 수율(▼)이 변수.
- **수요 변곡 EWI와의 통합**: 수요 EWI의 ①수요청산가(GPU 임대가)·②돈(capex·신용)·③발주(CoWoS) 신호가 본 트리의 CAPEX·패키징 상류와 **동일 신호의 양면**임이 트리에서 명시됨(`연계` 표기) — 대시보드에서 두 체계는 Bottleneck Model 탭 안에 통합(병목 모델 + 수요 변곡 EWI 서브탭).
