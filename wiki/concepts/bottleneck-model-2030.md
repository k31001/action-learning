...
#### 제약지수 업데이트 (2026-07-04, 이전 2026-06-14 대비)

| 병목 | 이전 지수 | 현재 지수 | 변동 | 주요 근거 |
|---|---:|---:|---:|---|
| **전력** | 70 | **72** | **▲ +2** | ERCOT 대형부하 접속 큐 1년 만에 거의 4배(410GW+, 87% DC·크립토)·PJM 2030년까지 최대 15GW 구조적 부족(신규 공급 연 2~3GW < 신규 DC 부하 연 5~7GW)·변압기·개폐장치 리드타임 최대 5년으로 가스터빈(~3년)보다 강한 신규 병목으로 부상. SemiAnalysis: 그리드 신규 대형부하 여력이 2027년 조기 마이너스 전환 가능 |
| **CAPEX/ROI** | 42 | **40** | **▼ −2** | Microsoft $190B(+61%, 증가분 중 $25B을 메모리 원가 상승에 직접 귀속)·Alphabet $180~190B·Amazon $200B 추가 상향. Dell'Oro 2026 글로벌 DC CAPEX **$1조 돌파** 전망(2026-06-10, 메모리·스토리지 가격 인플레를 CAPEX 상승 요인으로 명시). JPMorgan 2030 누적 AI 인프라 CAPEX $5.1조→**$5.5조** 상향. HY OAS ~285bp 안정(스트레스 신호 없음) |
| **파운드리** | 52 | **50** | **▼ −2** | TSMC N2 2026-03 매출 기여 개시·2026 Q3 유의미 기여 전망으로 램프 순항 재확인. ASML High-NA EUV 도입을 TSMC가 최소 **2029년까지 연기**(근시일 기술 리스크 축소). NVIDIA Rubin 출하 비중 22% 하향 유지로 캐파 여유 지속 |
| **패키징** | 68 | **67** | **▼ −1** | NVIDIA CoWoS 배정 ~60%(~59.5만 장) 확정·TSMC 2026년 24~27만 장 OSAT 외주로 공급 대응 진행. 단 CoPoS 전면 양산이 **2028년 하반기~2029년**으로 재확인(지연 유지)·HBM4 하이브리드본딩 시험 수율 ~10%에 그쳐 업계가 **마이크로범프 유지로 선회**(하이브리드본딩 연기) — 완화는 지속되나 폭이 이전 대비 축소 |

**전력: 4개 병목 중 유일하게 2개 분기 연속 재상승(68→70→72) — 큐 폭증(ERCOT 4배)과 변압기 리드타임(5년, 신규 최대 병목)이 구조적 악화를 재확인. 하방 위험 순서 불변: CAPEX/ROI > 전력 ≈ 패키징 > 파운드리. 파운드리·CAPEX는 완화 지속, 패키징은 완화 속도 둔화.**

#### 2026-07-04 핵심 신규 신호

- **전력 그리드 — 큐·설비 이중 병목**: PJM 최초 개편 큐 Cycle 1(2026-04-29)에 811개 프로젝트·220GW nameplate 신청(가스 105.8GW 주도), 그러나 PJM 자체 전망은 2027~28년 6.6GW 부족·2030년까지 최대 15GW 구조적 부족을 경고. ERCOT 큐는 1년 만에 410GW+로 거의 4배 폭증(87%가 DC·크립토)했으나 실제 신규 발전은 2024~25년 ~23GW+2026년 9GW뿐 — 신청:증설 배율 약 45:1. **변압기·개폐장치 리드타임이 최대 5년으로 확대되며 가스터빈(~3년, 2028년까지 예약 마감)보다 강한 신규 병목**으로 부상. 원자력·SMR DC향 커밋은 9.8GW+로 확대(전 주요 빅테크 참여)되었으나 가동 시점은 대부분 2028년 이후 — 단기 완화 효과 제한적 ([july-2026-market-update-2026-07-04.md](../../sources/articles/july-2026-market-update-2026-07-04.md)).
- **CAPEX 완화 신호 강화**: 빅테크 5사(MS·Alphabet·Amazon·Meta·Oracle) 개별 가이던스가 일제히 추가 상향되며, 흥미롭게도 **메모리 가격 상승 자체가 CAPEX 상승의 명시적 요인으로 지목**(MS CFO: $25B을 메모리 원가에 귀속, Dell'Oro도 동일 언급) — 메모리 공급업체엔 매출 단가 상승이 곧 고객 CAPEX 확대로 되먹임되는 구조. JPMorgan·Dell'Oro 모두 장기 전망을 상향 조정. 신용시장은 안정적(HY OAS 285bp)이나 SPV·부외부채 의존(~$120B)은 잔존.
- **파운드리·패키징 혼조**: 파운드리는 N2 램프·ASML High-NA 연기(리스크 축소)로 완화 지속. 패키징은 NVIDIA 배정 확정(60%)·OSAT 외주 확대로 공급 대응이 진전됐으나, **CoPoS 전면 양산 2028~29 재확인**과 **HBM4 하이브리드본딩 수율 10%로 인한 마이크로범프 유지 결정**이 완화 속도를 늦춤 — 최종 상방 병목 축이 파운드리에서 패키징 쪽으로 일부 이동할 가능성 모니터링 필요.
- **HBM 공급망 재편 — Micron 일시 배제 후 복귀**: NVIDIA가 HBM4 검증·핀속도 이슈로 초기 Vera Rubin 빌드를 삼성·SK 2사 중심으로 계획했다가, 2026-06-01 GTC 타이베이에서 3사 전원 공급을 재확인 — 공급망 자격 검증이 알려진 것보다 굴곡이 컸음을 시사. Vera Rubin HBM4 배정은 SK하이닉스 60~70%·삼성 25~30%·Micron 잔여로 세분화.
...
#### 제약지수 업데이트 (2026-09-01, 이전 2026-07-04 대비)

| 병목 | 이전 지수 | 현재 지수 | 변동 | 주요 근거 |
|---|---:|---:|---:|---|
| **전력** | 72 | **76** | **▲ +4** | ERCOT가 2026-08-03 Abbott 주지사 지시로 신규 데이터센터 접속 절차를 **전면 일시중단**(감사 완료 시한 2026-12-10) — 접속 신청 474GW 중 ~90%가 DC. Batch Zero 접속연구가 첫 마일스톤(08-07)부터 지연돼 PUC에 예외 신청. BNEF: 최대 49.8GW 지연·비용 최대 $15억 추가 전망. 07-04의 "큐 적체"(점증적 병목)에서 "접속 절차 자체의 정지"(레짐 전환)로 성격이 악화 |
| **CAPEX/ROI** | 40 | **38** | **▼ −2** | Q2 2026 실적 시즌 완료 — 빅4(MS·Alphabet·Amazon·Meta) 2026 CapEx 가이던스 전원 추가 상향(합산 ~$745~750B, +82% YoY), 삭감 0건 재확인. HY OAS 안정 유지. 단 Meta FCF -91%·Amazon TTM FCF 적자 등 CapEx-FCF 다이버전스 실측 확인(잔존 리스크) |
| **파운드리** | 50 | **48** | **▼ −2** | TSMC N3(3nm) 월 웨이퍼 투입이 18만 장 목표를 원 예상보다 **2~3개월 조기 달성**(2026 Q4 초) 전망. N2(2nm)는 현재 대만 5개 팹 합산 2만 장/월·연말 10만 장/월 목표 경로 그대로 순항. ASML High-NA 2029년 연기 유지 |
| **패키징** | 67 | **65** | **▼ −2** | TSMC CoWoS 수급 갭이 2026년 말까지 **20%→10%로 축소**된다는 최초의 정량 전망 확인(수율 98~99%). **삼성 HBM4(마이크로범프) 수율이 80% "골든수율"에 업계 예상보다 약 4개월 조기 도달**(연말 목표 85%로 상향, 3Q26 HBM4 매출 전분기 대비 3배+ 전망) — 07-04에 "미지수·악화" 플래그였던 적층 수율 드라이버의 구체적 완화 신호. 단 HBM4 하이브리드본딩은 3사 전원 여전히 마이크로범프 유지(HBM4E/5E로 이연), CoPoS 전면 양산도 2028~29로 불변 — 완화는 지속되나 구조적 지연축은 그대로 |

**전력이 4대 병목 중 최초로 Red 밴드(75~85) 진입 — 4개 점검 연속 재상승(64→68→70→72→76)이자 유일하게 악화 방향이 계속 이어지는 병목. CAPEX/파운드리/패키징 3개는 모두 완화 지속 — 자본·공급망 측 제약은 점진 해소되는 반면, 전력은 큐 적체를 넘어 규제 절차 정지라는 더 강한 형태로 구조 악화가 재확인됐다. 하방 위험 순서(충격 % 기준)는 CAPEX/ROI > 전력 ≈ 패키징 > 파운드리로 불변이나, 현재 지수(발생 근접도) 기준으로는 전력이 처음으로 단독 최고치다.**

#### 2026-09-01 핵심 신규 신호

- **ERCOT 접속 전면 중단**: 2026-08-03 텍사스 주지사 Greg Abbott가 PUC·ERCOT에 데이터센터 프로젝트 전수 "포괄적 검증·감사"를 지시하고, ERCOT는 즉시 신규 접속 절차를 중단했다. 감사 범위는 세제 혜택·공적 재정지원·전력수요·자가발전·냉각기술·지역사회 영향·소유구조까지 포괄 — 접속 심사를 넘어선 사업 전반 재검증. PUC 보고 시한은 2026-12-10, Batch Zero 연구는 이미 첫 마일스톤(08-07)을 놓쳐 예외 신청 중 ([september-2026-market-update-2026-09-01.md](../../sources/articles/september-2026-market-update-2026-09-01.md)).
- **TSMC 선단 램프 가속**: N3 월 웨이퍼 투입이 4Q26 초 18만 장에 도달할 전망(원 예상 대비 2~3개월 조기, NVIDIA·AMD·Broadcom 주문 견인). N2는 목표 경로(연말 10만 장) 그대로 순항 — 파운드리 축의 완화가 속도까지 붙는 방향으로 재확인.
- **CoWoS 수급갭 최초 정량 축소**: TSMC CoWoS 수급 갭이 2026년 말까지 20%에서 10%로 좁혀진다는 정량 전망이 처음 제시됨(TrendForce). 캐파(12만~14만 장/월 + OSAT 5만~6만 장)와 수율(98~99%) 수치는 07-04와 대체로 정합.
- **삼성 HBM4 골든수율 조기 달성**: 2026-02 양산 개시 이후 약 6개월 만에 수율이 60%대에서 **80%("골든수율")**로 상승 — 업계 예상보다 약 4개월 빠르다. 이는 07-04에 확인된 **하이브리드본딩 프로토타입**(16-Hi, 시험수율 ~10%)과는 별개로 **양산에 쓰이는 마이크로범프 기반 정규 HBM4** 라인의 수율이다. 삼성은 연말 목표를 85%로 상향하고 3Q26 HBM4 매출을 전분기 대비 3배 이상으로 전망 — HBM4 전략이 "기술 추격"에서 "물량 확대"로 이동하는 신호.
- **HBM4 하이브리드본딩 재이연 재확인**: 삼성·SK하이닉스·Micron 3사 모두 HBM4는 마이크로범프를 유지하고, 하이브리드본딩은 HBM4E 또는 HBM5E로 이연한다는 07-04 판단이 재확인됐다 — 변동 없음.
- **CAPEX 축 신규 소스 없음**: Q2 2026 실적 시즌(빅4 전원 상향, 합산 ~$745~750B)은 이미 08-11 소스에 반영돼 있어 이번 점검은 해당 수치를 그대로 이월 인용했다.

- **전력**: **현재 지수 4 상향(72→76)** — 처음으로 Red 밴드 진입. 상류(d2)의 변압기 리드타임(5년)·BTM 공급망 장주기는 불변인 가운데, 중류(d1)의 계통 접속(interconnect) 드라이버가 "긴장"에서 텍사스 한정 **"임계" 수준으로 재확인**됐다 — 큐 적체라는 정량적 악화에 이어 접속 절차 정지라는 정성적(규제) 악화가 겹쳤다. §5 조기경보 규칙상 상류-중류 괴리는 유지되나, 이번엔 중류(d1) 자체가 상류만큼 나빠진 드문 사례.
- **CAPEX**: 지수 **2 추가 하향(40→38)** — 중류(d1) 완화가 Q2 실적 실현으로 재확인. 상류(d2)의 GPU 임대가·자금조달 긴장은 07-04 대비 변동 신호 없음(신규 소스 부재), Meta FCF -91% 등 CapEx-FCF 다이버전스는 잔존 리스크로 유지.
- **파운드리**: 지수 **2 추가 하향(50→48)** — 중류(d1)의 N2/N3 램프가 목표 대비 오히려 조기화(N3 2~3개월 단축)되며 완화 폭이 확대. 상류(d2)의 지정학(대만 집중)·ASML 백로그는 불변 — d2−d1 괴리는 유지되나 절대 수준은 낮아지는 추세.
- **패키징**: 지수 **2 추가 하향(67→65)** — 중류(d1)의 CoWoS 가동률·수율은 완화 지속(갭 20%→10%). 상류(d2)의 적층 수율(stack_yield) 드라이버가 삼성 HBM4 골든수율(80%)로 **부분 완화** — 단 이는 마이크로범프 축이고, 하이브리드본딩 축(16-Hi)은 여전히 ~10% 수준에 머물러 있어 드라이버 판정은 "긴장"에서 완전히 벗어나진 않음. CoPoS 전면 양산 지연(2028~29)도 불변이라 완화 폭은 확대되지 않고 유지.
- **수요 변곡 EWI와의 관계 (분리 운영 유지)**: 2026-06-11 사용자 결정 유지. 08-25·08-28 정기 재평가(assessment 로그)가 SP-1/SP-2 시나리오 확률·EWI 6종을 이미 갱신했으므로, 본 절은 병목(상방 제약) 축만 갱신하고 중복 서술하지 않는다.
...
- [sources/articles/september-2026-market-update-2026-09-01.md](../../sources/articles/september-2026-market-update-2026-09-01.md) — 2026-09-01 병목 제약지수 갱신 데이터 (ERCOT 접속 전면 중단·TSMC N3 조기 램프·CoWoS 갭 20%→10%·삼성 HBM4 골든수율 80%)
