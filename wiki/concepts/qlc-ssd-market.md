...
**현행 덱 = v7.1(7장, 2026-09-23).** 스토리 스파인은 ① 문제(왜 지금인가 — 교훈 **하나**: 고객과 함께 수요를 설계하고 그 판단을 끊지 않은 쪽이 이겼다, [codesign-demand-lesson.md](codesign-demand-lesson.md); 이어 **그 수요가 지금 어디에 있는가** — HBM에서 넘쳐 NVMe SSD로 내려오는 스필오버, [hbm-to-storage-spillover.md](hbm-to-storage-spillover.md); 구매 기준의 세 국면 [essd-purchase-criteria-shift.md](essd-purchase-criteria-shift.md)와 "TLC는 만족하나 고객은 QLC를 원한다"는 문제 제기로 마무리 — 결론을 내리지 않는다) → ② 신뢰성(다이 고장률 — 요구는 ppm 단위로 내려가고 다이 품질 강화는 **수율·원가가 막는다**, 해법은 보호 구조, [ssd-die-reliability-ppm.md](ssd-die-reliability-ppm.md)) → ③ 내구성(WAF — 핵심 사슬만: 요구 DWPD 1~3 → WAF 저감 → 호스트가 같은 수명끼리 묶어 힌트와 함께 내려보냄, [solution-ladder-component-to-system.md](solution-ladder-component-to-system.md)) → ④ **배치 힌트**(신설 — 같은 QLC 드라이브에서 무배치 3.0 ✅ / 멀티스트림 1.8 ⚠️ / FDP 전면 1.05 ✅ / 혼재 2.2 ⚠️, 유효 DWPD 0.6·1.0·1.7·0.8, [fdp-placement-mechanics.md](fdp-placement-mechanics.md)) → ⑤ 역량([qlc-workload-capability-phases.md](../strategies/qlc-workload-capability-phases.md), 삼성 현 위치 Phase 1 진행 중으로 정직 표기) → ⑥ 실행([qlc-execution-strategy.md](../strategies/qlc-execution-strategy.md) 중 개발실 내부 범위; 단계별 통과 조건·판돈·비용 블록은 덱에서 제외하고 보고서에만 유지) → ⑦ 대응 기술(조건부 보증이 아니라 런타임 감지·대응 기술 확보, [waf-runtime-response.md](waf-runtime-response.md))이다. 제목 7개만 이어 읽어도 한 문단이 되며, 비유 표현은 쓰지 않는다(지렛대 → 잔여 변수, 닫힌다 → 충족·해소; "계층"으로 통일). 시각화 강화판(`qlc-ssd-strategy-visual.pptx` v2.0)은 v5.2 내용 기준으로 **동결**돼 있어 원본 덱과 수치가 다르다 — 의도된 차이다(사용자 지시 2026-09-22).
