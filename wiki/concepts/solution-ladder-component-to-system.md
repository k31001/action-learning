...
| **정격 DWPD(5년)** | 컨트롤러가 산출, 고객이 요구 | 17(X25-E 2008) | 10(S3700 2012) | 0.7(P4510 2018) | 0.6(P5336 122TB 2024) · 0.3(LC9 245TB 2025) | **57배 하락** — 단, 드라이브당 허용 기입량은 1.1 → 73.7 TB/day로 **67배 상승**. DWPD는 용량으로 나눈 지표라 단독으로 읽으면 오독이다(2026-09-22 추가) |
| **고객 요구 DWPD** | 고객(워크로드) | 엔터프라이즈 3~10 | 동일 | 캐시 계층 1~3 | 캐시 계층 1~3 | **그대로** — 현 QLC 정격 0.6 기준 **2~5배 갭**(2026-09-22 재계산. ScaleFlux의 유효 7~10은 벤더 발표값이라 제외) |
| **WAF** | 호스트·애플리케이션 | 랜덤 쓰기 ~3 | ~3 | 3.22(CacheLib, FDP 없음) | **1.03(CacheLib + FDP)** | **−68%** — 데이터 수명을 호스트가 알려 줄 때만 1에 수렴(F34) |

**산식**: DWPD = P/E × (1 + OP) ÷ (WAF × 365 × 보증연수) (F29). P/E는 셀이, OP는 디바이스가, WAF는 호스트·애플리케이션이, 보증연수와 요구 DWPD는 고객이 정한다. SLC→QLC로 P/E가 100배 줄었으니 요구 DWPD를 지키려면 남은 변수는 WAF뿐이고, WAF를 3에서 1로 내리는 것은 컨트롤러가 아니라 **호스트가 데이터 수명을 알려 줄 때**만 가능하다(F14·F15). 유효 DWPD는 정격 × (WAF_정격 / WAF_실제)이므로 3.22 → 1.03은 곧 ×3.1이다. 이것이 "이제 상위 계층과의 협력이 필요한 시점"의 수치적 근거이며, ScaleFlux의 FDP 스트림 200+·유효 7~10 DWPD 주장(F17)도 같은 산식 위에 있으나 제3자 검증이 없어 요구 수준의 근거로는 쓰지 않는다(2026-09-22 정정).
...
- 다음 장: [fdp-placement-mechanics.md](fdp-placement-mechanics.md) — 잔여 변수 WAF를 호스트가 어떻게 가르는가(네 경우의 WAF·유효 DWPD)
- 상위: [qlc-ssd-market.md](qlc-ssd-market.md)(§3.4 구매 기준·§3.5 다운턴 교훈) · [codesign-demand-lesson.md](codesign-demand-lesson.md) · [nand-process-transition.md](nand-process-transition.md) · [hbm-roadmap.md](hbm-roadmap.md) · [dram-technology.md](dram-technology.md)
...
- 산출물: [memory-solution-ladder-report.md](../../outputs/report/memory-solution-ladder-report.md) · 슬라이드 `outputs/presentation/memory-solution-ladder.pptx`(**v3.0(2026-09-22)**: 셀 차트 확대 + 정격 DWPD·허용 기입량 추이와 WAF before→after 쌍 신설, 연도축 WAF 추이선은 조건 교란 때문에 쓰지 않음 · v1.2: 네 지표 차트, 공식 문안 — 타이틀 "RBER 상승은 컨트롤러 ECC가 보상했고, P/E 감소의 보상 변수 WAF는 호스트 계층에 있습니다", v2.0 이관 매트릭스: 열 = 요구(고정) → 단품 지표(악화) → SSD 계층의 보상(1 ECC 완결 · 2 SSD 단독 최적화 부분 성공) → 호스트·시스템 계층의 보상(3 공동 설계 = QLC 덱 2~5장), 행 = 내구성 축 / 신뢰성 축(다이 수 128→1,024 · FFR ≤3%), 셀마다 미니 차트·빅넘버 1개; 같은 슬라이드가 QLC eSSD 전략 덱 `qlc-ssd-strategy.pptx` 3장(**현행 v7.0(2026-09-23): 빌더 v4.0** — 4열 이관 매트릭스와 격차 축소 변수 스트립을 제거하고 DWPD 산식 칩·3단 논증 사슬·5.90in 차트 3종만 남긴 압축판. 매트릭스와 격차 축소 변수는 보고서 §3.1·§3.4에 유지하며, 배치 힌트의 메커니즘은 덱 4장으로 분리했다([fdp-placement-mechanics.md](fdp-placement-mechanics.md)). 이전 v5.2, 2장 신뢰성 다음; v2.2~2.3 빌더 `build(lead·band_main·notes_tail)` 오버라이드로 "둘째 축 내구성은 SSD 단독 최적화로 WAF ≈3에 머물렀고 → 잔여 변수 WAF는 SSD 밖 호스트가 결정 → 고객 협업·새 역량" 문안 주입; v2.3(2026-09-19) 타이틀 오버라이드 추가 + "지렛대" → "변수" 용어 통일, QLC 덱 3장 제목 "둘째 축 내구성은 SSD 단독 최적화로 WAF ≈3에 머물렀고, 잔여 변수 WAF는 호스트가 결정합니다") — `scripts/solution_ladder_slide.py` 공용 빌더 `assets/solution_metrics_chart.png`; 계층 사다리 차트 `assets/solution_ladder_chart.png`는 보고서 그림)
