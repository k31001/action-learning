---
type: entity
last_reviewed: 2026-05-18
sources: [sources/README.md (Bloomberg, Reuters, Yole Group, Morgan Stanley, TechInsights, Caixin Global, Digitimes)]
---

# 중국 메모리 업체 그룹 (CXMT · YMTC)

중국이 메모리 자립을 위해 국가 자본과 지방정부 지원을 결합해 키운 두 핵심 업체. DRAM(CXMT)·NAND(YMTC)를 분담하면서 미국 수출통제 환경에서도 빠르게 캐파와 점유율을 확대 중.

**수집일**: 2026-05-05  
**위키 내 위치**: 그룹 인덱스. 업체별 상세는 분리된 entity 페이지 참조.

---

## 업체별 페이지

| 업체 | 영역 | 본사 | 점유 (Q3 2025) | 상세 |
|------|------|------|--------------|------|
| **CXMT** (창신메모리) | DRAM | 안후이성 허페이 | 글로벌 DRAM 8% (4위 진입) | [cxmt.md](cxmt.md) |
| **YMTC** (양쯔메모리) | NAND Flash | 후베이성 우한 | 글로벌 NAND 13% (출하 기준) | [ymtc.md](ymtc.md) |

---

## 공통 자금 구조: 국가 빅펀드 + 허페이 모델

중국 반도체 굴기의 핵심 재원은 **국가 집성전로 산업투자기금(國家集成電路産業投資基金, "빅펀드/Big Fund")**. CXMT와 YMTC 모두 직·간접 수혜자.

### 빅펀드 단계별 규모

| 단계 | 설립 | 규모 | 주요 투자 대상 |
|------|------|------|------------|
| 빅펀드 I | 2014 | ~1,387억 위안 ($190억) | YMTC, SMIC, CXMT 전신 등 |
| 빅펀드 II | 2019 | ~2,041억 위안 ($280억) | 제조·장비·소재 확대 |
| 빅펀드 III | 2024-03 | ~3,440억 위안 ($470억) | 첨단 공정·장비·메모리 자립 집중 — 역대 최대 |

- 출처: Bloomberg (2024-05-24), Reuters (2024-03-27), Yole Group
- 관리: CICC 및 재정부 산하 기관
- 빅펀드 III는 미국 수출통제 강화 대응용 전 분야 자립화 자금

### 지방정부 직접 지원 (허페이 모델)

| 업체 | 출자 기관 | 누적 지원 | 특징 |
|------|---------|---------|------|
| CXMT | 안후이성 정부, 허페이시 국유투자 | 수조 위안 | 토지·인프라 무상/할인, 지분 직접 보유 |
| YMTC | 후베이성 정부, 칭화유니그룹 | 수조 위안 | 우한시 국유기업 지분 참여, 저금리 정책금융 |

**허페이 모델**: 지방정부가 직접 지분 투자자로 참여. 같은 구조를 BOE(디스플레이)·NIO(전기차)에도 적용 → "국유 VC 모델".

---

## 14~15차 5개년 계획

| 항목 | 내용 |
|------|------|
| 기간 | 14차(~2025), 15차(2026~2030) |
| 핵심 목표 | **메모리 반도체 자급률 40% 이상** |
| 주요 과제 | ① 국산 EUV/DUV 장비 개발 ② 국산 화학·소재 공급망 ③ DRAM/NAND 전 세대 자립 생산 |
| 지원 기관 | 공업정보화부(MIIT), 국가발전개혁위원회(NDRC), 과학기술부 |

- 출처: Yole Group (2025)
- 보다 자세한 정책 분석은 [china-policy.md](../concepts/china-policy.md) 참조

---

## 미국 수출통제 영향

- **YMTC**: 2022-12 엔티티 리스트 등재 — 미국 장비·소재 차단
- **CXMT**: 엔티티 리스트 등재 상태, 첨단 미국 장비 접근 제한
- **대응**: 국산 DUV(SMEE) + 다중 패터닝 + Xtacking 자체 IP + 국산 장비 라인 구축
- 자세히는 [us-export-controls.md](../concepts/us-export-controls.md) 참조

---

## 주요 발견

- **공급 공백 수혜**: 한국·미국 업체가 HBM 등 고부가 제품에 집중하는 과정에서 발생하는 범용 DRAM/NAND 공급 공백을 CXMT, YMTC가 흡수 (Economy.ac, 2026-02-28)
- **CXMT DRAM 4강 진입**: Q3 2025 기준 글로벌 DRAM 8% → 2027E 13.9% 전망
- **YMTC NAND 13% 돌파**: Q3 2025 기준 글로벌 NAND 출하량 13%, 마이크론 추격 단계
- **YMTC 3공장 건설 중**: 2027년 가동 목표, 신규 설비 50%를 DRAM(HBM 포함 가능성) 전환
- **YMTC hybrid bonding IP 지배**: 한국·미국 메모리사가 V11·HBM4 세대에서 라이선스 의존 가능 → 디커플링 시 차단 리스크

---

## 삼성전자 전략 시사점

1. **포트폴리오 전환 압력** — 범용 DRAM/NAND 수익성 직격. HBM·고부가 비중 빠르게 확대 필요 → [RS2 바벨 포트폴리오](../strategies/invariant/rs2-barbell-portfolio.md)
2. **세그먼트 분리 전략** — 범용 공백을 의도적으로 CXMT/YMTC에 넘기고 자신은 고마진 AI 메모리 집중
3. **JV 모델 참고** — CXMT의 "지방정부 자본 + 메모리 업체" 조합 vs 삼성의 "글로벌 빅테크자본 + 삼성" 조합 (SanDisk+Kioxia 모델 변형)
4. **기술 격차 유지** — HBM4/HBM5, CXL, PIM 등 5~10년 내 추격 어려운 영역 리더십 → [RS6 공정 리더십](../strategies/invariant/rs6-process-leadership.md)
5. **Hybrid bonding 자체 IP 확보** — YMTC IP 종속 회피 → [RS7 AI 엔지니어링 자동화](../strategies/invariant/rs7-ai-engineering-automation.md)의 4 R&D 트랙

---

## 위키 내 관련 페이지

- [cxmt.md](cxmt.md), [ymtc.md](ymtc.md) — 업체별 상세
- [china-policy.md](../concepts/china-policy.md) — 정책·빅펀드 상세
- [us-export-controls.md](../concepts/us-export-controls.md) — 미국 수출통제
- [dram-market-share.md](../concepts/dram-market-share.md) — DRAM 점유율 변화
- [nand-process-transition.md](../concepts/nand-process-transition.md) — NAND 적층·hybrid bonding
- [wiki/scenarios/scenario-C.md](../scenarios/scenario-C.md) — 기술 냉전 시나리오

---

## [Update 2026-08-03] 크리스 밀러의 중국 이중 평가

크리스 밀러(Chip War 저자)의 2025-12~2026-07 발언 종합 ([chris-miller-interviews-2025-12-to-2026-07.md](../../sources/articles/chris-miller-interviews-2025-12-to-2026-07.md)) — 중국 평가는 **구조적 제약과 비대칭 위협의 이중 구도**.

### 구조적 제약 (AI 칩)

- **"중국은 지난 4년간 AI에 과소투자"** — "중국 정부는 AI가 우리 생각만큼 중요해질 거라고 진짜로 믿지 않는다" (2026-06 발언, §8).
- **Huawei Ascend 910B 280~400 TFLOPS vs Nvidia Blackwell 2,250 TFLOPS** (5~8배 격차) + 장비 통제로 SMIC가 규모 생산 불가 → 중국은 **AI 데이터센터 기술 수출국이 될 수 없고**, 자국 수요조차 스펙다운된 서방 칩 수입으로 채워야 하는 처지 (AEI 기고, §5).
- 함의: 첨단 AI 칩·HBM에서의 중국 추격은 단기 위협이 아님 — 위키의 "CXMT HBM 대규모 양산 미확정" 판단과 정합.

### 비대칭 위협 (전력·피지컬 AI·범용 침투)

- **전력**: AI 3대 투입(컴퓨팅·인재·전력) 중 중국이 전력에서 우위 (상원 증언 2025-12-02, §1).
- **피지컬 AI**: 중국은 **제조 기반과 소프트웨어 역량을 동시에 갖춘 거의 유일한 국가** — 로보틱스/피지컬 AI 시대의 최강 경쟁자. 각국은 중국산 피지컬 AI 의존을 피할 산업 정책을 고민해야 (인사이트코리아, §10).
- **범용 침투**: 애플–CXMT 류의 완제품 채택 경로 경계 — [cxmt.md [Update 2026-08-03]](cxmt.md) 참조.

### 삼성 함의

- 위협의 실현 경로는 **첨단 정면 추월이 아니라 ① 범용·완제품 채택(CXMT), ② 피지컬 AI 수요의 중국 내 수직계열화** — 시나리오 C·D의 중국 변수 해석과 일치하며, HBM·추론 아키텍처 등 추격 난이도 높은 영역의 격차 유지가 방어의 핵심이라는 기존 결론을 강화.

**출처**: [chris-miller-interviews-2025-12-to-2026-07.md](../../sources/articles/chris-miller-interviews-2025-12-to-2026-07.md)

---

## [Update 2026-09-08] CXMT DRAM 10% 돌파(4강 과점 붕괴)·YMTC NAND 15% 목표·美 제재 혼재

- **CXMT**: DRAM 매출 점유율 **10%** 돌파(Q2 2026, 전년 4%) — 삼성·SK하이닉스·Micron 90%+ 과점 붕괴. 연말 캐파 ~350K WSPM(Micron 근접), 2030년 30% 점유 목표. HBM3 배정은 2027년 55K WSPM까지 램프 전망이나 경쟁력 있는 양산은 2028년+ 로 시간 여유 존재. 상세는 [cxmt.md](cxmt.md) [Update 2026-09-08] 참조.
- **YMTC**: 2026년 말까지 NAND 글로벌 점유율 **~15%**·**~150K WSPM** 목표, 전(全) 국산 장비 시험라인 구축 추진, IPO 절차 진행 중 — 제재 환경에서도 확장을 지속.
- **美 제재 상태 혼재**: 두 회사 모두 국방부 **Section 1260H "Restricted Companies" 리스트에서는 제외**됐으나, 상무부 **Entity List는 유지**되고 H200급 수출은 케이스바이케이스 심사가 오히려 강화(2026-01 규칙)됐다 — 정치적 신호는 완화, 실질 통제는 유지·일부 강화라는 이중 국면.
- **中 빅펀드 III($47.5B) 무게중심 이동**: 2026-08-07 확정된 자금 배분에서 팹 건설보다 **첨단 패키징·장비·AI 칩 쪽으로 비중이 이동** — 메모리를 향한 신규 직접 보조 신호는 상대적으로 약화됐다(기존 팹 투자는 이미 진행 중인 CXMT·YMTC 자체 캐파 확장으로 계속).
- **삼성 함의**: DRAM 범용 시장의 점유율 유출(CXMT 10%)은 구조적으로 진행 중이나, DRAM 자체 수급은 2028 Q2까지 타이트가 유지될 전망이어서 즉각적 가격 압박으로 연결되지는 않는다 — "지금 당장의 가격 리스크"보다 "장기 파이 축소 리스크"로 프레이밍하는 것이 정확하다.

**출처**: [september-2026-market-update-2026-09-08.md](../../sources/articles/september-2026-market-update-2026-09-08.md) §4~5
