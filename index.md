...
- [outputs/presentation/qlc-ssd-strategy-outline.md](outputs/presentation/qlc-ssd-strategy-outline.md) — (NEW) QLC eSSD 전략 발표 **4장** 기획서 (다운턴 교훈 / 배경 / 요구사항·역량·기술 전략 / 실행 전략·고객 협업, ssd-strategy 디자인 승계 + 컨설팅 문법, 로고 타일)
- [outputs/presentation/qlc-ssd-strategy.pptx](outputs/presentation/qlc-ssd-strategy.pptx) — (NEW) QLC eSSD 전략 **4장** 덱 본체 — v3.5: S1 다운턴이 가르친 것(DT19·DT23 타임라인+교훈 카드 3) / S2 고객이 왜 QLC를 원하는가(3기 스트립: 고객이 산 것 = TB당 TCO·왜·조건·삼성)+이정표 타임라인+통합 그래프(EB·%·$B)+키넘버+송용호 인용 / S3(리드 = 신문섭 인용) 갭 타일 4+Phase 1·2·3 스택 그림(층별 기술)+진행 바 / S4 두 트랙 그림(FDE·SCA)+선례 카드 2+조직·인사·문화 3축 타일(시스템 SW 전문가 채용·양성·미주 현지 채용·오픈소스 생태계 주도)+3장 요약 결론 밴드 (`scripts/generate_qlc_ssd_strategy_pptx.py`, 그래프는 `scripts/generate_qlc_chart.py`, 커밋 예외 등록)
