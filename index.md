...
- [outputs/report/memory-solution-ladder-report.md](outputs/report/memory-solution-ladder-report.md) — (NEW) 해법 사다리 분석 보고서 v1.4 (QLC 전략 부속) — 5칸 사다리 모델·NAND 이관 연혁·§2.5 지표(P/E·RBER·UBER·ECC·DWPD·WAF 표 + 산식 + 이관의 3단계 + 병렬 축 다이 수·고장률)·DRAM 검증(부분 성립: PRAC = FDP의 DRAM 판)·순서 차이·함의 4·부록 A 팩트 대장 47건·슬라이드 맵 v1.4
- [outputs/presentation/memory-solution-ladder.pptx](outputs/presentation/memory-solution-ladder.pptx) — (NEW) 해법 사다리 **1장** 슬라이드 v1.4 — 타이틀 "ECC가 RBER을 보상했듯 P/E 감소는 WAF로 보상해야 하며, 이는 호스트 공동 설계로만 가능합니다" / 좌 네 지표 차트(P/E 100배↓ · RBER 10⁶배↑ vs UBER 고정·ECC 60배 · 정격 DWPD 17→0.41 vs 요구 1~3 · WAF: SSD 단독 ≈3 vs 호스트 공동 설계 1.03, `assets/solution_metrics_chart.png`) / 우 DWPD 산식 카드(항별 결정 주체) + 이관의 3단계 카드(1 ECC 완결 / 2 SSD 단독 워크로드 최적화 부분 성공: QoS·성능↑·WAF ≈3 / 3 호스트 시스템 공동 설계 WAF 1.0 = QLC 덱 2~5장, 유일한 경로) + 병렬 축 스트립(다이 고장률: FFR ≤3% 고정 · SSD당 다이 수 128→1,024 · 다이 RAID·여분 다이·FIP → 상위 계층) / 결론 (`scripts/generate_solution_ladder_pptx.py` → `solution_ladder_slide.py` 공용 빌더 + `generate_solution_metrics_chart.py` + `deck_kit.py`; QLC 5장 덱 1장과 동일 코드; v1.0 계층 사다리 차트 `assets/solution_ladder_chart.png`는 보고서 그림)
...
- [outputs/presentation/qlc-ssd-strategy-outline.md](outputs/presentation/qlc-ssd-strategy-outline.md) — (NEW) QLC eSSD 전략 발표 **5장** 기획서 v3.10 (스토리 스파인 ① 산식 → ② 시점 → ③ 수요 → ④ 역량 → ⑤ 실행, 스토리 레일·다음 장 포인터·요약 체인, ssd-strategy 디자인 승계 + 컨설팅 문법, 로고 타일)
- [outputs/presentation/qlc-ssd-strategy.pptx](outputs/presentation/qlc-ssd-strategy.pptx) — (NEW) QLC eSSD 전략 **5장** 덱 본체 — v3.12(1장 병렬 축 스트립: 다이 수 128→1,024·FFR 고정·다이 RAID·FIP → 상위 계층) · v3.11(1장 이관의 3단계: ECC 완결 → SSD 단독 최적화 부분 성공 → 호스트 공동 설계 유일) · v3.10 스토리라인 통일(헤더 스토리 레일 ① 산식 → ② 시점 → ③ 수요 → ④ 역량 → ⑤ 실행, 결론 밴드 다음 장 포인터, 연결 리드, 공식 용어): S1 산식·해법 사다리(P/E·BER·DWPD·WAF 네 지표 차트+산식+이관 구조의 반복 3행, `scripts/solution_ladder_slide.py` 공용) / S2 시점·다운턴 교훈(DT19·DT23 타임라인+교훈 카드 3, 연결 칩) / S3 수요·고객의 QLC 채택 동인(3기 스트립: 구매 기준 = TB당 TCO·동인·조건·삼성)+통합 그래프(EB·%·$B)+키넘버+송용호 인용 / S4 역량(리드 = 신문섭 인용) 격차 타일 4+Phase 1·2·3 스택 그림(층별 기술, 고객 보증 범위)+진행 바+결론 밴드 / S5 실행 두 트랙 그림(FDE 상주·SCA)+선례 카드 2+조직·인사·문화 3축 타일+**5장 요약 체인** (`scripts/generate_qlc_ssd_strategy_pptx.py`, 그래프는 `scripts/generate_qlc_chart.py`, 커밋 예외 등록)
