...
- [sources/articles/component-to-system-solution-ladder-facts-2026-09.md](sources/articles/component-to-system-solution-ladder-facts-2026-09.md) — (NEW) 해법 사다리 팩트체크 대장 F1~F35 (SSD·FTL·LDPC·TRIM·ZNS·FDP·CacheLib / Chipkill·Row Hammer·DDR5 on-die ECC·RFM·PRAC·CXL·MRDIMM / HBM·HBM4E 커스텀 베이스 다이·HBF OCP 사양 / §7 지표: JESD218 UBER·보존, DWPD 산식, 정격 DWPD 추이 X25-E→P5316, RBER·ECC 추이, WAF / §8 SSD 단독 워크로드 최적화의 한계 F36~F40: Multi-stream·AutoStream·FTL 핫/콜드·NVMe IOD·FDP 전제 / §9 병렬 축 다이 수·고장률 F41~F47: JESD218 FFR·S3700 128다이→Kioxia LC9 1,024다이·수율 모델·Cai 패리티·Micron RAIN·삼성 FIP·OCP SMART·MS Hyrax) — 웹 검색 요약 기반, 원문 열람 차단으로 등급 병기
...
- [outputs/presentation/qlc-ssd-strategy-outline.md](outputs/presentation/qlc-ssd-strategy-outline.md) — (NEW) QLC eSSD 전략 발표 **4장** 기획서 v4.0 (스토리 스파인 ① 교훈·문제 → ② 해법 사다리 → ③ 역량 → ④ 실행, 스토리 레일·다음 장 포인터·요약 체인, ssd-strategy 디자인 승계 + 컨설팅 문법, 로고 타일)
- [outputs/presentation/qlc-ssd-strategy.pptx](outputs/presentation/qlc-ssd-strategy.pptx) — (NEW) QLC eSSD 전략 **4장** 덱 본체 — v4.1(1 교훈·문제 제기 = 시간축이 곧 논리축: 타임라인 세 구간 아래 교훈 2018~23 / 지금·요구의 이동 2024~26 / 문제·해법이 필요한 상황 2027~30 세 패널 정렬, 타이틀은 결론 아닌 문제 제기 / 2 해법 사다리 이관 매트릭스 / 3 역량 3단계 / 4 실행 FDE·SCA·조직 + 요약 체인) · v3.10 스토리라인 통일(헤더 스토리 레일 ① 산식 → ② 시점 → ③ 수요 → ④ 역량 → ⑤ 실행, 결론 밴드 다음 장 포인터, 연결 리드, 공식 용어): S1 산식·해법 사다리(P/E·BER·DWPD·WAF 네 지표 차트+산식+이관 구조의 반복 3행, `scripts/solution_ladder_slide.py` 공용) / S2 시점·다운턴 교훈(DT19·DT23 타임라인+교훈 카드 3, 연결 칩) / S3 수요·고객의 QLC 채택 동인(3기 스트립: 구매 기준 = TB당 TCO·동인·조건·삼성)+통합 그래프(EB·%·$B)+키넘버+송용호 인용 / S4 역량(리드 = 신문섭 인용) 격차 타일 4+Phase 1·2·3 스택 그림(층별 기술, 고객 보증 범위)+진행 바+결론 밴드 / S5 실행 두 트랙 그림(FDE 상주·SCA)+선례 카드 2+조직·인사·문화 3축 타일+**5장 요약 체인** (`scripts/generate_qlc_ssd_strategy_pptx.py`, 그래프는 `scripts/generate_qlc_chart.py`, 커밋 예외 등록)
