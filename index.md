...
- [wiki/concepts/solution-ladder-component-to-system.md](wiki/concepts/solution-ladder-component-to-system.md) — (NEW) 해법 사다리: 단품이 요구를 못 채우면 해법은 상위 계층(컨트롤러→호스트→애플리케이션)으로 이관된다 — NAND(SSD 1991→FDP 2022→CacheLib 2025)·DRAM(Chipkill 1997·HBM 2013·on-die ECC 2020·RFM 2021·PRAC 2024·CXL) 사다리 표, §2.5 지표(P/E 100배↓·RBER 10⁶배↑ vs UBER 고정·ECC 60배·DWPD 17→0.41·WAF 3.22→1.03, DWPD 산식의 항별 소유자, 이관의 3단계: ECC 완결 → SSD 단독 최적화 부분 성공 → 호스트 공동 설계), 두 축(이관 vs 자기 진화)과 순서 차이(HBF 35년 뒤 vs HBM 6~11년 앞), QLC·DRAM 함의
...
- [sources/articles/component-to-system-solution-ladder-facts-2026-09.md](sources/articles/component-to-system-solution-ladder-facts-2026-09.md) — (NEW) 해법 사다리 팩트체크 대장 F1~F35 (SSD·FTL·LDPC·TRIM·ZNS·FDP·CacheLib / Chipkill·Row Hammer·DDR5 on-die ECC·RFM·PRAC·CXL·MRDIMM / HBM·HBM4E 커스텀 베이스 다이·HBF OCP 사양 / §7 지표: JESD218 UBER·보존, DWPD 산식, 정격 DWPD 추이 X25-E→P5316, RBER·ECC 추이, WAF / §8 SSD 단독 워크로드 최적화의 한계 F36~F40: Multi-stream·AutoStream·FTL 핫/콜드·NVMe IOD·FDP 전제) — 웹 검색 요약 기반, 원문 열람 차단으로 등급 병기
...
- [outputs/report/memory-solution-ladder-report.md](outputs/report/memory-solution-ladder-report.md) — (NEW) 해법 사다리 분석 보고서 v1.3 (QLC 전략 부속) — 5칸 사다리 모델·NAND 이관 연혁·§2.5 지표(P/E·RBER·UBER·ECC·DWPD·WAF 표 + 산식 + 이관의 3단계)·DRAM 검증(부분 성립: PRAC = FDP의 DRAM 판)·순서 차이·함의 4·부록 A 팩트 대장 40건·슬라이드 맵 v1.3
- [outputs/presentation/memory-solution-ladder.pptx](outputs/presentation/memory-solution-ladder.pptx) — (NEW) 해법 사다리 **1장** 슬라이드 v1.3 — 타이틀 "ECC가 RBER을 보상했듯 P/E 감소는 WAF로 보상해야 하며, 이는 호스트 공동 설계로만 가능합니다" / 좌 네 지표 차트(P/E 100배↓ · RBER 10⁶배↑ vs UBER 고정·ECC 60배 · 정격 DWPD 17→0.41 vs 요구 1~3 · WAF: SSD 단독 ≈3 vs 호스트 공동 설계 1.03, `assets/solution_metrics_chart.png`) / 우 DWPD 산식 카드(항별 결정 주체) + 이관의 3단계 카드(1 ECC 완결 / 2 SSD 단독 워크로드 최적화 부분 성공: QoS·성능↑·WAF ≈3 / 3 호스트 시스템 공동 설계 WAF 1.0 = QLC 덱 2~5장, 유일한 경로) / 결론 (`scripts/generate_solution_ladder_pptx.py` → `solution_ladder_slide.py` 공용 빌더 + `generate_solution_metrics_chart.py` + `deck_kit.py`; QLC 5장 덱 1장과 동일 코드; v1.0 계층 사다리 차트 `assets/solution_ladder_chart.png`는 보고서 그림)
