...
### 시장
- [wiki/concepts/qlc-ssd-market.md](wiki/concepts/qlc-ssd-market.md) — (NEW) QLC eSSD 시장: 초기(2022 준비기)·현재(용량 경쟁·HDD 부족)·향후(추론 캐시 티어) 3기 비교 + §3.4 고객이 QLC를 원하는 이유(불변 = 계산 옆의 가장 싼 바이트, 변화 = 쓰기 많은 캐시 티어 → 내구성이 문)표 + 2022~2030 수요(EB)·비중(%)·매출($B) 모델(비트 10배·매출 정체, KV 티어 QLC 조건부 상방)
...
- [outputs/presentation/qlc-ssd-strategy.pptx](outputs/presentation/qlc-ssd-strategy.pptx) — (NEW) QLC eSSD 전략 **3장** 덱 본체 — v3.3: S1 고객이 왜 QLC를 원하는가(3기 스트립: 고객이 산 것·왜·조건·삼성)+이정표 타임라인+통합 그래프(EB·%·$B)+키넘버 / S2 갭 타일 4+Phase 1·2·3 스택 그림(층별 기술)+진행 바 / S3 두 트랙 그림(FDE·SCA)+선례 카드 2+조직·인사·문화 3축 타일(시스템 SW 전문가 채용·양성·미주 현지 채용·오픈소스 생태계 주도)+3장 요약 결론 밴드 (`scripts/generate_qlc_ssd_strategy_pptx.py`, 그래프는 `scripts/generate_qlc_chart.py`, 커밋 예외 등록)
