...
- [outputs/presentation/memory-solution-ladder.pptx](outputs/presentation/memory-solution-ladder.pptx) — (NEW) 해법 사다리 **1장** 슬라이드 v2.1 이관 매트릭스(내구성 행 + 지렛대 스트립) — 타이틀 "요구는 고정, 단품은 악화, 보상은 상위 계층으로: QLC의 DWPD는 호스트 공동 설계로만 충족됩니다" / 열 = 요구(고정) → 단품 지표(악화) → SSD 계층의 보상(1 ECC 완결 · 2 SSD 단독 최적화 부분 성공) → 호스트·시스템 계층의 보상(3 공동 설계 = QLC 덱 2~5장), 행 = 내구성 축(DWPD 1~3·UBER 빅넘버 → P/E 막대 + RBER 로그 선 이중 축 미니 차트 → ECC 타일 + 정격 DWPD 스파크라인 → WAF 3.22→1.03) / 신뢰성 축(FFR ≤3% → 다이 수 128→1,024 → 다이 RAID·FIP → 상위 계층 점선), 행 사이 DWPD 산식 칩 (`assets/ladder_cell_*.png`, `scripts/generate_ladder_cells.py`; 2×2 차트는 보고서 그림) / 결론 (`scripts/generate_solution_ladder_pptx.py` → `solution_ladder_slide.py` 공용 빌더 + `generate_solution_metrics_chart.py` + `deck_kit.py`; QLC 5장 덱 1장과 동일 코드; v1.0 계층 사다리 차트 `assets/solution_ladder_chart.png`는 보고서 그림)
...
- [outputs/presentation/qlc-ssd-strategy-outline.md](outputs/presentation/qlc-ssd-strategy-outline.md) — (NEW) QLC eSSD 전략 발표 **6장** 기획서 v5.0 (스토리 스파인 ① 문제 → ② 해법 사다리 → ③ 신뢰성 → ④ 역량 → ⑤ 실행 → ⑥ 보증·SLA, 스토리 레일·다음 장 포인터·요약 체인, ssd-strategy 디자인 승계 + 컨설팅 문법)
- [outputs/presentation/qlc-ssd-strategy.pptx](outputs/presentation/qlc-ssd-strategy.pptx) — (NEW) QLC eSSD 전략 **6장** 덱 본체 — v5.0(1 문제: 시간축 = 논리축, 동급 비교 2~10배 / 2 해법 사다리: 내구성 매트릭스 + 지렛대 스트립, "유일한 경로" 철회, KV 캐시 WAF 미실측 가설 / 3 신뢰성: 다이 수·고장률의 SSD 내부 해법 / 4 역량: 현 위치 Phase 1 진행 중 / 5 실행: FDE 선별 집중 + 개발실 내부 실행 + 단계별 통과 조건 + 판돈·비용 / 6 보증·SLA: 유효 DWPD 조건부 보증 설계 + 요약 체인) · v3.10 스토리라인 통일(헤더 스토리 레일 ① 산식 → ② 시점 → ③ 수요 → ④ 역량 → ⑤ 실행, 결론 밴드 다음 장 포인터, 연결 리드, 공식 용어): S1 산식·해법 사다리(P/E·BER·DWPD·WAF 네 지표 차트+산식+이관 구조의 반복 3행, `scripts/solution_ladder_slide.py` 공용) / S2 시점·다운턴 교훈(DT19·DT23 타임라인+교훈 카드 3, 연결 칩) / S3 수요·고객의 QLC 채택 동인(3기 스트립: 구매 기준 = TB당 TCO·동인·조건·삼성)+통합 그래프(EB·%·$B)+키넘버+송용호 인용 / S4 역량(리드 = 신문섭 인용) 격차 타일 4+Phase 1·2·3 스택 그림(층별 기술, 고객 보증 범위)+진행 바+결론 밴드 / S5 실행 두 트랙 그림(FDE 상주·SCA)+선례 카드 2+조직·인사·문화 3축 타일+**5장 요약 체인** (`scripts/generate_qlc_ssd_strategy_pptx.py`, 그래프는 `scripts/generate_qlc_chart.py`, 커밋 예외 등록)
