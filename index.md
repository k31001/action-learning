...
- [outputs/presentation/memory-solution-ladder.pptx](outputs/presentation/memory-solution-ladder.pptx) — (NEW) 해법 사다리 **1장** 슬라이드 v1.1 — 좌 네 지표 차트(P/E 100배↓ · RBER 10⁶배↑ vs UBER 고정·ECC 60배 · 정격 DWPD 17→0.41 vs 요구 1~3 · WAF 3.22→1.03, `assets/solution_metrics_chart.png`) / 우 DWPD 산식 카드(항별 소유자, WAF = 남은 지렛대) + 두 단계의 대칭(BER→ECC / DWPD→WAF) / 결론 (`scripts/generate_solution_ladder_pptx.py` → `solution_ladder_slide.py` 공용 빌더 + `generate_solution_metrics_chart.py` + `deck_kit.py`; QLC 5장 덱 1장과 동일 코드; v1.0 계층 사다리 차트 `assets/solution_ladder_chart.png`는 보고서 그림)
...
- [outputs/presentation/qlc-ssd-strategy-outline.md](outputs/presentation/qlc-ssd-strategy-outline.md) — (NEW) QLC eSSD 전략 발표 **5장** 기획서 (해법 사다리 / 다운턴 교훈 / 배경 / 요구사항·역량·기술 전략 / 실행 전략·고객 협업, ssd-strategy 디자인 승계 + 컨설팅 문법, 로고 타일)
- [outputs/presentation/qlc-ssd-strategy.pptx](outputs/presentation/qlc-ssd-strategy.pptx) — (NEW) QLC eSSD 전략 **5장** 덱 본체 — v3.9: S1 해법 사다리(P/E·BER·DWPD·WAF 네 지표 차트+산식+두 단계의 대칭, `scripts/solution_ladder_slide.py` 공용) / S2 다운턴이 가르친 것(DT19·DT23 타임라인+교훈 카드 3) / S3 고객이 왜 QLC를 원하는가(3기 스트립: 고객이 산 것 = TB당 TCO·왜·조건·삼성)+이정표 타임라인+통합 그래프(EB·%·$B)+키넘버+송용호 인용 / S4(리드 = 신문섭 인용) 갭 타일 4+Phase 1·2·3 스택 그림(층별 기술)+진행 바 / S5 두 트랙 그림(FDE·SCA)+선례 카드 2+조직·인사·문화 3축 타일(시스템 SW 전문가 채용·양성·미주 현지 채용·오픈소스 생태계 주도)+3장 요약 결론 밴드 (`scripts/generate_qlc_ssd_strategy_pptx.py`, 그래프는 `scripts/generate_qlc_chart.py`, 커밋 예외 등록)
