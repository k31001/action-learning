...
- [outputs/presentation/qlc-ssd-strategy-visual.pptx](outputs/presentation/qlc-ssd-strategy-visual.pptx) — (NEW) QLC eSSD 전략 **시각화 강화판 7장** v1.0 (2026-09-20) — 요약 스토리 맵 + 1~6장, 제목 문단·수치는 v5.2 그대로, 장당 히어로 그림 1장, 텍스트 원본 대비 47%(pptx 텍스트 19%) · 생성기 `scripts/generate_qlc_ssd_strategy_visual_pptx.py`
- [outputs/presentation/scripts/generate_qlc_visual_figures.py](outputs/presentation/scripts/generate_qlc_visual_figures.py) — (NEW) 시각화 강화판 히어로 그림 7종 SVG 작성 + Chromium 3배 PNG 렌더(`assets/visual/qlc_vis_s{0..6}.svg|png`): 스토리 맵 · 문제(타임라인·요구 이동·동급 격차·두 축) · 신뢰성(다이 픽토그램·SSD 내부 메커니즘) · 해법 사다리(3단 계단·산식·WAF 실증·변수 칩) · 역량(Phase 카드·5계층) · 실행(고객 선별 맵·통과 조건·판돈) · 보증(유효 DWPD 곡선·5원칙)
