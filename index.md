...
- [outputs/presentation/storyline-common-overview-outline.md](outputs/presentation/storyline-common-overview-outline.md) — (NEW) 「호황은 전략을 심는 계절이다」 공통 개요편 발표 17장 기획서 (핵심 메시지 밴드·시각 스펙·전환 논리·발표자 노트)
- [outputs/presentation/storyline-common-overview.pptx](outputs/presentation/storyline-common-overview.pptx) — (NEW) 공통 개요편 17장 덱: 본문 13장(CMO 매트릭스 3장 = 다운턴별 M×C→O 포함) + 2차 저지선 골격 4장. 삼성 블루 모노크롬 · 전 텍스트 20pt 이상 · 도형 드로잉 데이터 그래픽 (`scripts/generate_storyline_overview.cjs`로 재생성)
