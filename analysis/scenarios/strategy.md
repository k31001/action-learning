...
- **구체적 실행 방안**:
  ...
  **[AI 개발 효율화와의 연결]**:
  - FDP 소프트웨어 개발, HBM 베이스다이 커스텀 로직 설계, 고객별 Co-Validation 엔지니어링 작업 모두 기존 인력 대비 상당한 추가 엔지니어링 자원을 요구한다
  - 이 추가 자원은 신규 채용이 아니라 RS2에서 확보한 AI 개발 효율화 잉여 자원에서 조달
  - 전사 AI 코딩 도구 도입으로 FW(펌웨어) 개발 사이클 30% 단축 → 기존 FW 엔지니어 일부를 고객특화 FDP 소프트웨어 개발에 전환 배치하는 것이 핵심 실행 경로

- **기간**: 중기~장기 (FDP 파일럿 2026년, 양산 적용 2027~2028년, 소프트웨어 구독 모델 2028년+)
- **핵심 리스크**: 고객 사이트에 삼성 엔지니어를 파견하는 Co-Validation 모델은 인력 부담이 크다. AI 효율화 선행 없이 이 전략을 추진하면 기존 제품 개발 인력이 희석된다. 따라서 AI 도구 전사 도입(RS2 선행 조건)이 RS3의 전제 조건이다.
